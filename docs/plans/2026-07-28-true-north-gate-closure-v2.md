...
**Interfaces:**
- **Scope** (deterministic, tightened — the 4b fragment screen at 157/243 was uninformative and is retired): a retained candidate is screened in when any of: (a) repetition — token-Jaccard of its `claim_text ∪ evidence_text` against another retained candidate's `claim_text ∪ evidence_text` ≥0.45 (pairs recorded); (a2) *(Amendment 2a — preflight correction)* local paraphrase repetition — another retained or revised candidate in the **same segment** whose evidence span lies within ±600 characters shares ≥2 informative stems (tokens ≥4 chars, stopwords excluded, simple suffix-stemmed) with the candidate's `claim_text ∪ evidence_text`; the paired neighbor joins the packet. Rationale: conversational repetition is local — a speaker restates what was just asserted — so evidence proximity plus shared informative stems admits semantic paraphrase duplicates (e.g. a 0.297-Jaccard pair 206 characters apart) without lowering the global Jaccard threshold to fit any single item, and without a new embedding dependency. If the Step 3 dry-run still misses a known junk fixture, do not tune constants further — implement the doer-proposed deterministic semantic-neighbor retrieval instead and re-run the dry-run; (b) chrome/bare-mention — `evidence_text` contains bracketed link text (`[...]`), or `claim_text`'s main verb is a reference verb (references, mentions, cites, names, points to) with no further predicate; (c) pure-interrogative — `evidence_text` consists only of interrogative sentences; (d) dangling fragment — no terminal punctuation AND begins lowercase AND under 160 characters; (e) any ensemble value-state disagreement. Constants frozen; per-class counts reported. Expected scope ≈30–50.
