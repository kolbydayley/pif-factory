...
from research_factory import (
    app_server_judge_v5_selection_v273_graph_canonicalizer_schema_compat as v273,
)


@pytest.fixture(scope="module")
def lineage() -> dict:
    return v273._validate_lineage()


@pytest.fixture(scope="module")
def turn(lineage: dict) -> dict:
    return v273.prepare_turn(lineage)


def _synthetic_output(turn: dict) -> dict:
    source_root = v273._source_turn_root(
        v273.V249_ROOT, "v249-explicit-applicability-"
    )
    ...
    assert not v273.labels_module._validate_schema(turn["schema"], result, path="$")
    return result


def test_turn_is_ranked_distinct_bounded_architecture(turn: dict) -> None:
    assert v273.MODEL == "gpt-5.6-sol"
    assert v273.EFFORT == "low"
    assert [len(turn["graph_ids"][item]) for item in turn["segment_ids"]] == [31, 1]
    assert turn["prompt_bytes"] <= v273.MAX_PROMPT_BYTES
    assert turn["base_bytes"] <= v273.MAX_BASE_BYTES
    assert turn["schema_bytes"] <= v273.MAX_SCHEMA_BYTES
    assert v273.llm_judge.validate_app_server_output_schema_subset(turn["schema"]) == []
    total, ratio = v273._production_ratio(v273.MAX_COMBINED_TOKENS)
...
def test_projection_accounts_every_graph_node_exactly_once(turn: dict) -> None:
    normalized, _, diagnostics, applicability, coverage = v273.project_output(
...
def test_projection_rejects_duplicate_graph_ownership(turn: dict) -> None:
    ...
    with pytest.raises(v273.V273OutputContractError):
        v273.project_output(output, turn)
...
def test_freeze_and_fake_run_reuse_semantics_with_compatible_schema(tmp_path: Path) -> None:
    root = tmp_path / "v273"
    frozen = v273.freeze_v273(output_dir=root)
    lock = v273.verify_runtime_lock(frozen["runtime_lock"])
    audit = json.loads((root / "v270-schema-compat-audit.json").read_text())
    predecessor = frozen["turn"]
    source_paths = v273._lineage_paths()
    ...
    assert audit["frozen_semantic_request_reused_byte_for_byte"] is True
    assert audit["unsupported_schema_paths"] == [
        "$.$schema",
    ...
    for name, source_name in {
        ...
        assert v273._record(predecessor["paths"][name])["sha256"] == v273._record(
            source_paths[source_name]
        )["sha256"]
    predecessor_schema = json.loads(source_paths["v270_schema"].read_text())
    assert v273.llm_judge.validate_app_server_output_schema_subset(predecessor_schema) == [
        "$.$schema",
        "$.properties.segments.items.properties.events.items.properties.source_graph_event_ids.uniqueItems",
    ]
    assert v273.llm_judge.validate_app_server_output_schema_subset(predecessor["schema"]) == []
    assert not (root / "launch-receipt.json").exists()
    assert not list(root.rglob("capacity.json"))
    assert not list(root.rglob("sidecar.json"))
    fake = _FakeClient(root, _synthetic_output(frozen["turn"]))
    terminal = asyncio.run(
        v273.run_v273(output_dir=root, client_factory=lambda _path: fake)
    )
    assert fake.calls == 1
    assert terminal["terminal_reason"] == "v273_graph_canonicalizer_structural_cost_gate_passed"
