...
def lineage() -> dict:
    return v249._validate_lineage()


@pytest.fixture(scope="module")
def turn(lineage: dict) -> dict:
    return v249.prepare_turn(lineage)
...
def _synthetic_output(turn: dict) -> dict:
    old_path = v249._lineage_paths()["v239_output"]
...
def test_predeclared_canary_is_blind_bounded_and_below_cost_target(turn: dict) -> None:
    assert turn["segment_ids"] == [v249.v239.DENSE_SEGMENT_ID, v249.v239.NO_SIGNAL_SEGMENT_ID]
    assert turn["prompt_bytes"] <= v249.MAX_PROMPT_BYTES
    assert turn["base_bytes"] <= v249.MAX_BASE_BYTES
    assert turn["schema_bytes"] <= v249.MAX_SCHEMA_BYTES
    assert round(v249._production_ratio(v249.MAX_TOTAL_TOKENS)[1], 6) == 0.202727
    assert v249.MODEL == "gpt-5.6-sol"
    assert v249.EFFORT == "high"
    assert v249.MAX_TOTAL_TOKENS == 48_000


def test_explicit_applicability_projects_without_semantic_code(turn: dict) -> None:
    normalized, provenance, diagnostics, receipt = v249.project_output(
...
def test_metric_applicability_conflict_fails_closed(turn: dict) -> None:
    ...
    metric = output["segments"][0]["events"][0]["metric"]
    metric.update(
        {
    ...
    with pytest.raises(v249.V249OutputContractError):
        v249.project_output(output, turn)


def test_freeze_is_presemantic_and_hash_bound(tmp_path: Path) -> None:
    root = tmp_path / "v249"
    frozen = v249.freeze_v249(output_dir=root)
    lock = v249.verify_runtime_lock(frozen["runtime_lock"])
    assert lock["declared_turn_count"] == 1
    assert lock["retry_count"] == 0
    assert lock["max_total_tokens"] == 48_000
...
def test_fake_run_passes_and_has_one_measured_turn(tmp_path: Path) -> None:
    ...
    terminal = asyncio.run(v249.run_v249(output_dir=root, client_factory=lambda _path: fake))
    assert fake.calls == 1
    assert terminal["state"] == "v249_architecture_structural_gate_passed"
...
def test_runtime_lock_rejects_schema_mutation(tmp_path: Path) -> None:
    ...
    with pytest.raises(v249.V249ExplicitApplicabilityError):
        v249.verify_runtime_lock(frozen["runtime_lock"])
