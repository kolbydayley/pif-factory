...
from research_factory.app_server_judge_v5_calibration_v31_diagnostic import (
    DEFAULT_V31_DESIGN_ROOT,
    MAX_PROMPT_BYTES,
    V28_CANARY_CASE_IDS,
    V28_CASE_IDS,
    V31_DIAGNOSTIC_SPEC_VERSION,
    freeze_v31_repair_pass_diagnostic,
...
class FakeV31RepairClient:
    ...
    def _capacity(self, path):
        ...
        remaining = max(1, 17 - len(self.calls))
        payload = {
            "schema_version": RESERVE_CAPACITY_CHECKPOINT_VERSION,
            "checked_at": "2026-07-14T00:00:00Z",
            "policy_path": str(policy_path),
            "policy_sha256": _sha256_file(policy_path),
            "phase_id": "judge_v5_4_v31_repair_pass_diagnostic",
...
class JudgeV5CalibrationV31DiagnosticTests(unittest.IsolatedAsyncioTestCase):
    async def test_freeze_binds_v31_design_and_declares_repair_turns(self):
        with tempfile.TemporaryDirectory() as temp:
            output_dir = Path(temp) / "v31"
            frozen = freeze_v31_repair_pass_diagnostic(output_dir=output_dir)
            frozen_again = freeze_v31_repair_pass_diagnostic(output_dir=output_dir)
            policy = load_reserve_capacity_policy(frozen["capacity_policy"])
            spec = frozen["spec"]
        self.assertEqual(frozen_again["spec"], spec)
        self.assertEqual(spec["schema_version"], V31_DIAGNOSTIC_SPEC_VERSION)
        self.assertEqual(spec["case_count"], 12)
        self.assertEqual(spec["canary_case_count"], 6)
        self.assertEqual(spec["case_ids"], V28_CASE_IDS)
        self.assertEqual(spec["canary_case_ids"], V28_CANARY_CASE_IDS)
        self.assertEqual(len(spec["turn_plan"]), 17)
        self.assertEqual(policy["phase_id"], "judge_v5_4_v31_repair_pass_diagnostic")
        self.assertEqual(
            spec["predecessors"]["v31_design"]["sha256"],
            _sha256_file(DEFAULT_V31_DESIGN_ROOT / "repair-pass-design.json"),
        )
        for shard in spec["frozen_inputs"]["pointwise_shards"]:
            self.assertLessEqual(shard["prompt"]["size_bytes"], MAX_PROMPT_BYTES)

    async def test_perfect_fake_v31_diagnostic_passes_and_authorizes_full_calibration(self):
        with tempfile.TemporaryDirectory() as temp:
            frozen = freeze_v31_repair_pass_diagnostic(output_dir=Path(temp) / "v31")
            client = FakeV31RepairClient(frozen["truth"])

            def factory(_policy_path):
                return client

            terminal = await run_v31_repair_pass_diagnostic(
                output_dir=Path(temp) / "v31",
        ...
        self.assertEqual(terminal["turn_count"], 16)
        self.assertEqual(terminal["usage"]["total_tokens"], 19200)
        self.assertEqual(len(client.calls), 16)
        self.assertEqual(client.enter_count, 1)
        self.assertEqual(client.exit_count, 1)
        names = [Path(call["sidecar_path"]).parent.name for call in client.calls]
        self.assertIn("pointwise-repair-shard-00", names)
        self.assertLess(names.index("pointwise-repair-shard-00"), names.index("neutral-alignment-base-shard-00"))
        self.assertLess(names.index("neutral-alignment-repair-shard-00"), names.index("neutral-alignment-canary-shard-00"))
