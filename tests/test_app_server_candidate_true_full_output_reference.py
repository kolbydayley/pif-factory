...
from research_factory import app_server_candidate_true_full_output_reference as repair
...
def test_contract_builds_exact_full_60_witness_pool():
...
def test_runtime_lock_rejects_direct_request_mutation(tmp_path: Path):
    frozen = repair.freeze_run(tmp_path / "full-output")
    repair.verify_runtime_lock(frozen["runtime_lock"])
    prompt = tmp_path / "full-output" / "judge" / "prompt-ab.private.md"
    prompt.write_text(prompt.read_text(encoding="utf-8") + "drift", encoding="utf-8")
    with pytest.raises(repair.TrueFullOutputReferenceError):
        repair.verify_runtime_lock(frozen["runtime_lock"])
