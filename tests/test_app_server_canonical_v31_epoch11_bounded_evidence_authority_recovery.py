...
from research_factory import (
    app_server_canonical_v31_epoch11_bounded_evidence_authority_recovery as recovery,
)


def _freeze(tmp_path: Path) -> tuple[Path, str]:
    root = tmp_path / "epoch11-bounded-evidence"
    authorization_id = "kolby-epoch11-bounded-evidence-test"
...
def test_epoch11_plan_and_directive_are_checksum_bound() -> None:
    directive_raw = recovery.DIRECTIVE_PATH.read_bytes()
    directive = json.loads(directive_raw)
    plan_path = recovery.PROJECT_ROOT / "automation" / "pif-evaluation-semantic-plan-v11.json"
    plan = json.loads(plan_path.read_bytes())

    assert hashlib.sha256(directive_raw).hexdigest() == recovery.DIRECTIVE_SHA256
    assert plan["plan_epoch"] == 11
...
def test_epoch10_rejection_is_one_measured_bounded_evidence_repair() -> None:
    predecessor = recovery._validate_epoch10_predecessor()

    assert predecessor["measured_usage"]["total_tokens"] == 29_005
...
def test_freeze_binds_five_turns_and_sanitized_repair_request(tmp_path: Path) -> None:
    ...
    assert contract["adopted_development_qa_total_tokens"] == 767_071
    ...
    assert request["base_instructions"].endswith(
        recovery.adapter.EVIDENCE_BOUND_INSTRUCTION
    )
    ...
    assert recovery._load_object(
        recovery._verify_record(
            contract["adopted_evidence"]["epoch10_representability_diagnostic"],
            label="diagnostic",
        ),
        label="diagnostic",
    ) == {
        ...
        "existing_canonical_evidence_max_chars": 1000,
...
def test_partial_attempt_terminalizes_waiting_before_client_creation(
    ...
    assert receipt["terminal_reason"] == (
        "epoch11_partial_attempt_preserved_no_replay"
    )
    ...
    with pytest.raises(recovery.CanonicalV31Epoch11Error, match="attempt drifted"):
...
def test_runtime_lock_tamper_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(recovery.CanonicalV31Epoch11Error, match="runtime lock drifted"):
...
def test_api_key_environment_is_rejected_before_runtime_work(
    ...
    with pytest.raises(
        recovery.CanonicalV31Epoch11Error,
