...
from research_factory import app_server_canonical_v31_tagged_metric_token_id_adjudication_runtime as adjudication
...
def _frozen_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    contract = adjudication.load_contract()
    root = tmp_path / "epoch29"
    changed = copy.deepcopy(contract)
    changed["receipt_path"] = root / "plan-step-receipt.json"
    monkeypatch.setattr(adjudication, "load_contract", lambda: copy.deepcopy(changed))
    adjudication.freeze_run(root)
    return root
...
class _CompletedClient:
    def __init__(self, root: Path, *, identity_from_epoch28: bool = False) -> None:
        self.root = root.resolve()
        self.policy_path = self.root / "capacity-policy.json"
        self.policy = adjudication.reserve.load_reserve_capacity_policy(self.policy_path)
        self.calls: list[dict] = []
        self.identity_from_epoch28 = identity_from_epoch28
    ...
    def _write_capacity(self, path: Path) -> None:
        evaluation = adjudication.reserve.evaluate_reserve_capacity(
            {"primary_used_percent": 10, "rate_limit_reached_type": None},
            policy=self.policy,
            remaining_turn_count=1,
        )
        value = {
            "schema_version": adjudication.reserve.RESERVE_CAPACITY_CHECKPOINT_VERSION,
            "checked_at": "2026-07-20T05:00:00+00:00",
            "policy_path": str(self.policy_path),
            "policy_sha256": adjudication._sha256_file(self.policy_path),  # noqa: SLF001
            "phase_id": adjudication.STEP_ID,
            "turn_name": adjudication.CAPACITY_TURN_NAMES[0],
            "turn_ordinal": 0,
    ...
    async def run_ephemeral_structured_turn(self, **kwargs):
        ...
        schema_text = adjudication._canonical_json(kwargs["output_schema"])  # noqa: SLF001
        usage = {
            "input_tokens": 100,
        ...
        prior_turn = adjudication._validate_epoch28_terminal()["turns"][0]  # noqa: SLF001
        thread_id = (
            prior_turn["thread_id"]
            if self.identity_from_epoch28
            else "fixture-epoch29-thread"
        )
        turn_id = (
            prior_turn["turn_id"] if self.identity_from_epoch28 else "fixture-epoch29-turn"
        )
        sidecar = {
            "schema_version": adjudication.codex_app_server.TURN_SIDECAR_SCHEMA_VERSION,
            "state": "completed",
            "status": "completed",
            "cli_version": adjudication.codex_app_server.PINNED_CODEX_CLI_VERSION,
            "client_version": adjudication.codex_app_server.APP_SERVER_CLIENT_VERSION,
            "protocol_schema_sha256": adjudication.codex_app_server.PROTOCOL_SCHEMA_SHA256,
            "transport": "stdio",
            "auth_type": "chatgpt",
            "plan_type": "pro",
            ...
            "instruction_sources_count": adjudication.EXPECTED_INSTRUCTION_SOURCES_COUNT,
            "instruction_sources_sha256": adjudication.EXPECTED_INSTRUCTION_SOURCES_SHA256,
            "thread_id": thread_id,
            "turn_id": turn_id,
            "app_server_user_agent": "fixture-codex-app-server",
            "output_path": str(output_path),
            "prompt_sha256": adjudication._sha256_text(kwargs["prompt"]),  # noqa: SLF001
            "prompt_bytes": len(kwargs["prompt"].encode("utf-8")),
            "base_instructions_sha256": adjudication._sha256_text(  # noqa: SLF001
                kwargs["base_instructions"]
            ),
            "base_instructions_bytes": len(kwargs["base_instructions"].encode("utf-8")),
            "output_schema_sha256": adjudication._sha256_text(schema_text),  # noqa: SLF001
            "output_schema_bytes": len(schema_text.encode("utf-8")),
            "output_sha256": adjudication._sha256_text(output_text),  # noqa: SLF001
...
def _started_sidecar(root: Path) -> dict:
    variant, base, prompt, schema = adjudication._request_material()  # noqa: SLF001
    schema_text = adjudication._canonical_json(schema)  # noqa: SLF001
    return {
        "schema_version": adjudication.codex_app_server.TURN_SIDECAR_SCHEMA_VERSION,
        "state": "started",
        "status": "in_progress",
        ...
        "instruction_sources_count": adjudication.EXPECTED_INSTRUCTION_SOURCES_COUNT,
        "instruction_sources_sha256": adjudication.EXPECTED_INSTRUCTION_SOURCES_SHA256,
        "thread_id": "fixture-partial-epoch29-thread",
        "turn_id": "fixture-partial-epoch29-turn",
        "output_path": str(
            (root / "turns/epoch29-quality-adjudication/output.private.json").resolve()
        ),
        "prompt_sha256": adjudication._sha256_text(prompt),  # noqa: SLF001
        "prompt_bytes": len(prompt.encode("utf-8")),
        "base_instructions_sha256": adjudication._sha256_text(base),  # noqa: SLF001
...
def test_exact_predecessor_disagreement_is_one_side_free_31_witness_case() -> None:
    contract = adjudication.load_contract()
    bundle = adjudication._base_bundle()  # noqa: SLF001
    case_ids = adjudication.observable_disagreement_case_ids(
        bundle["pool"], bundle["outputs"]
    )
    variant = adjudication.build_adjudication_variant(bundle["pool"], case_ids)

    assert contract["predecessor_receipt"]["state"] == "waiting"
    assert case_ids == list(adjudication.EXPECTED_DISAGREEMENT_CASE_IDS)
    assert len(variant["cases"]) == 1
    assert len(variant["cases"][0]["event_set_a"]) == 31
    assert variant["cases"][0]["event_set_b"] == []
    serialized = json.dumps(variant, sort_keys=True)
    assert adjudication.epoch28.SYSTEM_BASELINE not in serialized
    assert adjudication.epoch28.SYSTEM_CANDIDATE not in serialized
...
def test_freeze_is_zero_call_and_binds_predecessor_reserve_and_transport(
    ...
    lock = adjudication.verify_runtime_lock(root / "runtime-lock.json")
    policy = json.loads((root / "capacity-policy.json").read_text(encoding="utf-8"))

    assert lock["plan_epoch"] == 29
    assert lock["semantic_model_call_cap"] == 1
    assert lock["prior_ab_ba_replay_allowed"] is False
    assert lock["predecessor_records"] == adjudication._expected_predecessor_records()  # noqa: SLF001
    assert policy["ordered_turn_names"] == ["epoch29_quality_adjudication"]
...
def test_completed_fixture_terminalizes_once_with_exact_accounting(
    ...
    receipt = _run(adjudication.run(root, client_factory=lambda: fake))
    ...
    assert receipt["quality_total_tokens_including_epoch28"] == 142_182
    assert receipt["quality_measured_by_this_step"] is True
    assert len(fake.calls) == 1
    assert adjudication.verify_receipt(root)["state"] == "rejected"
...
def test_partial_attempt_terminalizes_waiting_without_replay(
    ...
    root = _frozen_root(tmp_path, monkeypatch)
    adjudication._launch_receipt(root)  # noqa: SLF001
    sidecar_path = root / "turns/epoch29-quality-adjudication/sidecar.json"
    ...
    def must_not_create_client():
        nonlocal called
        called = True
        raise AssertionError("partial adjudication was replayed")

    receipt = _run(adjudication.run(root, client_factory=must_not_create_client))

    assert called is False
    assert receipt["state"] == "waiting"
    assert receipt["terminal_reason"] == (
        "epoch29_partial_or_interrupted_adjudication_preserved_without_replay"
    )
    assert receipt["semantic_model_call_count"] == 1
    assert receipt["unknown_usage_turn_count"] == 1
    assert adjudication.verify_receipt(root)["state"] == "waiting"
...
def test_complete_turn_crash_finalizes_without_new_client(
    ...
    adjudication._launch_receipt(root)  # noqa: SLF001
    variant, base, prompt, schema = adjudication._request_material()  # noqa: SLF001
    turn_root = root / "turns/epoch29-quality-adjudication"
    _run(
        fake.run_ephemeral_structured_turn(
            ...
            batch_size=len(variant["cases"]),
            thread_mode="new_thread",
            timeout_seconds=adjudication.TIMEOUT_SECONDS,
        )
    )
    called = False
    ...
    def must_not_create_client():
        nonlocal called
        called = True
        raise AssertionError("complete adjudication was replayed")
...
def test_prior_thread_turn_identity_reuse_is_rejected(
    ...
    root = _frozen_root(tmp_path, monkeypatch)
    fake = _CompletedClient(root, identity_from_epoch28=True)

    with pytest.raises(adjudication.Epoch29AdjudicationError, match="reused"):
        _run(adjudication.run(root, client_factory=lambda: fake))
...
def test_runtime_rejects_request_tamper(
    ...
    with pytest.raises(adjudication.Epoch29AdjudicationError, match="runtime lock"):
        adjudication.verify_runtime_lock(root / "runtime-lock.json")
...
def test_single_valid_terminal_mirror_repairs_without_client(
    ...
    root = _frozen_root(tmp_path, monkeypatch)
    receipt = _run(
        adjudication.run(root, client_factory=lambda: _CompletedClient(root))
    )
    ...
    recovered = _run(adjudication.run(root, client_factory=must_not_create_client))
    assert called is False
...
def test_external_auth_and_default_root_injection_are_rejected(
    ...
    with pytest.raises(adjudication.Epoch29AdjudicationError, match="forbids injected"):
        _run(adjudication.run(adjudication.DEFAULT_ROOT, client_factory=lambda: object()))
    monkeypatch.setenv("OPENAI_API_KEY", "forbidden-fixture-value")
    with pytest.raises(
        adjudication.Epoch29AdjudicationError,
        match="managed ChatGPT execution rejects API-key/raw-session auth",
    ):
        adjudication._reject_external_auth_material()  # noqa: SLF001


def test_cli_exposes_all_bounded_phases() -> None:
    for command in ("freeze", "verify-runtime", "run", "verify-receipt", "status"):
        assert adjudication._parser().parse_args([command]).command == command  # noqa: SLF001
