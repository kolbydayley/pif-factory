...
from research_factory import app_server_judge_v5_selection_v266_frozen_support as v266


@pytest.fixture(scope="module")
def pool() -> dict:
    return v266.build_support_pool(v266._validate_lineage())
...
def test_pool_is_complete_origin_neutral_and_protocol_frozen(pool: dict) -> None:
    ...
    assert pool["prompt_bytes"] <= v266.MAX_PROMPT_BYTES
    assert pool["schema_bytes"] <= v266.MAX_SCHEMA_BYTES
    assert (
        v266._lineage_paths()["v223_instructions"].read_text(encoding="utf-8")
        == v266.v250.v245.v143.support_base_instructions_v143()
    )


def test_scoring_preserves_one_sided_residual_support(pool: dict) -> None:
    audit, private = v266.v250.v245.v223.score_support_output(
        output=_output(pool),
...
def test_freeze_is_presemantic_and_hash_bound(tmp_path: Path) -> None:
    root = tmp_path / "v266"
    frozen = v266.freeze_v266(output_dir=root)
    lock = v266.verify_runtime_lock(frozen["runtime_lock"])
...
def test_fake_run_is_one_measured_turn_and_authorizes_alignment(
    ...
    terminal = asyncio.run(
        v266.run_v266(
            output_dir=tmp_path / "v266-run", client_factory=lambda _path: fake
        )
    )
    assert fake.calls == 1
...
def test_runtime_lock_rejects_mutated_support_prompt(tmp_path: Path) -> None:
    ...
    with pytest.raises(v266.V266FrozenSupportError):
        v266.verify_runtime_lock(frozen["runtime_lock"])
