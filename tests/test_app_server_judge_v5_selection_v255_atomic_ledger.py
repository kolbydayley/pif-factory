...
from research_factory import app_server_judge_v5_selection_v255_atomic_ledger as v255


@pytest.fixture(scope="module")
def lineage() -> dict:
    return v255._validate_lineage()


@pytest.fixture(scope="module")
def turn(lineage: dict) -> dict:
    return v255.prepare_turn(lineage)
...
def _stateful_event(event: dict, proposition_id: str) -> dict:
...
def _synthetic_output(turn: dict) -> dict:
    ...
    for segment in output["segments"]:
        source_events = segment["events"]
        propositions = []
        events = []
        for index, event in enumerate(source_events):
            proposition_id = f"P{index:02d}"
            propositions.append(
                {
                    "proposition_id": proposition_id,
            ...
            events.append(_stateful_event(event, proposition_id))
        segment["atomic_propositions"] = propositions
        segment["events"] = events
    v255._validate_schema(turn["schema"], output, path="$")
    return output


def test_design_is_blind_bounded_and_below_cost_target(turn: dict) -> None:
    assert turn["segment_ids"] == [
        v249.v239.DENSE_SEGMENT_ID,
        v249.v239.NO_SIGNAL_SEGMENT_ID,
    ]
    assert turn["prompt_bytes"] <= v255.MAX_PROMPT_BYTES
    assert turn["base_bytes"] <= v255.MAX_BASE_BYTES
    assert turn["schema_bytes"] <= v255.MAX_SCHEMA_BYTES
    assert round(v255._production_ratio(v255.MAX_TOTAL_TOKENS)[1], 6) == 0.223591
    assert v255.MODEL == "gpt-5.6-sol"
    assert v255.EFFORT == "high"
    assert v255.MAX_TOTAL_TOKENS == 55_000
    assert "reference answer" in v255.ATOMIC_LEDGER_INSTRUCTIONS


def test_atomic_ledger_projects_one_to_one_without_semantic_code(turn: dict) -> None:
    normalized, provenance, diagnostics, receipt = v255.project_output(
    ...
    assert [row["proposition_count"] for row in receipt["segments"]] == [31, 1]
    assert receipt["all_proposition_semantics_selected_by_llm"] is True
    assert receipt["one_to_one_structure_validated_deterministically"] is True


def test_atomic_ledger_rejects_nonbijective_order(turn: dict) -> None:
    output = _synthetic_output(turn)
    output["segments"][0]["events"][1]["proposition_id"] = "P00"
    with pytest.raises(v255.V255OutputContractError):
        v255.project_output(output, turn)


def test_freeze_is_presemantic_hash_bound_and_ranked(tmp_path: Path) -> None:
    root = tmp_path / "v255"
    frozen = v255.freeze_v255(output_dir=root)
    lock = v255.verify_runtime_lock(frozen["runtime_lock"])
    ...
    assert ranking["selected_architecture_id"] == (
        "single_turn_atomic_ledger_one_to_one_realization"
    )
...
def test_fake_run_passes_once_with_measured_usage(tmp_path: Path) -> None:
    ...
    terminal = asyncio.run(v255.run_v255(output_dir=root, client_factory=lambda _path: fake))
    assert fake.calls == 1
    assert terminal["state"] == "v255_architecture_structural_gate_passed"
...
def test_measured_over_cap_is_quality_cost_failure_not_unknown(tmp_path: Path) -> None:
    root = tmp_path / "v255"
    frozen = v255.freeze_v255(output_dir=root)
    fake = _FakeClient(root, _synthetic_output(frozen["turn"]), total_tokens=56_000)
    terminal = asyncio.run(v255.run_v255(output_dir=root, client_factory=lambda _path: fake))
    assert fake.calls == 1
    ...
    assert terminal["usage"]["total_tokens"] == 56_000
...
def test_runtime_lock_rejects_schema_mutation(tmp_path: Path) -> None:
    ...
    with pytest.raises(v255.V255AtomicLedgerError):
        v255.verify_runtime_lock(frozen["runtime_lock"])
