...
from research_factory import app_server_judge_v5_selection_v257_frozen_alignment as v257


@pytest.fixture(scope="module")
def bundle() -> dict:
    return v257.build_alignment_bundle(v257._validate_lineage())


def _equivalent_output(bundle: dict, turn: dict) -> dict:
    mapping = [row for row in bundle["mapping"]["rows"] if row["witness_id"]]
    references = sorted(
        row["witness_id"] for row in mapping if row["origin"] == "reference"
    )
    candidates = sorted(
        row["witness_id"] for row in mapping if row["origin"] == "candidate"
    )
    groups = []
    pairs = []
    ...
    assert not v257.judge.validate_neutral_alignment_output(output, turn["value"])
    return output
...
def test_bundle_is_origin_neutral_complete_and_balanced(bundle: dict) -> None:
    ...
    assert sum(row["origin"] == "candidate" for row in bundle["mapping"]["rows"]) == 31
    base_ids = [
    ...
    assert len(base_ids) == 58
    assert canary_ids == list(reversed(base_ids))
    assert bundle["turns"][0]["prompt_bytes"] <= v257.MAX_PROMPT_BYTES
    assert bundle["turns"][0]["schema_bytes"] <= v257.MAX_SCHEMA_BYTES


def test_freeze_is_presemantic_and_hash_bound(tmp_path: Path) -> None:
    root = tmp_path / "v257"
    frozen = v257.freeze_v257(output_dir=root)
    lock = v257.verify_runtime_lock(frozen["runtime_lock"])
...
def test_fake_run_freezes_winner_and_authorizes_holdout(tmp_path: Path) -> None:
    ...
    frozen = v257.freeze_v257(output_dir=root)
    outputs = [
        _equivalent_output(frozen["bundle"], turn)
        for turn in frozen["bundle"]["turns"]
    ]
    fake = _FakeClient(root, outputs)
    terminal = asyncio.run(
        v257.run_v257(output_dir=root, client_factory=lambda _path: fake)
    )
    assert fake.calls == 2
...
def test_runtime_lock_rejects_mutated_alignment_prompt(tmp_path: Path) -> None:
    ...
    with pytest.raises(v257.V257FrozenAlignmentError):
        v257.verify_runtime_lock(frozen["runtime_lock"])
