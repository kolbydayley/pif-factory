...
from research_factory import app_server_flat_request_model_lane as lane
...
def _fixture(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, Path]:
    ...
    config = {
        ...
        "experiment_id": "synthetic_flat_model_lane",
    ...
    return _write_json(root / "experiment-config.json", config), request_prompt
...
def test_freeze_copies_exact_request_and_verifies(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config_path, source_prompt = _fixture(tmp_path, monkeypatch)
    frozen = lane.freeze_experiment(config_path)
    assert frozen["turn"]["prompt"].read_bytes() == source_prompt.read_bytes()
    assert lane.verify_frozen(config_path.parent)["config"]["model"] == lane.MODEL


def test_success_runs_once_projects_evidence_and_uses_no_count_proxy(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_nonliteral_metric_is_measured_semantic_rejection_without_retry(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config_path, _ = _fixture(tmp_path, monkeypatch)
    output = _output()
    ...
    assert terminal["terminal_reason"] == "flat_request_model_lane_structural_or_cost_gate_not_passed"
...
def test_lineage_and_copied_request_mutations_fail_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config_path, source_prompt = _fixture(tmp_path, monkeypatch)
    frozen = lane.freeze_experiment(config_path)
    source_prompt.write_text("mutated source prompt\n", encoding="utf-8")
    with pytest.raises(lane.FlatRequestModelLaneError):
