...
from research_factory import app_server_canonical_v31_opaque_source_unit_adjudication_runtime as source_unit
...
class _CompletedClient:
    def __init__(
        ...
        root: Path,
        *,
        identity_from_epoch28: bool = False,
    ...
    async def run_ephemeral_structured_turn(self, **kwargs):
        ...
        prior_turn = source_unit._validate_epoch28_terminal()["turns"][0]  # noqa: SLF001
        thread_id = (
            prior_turn["thread_id"]
            if self.identity_from_epoch28
            else "fixture-epoch31-thread"
        )
        turn_id = (
            prior_turn["turn_id"] if self.identity_from_epoch28 else "fixture-epoch31-turn"
        )
        sidecar = {
...
def _started_sidecar(root: Path) -> dict:
    ...
    return {
        ...
        "instruction_sources_count": source_unit.EXPECTED_INSTRUCTION_SOURCES_COUNT,
        "instruction_sources_sha256": source_unit.EXPECTED_INSTRUCTION_SOURCES_SHA256,
        "thread_id": "fixture-partial-epoch31-thread",
        "turn_id": "fixture-partial-epoch31-turn",
        "output_path": str(
...
def test_exact_predecessor_disagreement_is_one_side_free_31_witness_case() -> None:
    ...
    assert contract["predecessor_receipt"]["state"] == "rejected"
    ...
    assert len(variant["cases"][0]["source_units"]) == 68
    assert max(len(unit["text"]) for unit in variant["cases"][0]["source_units"]) <= 900
    serialized = json.dumps(variant, sort_keys=True)
    assert source_unit.epoch28.SYSTEM_BASELINE not in serialized
    assert source_unit.epoch28.SYSTEM_CANDIDATE not in serialized
...
def test_source_unit_projection_is_exact_and_standard_judge_valid() -> None:
    ...
    variant = source_unit.build_adjudication_variant(bundle["pool"], case_ids)
    output = _valid_output(variant)
    standard = source_unit.epoch29.build_adjudication_variant(
...
def test_freeze_is_zero_call_and_binds_predecessor_reserve_and_transport(
    ...
    assert policy["ordered_turn_names"] == ["epoch31_quality_adjudication"]
...
def test_completed_fixture_terminalizes_once_with_exact_accounting(
    ...
    assert receipt["quality_total_tokens_including_prior_quality_calls"] == 191_586
...
def test_invalid_unit_reference_terminalizes_measured_rejection_without_retry(
    ...
    assert receipt["state"] == "rejected"
    assert receipt["terminal_reason"] == "epoch31_source_unit_output_rejected"
    assert receipt["semantic_model_call_count"] == 1
    assert receipt["measured_model_call_count"] == 1
    assert receipt["unknown_usage_turn_count"] == 0
    assert receipt["usage"]["total_tokens"] == 120
...
def test_partial_attempt_terminalizes_waiting_without_replay(
    ...
    assert receipt["terminal_reason"] == (
        "epoch31_partial_or_interrupted_adjudication_preserved_without_replay"
    )
    assert receipt["semantic_model_call_count"] == 1
...
def test_prior_thread_turn_identity_reuse_is_rejected(
    ...
    with pytest.raises(source_unit.Epoch31SourceUnitAdjudicationError, match="reused"):
...
def test_runtime_rejects_request_tamper(
    ...
    with pytest.raises(source_unit.Epoch31SourceUnitAdjudicationError, match="runtime lock"):
...
def test_external_auth_and_default_root_injection_are_rejected(
    ...
    with pytest.raises(source_unit.Epoch31SourceUnitAdjudicationError, match="forbids injected"):
    ...
    with pytest.raises(
        source_unit.Epoch31SourceUnitAdjudicationError,
