...
def test_v86_selection_excludes_all_three_prior_diagnostic_cohorts() -> None:
    ...
    assert canary["task_count"] == audit["permutation_canary_count"] == 4


def test_v86_perfect_result_authorizes_full_development_only() -> None:
    _, truth, _, _ = _inputs()
    primary, canary = _perfect_outputs(truth)
    score = score_v86(primary, canary, truth)
...
def test_v86_canary_disagreement_authorizes_only_bounded_repair() -> None:
    _, truth, _, _ = _inputs()
    primary, canary = _perfect_outputs(truth)
...
def test_v86_freeze_is_six_presemantic_turns_and_immutable() -> None:
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp) / "v86"
        frozen = freeze_v86(output_dir=root)
        assert freeze_v86(output_dir=root)["spec"] == frozen["spec"]
