...
from research_factory import app_server_judge_v5_selection_v264_local_owner_adjudication as v264


@pytest.fixture(scope="module")
def lineage() -> dict:
    return v264._validate_lineage()


@pytest.fixture(scope="module")
def bundle(lineage: dict) -> dict:
    return v264.build_local_bundle(lineage)
...
def _base_local_output(bundle: dict) -> dict:
    ...
    output = {"cases": [v264._restrict_raw_case(raw_case, set(bundle["closure"]))]}
    assert not v264.judge.validate_neutral_alignment_output(
...
def test_mechanical_closure_is_small_origin_neutral_and_complete(bundle: dict) -> None:
    ...
    assert len(bundle["closure"]) == 6
    assert len(bundle["local_input"]["cases"][0]["witnesses"]) == 6
    ...
    assert bundle["schema_bytes"] <= v264.MAX_SCHEMA_BYTES


def test_base_local_owner_reconciles_to_passing_score(bundle: dict) -> None:
    _, reconciled, score = v264.reconcile_and_score(
        bundle=bundle, output=_base_local_output(bundle)
    )
    assert reconciled["mechanically_local_adjudication_witness_count"] == 6
...
def test_freeze_is_presemantic_hash_bound_and_zero_retry(tmp_path: Path) -> None:
    root = tmp_path / "v264"
    frozen = v264.freeze_v264(output_dir=root)
    lock = v264.verify_runtime_lock(frozen["runtime_lock"])
    assert lock["declared_turn_count"] == 1
    assert lock["adjudication_call_cap"] == 1
    assert lock["mechanical_local_witness_count"] == 6
...
def test_fake_run_freezes_winner_and_authorizes_holdout(tmp_path: Path) -> None:
    ...
    terminal = asyncio.run(
        v264.run_v264(output_dir=root, client_factory=lambda _path: fake)
    )
    assert fake.calls == 1
...
def test_runtime_lock_rejects_mutated_prompt(tmp_path: Path) -> None:
    ...
    with pytest.raises(v264.V264LocalOwnerError):
        v264.verify_runtime_lock(frozen["runtime_lock"])
