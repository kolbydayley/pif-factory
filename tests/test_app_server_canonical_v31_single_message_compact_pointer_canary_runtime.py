...
from research_factory import app_server_canonical_v31_single_message_compact_pointer_canary_runtime as runtime
from research_factory import app_server_one_turn_canary_runtime as one_turn
from test_app_server_canonical_v31_unit_local_pointer_canary_runtime import (
    _no_signal_output,
)


AUTHORIZATION_ID = "kolby-epoch19-single-message-compact-test"
...
def prepared_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    root = tmp_path / "epoch19"
    assert runtime.prepare_canary(root)["state"] == "verified_zero_call_runtime"
    return root


def _complete_fixture(root: Path, *, total_tokens: int) -> dict:
    ...
    thread = SimpleNamespace(
        thread_id="thread-epoch19-completed-fixture",
    ...
    one_turn._write_json(
        paths["dispatch"],
        {
            ...
            "initial_capacity": one_turn._capacity_records(
                paths, "initial_capacity"
            ),
            "preturn_capacity": one_turn._capacity_records(
    ...
    sidecar = {
        ...
        "finished_at": "2026-07-19T22:00:01+00:00",
        ...
        "turn_id": "turn-epoch19-completed-fixture",
        "model": runtime.adapter.MODEL,
        ...
        "stderr_sha256": "0" * 64,
        "stderr_bytes": 0,
        "output_sha256": runtime.adapter.base._output_message_hash(  # noqa: SLF001
            paths["output"]
        ),
...
def test_directive_plan_predecessor_and_semantic_identity_are_exact() -> None:
...
def test_prepare_freezes_single_message_shape(prepared_root: Path) -> None:
    ...
    compact_request = runtime.adapter._compact_request(request)  # noqa: SLF001
    assert status["semantic_model_call_count"] == 0
    assert status["base_bytes"] == runtime.EXPECTED_BASE_BYTES
    assert status["schema_bytes"] == runtime.EXPECTED_SCHEMA_BYTES
    assert status["request_bytes"] == runtime.EXPECTED_REQUEST_BYTES
    assert request["output_schema"] == compact_request["output_schema"]
    assert request["prompt"] == compact_request["prompt"]
    assert request["base_instructions"].startswith(
        runtime.adapter.SINGLE_MESSAGE_INSTRUCTIONS
    )
...
def test_completed_turn_uses_cumulative_usage_and_passes_cost_gate(
    prepared_root: Path,
) -> None:
    receipt = _complete_fixture(prepared_root, total_tokens=56_525)
    assert receipt["state"] == "passed"
    assert receipt["new_measured_usage"]["total_tokens"] == 56_525
    assert receipt["production_amortized_ratio"] == runtime._production_ratio(56_525)
    assert all(receipt["production_cost_gate"].values())
    assert runtime.verify_receipt(prepared_root) == receipt


def test_total_above_acceptance_ceiling_is_rejected(prepared_root: Path) -> None:
    receipt = _complete_fixture(prepared_root, total_tokens=73_001)
    assert receipt["state"] == "rejected"
    assert receipt["failed_checks"] == ["measured_total_token_acceptance_ceiling"]
    assert runtime.verify_receipt(prepared_root) == receipt


def test_runtime_lock_rejects_base_instruction_tamper(prepared_root: Path) -> None:
    base = prepared_root / "prepared-turn/base-instructions.private.md"
    base.write_text(base.read_text(encoding="utf-8") + "tamper", encoding="utf-8")
