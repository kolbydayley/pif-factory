...
from research_factory import app_server_canonical_v31_unit_local_pointer_canary_runtime as runtime
from research_factory import app_server_one_turn_canary_runtime as one_turn


AUTHORIZATION_ID = "kolby-epoch17-unit-local-pointer-test"
...
def _complete_fixture(root: Path, *, total_tokens: int) -> dict:
    ...
    thread = SimpleNamespace(
        thread_id="thread-epoch17-completed-fixture",
    ...
    sidecar = {
        ...
        "turn_id": "turn-epoch17-completed-fixture",
        "model": runtime.adapter.MODEL,
...
def test_prepare_freezes_exact_full_episode_shape(prepared_root: Path) -> None:
...
def test_partial_attempt_is_waiting_without_replay(
    ...
    assert "no_replay" not in receipt["failed_checks"]
...
def test_runtime_lock_rejects_prompt_tamper(prepared_root: Path) -> None:
