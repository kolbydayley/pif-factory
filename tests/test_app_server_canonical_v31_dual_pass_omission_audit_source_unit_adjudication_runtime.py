...
from research_factory import app_server_canonical_v31_dual_pass_omission_audit_source_unit_adjudication_runtime as source_unit
...
class _CompletedClient:
    def __init__(
        ...
        root: Path,
        *,
        identity_from_epoch38: bool = False,
        ...
        self.identity_from_epoch38 = identity_from_epoch38
    ...
    async def run_ephemeral_structured_turn(self, **kwargs):
        ...
        prior_turn = source_unit._validate_epoch38_terminal()["turns"][0]  # noqa: SLF001
        thread_id = (
            prior_turn["thread_id"]
            if self.identity_from_epoch38
            else "fixture-epoch39-thread"
        )
        turn_id = (
            prior_turn["turn_id"] if self.identity_from_epoch38 else "fixture-epoch39-turn"
        )
        sidecar = {
...
def _started_sidecar(root: Path) -> dict:
    ...
    return {
        ...
        "instruction_sources_count": source_unit.EXPECTED_INSTRUCTION_SOURCES_COUNT,
        "instruction_sources_sha256": source_unit.EXPECTED_INSTRUCTION_SOURCES_SHA256,
        "thread_id": "fixture-partial-epoch39-thread",
        "turn_id": "fixture-partial-epoch39-turn",
        "output_path": str(
...
def test_exact_predecessor_disagreement_is_two_side_free_cases_covering_all_witnesses() -> None:
    ...
    assert [len(case["event_set_a"]) for case in variant["cases"]] == [40, 33]
    ...
    assert source_unit.epoch38.SYSTEM_BASELINE not in serialized
    assert source_unit.epoch38.SYSTEM_CANDIDATE not in serialized
...
def test_freeze_is_zero_call_and_binds_predecessor_reserve_and_transport(
    ...
    assert policy["ordered_turn_names"] == ["epoch39_quality_adjudication"]
...
def test_completed_fixture_terminalizes_once_with_exact_accounting(
    ...
    assert receipt["quality_total_tokens_including_prior_quality_calls"] == 151_221
...
def test_invalid_unit_reference_terminalizes_measured_rejection_without_retry(
    ...
    assert receipt["state"] == "rejected"
    assert receipt["terminal_reason"] == "epoch39_source_unit_output_rejected"
    assert receipt["semantic_model_call_count"] == 1
    assert receipt["measured_model_call_count"] == 1
    assert receipt["unknown_usage_turn_count"] == 0
    assert receipt["usage"]["total_tokens"] == 120
...
def test_partial_attempt_terminalizes_waiting_without_replay(
    ...
    assert receipt["terminal_reason"] == (
        "epoch39_partial_or_interrupted_adjudication_preserved_without_replay"
    )
    assert receipt["semantic_model_call_count"] == 1
...
def test_prior_thread_turn_identity_reuse_is_rejected(
    ...
    root = _frozen_root(tmp_path, monkeypatch)
    fake = _CompletedClient(root, identity_from_epoch38=True)

    with pytest.raises(source_unit.Epoch39SourceUnitAdjudicationError, match="reused"):
...
def test_runtime_rejects_request_tamper(
    ...
    with pytest.raises(source_unit.Epoch39SourceUnitAdjudicationError, match="runtime lock"):
...
def test_external_auth_and_default_root_injection_are_rejected(
    ...
    with pytest.raises(source_unit.Epoch39SourceUnitAdjudicationError, match="forbids injected"):
    ...
    with pytest.raises(
        source_unit.Epoch39SourceUnitAdjudicationError,
