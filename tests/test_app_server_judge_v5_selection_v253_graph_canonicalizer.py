...
from research_factory import app_server_judge_v5_selection_v253_graph_canonicalizer as v253


@pytest.fixture(scope="module")
def lineage() -> dict:
    return v253._validate_lineage()


@pytest.fixture(scope="module")
def turn(lineage: dict) -> dict:
    return v253.prepare_turn(lineage)


def _synthetic_output(turn: dict) -> dict:
    source_root = v253._source_turn_root(
        v253.V249_ROOT, "v249-explicit-applicability-"
    )
    ...
    assert not v253.labels_module._validate_schema(turn["schema"], result, path="$")
    return result


def test_turn_is_ranked_distinct_bounded_architecture(turn: dict) -> None:
    assert v253.MODEL == "gpt-5.6-sol"
    assert v253.EFFORT == "low"
    assert [len(turn["graph_ids"][item]) for item in turn["segment_ids"]] == [31, 1]
    assert turn["prompt_bytes"] <= v253.MAX_PROMPT_BYTES
    assert turn["base_bytes"] <= v253.MAX_BASE_BYTES
    assert turn["schema_bytes"] <= v253.MAX_SCHEMA_BYTES
    total, ratio = v253._production_ratio(v253.MAX_COMBINED_TOKENS)
...
def test_projection_accounts_every_graph_node_exactly_once(turn: dict) -> None:
    normalized, _, diagnostics, applicability, coverage = v253.project_output(
...
def test_projection_rejects_duplicate_graph_ownership(turn: dict) -> None:
    ...
    with pytest.raises(v253.V253OutputContractError):
        v253.project_output(output, turn)


def test_freeze_is_presemantic_and_hash_bound(tmp_path: Path) -> None:
    root = tmp_path / "v253"
    frozen = v253.freeze_v253(output_dir=root)
    lock = v253.verify_runtime_lock(frozen["runtime_lock"])
    audit = json.loads((root / "v252-measured-failure-audit.json").read_text())
    assert lock["declared_turn_count"] == 1
    assert lock["retry_count"] == 0
    assert lock["max_total_tokens"] == 35_000
    assert audit["authoritative_total_tokens"] == 98_728
    assert audit["predecessor_output_adopted"] is False
...
def test_fake_run_passes_structural_gate_without_holdout(tmp_path: Path) -> None:
    ...
    terminal = asyncio.run(
        v253.run_v253(output_dir=root, client_factory=lambda _path: fake)
    )
    assert fake.calls == 1
    assert terminal["terminal_reason"] == "v253_graph_canonicalizer_structural_cost_gate_passed"
...
def test_measured_over_cap_failure_is_accounted_not_unknown(tmp_path: Path) -> None:
    root = tmp_path / "v253"
    frozen = v253.freeze_v253(output_dir=root)
    ...
    terminal = v253._failure_terminal(root, frozen, RuntimeError("bound"))
    assert terminal["usage_status"] == "complete"
    assert terminal["accounting_complete"] is True
    assert terminal["measured_token_bound_exceeded"] is True
    assert terminal["terminal_reason"] == "v253_graph_canonicalizer_structural_quality_or_cost_gate_not_passed"
