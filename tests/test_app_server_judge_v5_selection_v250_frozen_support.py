...
from research_factory import app_server_judge_v5_selection_v250_frozen_support as v250
...
def test_pool_is_complete_origin_neutral_and_uses_unchanged_protocol() -> None:
    pool = v250.build_support_pool(v250._validate_lineage())
    ...
    assert pool["prompt_bytes"] <= v250.MAX_PROMPT_BYTES
    assert pool["schema_bytes"] <= v250.MAX_SCHEMA_BYTES
    assert (
        v250._lineage_paths()["v223_instructions"].read_text(encoding="utf-8")
...
def test_scoring_preserves_one_sided_residual_support() -> None:
    pool = v250.build_support_pool(v250._validate_lineage())
    audit, private = v250.v245.v223.score_support_output(
        output=_output(pool),
...
def test_freeze_is_presemantic_and_hash_bound(tmp_path: Path) -> None:
    root = tmp_path / "v250"
    frozen = v250.freeze_v250(output_dir=root)
    lock = v250.verify_runtime_lock(frozen["runtime_lock"])
...
def test_fake_run_is_one_measured_turn_and_authorizes_alignment(tmp_path: Path) -> None:
    ...
    terminal = asyncio.run(
        v250.run_v250(output_dir=tmp_path / "v250-run", client_factory=lambda _path: fake)
    )
    assert fake.calls == 1
...
def test_runtime_lock_rejects_mutated_support_prompt(tmp_path: Path) -> None:
    ...
    with pytest.raises(v250.V250FrozenSupportError):
        v250.verify_runtime_lock(frozen["runtime_lock"])
