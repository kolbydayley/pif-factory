...
from research_factory import app_server_judge_v5_selection_v251_frozen_alignment as v251


@pytest.fixture(scope="module")
def bundle() -> dict:
    return v251.build_alignment_bundle(v251._validate_lineage())
...
def _equivalent_output(bundle: dict, turn: dict) -> dict:
    ...
    assert not v251.judge.validate_neutral_alignment_output(output, turn["value"])
    return output
...
def test_bundle_is_origin_neutral_complete_and_balanced(bundle: dict) -> None:
    ...
    assert bundle["turns"][0]["prompt_bytes"] <= v251.MAX_PROMPT_BYTES
    assert bundle["turns"][0]["schema_bytes"] <= v251.MAX_SCHEMA_BYTES


def test_freeze_is_presemantic_and_hash_bound(tmp_path: Path) -> None:
    root = tmp_path / "v251"
    frozen = v251.freeze_v251(output_dir=root)
    lock = v251.verify_runtime_lock(frozen["runtime_lock"])
...
def test_fake_run_freezes_winner_and_authorizes_holdout(tmp_path: Path) -> None:
    ...
    terminal = asyncio.run(v251.run_v251(output_dir=root, client_factory=lambda _path: fake))
    assert fake.calls == 2
...
def test_runtime_lock_rejects_mutated_alignment_prompt(tmp_path: Path) -> None:
    ...
    with pytest.raises(v251.V251FrozenAlignmentError):
        v251.verify_runtime_lock(frozen["runtime_lock"])
