...
from research_factory import (
    app_server_canonical_v31_owner_local_semantic_shard_canary_runtime as runtime,
)
...
def test_request_is_exact_owner_local_medium_effort_canary() -> None:
    request = _request()
    ...
    assert integrity["shard_protocol_version"] == runtime.adapter.SHARD_PROTOCOL_VERSION
    assert integrity["semantic_shard_names"] == list(runtime.adapter.SHARD_NAMES)
    assert integrity["model_assigns_each_event_to_exactly_one_semantic_shard"] is True
    assert integrity["deterministic_shard_concatenation_only"] is True


def test_epoch40_unknown_usage_timeout_is_directly_bound_and_never_replayed() -> None:
    runtime._validate_predecessor(full_verify=False)  # noqa: SLF001
    records = runtime._predecessor_records()  # noqa: SLF001
    predecessor = records["epoch40_interrupted_unknown_usage_attempt"]
    assert predecessor["receipt"]["sha256"] == runtime.EPOCH40_HASHES["receipt"]
    assert predecessor["sidecar"]["sha256"] == runtime.EPOCH40_HASHES["sidecar"]
    assert predecessor["dispatch"]["sha256"] == runtime.EPOCH40_HASHES["dispatch"]
    assert runtime.PREDECESSOR_MODEL_CALLS == 19
    assert runtime.PREDECESSOR_UNKNOWN_USAGE == 3
    assert runtime.PREDECESSOR_MEASURED_TOKENS == 846_632
    assert not (runtime.EPOCH40_ROOT / "turn/output.private.json").exists()
...
def test_directive_predeclares_distinct_architecture_and_nonsemantic_boundary() -> None:
    ...
    assert contract["semantic_shard_names"] == list(runtime.adapter.SHARD_NAMES)
    assert contract["model_assigns_each_event_to_exactly_one_semantic_shard"] is True
    assert contract["model_authors_every_normalized_table_value"] is True
    assert contract["model_authors_every_full_canonical_event_value"] is True
    assert contract["deterministic_shard_concatenation_only"] is True
    assert contract["cross_shard_duplicate_rejection_allowed"] is False
...
def test_receipt_metadata_passes_only_structural_and_cost_gate() -> None:
    ...
    assert metadata["aggregate_architecture_semantic_model_call_count"] == 20
    assert metadata["aggregate_architecture_unknown_usage_turn_count"] == 3
    assert metadata["aggregate_architecture_measured_total_tokens"] == 871_632
...
def test_zero_call_runtime_freeze_rejects_request_tamper(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        (runtime.SemanticShardCanaryError, runtime.adapter.CanonicalV31EpisodeBatchError)
    ):
