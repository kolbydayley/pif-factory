...
from research_factory import app_server_judge_v5_selection_v276_frozen_support as v276
...
def test_pool_is_complete_origin_neutral_and_uses_unchanged_protocol() -> None:
    pool = v276.build_support_pool(v276._validate_lineage())
    assert len(pool["cases"]) == 2
    assert len(pool["units"]) == 60
    assert sum(row["origin"] == "reference" for row in pool["origin_rows"]) == 27
    assert sum(row["origin"] == "candidate" for row in pool["origin_rows"]) == 33
    assert all(
        set(row) == {"case_id", "witness_id", "proposition", "source_excerpt"}
        for row in pool["support_value"]["units"]
    )
    assert pool["prompt_bytes"] <= v276.MAX_PROMPT_BYTES
    assert pool["schema_bytes"] <= v276.MAX_SCHEMA_BYTES
    assert (
        v276._lineage_paths()["v223_instructions"].read_text(encoding="utf-8")
        == v276.v245.v143.support_base_instructions_v143()
    )


def test_scoring_preserves_one_sided_residual_support() -> None:
    pool = v276.build_support_pool(v276._validate_lineage())
    audit, private = v276.v245.v223.score_support_output(
        output=_output(pool),
        support_value=pool["support_value"],
        origin_rows=pool["origin_rows"],
    )
    assert audit["witness_count"] == 60
    ...
    assert len(private["rows"]) == 60


def test_freeze_is_presemantic_and_hash_bound(tmp_path: Path) -> None:
    root = tmp_path / "v276"
    frozen = v276.freeze_v276(output_dir=root)
    lock = v276.verify_runtime_lock(frozen["runtime_lock"])
    assert lock["declared_turn_count"] == 1
    assert lock["retry_count"] == 0
    assert frozen["spec"]["witness_count"] == 60
...
def test_fake_run_is_one_measured_turn_and_authorizes_alignment(tmp_path: Path) -> None:
    pool = v276.build_support_pool(v276._validate_lineage())
    fake = _FakeClient(_output(pool))
    terminal = asyncio.run(
        v276.run_v276(output_dir=tmp_path / "v276-run", client_factory=lambda _path: fake)
    )
    assert fake.calls == 1
...
def test_runtime_lock_rejects_mutated_support_prompt(tmp_path: Path) -> None:
    ...
    with pytest.raises(v276.V276FrozenSupportError):
        v276.verify_runtime_lock(frozen["runtime_lock"])
