...
from research_factory import app_server_judge_v5_selection_v268_inventory_atomic_realization as v268


@pytest.fixture(scope="module")
def lineage() -> dict:
    return v268._validate_lineage()


@pytest.fixture(scope="module")
def turn(lineage: dict) -> dict:
    return v268.prepare_turn(lineage)
...
def _synthetic_output(turn: dict) -> dict:
    ...
    v268._validate_schema(turn["schema"], output, path="$")
    return output


def test_design_adopts_measured_inventory_and_fits_cost_target(turn: dict) -> None:
    assert turn["segment_ids"] == [v268.v234.DENSE_SEGMENT_ID, v268.v234.NO_SIGNAL_SEGMENT_ID]
    assert [len(row["propositions"]) for row in turn["inventory"]["segments"]] == [32, 1]
    assert turn["prompt_bytes"] <= v268.MAX_PROMPT_BYTES
    assert turn["base_bytes"] <= v268.MAX_BASE_BYTES
    assert turn["schema_bytes"] <= v268.MAX_SCHEMA_BYTES
    assert v268.ADOPTED_INVENTORY_TOKENS + v268.MAX_NEW_TOKENS == 73_362
    assert round(v268._production_accounting(v268.MAX_COMBINED_TOKENS)[1], 6) == 0.278319
    assert "reference answer" in v268.v234.REALIZATION_INSTRUCTIONS
    assert v268.MODEL == "gpt-5.6-sol"
    assert v268.EFFORT == "low"


def test_atomic_realization_projects_exact_once_ownership(turn: dict) -> None:
    normalized, provenance, diagnostics, ownership = v268.project_output(
...
def test_atomic_realization_rejects_duplicate_proposition_ownership(turn: dict) -> None:
    ...
    with pytest.raises(v268.V268OutputContractError):
        v268.project_output(output, turn)
...
def test_freeze_and_fake_run_are_hash_bound_single_turn(tmp_path: Path) -> None:
    root = tmp_path / "v268"
    frozen = v268.freeze_v268(output_dir=root)
    lock = v268.verify_runtime_lock(frozen["runtime_lock"])
    ...
    terminal = asyncio.run(v268.run_v268(output_dir=root, client_factory=lambda _path: fake))
    assert fake.calls == 1
    assert terminal["state"] == "v268_architecture_structural_gate_passed"
