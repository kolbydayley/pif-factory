...
from research_factory import (
    app_server_canonical_v31_epoch10_unit_projection_authority_recovery as recovery,
)


def _freeze(tmp_path: Path) -> tuple[Path, str]:
    root = tmp_path / "epoch10-unit-projection"
    authorization_id = "kolby-epoch10-unit-projection-test"
...
def test_epoch10_plan_and_directive_are_checksum_bound() -> None:
    directive_raw = recovery.DIRECTIVE_PATH.read_bytes()
    directive = json.loads(directive_raw)
    plan_path = recovery.PROJECT_ROOT / "automation" / "pif-evaluation-semantic-plan-v10.json"
    plan = json.loads(plan_path.read_bytes())

    assert hashlib.sha256(directive_raw).hexdigest() == recovery.DIRECTIVE_SHA256
    assert plan["plan_epoch"] == 10
...
def test_epoch9_partition_is_one_projectable_and_one_fresh_repair() -> None:
    predecessor = recovery._validate_epoch9_predecessor()

    assert predecessor["measured_usage"]["total_tokens"] == 33_044
    assert predecessor["projected_valid_label"]["segment_id"] == (
        "seg_536dd966606a4980bff030f7"
    )
    assert predecessor["fresh_repair_segment_id"] == (
        "seg_75643e2d9d620af459f9dee3"
    )
    assert predecessor["projection_receipt"]["semantic_values_changed"] is False
    assert predecessor["projection_receipt"]["projected_item_count"] == 4
    assert "metric.raw_text must be an exact evidence substring" in (
        predecessor["sanitized_invalid_diagnostic"]["diagnostic_path"]
    )
...
def test_freeze_binds_five_turns_and_sanitized_repair_request(tmp_path: Path) -> None:
    ...
    assert contract["adopted_development_qa_total_tokens"] == 738_066
    ...
    assert recovery._load_object(
        recovery._verify_record(
            contract["adopted_evidence"]["epoch9_remaining_diagnostic"],
            label="diagnostic",
        ),
        label="diagnostic",
    )["transcript_text_present"] is False
...
def test_partial_attempt_terminalizes_waiting_before_client_creation(
    ...
    assert receipt["terminal_reason"] == (
        "epoch10_partial_attempt_preserved_no_replay"
    )
    ...
    with pytest.raises(recovery.CanonicalV31Epoch10Error, match="attempt drifted"):
...
def test_runtime_lock_tamper_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(recovery.CanonicalV31Epoch10Error, match="runtime lock drifted"):
...
def test_api_key_environment_is_rejected_before_runtime_work(
    ...
    with pytest.raises(
        recovery.CanonicalV31Epoch10Error,
