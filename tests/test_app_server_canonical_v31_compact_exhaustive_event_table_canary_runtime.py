...
from research_factory import (
    app_server_canonical_v31_compact_exhaustive_event_table_canary_runtime as runtime,
)
...
def test_request_is_exact_compact_table_high_effort_canary() -> None:
    request = _request()
    ...
    assert integrity["compact_table_protocol_version"] == (
        runtime.adapter.COMPACT_TABLE_PROTOCOL_VERSION
    )
    assert integrity["deterministic_reference_expansion_only"] is True


def test_epoch39_quality_rejection_is_directly_bound_and_never_replayed() -> None:
    runtime._validate_predecessor(full_verify=False)  # noqa: SLF001
    records = runtime._predecessor_records()  # noqa: SLF001
    assert records["epoch39_quality_rejection"]["receipt"]["sha256"] == (
        runtime.EPOCH39_HASHES["receipt"]
    )
    assert runtime.PREDECESSOR_MODEL_CALLS == 18
...
def test_directive_predeclares_distinct_architecture_and_nonsemantic_boundary() -> None:
    ...
    assert contract["deterministic_reference_expansion_only"] is True
...
def test_receipt_metadata_passes_only_structural_and_cost_gate() -> None:
    ...
    assert metadata["aggregate_architecture_semantic_model_call_count"] == 19
...
def test_zero_call_runtime_freeze_rejects_request_tamper(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        (runtime.CompactTableCanaryError, runtime.adapter.CanonicalV31EpisodeBatchError)
    ):
