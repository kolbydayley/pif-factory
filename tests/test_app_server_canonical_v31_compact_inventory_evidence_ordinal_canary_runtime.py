...
from research_factory import (
    app_server_canonical_v31_compact_inventory_evidence_ordinal_canary_runtime as runtime,
)
...
def test_request_is_exact_compact_inventory_medium_effort_canary() -> None:
    request = _request()
    ...
    assert (
        integrity["inventory_protocol_version"]
        == runtime.adapter.INVENTORY_PROTOCOL_VERSION
    )
    assert integrity["model_authors_ordered_source_unit_proposition_inventory"] is True
    ...
    assert integrity["deterministic_inventory_removal_only"] is True
    assert (
        integrity["deterministic_evidence_relative_metric_ordinal_projection_only"]
        is True
    )


def test_epoch41_rejection_is_directly_bound_and_never_replayed() -> None:
    runtime._validate_predecessor(full_verify=False)  # noqa: SLF001
    records = runtime._predecessor_records()  # noqa: SLF001
    predecessor = records["epoch41_rejected_owner_local_semantic_shard_attempt"]
    assert predecessor["receipt"]["sha256"] == runtime.EPOCH41_HASHES["receipt"]
    assert predecessor["sidecar"]["sha256"] == runtime.EPOCH41_HASHES["sidecar"]
    assert predecessor["output"]["sha256"] == runtime.EPOCH41_HASHES["output"]
    assert predecessor["rejection"]["sha256"] == runtime.EPOCH41_HASHES["rejection"]
    assert runtime.PREDECESSOR_MODEL_CALLS == 20
    assert runtime.PREDECESSOR_UNKNOWN_USAGE == 3
    assert runtime.PREDECESSOR_MEASURED_TOKENS == 871_445
...
def test_directive_predeclares_distinct_architecture_and_nonsemantic_boundary() -> None:
    ...
    assert (
        contract["inventory_protocol_version"]
        == runtime.adapter.INVENTORY_PROTOCOL_VERSION
    )
    assert contract["model_authors_ordered_source_unit_proposition_inventory"] is True
    ...
    assert contract["model_authors_evidence_relative_metric_token_ordinals"] is True
    assert contract["deterministic_inventory_removal_only"] is True
    assert (
        contract["deterministic_evidence_relative_metric_ordinal_projection_only"]
        is True
    )
    assert contract["table_duplicate_rejection_allowed"] is False
...
def test_receipt_metadata_passes_only_structural_and_cost_gate() -> None:
    ...
    assert metadata["aggregate_architecture_semantic_model_call_count"] == 21
    assert metadata["aggregate_architecture_unknown_usage_turn_count"] == 3
    assert metadata["aggregate_architecture_measured_total_tokens"] == 896_445
...
def test_zero_call_runtime_freeze_rejects_request_tamper(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        (runtime.CompactInventoryCanaryError, runtime.adapter.CanonicalV31EpisodeBatchError)
    ):
