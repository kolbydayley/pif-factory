...
from research_factory import app_server_canonical_v31_compact_unit_pointer_canary_runtime as runtime
...
AUTHORIZATION_ID = "kolby-epoch18-compact-unit-pointer-test"
...
def _complete_fixture(root: Path, *, total_tokens: int) -> dict:
    ...
    thread = SimpleNamespace(
        thread_id="thread-epoch18-completed-fixture",
    ...
    sidecar = {
        ...
        "turn_id": "turn-epoch18-completed-fixture",
        "model": runtime.adapter.MODEL,
...
def test_directive_plan_predecessor_and_representability_are_exact() -> None:
...
def test_prepare_freezes_compact_full_episode_shape(prepared_root: Path) -> None:
    ...
    assert status["unit_local_token_count"] == runtime.EXPECTED_LOCAL_TOKEN_COUNT
    assert tuple(request["segment_ids"]) == runtime.EXPECTED_SEGMENT_IDS
...
def test_completed_turn_uses_cumulative_usage_and_passes_cost_gate(
    ...
    receipt = _complete_fixture(prepared_root, total_tokens=70_000)
    assert receipt["state"] == "passed"
    assert receipt["new_measured_usage"]["total_tokens"] == 70_000
    assert receipt["new_measured_usage"]["total_tokens"] > 500
    assert receipt["production_amortized_ratio"] == runtime._production_ratio(70_000)
...
def test_runtime_lock_rejects_schema_tamper(prepared_root: Path) -> None:
    schema = prepared_root / "prepared-turn/schema.json"
