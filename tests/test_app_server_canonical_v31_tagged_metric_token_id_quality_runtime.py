...
from research_factory import app_server_canonical_v31_tagged_metric_token_id_quality_runtime as quality
...
def test_contract_builds_exact_43_plus_25_full_event_pool() -> None:
    contract = quality.load_contract()
    ...
    assert len(lineage) == quality.EXPECTED_CANDIDATE_WITNESSES == 25
    assert sum(
        len(case["event_set_a"]) + len(case["event_set_b"])
        for case in pool["cases"]
    ) == quality.EXPECTED_TOTAL_WITNESSES == 68
...
def test_partial_ab_attempt_terminalizes_waiting_without_replay(
    ...
    assert receipt["terminal_reason"] == (
        "epoch28_partial_or_interrupted_ab_ba_preserved_without_replay"
    )
...
def test_live_default_root_rejects_injected_client_or_runner() -> None:
    ...
    with pytest.raises(quality.Epoch28QualityError, match="forbids injected"):
