...
from research_factory.app_server_judge_v5_calibration_v28_diagnostic import (
    MAX_PROMPT_BYTES,
    V28_CANARY_CASE_IDS,
    V28_CASE_IDS,
    V28_DIAGNOSTIC_SPEC_VERSION,
    freeze_v28_field_guarded_diagnostic,
...
class FakeV28FieldGuardedClient:
...
class JudgeV5CalibrationV28DiagnosticTests(unittest.IsolatedAsyncioTestCase):
    async def test_freeze_binds_v27_failure_and_declares_small_canary(self):
        with tempfile.TemporaryDirectory() as temp:
            output_dir = Path(temp) / "v28"
            frozen = freeze_v28_field_guarded_diagnostic(output_dir=output_dir)
            taxonomy_sha = _sha256_file(output_dir / "v27-sanitized-error-taxonomy.json")
            frozen_again = freeze_v28_field_guarded_diagnostic(output_dir=output_dir)
            taxonomy_sha_again = _sha256_file(output_dir / "v27-sanitized-error-taxonomy.json")
            policy_phase = load_reserve_capacity_policy(frozen["capacity_policy"])["phase_id"]
            spec = frozen["spec"]
        self.assertEqual(frozen_again["spec"], spec)
        self.assertEqual(taxonomy_sha, taxonomy_sha_again)
        self.assertEqual(spec["schema_version"], V28_DIAGNOSTIC_SPEC_VERSION)
        ...
        self.assertEqual(policy_phase, "judge_v5_4_v28_field_guarded_diagnostic")
        self.assertEqual(
            spec["v27_predecessor"]["failure"]["sha256"],
            _sha256_file(
                Path(
    ...
    async def test_perfect_fake_v28_diagnostic_passes_and_authorizes_full_calibration(self):
        with tempfile.TemporaryDirectory() as temp:
            frozen = freeze_v28_field_guarded_diagnostic(output_dir=Path(temp) / "v28")
            client = FakeV28FieldGuardedClient(frozen["truth"])

            def factory(_policy_path):
                return client

            terminal = await run_v28_field_guarded_diagnostic(
                output_dir=Path(temp) / "v28",
