...
DIAGNOSTIC_ROOT="$PIPELINE_ROOT/judge-diagnostic-v1"
RUNTIME_LOCK="$REPO_ROOT/work/app-server-development-v2/unattended-runtime-lock-v6.json"
...
if [[ -e "$PIPELINE_ROOT/STOP" || -e "$DIAGNOSTIC_ROOT/STOP" ]]; then
  print -u2 -- "A pipeline-v5 STOP sentinel is present; refusing to start."
  exit 2
fi
...
/usr/bin/python3 -m research_factory.app_server_v5_reuse \
