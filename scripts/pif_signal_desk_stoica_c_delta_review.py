...
from research_factory.signal_desk_stoica_c_delta import prepare as proposal
...
OUT=run.OUT/'stoica-c-semantic-delta-review-v2'
...
LEDGER_SYSTEM=ledger.SYSTEM+'\nsource_quotes must contain exact source substrings only. Do not add enclosing quotation marks, ellipses, speaker labels or punctuation absent from the transcript. Review all dispositions against the corrected C output.\n'
...
def prepare(*,write=True):
    ...
    changed={fixed['records']['events'][i-1]['event_id'] for i in (5,7,8,9,12)}
    selected=[]
    for q in records:
        candidates=[e for e in q['candidates'] if e['event_id'] in changed]
        if not candidates:continue
        q=dict(q);q.pop('packet_sha256');q['candidates']=candidates
        q['schema_sha256']=digest(run.previous.review.schema(q));q['packet_sha256']=digest(q)
        selected.append(q)
    records=selected
    lineage=ledger.packets(fixed,p,token_count=count,system=LEDGER_SYSTEM)
    if ([e['event_id'] for packet in records for e in packet['candidates']]!=[e['event_id'] for e in fixed['records']['events'] if e['event_id'] in changed]
    ...
    plan=dict(record_packets=[x['packet_sha256'] for x in records],ledger_packets=[x['packet_sha256'] for x in lineage],
        records=15,reviewed_delta_records=5,input_dispositions=27,additions=0,proposal_sha256=digest(fixed),provenance_sha256=digest(proof),
...
async def run_reviews(records,lineage):
    for kind,ps,system,provider in [('records',records,SYSTEM,run.previous.review),('ledger',lineage,LEDGER_SYSTEM,ledger)]:
        code=await execute(ps,output_root=OUT/kind,task_prefix='stoica-c-delta-'+kind+'-review-v2',system=system,
...
def verified_reviews():
    ...
    values=[];proofs=[]
    for kind,ps,system,validator in [('records',records,SYSTEM,run.previous.review.validate_review),('ledger',lineage,LEDGER_SYSTEM,ledger.validate)]:
        for p in ps:
