...
def apply_verified(directory,p,fixed,proof,prefix,*,execute=False):
    ...
    try:
        ...
        if row['status']=='succeeded':
            if json.loads((directory/f'{sha}.result.json').read_text())!=fixed or json.loads((directory/'source-need-repair.json').read_text())!=proof:raise ValueError('completed repair differs')
            return {'already_applied':True,'gold_accepted':False}
        if row['status']!='terminal_failed' or row['semantic_failure_detail']!='source limitation requires recovery need':raise ValueError('not the inspected terminal failure')
        if execute:
            immutable_json(directory/'source-need-repair.json',proof);immutable_json(directory/f'{sha}.result.json',fixed)
            resurrect_task(db,task_key=key,resurrected_by='01a04040-77ee-77f2-9d26-a5ca1ae56986',
                reason='Source-reconciled GPT-5.5 supported inspected need correction; all original records, raw output and failure retained. Not accepted gold.')
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--case',choices=['hidden-brain','marketplace'],required=True);parser.add_argument('--execute',action='store_true');args=parser.parse_args()
