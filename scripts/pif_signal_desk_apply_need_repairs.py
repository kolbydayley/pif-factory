...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--case',choices=['ai-governance-audit-context','ai-governance-audit-reviewed','stoica','tsmc','tsmc-b','hidden-brain','hidden-brain-b','hidden-brain-c','hidden-brain-audit','ai-governance-a','ai-governance-b','ai-governance-c','marketplace','changelog-audit-offsets','coding-tools-v3'],required=True);parser.add_argument('--execute',action='store_true');args=parser.parse_args()
    with (lane.previous.parent.OUT/'runner.lock').open('a') as a,(lane.previous.OUT/'runner.lock').open('a') as b:
        ...
        if args.case=='ai-governance-audit-context':
            from scripts.pif_signal_desk_ai_governance_audit_review import proposal
            from research_factory.signal_desk_ai_governance_audit_context_recovery import recover as correction
            ...
            print(json.dumps(apply_verified(directory,p,fixed,proof,'full-event-lineage-qualification-v1',execute=args.execute,
                expected_failure='inexact record evidence',receipt_name='ai-governance-audit-context-repair.json')),flush=True);return
        ...
        if args.case=='stoica':
            ...
            raw=json.loads((directory/f"{p['packet_sha256']}.output.json").read_text());fixed,proof=correction(raw,p)
            print(json.dumps(apply_verified(directory,p,fixed,proof,'full-event-lineage-qualification-v1',execute=args.execute,
                expected_failure='inexact record evidence',receipt_name='stoica-repair.json')),flush=True);return
        if args.case=='tsmc-b':
            from scripts.pif_signal_desk_tsmc_b_review import proposal
            from research_factory.signal_desk_tsmc_b_recovery import recover as correction
            _,_,p=proposal();directory=lane.OUT/'calls'/p['window_id']/'B'
            raw=json.loads((directory/f"{p['packet_sha256']}.output.json").read_text());fixed,proof=correction(raw,p)
            print(json.dumps(apply_verified(directory,p,fixed,proof,'full-event-lineage-qualification-v1',execute=args.execute,
                expected_failure='source limitation requires recovery need',receipt_name='tsmc-b-repair.json')),flush=True);return
        if args.case=='tsmc':
