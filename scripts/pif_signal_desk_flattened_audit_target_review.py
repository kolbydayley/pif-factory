...
from research_factory.signal_desk_flattened_audit_target import prepare as proposal
from research_factory.signal_desk_full_event_v4_review import packets
from research_factory.signal_desk_rubric_reference_packets import digest
OUT=run.OUT/'flattened-audit-target-review-v3'
...
def prepare(*,write=True):
    ...
    changed={fixed['events'][i-1]['event_id'] for i in (9,)}
    selected=[]
    for q in ps:
    ...
    if write:
        ...
        run.immutable_json(OUT/'plan.json',dict(packets=[q['packet_sha256'] for q in ps],records=17,reviewed_delta_records=1,
...
if __name__=='__main__':
    ...
    with (run.previous.OUT/'runner.lock').open('a') as lock:
        ...
        if args.execute:
            ...
            raise SystemExit(asyncio.run(execute(ps,output_root=OUT,task_prefix='flattened-audit-target-review-v3',system=SYSTEM,
