"""Full independent source review of all seventeen AUDIT candidates."""
import argparse
import asyncio
import fcntl
...
from research_factory.signal_desk_flattened_audit_proposal import prepare as proposal
from research_factory.signal_desk_full_event_v4_review import packets
from research_factory.signal_desk_rubric_reference_packets import digest
OUT=run.OUT/'flattened-audit-full-proposal-review-v1'
SYSTEM=run.previous.review.SYSTEM+'''\nReview every AUDIT record independently against the complete flattened source.
No speaker identity may be inferred from biographies or other roles. Inspect all
claims, voices, entities, scope, modality, analytical attitude targets, omissions
and contexts. Preserve ASR uncertainty rather than silently correcting wording.
Distinguish substantive examples from unsupported prevalence or outcome claims.
Questions and withheld technical details are not factual mechanism evidence.
Every source_quote must be an exact substring without added enclosing quotation
marks, ellipses, labels or punctuation.
'''
...
def prepare(*,write=True):
    ...
    if write:
        ...
        run.immutable_json(OUT/'plan.json',dict(packets=[q['packet_sha256'] for q in ps],records=17,
...
if __name__=='__main__':
    ...
    with (run.previous.OUT/'runner.lock').open('a') as lock:
        ...
        if args.execute:
            ...
            raise SystemExit(asyncio.run(execute(ps,output_root=OUT,task_prefix='flattened-audit-full-review-v1',system=SYSTEM,
