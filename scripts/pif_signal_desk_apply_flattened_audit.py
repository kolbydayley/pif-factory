"""Apply only independently reviewed flattened AUDIT repair, preserving task lineage."""
import argparse
import fcntl
import json
...
if __name__=='__main__':
    ...
    with (lane.previous.parent.OUT/'runner.lock').open('a') as a,(lane.previous.OUT/'runner.lock').open('a') as b:
        ...
        _,_,p=prepare();d=lane.OUT/'calls'/p['window_id']/'AUDIT'
        raw=json.loads((d/f"{p['packet_sha256']}.output.json").read_text());fixed,proof=recover(raw,p)
        result=apply_verified(d,p,fixed,proof,'full-event-lineage-qualification-v1',execute=args.execute,
            expected_failure='inexact span',receipt_name='flattened-audit-repair.json')
