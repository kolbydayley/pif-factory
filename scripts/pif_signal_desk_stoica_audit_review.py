"""Full independent source review of all thirteen AUDIT candidates."""
import argparse
import asyncio
import fcntl
...
from research_factory.signal_desk_stoica_audit_proposal import prepare as proposal
from research_factory.signal_desk_full_event_v4_review import packets
from research_factory.signal_desk_rubric_reference_packets import digest
OUT=run.OUT/'stoica-audit-full-proposal-review-v1'
SYSTEM=run.previous.review.SYSTEM+'''\nReview all AUDIT records independently against the complete source, not other
roles or their approvals. Never assign the unlabeled opening to Ion based on a
later label. Check recap versus independent prediction, historical versus current
claims, resource comparisons and caveats, conditional forecasts, antecedents,
analytical attitude targets, and the speculative Nvidia threshold. Questions and
navigation text must not become factual claims. Check all fields and omissions.
...
def prepare(*,write=True):
    ...
    if write:
        ...
        run.immutable_json(OUT/'plan.json',dict(packets=[q['packet_sha256'] for q in ps],records=13,
...
if __name__=='__main__':
    ...
    with (run.previous.OUT/'runner.lock').open('a') as lock:
        ...
        if args.execute:
            ...
            raise SystemExit(asyncio.run(execute(ps,output_root=OUT,task_prefix='stoica-audit-full-review-v1',system=SYSTEM,
