...
def schema(cid):
    return {"type":"object", "additionalProperties":False, "required":["case_id","verdict","rationale","missing_context"], "properties":{
        "case_id":{"type":"string","enum":[cid]}, "verdict":{"type":"string","enum":["accept_repair","reject_repair","request_wider_context"]},
        "rationale":{"type":"string","minLength":1}, "missing_context":{"type":"string"}}}


def validate(value, cid):
    if value.get("case_id") != cid or value.get("verdict") not in {"accept_repair", "reject_repair", "request_wider_context"} or not value.get("rationale"):
        ...
        async with CodexAppServerClient(command=["codex","app-server","--stdio","--strict-config"],expected_cli_version="0.147.0") as client:
            for packet in packets:
                ...
                if target.exists():
                    ...
                    continue
                key="corrected-field-repair-v1:"+digest
