...
def prepare():
    ...
    for wid in plan["window_ids"]:
        source=json.loads((run.BASE/f"{plan['source_packets'][wid]}.packet.json").read_text());outputs={}
        for role in ("A","B","C","AUDIT"):
            ...
            raw=json.loads((d/f"{sha}.output.json").read_text());saved=json.loads((d/f"{sha}.result.json").read_text())
            sidecar=json.loads((d/f"{sha}.sidecar.json").read_text())
            if raw!=saved or sidecar.get("state")!="completed" or sidecar.get("error_class"):
