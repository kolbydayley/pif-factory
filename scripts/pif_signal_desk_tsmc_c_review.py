"""Independent review of all TSMC C records and every preserved lineage item."""
import argparse
import asyncio
import fcntl
import json
...
from research_factory.signal_desk_tsmc_c_proposal import prepare as proposal
...
OUT=run.OUT/'tsmc-c-explicit-proposal-review-v1'
SYSTEM=run.previous.review.SYSTEM+'''\nReview every assigned C record independently against the complete source.
Opening speakers remain unknown; do not inherit a name from later turns. Check
the executive's antecedent, the probably/almost-half hedge, source note wording,
David's customer clarification, prior-CEO zero-offer contrast, and the limits of
Ben's relationship interpretation. Sponsor records remain quarantined. Earlier
A/B approvals do not approve these C records. Check all fields and omissions.
'''
...
def prepare(*,write=True):
    ...
    if ([e['event_id'] for packet in records for e in packet['candidates']]!=[e['event_id'] for e in fixed['records']['events']]
        or sum(len(packet['candidates']) for packet in lineage)!=29):raise ValueError('C review population changed')
    plan=dict(record_packets=[x['packet_sha256'] for x in records],ledger_packets=[x['packet_sha256'] for x in lineage],
        records=15,input_dispositions=27,additions=2,proposal_sha256=digest(fixed),provenance_sha256=digest(proof),
...
async def run_reviews(records,lineage):
    for kind,ps,system,provider in [('records',records,SYSTEM,run.previous.review),('ledger',lineage,ledger.SYSTEM,ledger)]:
        code=await execute(ps,output_root=OUT/kind,task_prefix='tsmc-c-'+kind+'-review-v1',system=system,
