"""Apply only independently reviewed Stoica B repair, preserving task lineage."""
import argparse
import fcntl
import json
...
if __name__=='__main__':
    ...
    with (lane.previous.parent.OUT/'runner.lock').open('a') as a,(lane.previous.OUT/'runner.lock').open('a') as b:
        ...
        _,_,p=prepare();d=lane.OUT/'calls'/p['window_id']/'B'
        ...
        result=apply_verified(d,p,fixed,proof,'full-event-lineage-qualification-v1',execute=args.execute,
            expected_failure='source limitation requires recovery need',receipt_name='stoica-b-repair.json')
