"""Independent review of all fifteen luxury-interview proposal records."""
import argparse
import asyncio
import fcntl
...
from research_factory.signal_desk_luxury_proposal import prepare as proposal
from research_factory.signal_desk_full_event_v4_review import packets
from research_factory.signal_desk_rubric_reference_packets import digest
OUT=run.OUT/'luxury-full-proposal-review-v1'
SYSTEM=run.previous.review.SYSTEM+'''\nReview every assigned luxury-interview record against the complete source.
No speaker identity or currency can be filled from outside knowledge. Distinguish
actual reported management statements, generic consumer characterization, and the
narrator's own analysis. Check cost denominators, missing antecedents, garbled ASR,
currency-free revenue ranges, creative-leadership caveats, and negative operating
leverage. Do not silently repair names or rues from external knowledge. Review
all fields, claim relevance, atomicity, context and omissions, not just edits.
'''
...
if __name__=='__main__':
    ...
    with (run.previous.OUT/'runner.lock').open('a') as lock:
        ...
        if args.execute:
            ...
            raise SystemExit(asyncio.run(execute(ps,output_root=OUT,task_prefix='luxury-full-review-v1',system=SYSTEM,
