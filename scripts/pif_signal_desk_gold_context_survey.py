...
OUT = R / "oversized-context-survey-v2"
...
Return source-unit ranges using first_unit and last_unit IDs. NEVER retype quotes.
Select consecutive units containing each cue plus surrounding context; the harness copies
the exact original source text. A range can span multiple units when a cue crosses a boundary.
...
Exact quotes and offsets are reconstructed by the harness; your job is conservative range selection.
"""
SCHEMA = {"type": "object", "additionalProperties": False,
    "required": ["model", "findings", "limitations"], "properties": {
        "model": {"type": "string", "const": "gpt-5.5"},
        "findings": {"type": "array", "items": {"type": "object", "additionalProperties": False,
            "required": ["first_unit", "last_unit", "kind", "interpretation"], "properties": {
                "first_unit": {"type": "string"}, "last_unit": {"type": "string"},
...
def source_units(text, start, size=800):
    return [{"id": f"u{i:03d}", "start_char": start + offset, "end_char": start + min(len(text), offset + size),
             "text": text[offset:offset + size]} for i, offset in enumerate(range(0, len(text), size))]


def range_schema(packet):
    schema = json.loads(json.dumps(SCHEMA))
    for key in ("first_unit", "last_unit"):
        schema["properties"]["findings"]["items"]["properties"][key]["enum"] = [u["id"] for u in packet["source_units"]]
    return schema


def range_index(output, packet):
    if output.get("model") != "gpt-5.5" or not isinstance(output.get("findings"), list):
        raise ValueError("invalid survey envelope")
    ...
    findings = []
    for finding in output["findings"]:
        first, last = finding.get("first_unit"), finding.get("last_unit")
        if first not in positions or last not in positions or positions[first] > positions[last]:
            raise ValueError("invalid source range")
        selected = units[positions[first]:positions[last] + 1]
        findings.append({**finding, "quote": "".join(u["text"] for u in selected),
            "source_locations": [[selected[0]["start_char"], selected[-1]["end_char"]]],
            "location_ambiguous": False})
    return {"packet_sha256": packet["packet_sha256"], "transcript_sha256": packet["transcript_sha256"],
        "findings": findings, "limitations": output["limitations"], "gold_accepted": False,
        "warning": "Harness-copied source ranges do not certify interpretation or speaker assignment."}
    ...
    for digest, source in sorted(sources.items()):
        for start, end, text in source_sections(source["text"]):
            packet = {"transcript_sha256": digest, "manifest_sha256": manifest["manifest_sha256"],
                ...
                "source_length": len(source["text"]), "source_units": source_units(text, start), "system_sha256": _sha_json(SYSTEM)}
        ...
        async with CodexAppServerClient(command=["codex", "app-server", "--stdio", "--strict-config"], expected_cli_version="0.147.0") as client:
            for packet in packets:
                ...
                if target.exists():
                    ...
                    if range_index(raw, packet) != json.loads(target.read_text()):
                        raise ValueError("saved index mismatch")
                    continue
                key = "corrected-context-survey-v2:" + digest
                ...
                try:
                    result = await client.run_ephemeral_structured_turn(model="gpt-5.5", effort="high",
                        base_instructions=SYSTEM, prompt=json.dumps(packet, ensure_ascii=False), output_schema=range_schema(packet),
                    ...
                    immutable_json(target, range_index(result.output, packet))
