...
from research_factory.signal_desk_stoica_audit_delta import prepare as proposal
from research_factory.signal_desk_full_event_v4_review import packets
from research_factory.signal_desk_rubric_reference_packets import digest
OUT=run.OUT/'stoica-audit-delta-review-v2'
...
def prepare(*,write=True):
    ...
    changed={fixed['events'][i-1]['event_id'] for i in (3,5,7,8,9)}
    selected=[]
    for q in ps:
    ...
    if write:
        ...
        run.immutable_json(OUT/'plan.json',dict(packets=[q['packet_sha256'] for q in ps],records=13,reviewed_delta_records=5,
...
if __name__=='__main__':
    ...
    with (run.previous.OUT/'runner.lock').open('a') as lock:
        ...
        if args.execute:
            ...
            raise SystemExit(asyncio.run(execute(ps,output_root=OUT,task_prefix='stoica-audit-delta-review-v2',system=SYSTEM,
