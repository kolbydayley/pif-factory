"""Independent review of all seventeen flattened_audit-interview proposal records."""
import argparse
import asyncio
import fcntl
...
from research_factory.signal_desk_flattened_audit_delta import prepare as proposal
from research_factory.signal_desk_full_event_v4_review import packets
from research_factory.signal_desk_rubric_reference_packets import digest
OUT=run.OUT/'flattened-audit-delta-review-v2'
SYSTEM=run.previous.review.SYSTEM+'''\nReview each changed record completely against the full flattened source and index.
Check antecedents, exact attribution, product versus organization, speech acts,
analytical attitude targets, full mechanism evidence, and source uncertainty.
Do not infer speaker identities or silently normalize source wording. Review all
fields and omissions, not only edits. Source quotes must be exact substrings with
no added enclosing quotation marks.
'''
...
def prepare(*,write=True):
    ...
    changed={fixed['events'][i-1]['event_id'] for i in (2,4,7,9,15,17)}
    selected=[]
    for q in ps:
    ...
    if write:
        ...
        run.immutable_json(OUT/'plan.json',dict(packets=[q['packet_sha256'] for q in ps],records=17,reviewed_delta_records=6,
...
if __name__=='__main__':
    ...
    with (run.previous.OUT/'runner.lock').open('a') as lock:
        ...
        if args.execute:
            ...
            raise SystemExit(asyncio.run(execute(ps,output_root=OUT,task_prefix='flattened-audit-delta-review-v2',system=SYSTEM,
