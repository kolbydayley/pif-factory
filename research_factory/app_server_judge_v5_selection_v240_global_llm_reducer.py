...
"""Run the bounded v240 global LLM reducer over the immutable v239 map."""
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v240_global_llm_reducer_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v240_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v240_terminal_v1"
PHASE_ID = "development_selection_v5_4_v240_global_llm_reducer"
MODEL = "gpt-5.6-sol"
EFFORT = "low"
...
EXPECTED_LINEAGE_HASHES = {
    ...
    "v239_terminal": "5d8b4550bcc2e909923826fe23b95007008b6fc111cc117c4c01070acd6e3ecb",
    "v239_runtime_lock": "ee310845c66671054165432cf5e2c7eedce9d296baeebae12bb4dc46e3f054b1",
    "v239_design": "8b884117813169f3f260c1a43d5f482dd96c0a771a55b7f659b724b6d37e96e9",
    "v239_audit": "5d94eef303aa06b025e638666106ea0f8f25c6fd231ac183c75ab67f3c8a6076",
}


class V240ReducerError(RuntimeError):
    """The v240 global reducer cannot proceed safely."""


class V240OutputContractError(V240ReducerError):
    """A completed v240 output violated the frozen contract."""


class V240ArchitectureStop(V240ReducerError):
    """The v240 architecture did not clear its predeclared gate."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V240ReducerError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V240ReducerError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    return {
        "source_input": V233_TURN_ROOT / "input.private.json",
        ...
        "v239_design": V239_ROOT / "architecture-design.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    for name, expected in EXPECTED_LINEAGE_HASHES.items():
        if records[name]["sha256"] != expected:
            raise V240ReducerError(f"frozen lineage {name} drifted")
    v239.verify_runtime_lock(paths["v239_runtime_lock"])
    sidecar = _load_json(paths["v239_sidecar"], "v239 sidecar")
    terminal = _load_json(paths["v239_terminal"], "v239 terminal")
    audit = _load_json(paths["v239_audit"], "v239 audit")
    output = _load_json(paths["v239_output"], "v239 output")
    if (
        sidecar.get("usage_status") != "measured"
        or sidecar.get("usage_complete") is not True
        or (sidecar.get("usage") or {}).get("total_tokens") != PRIOR_MAP_TOKENS
        ...
        or audit.get("semantic_findings", {}).get(
            "global_llm_reducer_architecture_authorized"
        ) is not True
        or audit.get("replay_authorized") is not False
        or [len(row.get("events") or []) for row in output.get("segments") or []]
        != [31, 1]
    ):
        raise V240ReducerError("v239 map evidence drifted")
    return {"paths": paths, "records": records, "output": output}
    ...
    *, episode_id: str, segment_ids: Sequence[str], proposal_ids: Sequence[str],
    ...
    segment = {
        ...
        "required": [
            ...
            "coverage_audit", "proposal_receipts", "events",
        ],
        "properties": {
...
BASE_INSTRUCTIONS = """You are the global semantic reducer for a topic-general podcast event extraction pipeline. A prior blind map pass proposed events, but every proposal and every field may be wrong. You cannot see a reference answer, target count, density label, expected topic, model identity, or quality hint. Re-read all exact source units and independently adjudicate every opaque proposal. Never use keywords, regex, fixed topic rules, or proposal count as a semantic decision.

For every proposal choose keep, merge, or drop. Keep only when every material field is supported by its selected source evidence. Merge only proposals with the same truth conditions: actor, reported actor, speaker, target, stance, certainty, temporal scope, negation, attribution, event boundary, metric, and causal mechanism must not conflict. Preserve legitimately distinct premises, mechanisms, outcomes, comparisons, constraints, stances, and multi-lens events. Drop unsupported inference, bare mention, logistics, greeting, acknowledgement, or a proposal whose material semantics cannot be repaired from source.

Every kept proposal maps to exactly one final event. Multiple merged proposals map to one final event. Every final event lists all and only its source_proposal_ids. The LLM alone decides keep, merge, drop, event boundaries, evidence, and every semantic field. Return one receipt per proposal in exact input order and set coverage_audit to confirm all source units and proposals were reviewed with unresolved_count=0.

Select the smallest contiguous source-unit range supporting every material field. Every nonempty metric_value, metric_unit, metric_comparator, and metric_raw_text must be a literal contiguous substring of selected evidence. When all four metric strings are empty, use metric_direction=not_applicable. When any metric string is nonempty, do not use not_applicable. Populate the complete schema, order final events by source evidence, use coded if and only if events are returned, enforce the 32-event cap, and return schema-valid JSON only."""
...
def _prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    ...
    map_rows = list(lineage["output"]["segments"])
    source_rows = list(source["segments"])
    if [row["segment_id"] for row in map_rows] != [row["segment_id"] for row in source_rows]:
        raise V240ReducerError("map/source segment order drifted")
    private_segments = []
    prompt_segments = []
    all_unit_ids = []
    all_proposal_ids = []
    segment_ids = []
    for segment_index, (map_row, source_row) in enumerate(zip(map_rows, source_rows)):
        ...
        proposals = []
        for proposal_index, event in enumerate(map_row.get("events") or []):
            proposal_id = f"S{segment_index}P{proposal_index:03d}"
            all_proposal_ids.append(proposal_id)
            proposals.append({"proposal_id": proposal_id, "event": dict(event)})
        private_segments.append({**source_row, "proposals": proposals})
        prompt_segments.append(
            {
                ...
                "proposals": [
                    {"proposal_id": row["proposal_id"], "event": _compact_event(row["event"])}
                    for row in proposals
                ],
            }
        )
    if segment_ids != [DENSE_SEGMENT_ID, NO_SIGNAL_SEGMENT_ID]:
        raise V240ReducerError("canary segment membership drifted")
    prompt = "# Blind global-reducer packet\n" + json.dumps(
        {"episode_id": source["episode_id"], "segments": prompt_segments},
        ensure_ascii=True,
        separators=(",", ":"),
    ) + "\n"
    ...
    schema = _output_schema(
        ...
        max_proposal_count=max(len(row["proposals"]) for row in private_segments),
    )
    ...
    if (
        ...
        raise V240ReducerError("v240 request size cap exceeded")
    return {
        "turn_name": "v240_global_reducer_" + sha256_text(str(source["episode_id"]))[:20],
...
def _project_output(
    ...
    try:
        ...
        raise V240OutputContractError("global reducer output schema failed") from exc
    if output.get("episode_id") != turn["episode_id"]:
        raise V240OutputContractError("episode id drifted")
    rows = list(output.get("segments") or [])
    if [row.get("segment_id") for row in rows] != list(turn["segment_ids"]):
        raise V240OutputContractError("segment order or coverage drifted")
    source_by_id = {
    ...
    for row in rows:
        ...
        if (
            ...
            raise V240OutputContractError("global reducer coverage audit failed")
        receipts = list(row["proposal_receipts"])
        if [receipt.get("proposal_id") for receipt in receipts] != proposal_ids:
            raise V240OutputContractError("proposal receipt order or coverage drifted")
        raw_events = list(row["events"])
        if (row["status"] == "coded") != bool(raw_events):
            raise V240OutputContractError("coded status does not match final events")
        event_by_id = {}
        event_sources = {}
        projection_events = []
        ...
        for raw_event in raw_events:
            ...
            source_ids = [str(value) for value in event.pop("source_proposal_ids")]
            if event_id in event_by_id:
                raise V240OutputContractError("final event id duplicate")
            if len(source_ids) != len(set(source_ids)) or not source_ids:
                raise V240OutputContractError("final event proposal mapping duplicate or empty")
            if any(proposal_id not in proposal_ids for proposal_id in source_ids):
                raise V240OutputContractError("final event maps another segment proposal")
            event_by_id[event_id] = raw_event
            event_sources[event_id] = source_ids
        ...
        for receipt in receipts:
            ...
            if action == "drop":
                if final_event_id:
                    raise V240OutputContractError("dropped proposal names a final event")
                continue
            if not final_event_id or final_event_id not in event_by_id:
                raise V240OutputContractError("active proposal lacks a final event")
            active_ids.append(proposal_id)
            grouped_receipts.setdefault(final_event_id, []).append(receipt)
        mapped_ids = []
        for event_id, source_ids in event_sources.items():
            group = grouped_receipts.get(event_id, [])
            group_ids = [str(receipt["proposal_id"]) for receipt in group]
            if source_ids != group_ids:
                raise V240OutputContractError("event proposal receipt mapping drifted")
            if len(group) == 1:
                if group[0]["action"] != "keep":
                    raise V240OutputContractError("single proposal event must be kept")
            elif any(receipt["action"] != "merge" for receipt in group):
                raise V240OutputContractError("multi-proposal event must be merged")
            mapped_ids.extend(group_ids)
        if mapped_ids != active_ids:
            raise V240OutputContractError("active proposal accounting drifted")
    ...
    try:
        ...
        raise V240OutputContractError(str(exc)) from exc
    receipt_artifact = {
        "schema_version": SCHEMA_VERSION,
        "episode_id": turn["episode_id"],
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v240 runtime lock")
    ...
    if (
        ...
        raise V240ReducerError("v240 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(row or {}) for row in records):
        raise V240ReducerError("v240 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V240ReducerError("v240 lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    spec = _load_json(spec_path, "v240 spec")
    ...
    return {
        ...
        "turn": {
            ...
            "paths": paths,
            "private_input": _load_json(paths["input"], "v240 input"),
            "prompt": paths["prompt"].read_text(encoding="utf-8"),
            "base": paths["base"].read_text(encoding="utf-8"),
            "schema": _load_json(paths["schema"], "v240 schema"),
            "source_schema": _load_json(v233.SOURCE_TURN_ROOT / "schema.json", "source schema"),
        },
    }


def freeze_v240(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v240 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V240ReducerError("unfinished v240 root is not replayable")
    ...
    _write_stable_time(
        authorization_path,
        {
            ...
            "scope": "one bounded global LLM reducer architecture canary",
    ...
    _write_stable_time(
        ranking_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "selected_architecture_id": "adopted_map_plus_global_llm_reducer",
            "architectures": [
                {"rank": 1, "id": "adopted_map_plus_global_llm_reducer"},
            ...
            "on_failure": "freeze and reject map-reduce; no per-field repair",
        },
        "created_at",
    )
    projected_total = (
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "architecture_id": "adopted_map_plus_global_llm_reducer",
            "hypothesis": (
                "a separate global source-grounded LLM reducer can preserve at least 24 of the 31 dense "
                "v239 map proposals while correcting unsupported fields and semantic duplicates, avoiding "
                "the same-turn overmerge observed in v238"
            ),
            ...
            "production_cost_projection": {
                ...
                "reducer_turn_hard_max": MAX_NEW_TOKENS,
                "maximum_combined_development_tokens": MAX_COMBINED_DEVELOPMENT_TOKENS,
    ...
    _write_stable_time(
        spec_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "state": "frozen_before_one_turn_global_llm_reducer_canary",
            "declared_turn_count": 1,
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    if not isinstance(usage, Mapping):
        raise V240ReducerError("reducer usage absent")
    result = {}
    for field in USAGE_FIELDS:
        value = usage.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise V240ReducerError("reducer usage incomplete")
        result[field] = value
    if (
        ...
        or result["total_tokens"] > MAX_NEW_TOKENS
    ):
        raise V240ReducerError("measured reducer sidecar contract failed")
    return result
...
def _failure_terminal(
    ...
    if paths["sidecar"].is_file():
        ...
        try:
            raw = _load_json(paths["sidecar"], "v240 sidecar")
            values = raw.get("usage") or {}
            usage = {field: int(values[field]) for field in USAGE_FIELDS}
            unknown = 0
        except Exception:
            unknown = 1
    semantic = isinstance(exc, (V240OutputContractError, V240ArchitectureStop))
    ...
    terminal = {
        ...
        "terminal_reason": (
            "v240_global_llm_reducer_structural_or_count_gate_not_passed"
            if semantic
...
async def run_v240(
    *, output_dir: Path = DEFAULT_OUTPUT_ROOT,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v240 terminal")
    frozen = freeze_v240(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(root, frozen, V240ReducerError("launch exists; replay prohibited"))
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V240ReducerError("global reducer turn did not complete")
        usage = _usage(_load_json(paths["sidecar"], "v240 sidecar"))
        ...
        if not gate["passed"]:
            raise V240ArchitectureStop("v240 structural or count gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "v240_architecture_structural_gate_passed",
            "terminal_reason": "v240_global_llm_reducer_structural_cost_gate_passed",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v240 global LLM reducer canary")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v240(output_dir=Path(args.output_dir))
        design = _load_json(frozen["design_path"], "v240 design")
        result = {
        ...
        terminal = asyncio.run(
            run_v240(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
