...
"""Epoch-35 two-case full-event AB/BA quality gate for epoch 34.

The runtime consumes the immutable epoch-34 extraction output, places every
...
EPOCH34_ROOT = (
    PIPELINE_ROOT / "canonical-v31-epoch34-nested-ledger-predispatch-recovery-v1"
).resolve()
BASELINE_SEED_PATH = (
    PROJECT_ROOT / "work/app-server-development-v2/shared-reference-seed-v1.json"
).resolve()
DEFAULT_ROOT = (
    PIPELINE_ROOT / "canonical-v31-epoch35-nested-ledger-quality-v1"
).resolve()
DIRECTIVE_PATH = (
    PROJECT_ROOT / "automation/pif-evaluation-epoch35-nested-ledger-quality-v35.json"
).resolve()
PLAN_PATH = (PROJECT_ROOT / "automation/pif-evaluation-semantic-plan-v35.json").resolve()
...
SCHEMA_VERSION = "pif_canonical_v31_nested_ledger_quality_v35"
LOCK_VERSION = "pif_canonical_v31_nested_ledger_quality_lock_v1"
LAUNCH_VERSION = "pif_canonical_v31_nested_ledger_quality_launch_v1"
RECEIPT_VERSION = "pif_semantic_plan_step_receipt_v1"
THREAD_ID = "019f4cf1-c46e-7db3-acd2-bf03c4459a10"
PLAN_EPOCH = 35
STEP_ID = "canonical_v31_epoch35_nested_ledger_quality_v35"
MODEL = "gpt-5.5"
EFFORT = "high"
...
CAPACITY_TURN_NAMES = ("epoch35_quality_ab", "epoch35_quality_ba")
...
PROCESS_LOCK_NAME = ".canonical-v31-epoch35-quality.lock"
...
EPOCH34_EXPECTED = {
    "runtime_lock": "d1be4966de29021f79c13701850c23c62e36879fd7f2580f58c68c6d5fa7ff6c",
    "receipt": "69e6e051b7f44e2c8745d8b2c73c768ac25669fd510291f598327d4a03400fd6",
    "authorization": "9ec1dabf0534eadbdad528351cb544f6ca260b1bbf08a5b519f8ddd1aee1409e",
    "semantic_attempt": "c7f93a3958f0bc76e0870996fc2746b3b8c80b0d8bda2556e1af055787566173",
    "labels": "dea41764ebd33979417f64d219afb221ef8976405ce2982fa6ac5df0bc78fe7b",
    "provenance": "9989d76f00300744df62e33914c5756d28b93e15793678b407992565f604b402",
    "fidelity": "787c13c4c76af453da0fdf1405466624f44530822c948df7e0724250dd90b99d",
    "output": "5ce9ff49d77c8f83c78be7a9b16b92f070cca545c48816292a321d87da9eeefb",
    "sidecar": "04004b2b24143def9f9c6d398e461975d1ac36c353000630cfd1e6818c5dc45f",
    "request": "549104c9624d10d4f6b3d53700ac79b9855219ecc0b0f239af549fff8db24107",
    "baseline_seed": "167bdfb40af5608b1dd23d3199d30b9d12bc7573f369038a4f642e85adaf19b0",
}


class Epoch35QualityError(RuntimeError):
    """The epoch-35 contract, lineage, or immutable evidence drifted."""


class Epoch35QualityWaiting(Epoch35QualityError):
...
def _record(path: Path) -> dict[str, Any]:
    ...
    if not stat.S_ISREG(info.st_mode) or target.is_symlink():
        raise Epoch35QualityError(f"required artifact is not a real file: {target}")
    return {
        "path": str(target),
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise Epoch35QualityError(f"cannot read {label}") from exc
...
def _write_immutable_bytes(path: Path, payload: bytes) -> None:
    ...
    if target.exists():
        if target.is_symlink() or target.read_bytes() != payload:
            raise Epoch35QualityError(f"immutable artifact drifted: {target}")
        return
...
def _process_lock(root: Path) -> Iterator[None]:
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise Epoch35QualityWaiting("another epoch-35 evaluator owns the root") from exc
        try:
            yield
        finally:
...
def _direct_source_paths() -> dict[str, Path]:
    return {
        "epoch34_runtime_lock": EPOCH34_ROOT / "runtime-lock.json",
        "epoch34_receipt": EPOCH34_ROOT / "plan-step-receipt.json",
        "epoch34_terminal": EPOCH34_ROOT / "terminal.json",
        "epoch34_authorization": EPOCH34_ROOT / "operator-authorization.json",
        "epoch34_semantic_attempt": EPOCH34_ROOT / "turn/semantic-attempt.json",
        "epoch34_labels": EPOCH34_ROOT / "turn/canonical-labels.private.json",
        "epoch34_provenance": EPOCH34_ROOT / "turn/evidence-provenance.private.json",
        "epoch34_fidelity": EPOCH34_ROOT / "turn/semantic-fidelity.json",
        "epoch34_output": EPOCH34_ROOT / "turn/output.private.json",
        "epoch34_sidecar": EPOCH34_ROOT / "turn/sidecar.json",
        "epoch34_request": EPOCH34_ROOT / "prepared-turn/request.private.json",
...
def load_contract() -> dict[str, Any]:
    plan = _load_json(PLAN_PATH, "epoch-35 semantic plan")
    directive = _load_json(DIRECTIVE_PATH, "epoch-35 quality directive")
    ...
    if (
        ...
        raise Epoch35QualityError("epoch-35 semantic plan drifted")
    ...
    if (
        ...
        != "pif_evaluation_epoch35_nested_ledger_quality_directive_v1"
        ...
        raise Epoch35QualityError("epoch-35 directive values drifted")
    ...
    if not isinstance(frozen, list) or [row.get("role") for row in frozen] != list(expected_sources):
        raise Epoch35QualityError("epoch-35 frozen input roles drifted")
    records: dict[str, dict[str, Any]] = {}
    for row in frozen:
        if not isinstance(row, Mapping) or set(row) != {"role", "path", "sha256", "size_bytes"}:
            raise Epoch35QualityError("epoch-35 frozen input record is malformed")
        role = str(row["role"])
        expected = _record(expected_sources[role])
        supplied = {key: row[key] for key in ("path", "sha256", "size_bytes")}
        if supplied != expected:
            raise Epoch35QualityError(f"epoch-35 frozen input drifted: {role}")
        records[role] = expected
    expected_hashes = {
        "epoch34_runtime_lock": EPOCH34_EXPECTED["runtime_lock"],
        "epoch34_receipt": EPOCH34_EXPECTED["receipt"],
        "epoch34_terminal": EPOCH34_EXPECTED["receipt"],
        "epoch34_authorization": EPOCH34_EXPECTED["authorization"],
        "epoch34_semantic_attempt": EPOCH34_EXPECTED["semantic_attempt"],
        "epoch34_labels": EPOCH34_EXPECTED["labels"],
        "epoch34_provenance": EPOCH34_EXPECTED["provenance"],
        "epoch34_fidelity": EPOCH34_EXPECTED["fidelity"],
        "epoch34_output": EPOCH34_EXPECTED["output"],
        "epoch34_sidecar": EPOCH34_EXPECTED["sidecar"],
        "epoch34_request": EPOCH34_EXPECTED["request"],
        "baseline_seed": EPOCH34_EXPECTED["baseline_seed"],
    }
    if any(records[role]["sha256"] != digest for role, digest in expected_hashes.items()):
        raise Epoch35QualityError("epoch-35 direct source hash drifted")
    receipt = _load_json(EPOCH34_ROOT / "plan-step-receipt.json", "epoch-27 receipt")
    terminal = _load_json(EPOCH34_ROOT / "terminal.json", "epoch-27 terminal")
    sidecar = _load_json(EPOCH34_ROOT / "turn/sidecar.json", "epoch-27 sidecar")
    fidelity = _load_json(EPOCH34_ROOT / "turn/semantic-fidelity.json", "epoch-27 fidelity")
    if (
        receipt != terminal
        or receipt.get("state") != "passed"
        or receipt.get("terminal_reason") != "epoch34_one_turn_canary_passed"
        or receipt.get("new_semantic_model_call_count") != 1
        or receipt.get("semantic_retry_count") != 0
        or receipt.get("new_measured_usage", {}).get("total_tokens") != 26_852
        ...
        or sidecar.get("usage", {}).get("total_tokens") != 26_852
        ...
        raise Epoch35QualityError("epoch-27 terminal evidence drifted")
    return {
        "plan": plan,
        "directive": directive,
...
def build_complete_container(
    ...
    request = _load_json(Path(records["epoch34_request"]["path"]), "epoch-27 request")
    labels = _load_json(Path(records["epoch34_labels"]["path"]), "epoch-27 labels")
    provenance = _load_json(
        Path(records["epoch34_provenance"]["path"]), "epoch-27 provenance"
    )
    ...
    if (
        ...
        raise Epoch35QualityError("epoch-27 source segment order drifted")
    if not isinstance(labels, list) or [row.get("segment_id") for row in labels] != list(
        SOURCE_SEGMENT_ORDER
    ):
        raise Epoch35QualityError("epoch-27 candidate segment order drifted")
    ...
    if not isinstance(provenance_segments, list) or [
        ...
        raise Epoch35QualityError("epoch-27 provenance segment order drifted")
    references = baseline.get("references") if isinstance(baseline, Mapping) else None
    if not isinstance(references, list):
        raise Epoch35QualityError("baseline references are missing")
    ...
    if len(reference_by_id) != len(references):
        raise Epoch35QualityError("baseline segment identities are duplicated")
    ...
    for source_index, (segment, label, provenance_segment) in enumerate(
        ...
        if (
            ...
            raise Epoch35QualityError("epoch-35 selected event counts drifted")
        baseline_wrapped = []
        for event_index, raw_event in enumerate(baseline_events):
            if not isinstance(raw_event, Mapping):
                raise Epoch35QualityError("baseline event is malformed")
            ...
            if (
                ...
                raise Epoch35QualityError("baseline exact-evidence lineage drifted")
        ...
        for event_index, (raw_event, provenance_row) in enumerate(
            ...
            if not isinstance(raw_event, Mapping) or not isinstance(provenance_row, Mapping):
                raise Epoch35QualityError("candidate event provenance is malformed")
            event = copy.deepcopy(dict(raw_event))
            evidence = event.get("evidence")
            ...
            if (
                ...
                raise Epoch35QualityError("candidate exact-evidence lineage drifted")
            event_hash = _event_sha256(event)
            lineage = {
                "segment_id": segment_id,
    ...
    try:
        ...
        raise Epoch35QualityError("epoch-35 shared witness pool construction failed") from exc
    if judge.validate_shared_witness_pool(pool):
        raise Epoch35QualityError("epoch-35 shared witness pool failed validation")
    ...
    for mapped_case in mapping.get("cases") or []:
        for witness in mapped_case.get("witnesses") or []:
            ...
            if system_id not in observed_events or not isinstance(original, Mapping):
                raise Epoch35QualityError("private witness lineage drifted")
            ...
            if system_id == SYSTEM_CANDIDATE:
                ...
                if (
                    ...
                    raise Epoch35QualityError("candidate witness lineage hash drifted")
                observed_candidate_lineage[(segment_id, raw_index, event_hash)] += 1
    expected_counts = Counter(EXPECTED_WITNESS_COUNTS)
    if observed_counts != expected_counts:
        raise Epoch35QualityError("43-baseline plus 30-candidate witness counts drifted")
    if observed_events[SYSTEM_BASELINE] != expected_baseline_events:
        raise Epoch35QualityError("baseline raw-to-witness event multiset drifted")
    if observed_events[SYSTEM_CANDIDATE] != expected_candidate_events:
        raise Epoch35QualityError("candidate raw-to-witness event multiset drifted")
    ...
    if observed_candidate_lineage != expected_lineage:
        raise Epoch35QualityError("candidate provenance is not one-to-one with witnesses")
    total = sum(
        len(case["event_set_a"]) + len(case["event_set_b"]) for case in pool["cases"]
    )
    if total != EXPECTED_TOTAL_WITNESSES:
        raise Epoch35QualityError("epoch-35 witness total drifted")
    ...
    if [row["case_id"] for row in ab_cases] != [row["case_id"] for row in ba_cases]:
        raise Epoch35QualityError("AB/BA case order drifted")
    for ab_case, ba_case in zip(ab_cases, ba_cases):
        if (
            ...
            raise Epoch35QualityError("AB/BA opaque witness order drifted")
    if SYSTEM_BASELINE in _canonical_json(pool) or SYSTEM_CANDIDATE in _canonical_json(pool):
        raise Epoch35QualityError("system origin leaked into the model-facing pool")
...
def _freeze_unlocked(root: Path) -> dict[str, Any]:
    ...
    if root.exists() and _meaningful_entries(root):
        raise Epoch35QualityError("unfrozen epoch-35 root is not empty")
    contract = load_contract()
    if contract["receipt_path"] != root / "plan-step-receipt.json":
        raise Epoch35QualityError("epoch-35 output root differs from the contract")
...
def verify_runtime_lock(path: Path, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        lock = _load_json(lock_path, "epoch-35 runtime lock")
        records = [
        ...
        if (
            ...
            raise Epoch35QualityError("epoch-35 runtime lock drifted")
        contract = load_contract()
        source_records = list(contract["source_records"].values()) + [
            _record(PLAN_PATH),
            _record(DIRECTIVE_PATH),
        ...
        if lock.get("source_records") != source_records:
            raise Epoch35QualityError("epoch-35 source record set drifted")
        rebuilt_pool, rebuilt_mapping, rebuilt_lineage = build_complete_container(contract)
        if (
            _load_json(Path(lock["pool"]["path"]), "epoch-35 pool") != rebuilt_pool
            or _load_json(Path(lock["mapping"]["path"]), "epoch-35 mapping")
            != rebuilt_mapping
            or _load_json(Path(lock["candidate_lineage"]["path"]), "epoch-35 lineage")
            != {"event_count": len(rebuilt_lineage), "events": rebuilt_lineage}
        ):
            raise Epoch35QualityError("epoch-35 reconstructed witness lineage drifted")
        for name in ("ab", "ba"):
            _turn_material(lock_path.parent, name, rebuilt_pool)
        attempt_spec = _load_json(
            Path(lock["attempt_spec"]["path"]), "epoch-35 attempt spec"
        )
        if (
            ...
            raise Epoch35QualityError("epoch-35 attempt specification drifted")
        ...
        if (
            ...
            raise Epoch35QualityError("epoch-35 capacity policy drifted")
        return lock
...
class Epoch28ReserveCapacityClient(reserve.ReserveCapacityGatedCodexAppServerClient):
    """Map the mature judge layout onto the frozen epoch-35 reserve policy."""
    ...
    def _turn_index(self, checkpoint_path: Any) -> tuple[int, Path]:
        if checkpoint_path is None:
            raise reserve.ReserveCapacityError("epoch-35 capacity checkpoint is required")
    ...
    def _verify_prior_turns(self, turn_index: int) -> None:
        for index in range(turn_index):
            ...
            sidecar = _load_json(sidecar_path, "prior epoch-35 judge sidecar")
            ...
            if (
                ...
                raise reserve.ReserveCapacityError("prior epoch-35 judge turn drifted")
...
def _validate_capacity_checkpoint(path: Path) -> dict[str, Any]:
    value = _load_json(path, "epoch-35 capacity checkpoint")
    ...
    if not isinstance(value, Mapping) or any(value.get(key) != expected for key, expected in required.items()):
        raise Epoch35QualityError("epoch-35 capacity checkpoint drifted")
    ...
    for key in (
        ...
        if value.get(key) != evaluation[key]:
            raise Epoch35QualityError("epoch-35 capacity arithmetic drifted")
    if remaining != 100 - used:
        raise Epoch35QualityError("epoch-35 capacity remaining percentage drifted")
    return dict(value)
...
def _validate_launch_receipt(root: Path) -> dict[str, Any]:
    value = _load_json(root / "launch-receipt.json", "epoch-35 launch receipt")
    if (
        ...
        raise Epoch35QualityError("epoch-35 launch receipt drifted")
    return dict(value)


def _valid_usage(value: Any) -> dict[str, int]:
    if not isinstance(value, Mapping):
        raise Epoch35QualityError("judge usage is missing")
    result: dict[str, int] = {}
    for field in USAGE_FIELDS:
        item = value.get(field)
        if isinstance(item, bool) or not isinstance(item, int) or item < 0:
            raise Epoch35QualityError("judge usage is malformed")
        result[field] = item
    if (
        ...
        raise Epoch35QualityError("judge usage arithmetic drifted")
    return result
...
def _turn_material(root: Path, name: str, pool: Mapping[str, Any]) -> tuple[str, dict[str, Any], str]:
    ...
    if (
        ...
        raise Epoch35QualityError("epoch-35 frozen judge request drifted")
...
def _validate_completed_turn(
    ...
    try:
        ...
        raise Epoch35QualityError(f"epoch-35 {name} checkpoint drifted") from exc
    required = {
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch35QualityError(f"epoch-35 {name} managed sidecar contract drifted")
    usage = _valid_usage(sidecar.get("usage"))
    if sidecar.get("thread_total_usage") != usage:
        raise Epoch35QualityError(f"epoch-35 {name} thread usage drifted")
    if usage["total_tokens"] > MAXIMUM_TOTAL_TOKENS_PER_TURN:
        raise Epoch35QualityError(f"epoch-35 {name} exceeded the token ceiling")
    if judge.validate_judge_output(output, judge.build_judge_variants(pool)[name]):
        raise Epoch35QualityError(f"epoch-35 {name} judge output is invalid")
...
def _validate_partial_sidecar(
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch35QualityError(f"partial {name} sidecar lineage drifted")
    output_path = root / f"judge/output-{name}.private.json"
    if Path(str(sidecar.get("output_path") or "")).expanduser().resolve() != output_path:
        raise Epoch35QualityError(f"partial {name} output path drifted")
    thread_id = sidecar.get("thread_id")
    if not isinstance(thread_id, str) or not thread_id:
        raise Epoch35QualityError(f"partial {name} sidecar lacks thread identity")
    turn_id = sidecar.get("turn_id")
    if turn_id is not None and (not isinstance(turn_id, str) or not turn_id):
        raise Epoch35QualityError(f"partial {name} turn identity is malformed")
...
def _partial_accounting(root: Path) -> dict[str, Any]:
    ...
    turns = []
    ab_evidence = False
    pool = _load_json(root / "shared-witness-pool.private.json", "epoch-35 pool")
    for name in ("ab", "ba"):
        sidecar_path = root / f"judge/sidecars/{name}.json"
        ...
        if name == "ba" and any(present) and not ab_evidence:
            raise Epoch35QualityError("BA evidence exists without AB lineage")
        ...
        if output_path.is_file():
            ...
            if judge.validate_judge_output(output, judge.build_judge_variants(pool)[name]):
                raise Epoch35QualityError(f"partial {name} output is invalid")
        if sidecar_path.is_file():
            sidecar = _load_json(sidecar_path, f"partial {name} sidecar")
            if not isinstance(sidecar, Mapping):
                raise Epoch35QualityError(f"partial {name} sidecar is malformed")
            ...
            if state == "completed" and output_path.is_file() and capacity_path.is_file():
                ...
                raise Epoch35QualityError(f"partial {name} sidecar state drifted")
    ...
    if measured_calls + unknown_calls > MODEL_CALL_CAP:
        raise Epoch35QualityError("epoch-35 partial call count exceeded the cap")
    return {
    ...
    for case in mapping.get("cases") or []:
        for witness in case.get("witnesses") or []:
            ...
            if system_id not in {SYSTEM_BASELINE, SYSTEM_CANDIDATE} or witness_id in systems:
                raise Epoch35QualityError("private witness system lineage drifted")
    ...
    *, pool: Mapping[str, Any], mapping: Mapping[str, Any], consensus: Mapping[str, Any]
    ...
    for row in consensus.get("cases") or []:
        ...
        if case_id not in pool_cases or row.get("partition_abstained_witness_ids"):
            raise Epoch35QualityWaiting("AB/BA equivalence partition requires adjudication")
        events = {
        ...
        if grouped != set(events) or set(support) != set(events):
            raise Epoch35QualityError("shared-reference consensus omits a witness")
        group_by_witness = {
    ...
    if len(case_scores) != len(pool_cases) or not case_scores:
        raise Epoch35QualityError("macro score does not cover both cases")
    return {
        "systems": {
            system_id: {
...
def _aggregate_completed_usage(
    ...
    for name in ("ab", "ba"):
        ...
        if identity in identities:
            raise Epoch35QualityError("epoch-35 judge thread/turn identity was replayed")
    ...
    if usage["total_tokens"] > TOTAL_TOKEN_CAP:
        raise Epoch35QualityError("epoch-35 judge token cap exceeded")
    return usage, turns


def _score(root: Path) -> dict[str, Any]:
    pool = _load_json(root / "shared-witness-pool.private.json", "epoch-35 pool")
    mapping = _load_json(root / "private-witness-mapping.private.json", "epoch-35 mapping")
    consensus = _load_json(root / "judge/consensus.private.json", "epoch-35 consensus")
    report = _load_json(root / "judge/report.json", "epoch-35 judge report")
    if (
        ...
        raise Epoch35QualityError("epoch-35 judge report drifted")
    usage, turns = _aggregate_completed_usage(root, pool)
    if report.get("usage") != usage:
        raise Epoch35QualityError("epoch-35 report accounting drifted")
    try:
        ...
        if "abstained semantic equivalence partition" in str(exc):
            raise Epoch35QualityWaiting(
                "AB/BA disagreement requires separately frozen adjudication"
            ) from exc
        raise Epoch35QualityError("epoch-35 shared-reference score failed") from exc
    if set(shared.get("reference_system_ids") or []) != {SYSTEM_BASELINE, SYSTEM_CANDIDATE}:
        raise Epoch35QualityError("epoch-35 shared reference is not symmetric")
...
def _receipt(
    ...
    *,
    ...
    return {
        ...
        "aggregate_architecture_semantic_model_call_count": 5 + current_calls,
        "aggregate_architecture_unknown_usage_turn_count": 2 + unknown,
        "aggregate_architecture_measured_total_tokens": 74_326 + measured_total,
        "predecessor_semantic_model_call_count": 5,
        "predecessor_unknown_usage_turn_count": 2,
        "predecessor_measured_total_tokens": 74_326,
        ...
        "next_authorized_action": (
            "freeze_epoch34_nested_ledger_development_candidate_for_holdout_precommit"
            if state == "passed"
            else "reject_nested_ledger_architecture_without_field_repair"
            if state == "rejected"
...
def _recover_terminal_mirror(root: Path) -> dict[str, Any] | None:
    ...
    value = _load_json(existing, "single epoch-35 terminal mirror")
...
def _validate_receipt_payload(root: Path, receipt: Mapping[str, Any]) -> None:
    if (
        ...
        raise Epoch35QualityError("epoch-35 terminal receipt drifted")
    ...
    if any(isinstance(item, bool) or not isinstance(item, int) or item < 0 for item in (
        ...
        raise Epoch35QualityError("epoch-35 receipt call accounting drifted")
    usage = _valid_usage(receipt.get("usage"))
    if (
        receipt.get("aggregate_architecture_semantic_model_call_count") != 5 + current_calls
        or receipt.get("aggregate_architecture_unknown_usage_turn_count") != 2 + unknown
        or receipt.get("aggregate_architecture_measured_total_tokens")
        != 74_326 + usage["total_tokens"]
    ):
        raise Epoch35QualityError("epoch-35 aggregate accounting drifted")
    ...
    if receipt.get("state") == "passed" or quality_terminal:
        ...
        if (
            ...
            raise Epoch35QualityError("epoch-35 quality receipt drifted")
    else:
        ...
        if (
            ...
            raise Epoch35QualityError("epoch-35 waiting receipt drifted")
...
def verify_receipt(root: Path = DEFAULT_ROOT, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        ...
        receipt = _load_json(output_root / "plan-step-receipt.json", "epoch-35 receipt")
        terminal = _load_json(output_root / "terminal.json", "epoch-35 terminal")
        if receipt != terminal:
            raise Epoch35QualityError("epoch-35 terminal mirrors differ")
...
async def _run_unlocked(
    ...
    *,
    ...
    if artifacts_exist and not launch_exists:
        raise Epoch35QualityError("semantic artifacts exist without an epoch-35 launch")
    if launch_exists:
        ...
        if report_exists:
            try:
                score = _score(root)
            except Epoch35QualityWaiting as exc:
                ...
                return _write_terminal(
                    ...
                    _receipt(
                        root,
                        state="waiting",
                        terminal_reason="epoch35_ab_ba_disagreement_requires_versioned_adjudication",
            ...
            return _write_terminal(
                ...
                _receipt(
                    ...
                    terminal_reason=(
                        "epoch35_two_case_quality_gate_passed"
                        if score["passed"]
                        else "epoch35_two_case_quality_gate_rejected"
                    ),
                    accounting=accounting,
                    score=score,
                ),
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch35_partial_or_interrupted_ab_ba_preserved_without_replay",
                accounting=accounting,
    ...
    try:
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch35_judge_structured_output_rejected",
                accounting=accounting,
        ...
        return _write_terminal(
            ...
            _receipt(
                ...
                terminal_reason=(
                    "epoch35_judge_structured_output_rejected"
                    if rejected
                    else "epoch35_judge_operational_attempt_waiting_no_replay"
                ),
                accounting=accounting,
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch35_judge_operational_attempt_waiting_no_replay",
                accounting=accounting,
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch35_judge_exact_validation_rejected",
                accounting=accounting,
                diagnostic={"class": type(exc).__name__},
            ),
        )
...
async def run(
    ...
    *,
    ...
    if output_root == DEFAULT_ROOT and (
        ...
        raise Epoch35QualityError(
            "the live epoch-35 root forbids injected judge runners or clients"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 75
    except Epoch35QualityError as exc:
