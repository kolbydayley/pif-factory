...
"""Run the unchanged frozen side-free support judge over v261."""
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v262_frozen_support_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v262_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v262_terminal_v1"
PHASE_ID = "judge_v5_4_selection_v262_frozen_support"
TURN_NAME = "v262_frozen_support_v261_candidate"
MODEL = v250.MODEL
EFFORT = v250.EFFORT
...
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v262-frozen-support-v261"
).resolve()
PINNED_CODEX_0_144_1 = v261.PINNED_CODEX_0_144_1


class V262FrozenSupportError(RuntimeError):
    """The v262 frozen support gate cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V262FrozenSupportError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V262FrozenSupportError(f"frozen {path.name} drifted")
        return
...
def _v261_turn_root() -> Path:
    return next(V261_ROOT.glob("turns/v261-dual-decomposition-*"))


def _lineage_paths() -> dict[str, Path]:
    turn_root = _v261_turn_root()
    return {
        ...
        "v261_sidecar": turn_root / "sidecar.json",
        "v261_normalized": V261_ROOT / "normalized-output.private.json",
        "v261_provenance": V261_ROOT / "evidence-provenance.private.json",
        "v261_input": turn_root / "input.private.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    terminal = _load_json(paths["v261_terminal"], "v261 terminal")
    gate = _load_json(paths["v261_gate"], "v261 gate")
    sidecar = _load_json(paths["v261_sidecar"], "v261 sidecar")
    ...
    if (
        ...
        or gate.get("production_amortized_total_token_ratio") != 0.194895
        or (sidecar.get("usage") or {}).get("total_tokens") != 45_372
        ...
        raise V262FrozenSupportError("v262 frozen support lineage or protocol drifted")
...
def _event_claim(event: Mapping[str, Any]) -> str:
    ...
    if not isinstance(claim, str) or not claim.strip():
        raise V262FrozenSupportError("support witness has no claim text")
    return claim


def build_support_pool(lineage: Mapping[str, Any]) -> dict[str, Any]:
    source = _load_json(lineage["paths"]["v261_input"], "v261 source input")
    candidate = _load_json(
        lineage["paths"]["v261_normalized"], "v261 normalized output"
    )
    ...
    segment_ids = [v261.v249.v239.DENSE_SEGMENT_ID, v261.v249.v239.NO_SIGNAL_SEGMENT_ID]
    if any(
        ...
        raise V262FrozenSupportError("support pool segment coverage drifted")
    ...
    for segment_id in segment_ids:
        ...
        if sha256_text(source_excerpt) != reference_row["text_sha256"]:
            raise V262FrozenSupportError("support source text hash drifted")
        density = str(source_row["density_stratum"])
        case_id = "case_" + sha256_text(f"v262|{segment_id}")[:24]
        ...
        for origin, events in (
            ...
            for event_index, event in enumerate(events):
                event_hash = sha256_text(v261.v249._canonical_json(event))
                witness_id = "w_" + sha256_text(
                    f"v262|{case_id}|{origin}|{event_index}|{event_hash}"
                )[:24]
    ...
    if (
        ...
        raise V262FrozenSupportError("support pool witness coverage drifted")
    ...
    schema_text = v261.v249._canonical_json(schema)
    if (
        len(prompt.encode("utf-8")) > MAX_PROMPT_BYTES
        or len(schema_text.encode("utf-8")) > MAX_SCHEMA_BYTES
    ):
        raise V262FrozenSupportError("support request size cap exceeded")
    return {
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v262 runtime lock")
    if (
        ...
        raise V262FrozenSupportError("v262 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V262FrozenSupportError("v262 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V262FrozenSupportError("v262 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    return {
        "root": root,
        "spec_path": spec_path,
        "spec": _load_json(spec_path, "v262 spec"),
...
def freeze_v262(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v262 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V262FrozenSupportError("unfinished v262 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "strategy": "unchanged_frozen_side_free_support_over_v261_and_shared_reference",
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V262FrozenSupportError("v262 sidecar usage incomplete") from exc
    if (
        ...
        raise V262FrozenSupportError("v262 sidecar accounting or auth contract failed")
    return usage
...
def _failure_terminal(
    ...
    if paths["sidecar"].is_file():
        ...
        try:
            sidecar = _load_json(paths["sidecar"], "v262 sidecar")
            usage = {
    ...
    terminal = {
        ...
        "exact_next_action": "audit immutable v262 judge attempt; no retry",
    }
...
async def run_v262(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v262 terminal")
    frozen = freeze_v262(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V262FrozenSupportError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V262FrozenSupportError("v262 support turn did not complete")
        usage = _usage(_load_json(paths["sidecar"], "v262 sidecar"))
        ...
        terminal = {
            ...
            "terminal_reason": (
                ...
                if authorized
                else "v262_frozen_support_quality_gate_not_passed"
            ),
            ...
            "exact_next_action": (
                ...
                if authorized
                else "reject v261 on source support and advance to the next distinct architecture"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run frozen support over v261")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v262(output_dir=Path(args.output_dir))
        result = {
