...
"""Adopt the frozen v234 inventory and run one global LLM join/realization turn."""
...
from . import app_server_judge_v5_selection_v259_window_map_reduce as v259
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v260_inventory_global_join_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v260_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v260_terminal_v1"
PHASE_ID = "development_selection_v5_4_v260_inventory_global_join"
MODEL = "gpt-5.6-sol"
EFFORT = "low"
ADOPTED_INVENTORY_TOKENS = 29_362
MAX_NEW_TOKENS = 40_638
MAX_COMBINED_TOKENS = 70_000
...
V259_ROOT = v259.DEFAULT_OUTPUT_ROOT
V257_ROOT = (
...
EXPECTED_LINEAGE_HASHES = {
    ...
    "v259_runtime_lock": "7e7c8841d16bbfd13ca53b1842824e48f7f3dcd884a3462757f3d62eca0e3fc7",
...
GLOBAL_JOIN_INSTRUCTIONS = """You are the global semantic join and full-schema realization stage of a topic-general podcast event extractor. A prior blind LLM pass already inventoried compact source-supported propositions without seeing any reference answer, event output, density label, target count, topic list, or quality hint. Treat that frozen inventory as a high-recall proposal set, not as final event boundaries.

Read the complete source units and every frozen proposition for both segments. Within each segment, assign every proposition_id exactly once: cite it from exactly one final event in source_proposition_ids, or return it once in dropped_propositions with a concise source-based rationale. Never move a proposition across segments. The LLM alone decides whether a proposal is eligible, supported, duplicate, complementary, or part of a different event; the LLM alone decides merge/drop boundaries and every semantic field.

Merge propositions only when they are duplicate or inseparable support for one independently truth-valued event. Keep separate events whenever actor, reported actor, speaker, target, causal mechanism, stance, certainty, temporal horizon, attribution, main predicate, or evidential commitment materially differs. Do not add a final event without at least one frozen proposition. Do not infer any expected count.

For each final event, select the smallest self-contained contiguous evidence-unit range supporting every material field, including adjacent attribution or coreference only when needed. The range must stay in one fixed source window. Every nonempty metric string must be a literal contiguous substring of the selected evidence, and metric_direction must be not_applicable exactly when all metric strings are empty.

Order final events by the earliest cited proposition and source evidence. Use coded exactly when the segment has one or more final events; otherwise use no_signal. Return schema-valid JSON only. Never use keywords, regex, phrase lists, fixed-topic rules, embeddings, or reference-aware shortcuts."""


class V260InventoryGlobalJoinError(RuntimeError):
    """The v260 architecture cannot proceed or be adopted safely."""


class V260OutputContractError(V260InventoryGlobalJoinError):
    """A completed output violated the frozen ownership/projection contract."""


class V260ArchitectureStop(V260InventoryGlobalJoinError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V260InventoryGlobalJoinError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V260InventoryGlobalJoinError(f"frozen {path.name} drifted")
        return
...
def _single_turn(root: Path, prefix: str) -> Path:
    ...
    if len(matches) != 1:
        raise V260InventoryGlobalJoinError(f"{prefix} turn membership drifted")
    return matches[0]
...
def _lineage_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "v259_runtime_lock": V259_ROOT / "runtime-lock.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    for name, expected in EXPECTED_LINEAGE_HASHES.items():
        if records[name]["sha256"] != expected:
            raise V260InventoryGlobalJoinError(f"frozen lineage {name} drifted")
    v234.verify_runtime_lock(paths["v234_runtime_lock"])
    v259.verify_runtime_lock(paths["v259_runtime_lock"])
    ...
    if (
        ...
        or g259.get("failed_checks") != ["dense_event_count_gte_27"]
        ...
        or any(
            ...
            for terminal in (t234, t259, t257)
        )
    ):
        raise V260InventoryGlobalJoinError("v260 predecessor evidence drifted")
    packet = v234._source_packet(v234._validate_lineage())
    if [row["segment_id"] for row in inventory["segments"]] != packet["segment_ids"]:
        raise V260InventoryGlobalJoinError("v234 inventory segment order drifted")
    return {
        "paths": paths,
        "records": records,
        "inventory": inventory,
...
def _join_schema(packet: Mapping[str, Any], inventory: Mapping[str, Any]) -> dict[str, Any]:
    schema = v234._realization_schema(packet, inventory)
    ...
    event["properties"].pop("proposition_id")
    event["required"].remove("proposition_id")
    event["properties"] = {
        "source_proposition_ids": {
            "type": "array",
            "minItems": 1,
            "maxItems": v234.MAX_EVENTS_PER_SEGMENT,
            "items": {"type": "string", "enum": all_ids},
        },
        **event["properties"],
    }
    event["required"] = ["source_proposition_ids", *event["required"]]
    segment["properties"]["dropped_propositions"] = {
        "type": "array",
        "minItems": 0,
        "maxItems": v234.MAX_EVENTS_PER_SEGMENT,
        "items": {
            ...
            "required": ["proposition_id", "decision", "rationale"],
            "properties": {
                "proposition_id": {"type": "string", "enum": all_ids},
                "decision": {"type": "string", "enum": ["drop"]},
                "rationale": {"type": "string", "minLength": 1, "maxLength": 1000},
            },
        },
    }
    segment["required"].append("dropped_propositions")
    return schema
...
def prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    ...
    inventory_by_id = {str(row["segment_id"]): row for row in inventory["segments"]}
    segments = []
    ...
    private_input = {
        ...
        "segments": segments,
        "privacy": "private source units and adopted blind LLM inventory",
    }
    prompt = "# Adopted blind inventory global join packet\n" + json.dumps(
        {"episode_id": packet["episode_id"], "segments": segments},
        ensure_ascii=True,
        separators=(",", ":"),
    ) + "\n"
    base = (
        GLOBAL_JOIN_INSTRUCTIONS
        + "\n\n# Immutable episode context\n"
        + json.dumps(packet["context"], ensure_ascii=True, separators=(",", ":"))
        + "\n"
    )
    schema = _join_schema(packet, inventory)
    sizes = {
    ...
    if (
        ...
        raise V260InventoryGlobalJoinError("v260 request size cap failed")
    return {
        "turn_name": "v260_inventory_global_join_" + sha256_text(packet["episode_id"])[:20],
        "episode_id": packet["episode_id"],
        "segment_ids": list(packet["segment_ids"]),
...
def project_output(
    ...
    try:
        ...
        raise V260OutputContractError("global join output schema failed") from exc
    if output.get("episode_id") != turn["episode_id"]:
        raise V260OutputContractError("global join episode id drifted")
    rows = list(output.get("segments") or [])
    if [row.get("segment_id") for row in rows] != list(turn["segment_ids"]):
        raise V260OutputContractError("global join segment coverage or order drifted")
    source_by_id = {
        str(row["segment_id"]): row for row in turn["packet"]["private_input"]["segments"]
    }
    inventory_by_id = {
    ...
    for row in rows:
        ...
        expected_ids = [str(item["proposition_id"]) for item in propositions]
        position = {item: index for index, item in enumerate(expected_ids)}
        events = list(row.get("events") or [])
        dropped = list(row.get("dropped_propositions") or [])
        observed = []
        prior_proposition_position = -1
        prior_evidence_position = -1
        projected_events = []
        for event_index, raw_event in enumerate(events):
            event = dict(raw_event)
            source_ids = [str(item) for item in event.pop("source_proposition_ids")]
            if not source_ids or any(item not in position for item in source_ids):
                raise V260OutputContractError("global join cross-segment proposition ownership")
            earliest = min(position[item] for item in source_ids)
            if earliest < prior_proposition_position:
                raise V260OutputContractError("global join event proposition order drifted")
            prior_proposition_position = earliest
            ...
            except v234.V234OutputContractError as exc:
                raise V260OutputContractError(str(exc)) from exc
            if int(projection["start_index"]) < prior_evidence_position:
                raise V260OutputContractError("global join evidence order drifted")
            prior_evidence_position = int(projection["start_index"])
            metric_fields = (
                "metric_value",
                "metric_unit",
                "metric_comparator",
            ...
            if any(value and value not in projection["evidence"] for value in metric_values):
                raise V260OutputContractError("global join metric literal is not in evidence")
            if any(metric_values) == (event.get("metric_direction") == "not_applicable"):
                raise V260OutputContractError("global join metric applicability drifted")
            identity = v234._event_identity(event)
            if identity in seen_global:
                raise V260OutputContractError("global join exact event identity duplicate")
        ...
        dropped_ids = [str(item["proposition_id"]) for item in dropped]
        if any(item not in position for item in dropped_ids):
            raise V260OutputContractError("global join cross-segment dropped proposition")
        observed.extend(dropped_ids)
        counts = Counter(observed)
        if set(counts) != set(expected_ids) or any(value != 1 for value in counts.values()):
            raise V260OutputContractError("inventory propositions are not owned exactly once")
        if (row.get("status") == "coded") != bool(projected_events):
            raise V260OutputContractError("global join coded status drifted")
        if not projected_events and row.get("status") != "no_signal":
            raise V260OutputContractError("empty global join must be no_signal")
        ...
        diagnostics.append(
            {
                ...
                "dropped_proposition_count": len(dropped),
            }
        )
        ownership_rows.append(
            {
                "segment_id": segment_id,
                "inventory_proposition_count": len(expected_ids),
                "final_event_count": len(projected_events),
                "dropped_proposition_count": len(dropped),
                "all_propositions_accounted_exactly_once": True,
            }
        )
    ...
    ownership = {
        ...
        "all_join_semantics_selected_by_llm": True,
...
def _gate(
    *,
    ...
    checks = {
        ...
        "new_turn_tokens_lte_40638": int(new_usage["total_tokens"]) <= MAX_NEW_TOKENS,
        "combined_tokens_lte_70000": int(combined["total_tokens"]) <= MAX_COMBINED_TOKENS,
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v260 runtime lock")
    root = path.parent.resolve()
    spec = _load_json(root / "attempt-spec.json", "v260 spec")
    ...
    if (
        ...
        raise V260InventoryGlobalJoinError("v260 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V260InventoryGlobalJoinError("v260 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V260InventoryGlobalJoinError("v260 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    spec = _load_json(spec_path, "v260 spec")
    paths = _turn_paths(root, spec["turn_name"])
    lineage = _validate_lineage()
    ...
    return {
        ...
        "turn": {
            **turn,
            "private_input": _load_json(paths["input"], "v260 input"),
            "prompt": paths["prompt"].read_text(encoding="utf-8"),
            "base": paths["base"].read_text(encoding="utf-8"),
            "schema": _load_json(paths["schema"], "v260 schema"),
            "paths": paths,
        },
    }


def freeze_v260(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v260 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V260InventoryGlobalJoinError("unfinished v260 root is not replayable")
    ...
    _write_stable_time(
        authorization_path,
        {
            ...
            "scope": "adopt one immutable blind inventory and run one global LLM join turn",
    ...
    _write_stable_time(
        ranking_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "selected_architecture_id": "adopted_blind_inventory_global_llm_join",
            "architectures": [
                {"rank": 1, "id": "adopted_blind_inventory_global_llm_join"},
            ...
            "selection_basis": {
                ...
                "v259_projected_ratio": 0.202903,
                "decision": "preserve measured global inventory recall and test LLM-owned consolidation without rerunning extraction",
            },
            "on_failure": "reject v260 and advance to the next distinct architecture without field repair",
        },
        "created_at",
    )
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "architecture_id": "adopted_blind_inventory_global_llm_join",
            "hypothesis": (
                "the measured 32-proposition blind inventory preserves dense recall while a separate global LLM join "
                "can merge or drop proposals and realize full fields within the remaining 40638-token budget"
            ),
            ...
            "predeclared_stop_rules": {
                "retry_count": 0,
                "all_inventory_propositions_accounted_exactly_once": True,
    ...
    _write_stable_time(
        spec_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "state": "frozen_before_one_turn_inventory_global_join_canary",
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V260InventoryGlobalJoinError("v260 sidecar usage incomplete") from exc
    if (
        ...
        raise V260InventoryGlobalJoinError("v260 sidecar accounting or auth failed")
    return usage
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        try:
            sidecar = _load_json(paths["sidecar"], "v260 sidecar")
    ...
    semantic = isinstance(exc, (V260OutputContractError, V260ArchitectureStop)) or measured_over_cap
    ...
    terminal = {
        ...
        "exact_next_action": (
            "reject v260 and advance to the next distinct architecture without field repair"
            if semantic
            else "audit immutable v260 infrastructure attempt; no retry"
        ),
    }
...
async def run_v260(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v260 terminal")
    frozen = freeze_v260(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V260InventoryGlobalJoinError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V260InventoryGlobalJoinError("v260 turn did not complete")
        new_usage = _usage(_load_json(paths["sidecar"], "v260 sidecar"))
        if new_usage["total_tokens"] > MAX_NEW_TOKENS:
            raise V260ArchitectureStop("v260 measured turn exceeded frozen token bound")
        ...
        if not gate["passed"]:
            raise V260ArchitectureStop("v260 structural quality or cost gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "v260_architecture_structural_gate_passed",
            "terminal_reason": "v260_inventory_global_join_structural_cost_gate_passed",
            "new_semantic_attempt_count": 1,
            "semantic_retry_count": 0,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v260 adopted inventory global join canary")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v260(output_dir=Path(args.output_dir))
        result = {
        ...
        terminal = asyncio.run(
            run_v260(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
