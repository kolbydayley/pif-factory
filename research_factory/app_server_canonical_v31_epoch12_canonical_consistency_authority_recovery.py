...
from . import app_server_canonical_v31_audited_episode_batch as adapter
from . import app_server_canonical_v31_bounded_evidence_episode_batch as bounded_adapter
...
from . import app_server_canonical_v31_epoch11_bounded_evidence_authority_recovery as epoch11
from . import codex_app_server


PROJECT_ROOT = Path(__file__).resolve().parents[1]
PREDECESSOR_ROOT = epoch11.DEFAULT_ROOT
DEFAULT_ROOT = (
    PROJECT_ROOT
    / "work"
...
DIRECTIVE_PATH = (
    PROJECT_ROOT
    / "automation"
    / "pif-evaluation-epoch12-canonical-consistency-authority-recovery-v12.json"
)
DIRECTIVE_SHA256 = "dc021149e02354cee74ccd0d219129f8898e657b3a0589166671481490fcc165"

THREAD_ID = "019f4cf1-c46e-7db3-acd2-bf03c4459a10"
PLAN_EPOCH = 12
STEP_ID = "canonical_v31_epoch12_canonical_consistency_authority_recovery_v12"
...
CONTRACT_VERSION = "pif_canonical_v31_epoch12_canonical_consistency_contract_v1"
RUNTIME_LOCK_VERSION = "pif_canonical_v31_epoch12_canonical_consistency_runtime_lock_v1"
PREAUTHORIZATION_VERSION = "pif_canonical_v31_epoch12_canonical_consistency_preauthorization_v1"
AUTHORIZATION_VERSION = "pif_canonical_v31_epoch12_canonical_consistency_authorization_v1"
TURN_MANIFEST_VERSION = "pif_canonical_v31_epoch12_canonical_consistency_turn_manifest_v1"
IO_BINDING_VERSION = "pif_canonical_v31_epoch12_canonical_consistency_io_binding_v1"
TURN_RESULT_VERSION = "pif_canonical_v31_epoch12_canonical_consistency_turn_result_v1"
REJECTION_VERSION = "pif_canonical_v31_epoch12_semantic_rejection_v1"
...
ADOPTED_EPOCH11_TOTAL_TOKENS = 28_053
...
class CanonicalV31Epoch12Error(RuntimeError):
    pass


class CanonicalV31Epoch12Waiting(CanonicalV31Epoch12Error):
    pass


class CanonicalV31Epoch12Rejected(CanonicalV31Epoch12Error):
    pass
...
def _record(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CanonicalV31Epoch12Error(str(exc)) from exc
...
def _now(value: datetime | None = None) -> datetime:
    ...
    if current.tzinfo is None:
        raise CanonicalV31Epoch12Error("current time is not timezone-aware")
    return current.astimezone(timezone.utc)


def _verify_directive() -> dict[str, Any]:
    if _record(DIRECTIVE_PATH)["sha256"] != DIRECTIVE_SHA256:
        raise CanonicalV31Epoch12Error("epoch-12 directive drifted")
    directive = _load_object(DIRECTIVE_PATH, label="epoch-12 directive")
    fixed = {
        "schema_version": "pif_evaluation_epoch12_canonical_consistency_authority_recovery_directive_v1",
        "thread_id": THREAD_ID,
    ...
    for key, expected in fixed.items():
        if directive.get(key) != expected:
            raise CanonicalV31Epoch12Error(f"epoch-12 directive drifted at {key}")
    ...
    if (
        ...
        or predecessor.get("epoch11_root") != str(PREDECESSOR_ROOT.resolve())
        or predecessor.get("epoch11_state") != "rejected"
        or predecessor.get("epoch11_terminal_reason")
        != "epoch11_bounded_evidence_semantic_output_rejected"
        or predecessor.get("epoch11_semantic_model_call_count") != 1
        or predecessor.get("epoch11_full_thread_total_tokens")
        != ADOPTED_EPOCH11_TOTAL_TOKENS
        or predecessor.get("epoch11_completed_turn_replay_allowed") is not False
        or predecessor.get("epoch11_later_turns_started") is not False
        or predecessor.get("epoch11_semantic_failure")
        != "canonical_metric_applicability_cross_field_inconsistency"
        ...
        or architecture.get("llm_owned_canonical_cross_field_self_audit") is not True
        ...
        raise CanonicalV31Epoch12Error("epoch-12 directive contract drifted")
    return directive


def _validate_epoch11_predecessor() -> dict[str, Any]:
    receipt = epoch11.verify_recovery_receipt(PREDECESSOR_ROOT)
    receipt_path = PREDECESSOR_ROOT / epoch11.RECEIPT_FILENAME
    terminal_path = PREDECESSOR_ROOT / epoch11.TERMINAL_FILENAME
    if receipt_path.read_bytes() != terminal_path.read_bytes():
        raise CanonicalV31Epoch12Error("epoch-11 terminal mirrors drifted")
    expected_fields = {
        "schema_version": epoch11.RECEIPT_VERSION,
        "state": "rejected",
        "terminal_reason": "epoch11_bounded_evidence_semantic_output_rejected",
    ...
    for key, expected in expected_fields.items():
        if receipt.get(key) != expected:
            raise CanonicalV31Epoch12Error(f"epoch-11 receipt drifted at {key}")
    expected_records = {
        "runtime_contract": _record(PREDECESSOR_ROOT / epoch11.CONTRACT_FILENAME),
        "runtime_lock": _record(PREDECESSOR_ROOT / epoch11.RUNTIME_LOCK_FILENAME),
        "preauthorization_receipt": _record(
            PREDECESSOR_ROOT / epoch11.PREAUTHORIZATION_FILENAME
        ),
        "operator_authorization": _record(
            PREDECESSOR_ROOT / epoch11.AUTHORIZATION_FILENAME
        ),
    }
    for key, expected in expected_records.items():
        if receipt.get(key) != expected:
            raise CanonicalV31Epoch12Error(f"epoch-11 lineage drifted at {key}")
    measured = _usage(receipt.get("new_measured_usage"), label="epoch-11 measured")
    if measured["total_tokens"] != ADOPTED_EPOCH11_TOTAL_TOKENS:
        raise CanonicalV31Epoch12Error("epoch-11 measured usage drifted")
    contract = _load_object(
        PREDECESSOR_ROOT / epoch11.CONTRACT_FILENAME, label="epoch-11 contract"
    )
    if [epoch11._turn_state(PREDECESSOR_ROOT, turn) for turn in contract["turns"]] != [
        ...
        raise CanonicalV31Epoch12Error("epoch-11 turn lifecycle drifted")
    turn = contract["turns"][0]
    paths = epoch11._turn_paths(PREDECESSOR_ROOT, str(turn["turn_id"]))  # noqa: SLF001
    manifest = epoch11._manifest(turn)  # noqa: SLF001
    io = epoch11._io_binding(PREDECESSOR_ROOT, turn)  # noqa: SLF001
    request = _load_object(
        _verify_record(io["request"], label="epoch-11 request"),
        label="epoch-11 request",
    )
    raw_output = _load_object(paths["raw_output"], label="epoch-11 raw output")
    try:
        bounded_adapter.validate_and_project_output(request, raw_output)
    except canonical_base.CanonicalV31OutputError as exc:
        cause = exc.__cause__
        if cause is None or type(cause).__name__ != "ValidationError":
            raise CanonicalV31Epoch12Error(
                "epoch-11 canonical validation cause is unavailable"
            ) from exc
        diagnostic = str(cause)
    else:
        raise CanonicalV31Epoch12Error("epoch-11 rejection no longer reproduces")
    expected_diagnostic = (
        "$.discourse_events[4].metric.raw_text must be an exact evidence substring"
    )
    if diagnostic != expected_diagnostic:
        raise CanonicalV31Epoch12Error("epoch-11 canonical diagnostic drifted")
    ...
    rejection = _load_object(
        _verify_record(receipt["semantic_rejection"], label="epoch-11 rejection"),
        label="epoch-11 rejection",
    )
    if (
        ...
        raise CanonicalV31Epoch12Error("epoch-11 rejection lineage drifted")
    return {
        "receipt": receipt,
        "receipt_record": _record(receipt_path),
        "terminal_record": _record(terminal_path),
        "runtime_contract_record": _record(
            PREDECESSOR_ROOT / epoch11.CONTRACT_FILENAME
        ),
        "runtime_lock_record": _record(
            PREDECESSOR_ROOT / epoch11.RUNTIME_LOCK_FILENAME
        ),
...
def _episode_packet(
    ...
    *,
    ...
    if set(segment_ids) - set(repairs):
        raise CanonicalV31Epoch12Error("authority repair segment is unavailable")
    segments = []
    for segment_id in segment_ids:
        source = repairs[segment_id]
        text = source["segment_text"]
        if _sha256_bytes(text.encode("utf-8")) != source["segment_text_sha256"]:
            raise CanonicalV31Epoch12Error("authority segment text drifted")
        segments.append(
            {
...
def _prepare_canonical_request(
    ...
    *,
    ...
    if len(requests) != 1 or requests[0]["segment_ids"] != list(segment_ids):
        raise CanonicalV31Epoch12Error("canonical authority request partition drifted")
    return requests[0]
...
def _turn_id(stage: str, episode_id: str, segment_ids: Sequence[str]) -> str:
    ...
    return "authority12_" + stage + "_" + _sha256_bytes(
...
def _prepare_contract(root: Path) -> dict[str, Any]:
    predecessor = _validate_epoch11_predecessor()
    v11_contract = predecessor["contract"]
    adopted = root / ADOPTED_DIRECTORY
    adopted.mkdir(parents=True, exist_ok=True)
    diagnostic_record = _write_json(
        adopted / "epoch11-canonical-consistency-diagnostic.json",
        {
    ...
    context_record = copy.deepcopy(
        v11_contract["adopted_evidence"]["epoch8_valid_context_output"]
    )
    ...
    for episode_index, source_episode in enumerate(v11_contract["episodes"]):
        ...
        source_context_turn = next(
            turn
            for turn in v11_contract["turns"]
            if turn["turn_id"] == source_episode["context_turn_id"]
        )
        source_context_io = epoch11._io_binding(  # noqa: SLF001
    ...
    adopted_evidence = copy.deepcopy(v11_contract["adopted_evidence"])
    adopted_evidence.update(
        {
            "epoch11_receipt": predecessor["receipt_record"],
            "epoch11_terminal": predecessor["terminal_record"],
            "epoch11_runtime_contract": predecessor["runtime_contract_record"],
            "epoch11_runtime_lock": predecessor["runtime_lock_record"],
            "epoch11_preauthorization": predecessor["preauthorization_record"],
            "epoch11_authorization": predecessor["authorization_record"],
            "epoch11_semantic_rejection": predecessor[
                "semantic_rejection_record"
            ],
            "epoch11_sidecar": predecessor["sidecar_record"],
            "epoch11_raw_output": predecessor["raw_output_record"],
            "epoch11_request": predecessor["request_record"],
            "epoch11_canonical_consistency_diagnostic": diagnostic_record,
        }
    )
    return {
        "schema_version": CONTRACT_VERSION,
        "state": "frozen_canonical_consistency_authority_recovery_ready",
        "directive": _record(DIRECTIVE_PATH),
        "predecessor_root": str(PREDECESSOR_ROOT.resolve()),
        "adopted_evidence": adopted_evidence,
        "authority_plan": copy.deepcopy(v11_contract["authority_plan"]),
        "authority_plan_receipt": copy.deepcopy(
            v11_contract["authority_plan_receipt"]
        ),
        "source_authority_turns": copy.deepcopy(
            v11_contract["source_authority_turns"]
        ),
        "turns": turns,
        "episodes": episodes,
        "model": MODEL,
...
def _runtime_lock(root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "epoch11_runtime_module": _record(Path(epoch11.__file__)),
        ...
        "audited_adapter_module": _record(Path(adapter.__file__)),
        "bounded_evidence_adapter_module": _record(Path(bounded_adapter.__file__)),
        ...
        "trusted_client_factory": "audited_adapter._client_factory",
        "model": MODEL,
        "effort": EFFORT,
...
def _preauthorization(root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "adopted_evidence": copy.deepcopy(contract["adopted_evidence"]),
        "turn_manifests": [copy.deepcopy(row["turn_manifest"]) for row in contract["turns"]],
        "adopted_semantic_app_server_turn_count": 5,
        "adopted_development_qa_total_tokens": ADOPTED_DEVELOPMENT_QA_TOTAL_TOKENS,
...
def _authorization(
    ...
    *,
    ...
    if (
        ...
        raise CanonicalV31Epoch12Error("authorization identity or window is invalid")
    return {
        "schema_version": AUTHORIZATION_VERSION,
        "state": "authorized_for_five_canonical_consistency_authority_turns",
        ...
        "authorized_scope": (
            "one_canonical_consistency_label_repair_plus_two_context_label_pairs"
        ),
        "runtime_contract": _record(root / CONTRACT_FILENAME),
        "runtime_lock": _record(root / RUNTIME_LOCK_FILENAME),
...
def freeze_recovery(
    *,
    ...
    if output_root.exists():
        raise CanonicalV31Epoch12Error("epoch-12 output root already exists")
...
def verify_preauthorization(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    if (
        ...
        != "frozen_canonical_consistency_authority_recovery_ready"
    ...
    if preauthorization != _preauthorization(output_root, contract):
        raise CanonicalV31Epoch12Error("preauthorization drifted")
...
def _io_binding(root: Path, turn: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise CanonicalV31Epoch12Error("turn I/O binding drifted")
    ...
    if value["stage"] == "canonical_labels":
        ...
        try:
            ...
            raise CanonicalV31Epoch12Error("canonical request drifted") from exc
        if (
            ...
            raise CanonicalV31Epoch12Error("canonical I/O artifacts drifted")
    elif (
        value["stage"] != "context"
        or value["request"] is not None
        or value["segment_ids"] != []
    ):
        raise CanonicalV31Epoch12Error("context I/O binding drifted")
    return value
...
def _thread_binding(
    ...
    *,
    ...
    if (
        ...
        raise CanonicalV31Epoch12Error("semantic thread contract drifted")
    return {
...
def _verify_thread_binding(
    ...
    *,
    ...
    if (
        ...
        raise CanonicalV31Epoch12Error("thread binding drifted")
...
def _context_output(raw: Mapping[str, Any], *, episode_id: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise CanonicalV31Epoch12Rejected("context output is malformed")
...
def _combine_repair_output(
    ...
    if set(labels_by_id) != set(expected_ids):
        raise CanonicalV31Epoch12Rejected("repair label partition drifted")
    context = _load_object(
        _verify_record(
    ...
    try:
        ...
        raise CanonicalV31Epoch12Rejected(
...
def _combine_episode_output(
    ...
    if [row["segment_id"] for row in projected["labels"]] != expected_ids:
        raise CanonicalV31Epoch12Rejected("canonical labels changed authority order")
    combined = {
        "schema_version": authority_plan.OUTPUT_VERSION,
        "episode_id": str(turn["episode_id"]),
        "excluded_source_context": copy.deepcopy(context["excluded_source_context"]),
...
def _finalize_turn(
    *,
    ...
    if _load_object(paths["attempt"], label="semantic attempt") != expected_attempt:
        raise CanonicalV31Epoch12Error("semantic attempt drifted")
    ...
    try:
        ...
        raise CanonicalV31Epoch12Rejected("turn output is not structured JSON") from exc
    schema = _load_object(
    ...
    try:
        ...
        raise CanonicalV31Epoch12Rejected("turn output failed its frozen schema") from exc
    ...
    if not paths["validated_output"].exists():
        ...
        if _load_object(paths["validated_output"], label="validated output") != validated:
            raise CanonicalV31Epoch12Error("validated output drifted")
    result = {
    ...
    if paths["result"].exists():
        if _load_object(paths["result"], label="turn result") != result:
            raise CanonicalV31Epoch12Error("turn result drifted")
    else:
...
def _accounting(root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(thread_ids) != len(set(thread_ids)) or len(turn_ids) != len(set(turn_ids)):
        raise CanonicalV31Epoch12Error("semantic thread or turn ID was reused")
    return {
    ...
    *,
    ...
    if paths["thread"].is_file():
        if not paths["initial_capacity"].is_dir():
            raise CanonicalV31Epoch12Error("thread exists without initial capacity")
        io = _io_binding(root, turn)
        ...
        if paths["sidecar"].is_file():
            ...
            raise CanonicalV31Epoch12Error("raw output exists without a sidecar")
    elif any(path.exists() for path in semantic_paths[2:]):
        raise CanonicalV31Epoch12Error("turn artifact exists without thread binding")
...
def _merge_output(
    ...
    if path.exists():
        if _load_object(path, label="merged output") != merged:
            raise CanonicalV31Epoch12Error("merged output drifted")
    else:
...
def _terminal_payload(
    *,
    ...
    if state == "passed" and failed:
        state = "rejected"
        reason = "epoch12_canonical_consistency_cost_rejected"
    merged_record = None
    if state == "passed":
        if len(results) != NEW_TURN_COUNT:
            raise CanonicalV31Epoch12Error("passing receipt lacks every turn result")
    ...
    adopted_usage = _sum_usage(
        [
            ...
            _usage(
                _load_object(
                    _verify_record(
                        contract["adopted_evidence"]["epoch11_receipt"],
                        label="epoch-11 receipt",
                    ),
                    label="epoch-11 receipt",
                )["new_measured_usage"],
                label="epoch-11 usage",
            ),
        ]
    )
    if adopted_usage["total_tokens"] != ADOPTED_DEVELOPMENT_QA_TOTAL_TOKENS:
        raise CanonicalV31Epoch12Error("adopted usage drifted")
    return {
        ...
        "cumulative_semantic_app_server_turn_count": (
            5 + accounting["semantic_model_call_count"]
        ),
        "semantic_retry_count": 0,
...
def verify_recovery_receipt(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    if not receipt_path.is_file() or not terminal_path.is_file():
        raise CanonicalV31Epoch12Error("terminal mirrors are incomplete")
    if receipt_path.read_bytes() != terminal_path.read_bytes():
        raise CanonicalV31Epoch12Error("terminal mirrors differ")
    ...
    if state not in {"passed", "rejected", "waiting"}:
        raise CanonicalV31Epoch12Error("receipt state drifted")
    rejection = None
    ...
    if observed != expected:
        raise CanonicalV31Epoch12Error("plan-step receipt drifted")
    return observed
...
def status_recovery(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    if (output_root / RECEIPT_FILENAME).is_file():
        ...
        return {
            "schema_version": "pif_canonical_v31_epoch12_status_v1",
            "state": receipt["state"],
    ...
    return {
        ...
        "state": "ready",
        "reason": (
            "ready_for_one_canonical_consistency_repair_plus_two_context_label_pairs"
        ),
...
async def execute_recovery(
    *,
    ...
    try:
        ...
        for index, turn in enumerate(contract["turns"]):
            ...
            if state == "partial":
                return _write_terminal(
                    ...
                    contract=contract,
                    state="waiting",
                    reason="epoch12_partial_attempt_preserved_no_replay",
                )
        ...
        if len(results) == NEW_TURN_COUNT:
            ...
            return _write_terminal(
                ...
                contract=contract,
                state="passed",
                reason="epoch12_canonical_consistency_authority_recovery_passed",
            )
        async with adapter._client_factory() as client:  # noqa: SLF001
            ...
            if (
                ...
                raise CanonicalV31Epoch12Error(
                    "epoch-12 requires managed ChatGPT Pro auth"
                )
            for index in range(start_index, NEW_TURN_COUNT):
                ...
                if result.status_ok is not True:
                    raise CanonicalV31Epoch12Waiting(
                ...
                if (
                    ...
                    raise CanonicalV31Epoch12Error("in-memory turn lineage drifted")
    ...
    except CanonicalV31Epoch12Rejected as exc:
        current = contract["turns"][len(results)] if len(results) < NEW_TURN_COUNT else {}
        return _write_terminal(
            root=output_root,
            contract=contract,
            state="rejected",
            reason="epoch12_canonical_consistency_semantic_output_rejected",
            rejection={
        ...
        return _write_terminal(
            ...
            contract=contract,
            state="rejected",
            reason="epoch12_structured_output_rejected",
            rejection={
                "turn_id": current.get("turn_id"),
                "episode_id": current.get("episode_id"),
                "stage": current.get("stage"),
        ...
        return _write_terminal(
            ...
            contract=contract,
            state="waiting",
            reason="epoch12_operational_waiting_no_replay",
        )
    _merge_output(output_root, contract, results)
    return _write_terminal(
        root=output_root,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=(
            "python3 -m "
            "research_factory.app_server_canonical_v31_epoch12_canonical_consistency_authority_recovery"
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except CanonicalV31Epoch12Waiting as exc:
        print(_pretty_json({"ok": False, "waiting": True, "error": str(exc)}), end="")
        return 75
    except (
        CanonicalV31Epoch12Error,
