...
"""Run the unchanged two-permutation neutral alignment judge for v275."""
...
from . import app_server_judge_v5_selection_v275_inventory_compact_global_join as v275
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v277_frozen_alignment_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v277_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v277_terminal_v1"
WINNER_VERSION = "pif_app_server_judge_v5_selection_v277_winner_v1"
PHASE_ID = "judge_v5_4_selection_v277_frozen_alignment"
TURN_NAMES = ("v277_frozen_alignment_base", "v277_frozen_alignment_balanced_canary")
...
OBSERVED_PRODUCTION_RATIO = 0.27203
USAGE_FIELDS = v275.USAGE_FIELDS
PROJECT_ROOT = v275.PROJECT_ROOT
PIPELINE_ROOT = v275.PIPELINE_ROOT
V275_ROOT = v275.DEFAULT_OUTPUT_ROOT
V276_ROOT = v276.DEFAULT_OUTPUT_ROOT
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v277-frozen-alignment-v275"
).resolve()
PINNED_CODEX_0_144_1 = v275.PINNED_CODEX_0_144_1


class V277FrozenAlignmentError(RuntimeError):
    """The v277 frozen alignment gate cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V277FrozenAlignmentError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V277FrozenAlignmentError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    v275_turn = next(V275_ROOT.glob("turns/v275-inventory-compact-global-join-*"))
    return {
        "v275_terminal": V275_ROOT / "terminal.json",
        "v275_gate": V275_ROOT / "architecture-structural-gate.json",
        "v275_runtime_lock": V275_ROOT / "runtime-lock.json",
        "v275_sidecar": v275_turn / "sidecar.json",
        "v275_normalized": V275_ROOT / "normalized-output.private.json",
        "v276_terminal": V276_ROOT / "terminal.json",
        "v276_runtime_lock": V276_ROOT / "runtime-lock.json",
        "v276_pool": V276_ROOT / "support-pool.private.json",
        "v276_origin": V276_ROOT / "origin-map.private.json",
        "v276_receipts": V276_ROOT / "support-receipts.private.json",
        "v276_score": V276_ROOT / "support-score.private.json",
        "v276_audit": V276_ROOT / "support-audit.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    v275.verify_runtime_lock(paths["v275_runtime_lock"])
    v276.verify_runtime_lock(paths["v276_runtime_lock"])
    v246.v224.verify_runtime_lock(paths["v224_runtime_lock"])
    t275 = _load_json(paths["v275_terminal"], "v275 terminal")
    g275 = _load_json(paths["v275_gate"], "v275 gate")
    t276 = _load_json(paths["v276_terminal"], "v276 terminal")
    a276 = _load_json(paths["v276_audit"], "v276 audit")
    ...
    if (
        t275.get("terminal_reason")
        != "v275_inventory_compact_global_join_structural_cost_gate_passed"
        or t275.get("support_alignment_authorized") is not True
        or t275.get("production_amortized_total_token_ratio") != OBSERVED_PRODUCTION_RATIO
        or (t275.get("combined_usage") or {}).get("total_tokens") != 71_252
        or g275.get("passed") is not True
        or t276.get("terminal_reason")
        != "v276_frozen_support_completed_alignment_authorized"
        or t276.get("alignment_audit_authorized") is not True
        or (t276.get("usage") or {}).get("total_tokens") != 29_939
        or t276.get("usage_status") != "complete"
        or t276.get("accounting_complete") is not True
        or a276.get("exact_evidence_validated") is not True
        or a276.get("support_status_counts") != {"supported": 60}
        or a276.get("no_signal_candidate_event_support_status") != "supported"
        ...
        raise V277FrozenAlignmentError("v277 frozen alignment lineage or protocol drifted")
    return {"paths": paths, "records": records}


def _opaque_id(kind: str, value: str) -> str:
    prefix = "jcase_" if kind == "case" else "wit_"
    return prefix + sha256_text(f"v277|{kind}|{value}")[:24]


def build_alignment_bundle(lineage: Mapping[str, Any]) -> dict[str, Any]:
    raw_pool = _load_json(lineage["paths"]["v276_pool"], "v276 support pool")
    origin_rows = _load_json(lineage["paths"]["v276_origin"], "v276 origin map")["rows"]
    score_rows = _load_json(lineage["paths"]["v276_score"], "v276 support score")["rows"]
    receipts = _load_json(lineage["paths"]["v276_receipts"], "v276 support receipts")
    ...
    if set(origin_by_id) != set(status_by_id) or set(origin_by_id) != set(receipt_by_id):
        raise V277FrozenAlignmentError("support receipt and provenance coverage drifted")
    ...
    if (
        ...
        raise V277FrozenAlignmentError("supported alignment partition drifted")
    pool = {
        "schema_version": app_server_llm_judge.SHARED_WITNESS_POOL_VERSION,
        "seed_sha256": sha256_text("v277-v275-supported-dense-pool"),
        "cases": [
            {
    ...
    if errors:
        raise V277FrozenAlignmentError(
    ...
    if canary_ids != list(reversed(base_ids)):
        raise V277FrozenAlignmentError("balanced witness permutation drifted")
    turns = []
    for turn_name, permutation, value in zip(TURN_NAMES, PERMUTATIONS, (base, canary)):
        ...
        if prompt_bytes > MAX_PROMPT_BYTES or schema_bytes > MAX_SCHEMA_BYTES:
            raise V277FrozenAlignmentError("alignment request size cap exceeded")
        turns.append(
            {
                "turn_name": turn_name,
...
def score_alignment(
    *, base: Mapping[str, Any], canary: Mapping[str, Any], mapping: Mapping[str, Any]
    ...
    if set(base_rows) != set(canary_rows) or len(base_rows) != EXPECTED_ALIGNMENT_CASE_COUNT:
        raise V277FrozenAlignmentError("alignment normalized case coverage drifted")
        ...
        sorted(
            {
                ...
                *v276._runtime_files(),
                ...
                Path(v275.__file__).resolve(),
                Path(v276.__file__).resolve(),
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v277 runtime lock")
    ...
    if (
        ...
        raise V277FrozenAlignmentError("v277 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V277FrozenAlignmentError("v277 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V277FrozenAlignmentError("v277 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    return {
        "root": root,
        "spec_path": spec_path,
        "spec": _load_json(spec_path, "v277 spec"),
        "runtime_lock": root / "runtime-lock.json",
        "capacity_policy": root / "capacity-policy.json",
        "bundle": {
            "mapping": _load_json(root / "origin-map.private.json", "v277 origin map"),
...
def freeze_v277(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v277 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V277FrozenAlignmentError("unfinished v277 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "strategy": "unchanged_frozen_two_permutation_neutral_alignment_over_v275",
            "model": MODEL,
            "effort": EFFORT,
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V277FrozenAlignmentError("v277 sidecar usage incomplete") from exc
    if (
        ...
        raise V277FrozenAlignmentError("v277 sidecar accounting or auth failed")
    return usage
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    terminal = {
        ...
        "exact_next_action": "audit immutable v277 judge attempt; no retry",
    }
...
def _winner(score_path: Path) -> dict[str, Any]:
    turn_root = next(V275_ROOT.glob("turns/v275-inventory-compact-global-join-*"))
    return {
        "schema_version": WINNER_VERSION,
        "frozen_at": now_iso(),
        "system_id": "v275_adopted_inventory_compact_global_join_v277_frozen",
        "configuration": {
            "model": v275.MODEL,
            "reasoning_effort": v275.EFFORT,
            "inventory_model": v275.v234.MODEL,
            "inventory_reasoning_effort": v275.v234.EFFORT,
            "max_events_per_segment": v275.v234.MAX_EVENTS_PER_SEGMENT,
            "architecture_design": _record(V275_ROOT / "architecture-design.json"),
            "attempt_spec": _record(V275_ROOT / "attempt-spec.json"),
            "instructions": _record(turn_root / "base-instructions.private.md"),
            "schema": _record(turn_root / "schema.json"),
            "inventory_ownership_receipt": _record(
                V275_ROOT / "inventory-ownership-receipt.json"
            ),
            "adopted_inventory_output": _record(
                v275._lineage_paths()["v234_inventory_output"]
            ),
            "adopted_inventory_sidecar": _record(
                v275._lineage_paths()["v234_inventory_sidecar"]
            ),
            "runtime_lock": _record(V275_ROOT / "runtime-lock.json"),
        },
        "development_evidence": {
            "structural_gate": _record(V275_ROOT / "architecture-structural-gate.json"),
            "support_terminal": _record(V276_ROOT / "terminal.json"),
...
async def run_v277(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v277 terminal")
    frozen = freeze_v277(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(root, frozen, V277FrozenAlignmentError("launch exists; replay prohibited"))
    _write_immutable(
    ...
    try:
        ...
        terminal = {
            ...
            "terminal_reason": (
                "v277_alignment_passed_development_winner_frozen_holdout_authorized"
                if passed
                else "v277_alignment_quality_or_permutation_gate_not_passed"
            ),
            ...
            "exact_next_action": (
                ...
                if passed
                else "reject v275 and advance to the next distinct architecture"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run frozen alignment over v275")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v277(output_dir=Path(args.output_dir))
        result = {
        ...
        terminal = asyncio.run(
            run_v277(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
