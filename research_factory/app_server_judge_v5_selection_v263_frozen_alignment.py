...
"""Run the unchanged two-permutation neutral alignment judge for v261."""
...
from . import app_server_judge_v5_selection_v261_dual_decomposition as v261
from . import app_server_judge_v5_selection_v262_frozen_support as v262
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v263_frozen_alignment_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v263_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v263_terminal_v1"
WINNER_VERSION = "pif_app_server_judge_v5_selection_v263_winner_v1"
PHASE_ID = "judge_v5_4_selection_v263_frozen_alignment"
TURN_NAMES = ("v263_frozen_alignment_base", "v263_frozen_alignment_balanced_canary")
...
OBSERVED_PRODUCTION_RATIO = 0.194895
USAGE_FIELDS = v261.USAGE_FIELDS
PROJECT_ROOT = v261.PROJECT_ROOT
PIPELINE_ROOT = v261.PIPELINE_ROOT
V261_ROOT = v261.DEFAULT_OUTPUT_ROOT
V262_ROOT = v262.DEFAULT_OUTPUT_ROOT
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v263-frozen-alignment-v261"
).resolve()
PINNED_CODEX_0_144_1 = v261.PINNED_CODEX_0_144_1


class V263FrozenAlignmentError(RuntimeError):
    """The v263 frozen alignment gate cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V263FrozenAlignmentError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V263FrozenAlignmentError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    v261_turn = next(V261_ROOT.glob("turns/v261-dual-decomposition-*"))
    return {
        "v261_terminal": V261_ROOT / "terminal.json",
        "v261_gate": V261_ROOT / "architecture-structural-gate.json",
        "v261_runtime_lock": V261_ROOT / "runtime-lock.json",
        "v261_sidecar": v261_turn / "sidecar.json",
        "v261_normalized": V261_ROOT / "normalized-output.private.json",
        "v262_terminal": V262_ROOT / "terminal.json",
        "v262_runtime_lock": V262_ROOT / "runtime-lock.json",
        "v262_pool": V262_ROOT / "support-pool.private.json",
        "v262_origin": V262_ROOT / "origin-map.private.json",
        "v262_receipts": V262_ROOT / "support-receipts.private.json",
        "v262_score": V262_ROOT / "support-score.private.json",
        "v262_audit": V262_ROOT / "support-audit.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    v261.verify_runtime_lock(paths["v261_runtime_lock"])
    v262.verify_runtime_lock(paths["v262_runtime_lock"])
    v251.v246.v224.verify_runtime_lock(paths["v224_runtime_lock"])
    t255 = _load_json(paths["v261_terminal"], "v261 terminal")
    g255 = _load_json(paths["v261_gate"], "v261 gate")
    t256 = _load_json(paths["v262_terminal"], "v262 terminal")
    a256 = _load_json(paths["v262_audit"], "v262 audit")
    ...
    if (
        t255.get("terminal_reason") != "v261_dual_decomposition_structural_cost_gate_passed"
        ...
        != "v262_frozen_support_completed_alignment_authorized"
        or t256.get("alignment_audit_authorized") is not True
        or a256.get("support_status_counts") != {"supported": 60}
        ...
        raise V263FrozenAlignmentError("v263 frozen alignment lineage or protocol drifted")
    return {"paths": paths, "records": records}


def _opaque_id(kind: str, value: str) -> str:
    prefix = "jcase_" if kind == "case" else "wit_"
    return prefix + sha256_text(f"v263|{kind}|{value}")[:24]


def build_alignment_bundle(lineage: Mapping[str, Any]) -> dict[str, Any]:
    raw_pool = _load_json(lineage["paths"]["v262_pool"], "v262 support pool")
    origin_rows = _load_json(lineage["paths"]["v262_origin"], "v262 origin map")["rows"]
    score_rows = _load_json(lineage["paths"]["v262_score"], "v262 support score")["rows"]
    receipts = _load_json(lineage["paths"]["v262_receipts"], "v262 support receipts")
    ...
    if set(origin_by_id) != set(status_by_id) or set(origin_by_id) != set(receipt_by_id):
        raise V263FrozenAlignmentError("support receipt and provenance coverage drifted")
    ...
    if (
        ...
        raise V263FrozenAlignmentError("supported alignment partition drifted")
    pool = {
        "schema_version": app_server_llm_judge.SHARED_WITNESS_POOL_VERSION,
        "seed_sha256": sha256_text("v263-v261-supported-dense-pool"),
        "cases": [
            {
    ...
    if errors:
        raise V263FrozenAlignmentError(
    ...
    if canary_ids != list(reversed(base_ids)):
        raise V263FrozenAlignmentError("balanced witness permutation drifted")
    turns = []
    for turn_name, permutation, value in zip(TURN_NAMES, PERMUTATIONS, (base, canary)):
        ...
        if prompt_bytes > MAX_PROMPT_BYTES or schema_bytes > MAX_SCHEMA_BYTES:
            raise V263FrozenAlignmentError("alignment request size cap exceeded")
        turns.append(
            {
                "turn_name": turn_name,
...
def score_alignment(
    *, base: Mapping[str, Any], canary: Mapping[str, Any], mapping: Mapping[str, Any]
    ...
    if set(base_rows) != set(canary_rows) or len(base_rows) != EXPECTED_ALIGNMENT_CASE_COUNT:
        raise V263FrozenAlignmentError("alignment normalized case coverage drifted")
        ...
        sorted(
            {
                ...
                *v262._runtime_files(),
                ...
                Path(v261.__file__).resolve(),
                Path(v262.__file__).resolve(),
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v263 runtime lock")
    ...
    if (
        ...
        raise V263FrozenAlignmentError("v263 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V263FrozenAlignmentError("v263 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V263FrozenAlignmentError("v263 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    return {
        "root": root,
        "spec_path": spec_path,
        "spec": _load_json(spec_path, "v263 spec"),
        "runtime_lock": root / "runtime-lock.json",
        "capacity_policy": root / "capacity-policy.json",
        "bundle": {
            "mapping": _load_json(root / "origin-map.private.json", "v263 origin map"),
            "instructions": (
...
def freeze_v263(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v263 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V263FrozenAlignmentError("unfinished v263 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "strategy": "unchanged_frozen_two_permutation_neutral_alignment_over_v261",
            "model": MODEL,
            "effort": EFFORT,
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V263FrozenAlignmentError("v263 sidecar usage incomplete") from exc
    if (
        ...
        raise V263FrozenAlignmentError("v263 sidecar accounting or auth failed")
    return usage
...
def _failure_terminal(
    ...
    terminal = {
        ...
        "exact_next_action": "audit immutable v263 judge attempt; no retry",
    }
...
def _winner(score_path: Path) -> dict[str, Any]:
    turn_root = next(V261_ROOT.glob("turns/v261-dual-decomposition-*"))
    return {
        "schema_version": WINNER_VERSION,
        "frozen_at": now_iso(),
        "system_id": "v261_dual_decomposition_explicit_applicability_v263_frozen",
        "configuration": {
            "model": v261.MODEL,
            "reasoning_effort": v261.EFFORT,
            "max_events_per_segment": 32,
            "architecture_design": _record(V261_ROOT / "architecture-design.json"),
            "attempt_spec": _record(V261_ROOT / "attempt-spec.json"),
            ...
            "runtime_lock": _record(V261_ROOT / "runtime-lock.json"),
        },
        "development_evidence": {
            "structural_gate": _record(V261_ROOT / "architecture-structural-gate.json"),
            "support_terminal": _record(V262_ROOT / "terminal.json"),
...
async def run_v263(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v263 terminal")
    frozen = freeze_v263(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V263FrozenAlignmentError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        terminal = {
            ...
            "terminal_reason": (
                "v263_alignment_passed_development_winner_frozen_holdout_authorized"
                if passed
                else "v263_alignment_quality_or_permutation_gate_not_passed"
            ),
            ...
            "exact_next_action": (
                ...
                if passed
                else "reject v261 and advance to the next distinct architecture"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run frozen alignment over v261")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v263(output_dir=Path(args.output_dir))
        result = {
