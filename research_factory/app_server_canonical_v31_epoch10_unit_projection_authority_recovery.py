...
PREDECESSOR_ROOT = epoch9.DEFAULT_ROOT
DEFAULT_ROOT = (
    PROJECT_ROOT
    / "work"
    / "app-server-development-v2"
    / "unattended-pipeline-v5"
    / "canonical-v31-epoch10-unit-projection-authority-recovery-v1"
)
DIRECTIVE_PATH = (
    PROJECT_ROOT
    / "automation"
    / "pif-evaluation-epoch10-unit-projection-authority-recovery-v10.json"
)
DIRECTIVE_SHA256 = "b3d6121b340458c2ddd846157a1dda23715fd4cd04088a14bf5ac5c3933c9b8c"

THREAD_ID = "019f4cf1-c46e-7db3-acd2-bf03c4459a10"
PLAN_EPOCH = 10
STEP_ID = "canonical_v31_epoch10_unit_projection_authority_recovery_v10"
...
CONTRACT_VERSION = "pif_canonical_v31_epoch10_unit_projection_contract_v1"
RUNTIME_LOCK_VERSION = "pif_canonical_v31_epoch10_unit_projection_runtime_lock_v1"
PREAUTHORIZATION_VERSION = "pif_canonical_v31_epoch10_unit_projection_preauthorization_v1"
AUTHORIZATION_VERSION = "pif_canonical_v31_epoch10_unit_projection_authorization_v1"
TURN_MANIFEST_VERSION = "pif_canonical_v31_epoch10_unit_projection_turn_manifest_v1"
IO_BINDING_VERSION = "pif_canonical_v31_epoch10_unit_projection_io_binding_v1"
TURN_RESULT_VERSION = "pif_canonical_v31_epoch10_unit_projection_turn_result_v1"
PROJECTION_RECEIPT_VERSION = "pif_canonical_v31_epoch10_unique_offset_projection_v1"
REJECTION_VERSION = "pif_canonical_v31_epoch10_semantic_rejection_v1"
...
MAXIMUM_WALL_SECONDS_PER_TURN = epoch9.MAXIMUM_WALL_SECONDS_PER_TURN
...
class CanonicalV31Epoch10Error(RuntimeError):
    pass


class CanonicalV31Epoch10Waiting(CanonicalV31Epoch10Error):
    pass


class CanonicalV31Epoch10Rejected(CanonicalV31Epoch10Error):
    pass
...
def _record(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CanonicalV31Epoch10Error(str(exc)) from exc
...
def _now(value: datetime | None = None) -> datetime:
    ...
    if current.tzinfo is None:
        raise CanonicalV31Epoch10Error("current time is not timezone-aware")
    return current.astimezone(timezone.utc)


def _verify_directive() -> dict[str, Any]:
    if _record(DIRECTIVE_PATH)["sha256"] != DIRECTIVE_SHA256:
        raise CanonicalV31Epoch10Error("epoch-10 directive drifted")
    directive = _load_object(DIRECTIVE_PATH, label="epoch-10 directive")
    fixed = {
        "schema_version": "pif_evaluation_epoch10_unit_projection_authority_recovery_directive_v1",
        "thread_id": THREAD_ID,
    ...
    for key, expected in fixed.items():
        if directive.get(key) != expected:
            raise CanonicalV31Epoch10Error(f"epoch-10 directive drifted at {key}")
    ...
    if (
        ...
        or predecessor.get("epoch9_root") != str(PREDECESSOR_ROOT.resolve())
        or predecessor.get("epoch9_semantic_model_call_count") != 1
        or predecessor.get("epoch9_full_thread_total_tokens")
        != ADOPTED_EPOCH9_TOTAL_TOKENS
        or predecessor.get("epoch9_completed_turn_replay_allowed") is not False
        or predecessor.get("epoch9_valid_after_unique_exact_offset_projection_count")
        != 1
        or predecessor.get("epoch9_requires_fresh_llm_repair_count") != 1
        ...
        raise CanonicalV31Epoch10Error("epoch-10 directive contract drifted")
    return directive


def _without_offsets(label: Mapping[str, Any]) -> dict[str, Any]:
    value = copy.deepcopy(dict(label))
    for field in ("discourse_events", "concept_candidates"):
        for item in value.get(field, []):
            item.pop("evidence_start", None)
            item.pop("evidence_end", None)
    return value


def _project_unique_exact_offsets(
    label: Mapping[str, Any], *, segment_text: str
) -> tuple[dict[str, Any], dict[str, Any]]:
    projected = copy.deepcopy(dict(label))
    projection_rows: list[dict[str, Any]] = []
    for collection in ("discourse_events", "concept_candidates"):
        items = projected.get(collection)
        if not isinstance(items, list):
            raise CanonicalV31Epoch10Error("epoch-9 projected collection is malformed")
        for ordinal, item in enumerate(items):
            evidence = item.get("evidence")
            if not isinstance(evidence, str) or not evidence:
                raise CanonicalV31Epoch10Error("epoch-9 evidence is unavailable")
            if segment_text.count(evidence) != 1:
                raise CanonicalV31Epoch10Error(
                    "epoch-9 evidence is not uniquely projectable"
                )
    ...
    if _without_offsets(label) != _without_offsets(projected):
        raise CanonicalV31Epoch10Error(
            "unique-offset projection changed model semantic values"
        )
    receipt = {
        "schema_version": PROJECTION_RECEIPT_VERSION,
        "segment_id": projected.get("segment_id"),
        "source_text_sha256": _sha256_bytes(segment_text.encode("utf-8")),
        "input_model_semantics_sha256": _sha256_bytes(
            _canonical_json(_without_offsets(label)).encode("ascii")
        ),
        "projected_model_semantics_sha256": _sha256_bytes(
            _canonical_json(_without_offsets(projected)).encode("ascii")
        ),
        "projected_item_count": len(projection_rows),
...
def _validate_epoch9_predecessor() -> dict[str, Any]:
    epoch9.verify_preauthorization(PREDECESSOR_ROOT)
    authorization = epoch9.verify_authorization(
        PREDECESSOR_ROOT,
        expected_authorization_id=None,
        require_current=False,
    )
    receipt_path = PREDECESSOR_ROOT / epoch9.RECEIPT_FILENAME
    terminal_path = PREDECESSOR_ROOT / epoch9.TERMINAL_FILENAME
    if receipt_path.read_bytes() != terminal_path.read_bytes():
        raise CanonicalV31Epoch10Error("epoch-9 terminal mirrors drifted")
    receipt = _load_object(receipt_path, label="epoch-9 receipt")
    expected_receipt_fields = {
        "schema_version": epoch9.RECEIPT_VERSION,
        "state": "rejected",
        "terminal_reason": "epoch9_split_authority_structural_or_cost_rejected",
    ...
    for key, expected in expected_receipt_fields.items():
        if receipt.get(key) != expected:
            raise CanonicalV31Epoch10Error(f"epoch-9 receipt drifted at {key}")
    expected_receipt_records = {
        "runtime_contract": _record(PREDECESSOR_ROOT / epoch9.CONTRACT_FILENAME),
        "runtime_lock": _record(PREDECESSOR_ROOT / epoch9.RUNTIME_LOCK_FILENAME),
        "preauthorization_receipt": _record(
            PREDECESSOR_ROOT / epoch9.PREAUTHORIZATION_FILENAME
        ),
        "operator_authorization": _record(
            PREDECESSOR_ROOT / epoch9.AUTHORIZATION_FILENAME
        ),
    }
    for key, expected in expected_receipt_records.items():
        if receipt.get(key) != expected:
            raise CanonicalV31Epoch10Error(f"epoch-9 receipt lineage drifted at {key}")
    measured = _usage(receipt.get("new_measured_usage"), label="epoch-9 measured")
    if measured["total_tokens"] != ADOPTED_EPOCH9_TOTAL_TOKENS:
        raise CanonicalV31Epoch10Error("epoch-9 measured usage drifted")
    contract = _load_object(
        PREDECESSOR_ROOT / epoch9.CONTRACT_FILENAME, label="epoch-9 contract"
    )
    turn = contract["turns"][0]
    paths = epoch9._turn_paths(PREDECESSOR_ROOT, str(turn["turn_id"]))  # noqa: SLF001
    ...
    if _load_object(paths["attempt"], label="epoch-9 attempt") != expected_attempt:
        raise CanonicalV31Epoch10Error("epoch-9 attempt drifted")
    ...
    thread_binding = epoch7._verify_thread_binding(  # noqa: SLF001
        _load_object(paths["thread"], label="epoch-9 thread binding"),
        contract=contract,
        turn=turn,
    )
    manifest = _load_object(
        _verify_record(turn["turn_manifest"], label="epoch-9 manifest"),
        label="epoch-9 manifest",
    )
    telemetry = epoch9._validate_completed_sidecar_common(  # noqa: SLF001
        sidecar_path=paths["sidecar"],
        output_path=paths["raw_output"],
        prompt_path=_verify_record(manifest["prompt"], label="epoch-9 prompt"),
        base_path=_verify_record(
            manifest["base_instructions"], label="epoch-9 base"
        ),
        schema_path=_verify_record(manifest["output_schema"], label="epoch-9 schema"),
        thread_binding=thread_binding,
    )
    if telemetry["thread_total_usage"] != measured:
        raise CanonicalV31Epoch10Error("epoch-9 sidecar usage differs from receipt")
    ...
    repairs_by_id = {
        str(row["segment_id"]): row for row in original["reference_repairs"]
    }
    valid: list[tuple[dict[str, Any], dict[str, Any]]] = []
    invalid: list[dict[str, str]] = []
    for label in raw_output["repaired_reference_labels"]:
        segment_id = str(label["segment_id"])
        projected, projection = _project_unique_exact_offsets(
            label, segment_text=repairs_by_id[segment_id]["segment_text"]
        )
        try:
            ...
            valid.append((projected, projection))
    if (
        len(valid) != 1
        or len(invalid) != 1
        or valid[0][0]["segment_id"] != "seg_536dd966606a4980bff030f7"
        or invalid[0]["segment_id"] != "seg_75643e2d9d620af459f9dee3"
        or "metric.raw_text must be an exact evidence substring"
        not in invalid[0]["diagnostic_path"]
    ):
        raise CanonicalV31Epoch10Error("epoch-9 projection partition drifted")
    return {
        "receipt": receipt,
        "receipt_record": _record(receipt_path),
        "terminal_record": _record(terminal_path),
        "runtime_contract_record": _record(
            PREDECESSOR_ROOT / epoch9.CONTRACT_FILENAME
        ),
        "runtime_lock_record": _record(
            PREDECESSOR_ROOT / epoch9.RUNTIME_LOCK_FILENAME
        ),
        ...
        "projected_valid_label": valid[0][0],
        "projection_receipt": valid[0][1],
        "fresh_repair_segment_id": invalid[0]["segment_id"],
        "sanitized_invalid_diagnostic": invalid[0],
    }
...
def _episode_packet(
    ...
    *,
    ...
    if set(segment_ids) - set(repairs):
        raise CanonicalV31Epoch10Error("authority repair segment is unavailable")
    segments = []
    for segment_id in segment_ids:
        source = repairs[segment_id]
        text = source["segment_text"]
        if _sha256_bytes(text.encode("utf-8")) != source["segment_text_sha256"]:
            raise CanonicalV31Epoch10Error("authority segment text drifted")
        segments.append(
            {
                "segment_id": segment_id,
                "segment_text": text,
...
def _prepare_canonical_request(
    ...
    *,
    ...
    if len(requests) != 1 or requests[0]["segment_ids"] != list(segment_ids):
        raise CanonicalV31Epoch10Error("canonical authority request partition drifted")
    return requests[0]
...
def _turn_id(stage: str, episode_id: str, segment_ids: Sequence[str]) -> str:
    ...
    return "authority10_" + stage + "_" + _sha256_bytes(
...
def _prepare_contract(root: Path) -> dict[str, Any]:
    predecessor = _validate_epoch9_predecessor()
    v9_contract = predecessor["contract"]
    adopted = root / ADOPTED_DIRECTORY
    adopted.mkdir(parents=True, exist_ok=True)
    projected_label_record = _write_json(
        adopted / "epoch9-unique-offset-projected-label.private.json",
        predecessor["projected_valid_label"],
    )
    projection_receipt_record = _write_json(
        adopted / "epoch9-unique-offset-projection-receipt.json",
        predecessor["projection_receipt"],
    )
    diagnostic_record = _write_json(
        adopted / "epoch9-remaining-diagnostic.json",
        {
    ...
    context_record = copy.deepcopy(
        v9_contract["adopted_evidence"]["epoch8_valid_context_output"]
    )
    ...
    for episode_index, source_episode in enumerate(v9_contract["episodes"]):
        ...
        source_context_turn = next(
            turn
            for turn in v9_contract["turns"]
            if turn["turn_id"] == source_episode["context_turn_id"]
        )
        source_context_manifest = _load_object(
            _verify_record(source_context_turn["turn_manifest"], label="v9 context manifest"),
            label="v9 context manifest",
        )
        ...
        context_io = _write_context_io(
            root, turn_id=context_turn_id, source_manifest=source_context_manifest
        )
    ...
    adopted_evidence = {
        "epoch7_receipt": copy.deepcopy(
            v9_contract["adopted_evidence"]["epoch7_receipt"]
        ),
        "epoch7_validated_output": copy.deepcopy(
            v9_contract["adopted_evidence"]["epoch7_validated_output"]
        ),
        "epoch8_accounting_rejection": copy.deepcopy(
            v9_contract["adopted_evidence"]["epoch8_accounting_rejection"]
        ),
        "epoch8_valid_context_output": context_record,
        "epoch8_valid_reference_labels": copy.deepcopy(
            v9_contract["adopted_evidence"]["epoch8_valid_reference_labels"]
        ),
        "epoch9_receipt": predecessor["receipt_record"],
        "epoch9_terminal": predecessor["terminal_record"],
        "epoch9_runtime_contract": predecessor["runtime_contract_record"],
        "epoch9_runtime_lock": predecessor["runtime_lock_record"],
        "epoch9_sidecar": predecessor["sidecar_record"],
        "epoch9_raw_output": predecessor["raw_output_record"],
        "epoch9_projected_valid_label": projected_label_record,
        "epoch9_projection_receipt": projection_receipt_record,
        "epoch9_remaining_diagnostic": diagnostic_record,
    }
    return {
        "schema_version": CONTRACT_VERSION,
        "state": "frozen_unit_projection_authority_recovery_ready",
        "directive": _record(DIRECTIVE_PATH),
        "predecessor_root": str(PREDECESSOR_ROOT.resolve()),
        "adopted_evidence": adopted_evidence,
        "authority_plan": copy.deepcopy(v9_contract["authority_plan"]),
        "authority_plan_receipt": copy.deepcopy(
            v9_contract["authority_plan_receipt"]
        ),
        "source_authority_turns": copy.deepcopy(
            v9_contract["source_authority_turns"]
        ),
        "turns": turns,
        "episodes": episodes,
        "model": MODEL,
...
def _preauthorization(root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "adopted_evidence": copy.deepcopy(contract["adopted_evidence"]),
        "turn_manifests": [copy.deepcopy(row["turn_manifest"]) for row in contract["turns"]],
        "adopted_semantic_app_server_turn_count": 3,
        "adopted_development_qa_total_tokens": ADOPTED_DEVELOPMENT_QA_TOTAL_TOKENS,
...
def _authorization(
    ...
    *,
    ...
    if (
        ...
        raise CanonicalV31Epoch10Error("authorization identity or window is invalid")
    return {
        "schema_version": AUTHORIZATION_VERSION,
        "state": "authorized_for_five_unit_projection_authority_turns",
        ...
        "authorized_scope": "one_canonical_label_repair_plus_two_context_label_pairs",
        "runtime_contract": _record(root / CONTRACT_FILENAME),
        "runtime_lock": _record(root / RUNTIME_LOCK_FILENAME),
...
def freeze_recovery(
    *,
    ...
    if output_root.exists():
        raise CanonicalV31Epoch10Error("epoch-10 output root already exists")
...
def verify_preauthorization(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    if (
        ...
        or contract.get("state") != "frozen_unit_projection_authority_recovery_ready"
    ...
    if preauthorization != _preauthorization(output_root, contract):
        raise CanonicalV31Epoch10Error("preauthorization drifted")
...
def _io_binding(root: Path, turn: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise CanonicalV31Epoch10Error("turn I/O binding drifted")
    ...
    if value["stage"] == "canonical_labels":
        ...
        try:
            ...
            raise CanonicalV31Epoch10Error("canonical request drifted") from exc
        if (
            ...
            raise CanonicalV31Epoch10Error("canonical I/O artifacts drifted")
    elif (
        value["stage"] != "context"
        or value["request"] is not None
        or value["segment_ids"] != []
    ):
        raise CanonicalV31Epoch10Error("context I/O binding drifted")
    return value
...
def _thread_binding(
    ...
    *,
    ...
    if (
        ...
        raise CanonicalV31Epoch10Error("semantic thread contract drifted")
    return {
...
def _verify_thread_binding(
    ...
    *,
    ...
    if (
        ...
        raise CanonicalV31Epoch10Error("thread binding drifted")
...
def _context_output(raw: Mapping[str, Any], *, episode_id: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise CanonicalV31Epoch10Rejected("context output is malformed")
...
def _combine_repair_output(
    ...
    if set(labels_by_id) != set(expected_ids):
        raise CanonicalV31Epoch10Rejected("repair label partition drifted")
    context = _load_object(
        _verify_record(
    ...
    try:
        ...
        raise CanonicalV31Epoch10Rejected(
...
def _combine_episode_output(
    ...
    if [row["segment_id"] for row in projected["labels"]] != expected_ids:
        raise CanonicalV31Epoch10Rejected("canonical labels changed authority order")
    combined = {
        "schema_version": authority_plan.OUTPUT_VERSION,
        "episode_id": str(turn["episode_id"]),
        "excluded_source_context": copy.deepcopy(context["excluded_source_context"]),
...
def _finalize_turn(
    *,
    ...
    if _load_object(paths["attempt"], label="semantic attempt") != expected_attempt:
        raise CanonicalV31Epoch10Error("semantic attempt drifted")
    for boundary, bundle in (
        ("initial_before_thread", paths["initial_capacity"]),
        ("preturn_before_turn", paths["preturn_capacity"]),
    ):
    ...
    try:
        ...
        raise CanonicalV31Epoch10Rejected("turn output is not structured JSON") from exc
    schema = _load_object(
        _verify_record(io["output_schema"], label="turn output schema"),
        label="turn output schema",
    )
    try:
        adapter._validate_schema(raw_output, schema, "$")  # noqa: SLF001
    except adapter.CanonicalV31OutputError as exc:
        raise CanonicalV31Epoch10Rejected("turn output failed its frozen schema") from exc
    ...
    if not paths["validated_output"].exists():
        ...
        if _load_object(paths["validated_output"], label="validated output") != validated:
            raise CanonicalV31Epoch10Error("validated output drifted")
    result = {
    ...
    if paths["result"].exists():
        if _load_object(paths["result"], label="turn result") != result:
            raise CanonicalV31Epoch10Error("turn result drifted")
    else:
...
def _accounting(root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(thread_ids) != len(set(thread_ids)) or len(turn_ids) != len(set(turn_ids)):
        raise CanonicalV31Epoch10Error("semantic thread or turn ID was reused")
    return {
    ...
    *,
    ...
    if paths["thread"].is_file():
        if not paths["initial_capacity"].is_dir():
            raise CanonicalV31Epoch10Error("thread exists without initial capacity")
        io = _io_binding(root, turn)
        ...
        if paths["sidecar"].is_file():
            ...
            raise CanonicalV31Epoch10Error("raw output exists without a sidecar")
    elif any(path.exists() for path in semantic_paths[2:]):
        raise CanonicalV31Epoch10Error("turn artifact exists without thread binding")
...
def _merge_output(
    ...
    if path.exists():
        if _load_object(path, label="merged output") != merged:
            raise CanonicalV31Epoch10Error("merged output drifted")
    else:
...
def _terminal_payload(
    *,
    ...
    if state == "passed" and failed:
        state = "rejected"
        reason = "epoch10_unit_projection_cost_rejected"
    merged_record = None
    if state == "passed":
        if len(results) != NEW_TURN_COUNT:
            raise CanonicalV31Epoch10Error("passing receipt lacks every turn result")
    ...
    if adopted_usage["total_tokens"] != ADOPTED_DEVELOPMENT_QA_TOTAL_TOKENS:
        raise CanonicalV31Epoch10Error("adopted usage drifted")
    return {
        ...
        "cumulative_semantic_app_server_turn_count": (
            3 + accounting["semantic_model_call_count"]
        ),
        "semantic_retry_count": 0,
...
def verify_recovery_receipt(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    if not receipt_path.is_file() or not terminal_path.is_file():
        raise CanonicalV31Epoch10Error("terminal mirrors are incomplete")
    if receipt_path.read_bytes() != terminal_path.read_bytes():
        raise CanonicalV31Epoch10Error("terminal mirrors differ")
    ...
    if state not in {"passed", "rejected", "waiting"}:
        raise CanonicalV31Epoch10Error("receipt state drifted")
    rejection = None
    ...
    if observed != expected:
        raise CanonicalV31Epoch10Error("plan-step receipt drifted")
    return observed
...
def status_recovery(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    if (output_root / RECEIPT_FILENAME).is_file():
        ...
        return {
            "schema_version": "pif_canonical_v31_epoch10_status_v1",
            "state": receipt["state"],
    ...
    return {
        "schema_version": "pif_canonical_v31_epoch10_status_v1",
        "state": "ready",
        "reason": "ready_for_one_unit_repair_plus_two_context_label_pairs",
...
async def execute_recovery(
    *,
    ...
    try:
        ...
        for index, turn in enumerate(contract["turns"]):
            ...
            if state == "partial":
                return _write_terminal(
                    ...
                    contract=contract,
                    state="waiting",
                    reason="epoch10_partial_attempt_preserved_no_replay",
                )
        ...
        if len(results) == NEW_TURN_COUNT:
            ...
            return _write_terminal(
                ...
                contract=contract,
                state="passed",
                reason="epoch10_unit_projection_authority_recovery_passed",
            )
        async with adapter._client_factory() as client:  # noqa: SLF001
            ...
            if (
                ...
                raise CanonicalV31Epoch10Error(
                    "epoch-10 requires managed ChatGPT Pro auth"
                )
            for index in range(start_index, NEW_TURN_COUNT):
                ...
                if result.status_ok is not True:
                    raise CanonicalV31Epoch10Waiting(
                ...
                if (
                    ...
                    raise CanonicalV31Epoch10Error("in-memory turn lineage drifted")
    ...
    except CanonicalV31Epoch10Rejected as exc:
        current = contract["turns"][len(results)] if len(results) < NEW_TURN_COUNT else {}
        return _write_terminal(
            root=output_root,
            contract=contract,
            state="rejected",
            reason="epoch10_unit_projection_semantic_output_rejected",
            rejection={
        ...
        return _write_terminal(
            ...
            contract=contract,
            state="rejected",
            reason="epoch10_structured_output_rejected",
            rejection={
                "turn_id": current.get("turn_id"),
                "episode_id": current.get("episode_id"),
                "stage": current.get("stage"),
        ...
        return _write_terminal(
            ...
            contract=contract,
            state="waiting",
            reason="epoch10_operational_waiting_no_replay",
        )
    _merge_output(output_root, contract, results)
    return _write_terminal(
        root=output_root,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=(
            "python3 -m "
            "research_factory.app_server_canonical_v31_epoch10_unit_projection_authority_recovery"
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except CanonicalV31Epoch10Waiting as exc:
        print(_pretty_json({"ok": False, "waiting": True, "error": str(exc)}), end="")
        return 75
    except (
        CanonicalV31Epoch10Error,
