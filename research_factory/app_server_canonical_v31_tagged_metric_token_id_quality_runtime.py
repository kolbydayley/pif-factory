...
"""Epoch-28 two-case full-event AB/BA quality gate for epoch 27.

The runtime consumes the immutable epoch-27 extraction output, places every
...
DIRECTIVE_PATH = (
    PROJECT_ROOT / "automation/pif-evaluation-epoch28-tagged-metric-token-id-quality-v28.json"
).resolve()
PLAN_PATH = (PROJECT_ROOT / "automation/pif-evaluation-semantic-plan-v28.json").resolve()
...
SCHEMA_VERSION = "pif_canonical_v31_tagged_metric_token_id_quality_v28"
LOCK_VERSION = "pif_canonical_v31_tagged_metric_token_id_quality_lock_v1"
LAUNCH_VERSION = "pif_canonical_v31_tagged_metric_token_id_quality_launch_v1"
RECEIPT_VERSION = "pif_semantic_plan_step_receipt_v1"
THREAD_ID = "019f4cf1-c46e-7db3-acd2-bf03c4459a10"
PLAN_EPOCH = 28
STEP_ID = "canonical_v31_epoch28_tagged_metric_token_id_quality_v28"
MODEL = "gpt-5.5"
EFFORT = "high"
...
CAPACITY_TURN_NAMES = ("epoch28_quality_ab", "epoch28_quality_ba")
...
PROCESS_LOCK_NAME = ".canonical-v31-epoch28-quality.lock"
...
EPOCH27_EXPECTED = {
    "runtime_lock": "64c4fab02ade0458977fec6bbddaef170071e64179bb6f5e3ce9dffa50722c4c",
    "receipt": "47d068f553d776d22986312c31f2746b2616a2983f4fb9379901aca31deccb16",
    "authorization": "e0c33accf2fcca7350782af17b0392f10bda8f42ddc45e3975cae6d28f13a126",
    "semantic_attempt": "9df21c2cbe741ec91663341b37ab198674e8f79524340eebe83caa4a1f1f3831",
    "labels": "8e2666750b3fb67658d7786b318d5e337558743bb8e142b0fcf88a667ca437a6",
    "provenance": "f753eb545cf1c56624b53b7aefec80e473c3e286bb8243d461231cff0817a543",
    "fidelity": "94c7132270da9afc51d9dd39756dd6bbcb14403d8889338c46aa81df893db87d",
    "output": "6d2bbe7c4fd7cbfcd86d405ff66eb588495a2c9b11c92e995200cbc7998436fd",
    "sidecar": "a1b7fa4c3cfd5a1b1dc1ede596a487a5ac425619bc6fd691623fc16d2fcbbf20",
    "request": "f94df0a9f66f3de82a67f6442e37e5f6f0e563794e6722e5a6e56cc1e02513b6",
    "baseline_seed": "167bdfb40af5608b1dd23d3199d30b9d12bc7573f369038a4f642e85adaf19b0",
}


class Epoch28QualityError(RuntimeError):
    """The epoch-28 contract, lineage, or immutable evidence drifted."""


class Epoch28QualityWaiting(Epoch28QualityError):
...
def _record(path: Path) -> dict[str, Any]:
    ...
    if not stat.S_ISREG(info.st_mode) or target.is_symlink():
        raise Epoch28QualityError(f"required artifact is not a real file: {target}")
    return {
        "path": str(target),
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise Epoch28QualityError(f"cannot read {label}") from exc
...
def _write_immutable_bytes(path: Path, payload: bytes) -> None:
    ...
    if target.exists():
        if target.is_symlink() or target.read_bytes() != payload:
            raise Epoch28QualityError(f"immutable artifact drifted: {target}")
        return
...
def _process_lock(root: Path) -> Iterator[None]:
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise Epoch28QualityWaiting("another epoch-28 evaluator owns the root") from exc
        try:
            yield
        finally:
...
def _direct_source_paths() -> dict[str, Path]:
    return {
        "epoch27_runtime_lock": EPOCH27_ROOT / "runtime-lock.json",
        "epoch27_receipt": EPOCH27_ROOT / "plan-step-receipt.json",
        "epoch27_terminal": EPOCH27_ROOT / "terminal.json",
        "epoch27_authorization": EPOCH27_ROOT / "operator-authorization.json",
        "epoch27_semantic_attempt": EPOCH27_ROOT / "turn/semantic-attempt.json",
        "epoch27_labels": EPOCH27_ROOT / "turn/canonical-labels.private.json",
        "epoch27_provenance": EPOCH27_ROOT / "turn/evidence-provenance.private.json",
        "epoch27_fidelity": EPOCH27_ROOT / "turn/semantic-fidelity.json",
        "epoch27_output": EPOCH27_ROOT / "turn/output.private.json",
        "epoch27_sidecar": EPOCH27_ROOT / "turn/sidecar.json",
...
def load_contract() -> dict[str, Any]:
    plan = _load_json(PLAN_PATH, "epoch-28 semantic plan")
    directive = _load_json(DIRECTIVE_PATH, "epoch-28 quality directive")
    ...
    if (
        ...
        raise Epoch28QualityError("epoch-28 semantic plan drifted")
    ...
    if (
        ...
        != "pif_evaluation_epoch28_tagged_metric_token_id_quality_directive_v1"
        ...
        raise Epoch28QualityError("epoch-28 directive values drifted")
    ...
    if not isinstance(frozen, list) or [row.get("role") for row in frozen] != list(expected_sources):
        raise Epoch28QualityError("epoch-28 frozen input roles drifted")
    records: dict[str, dict[str, Any]] = {}
    for row in frozen:
        if not isinstance(row, Mapping) or set(row) != {"role", "path", "sha256", "size_bytes"}:
            raise Epoch28QualityError("epoch-28 frozen input record is malformed")
        role = str(row["role"])
        expected = _record(expected_sources[role])
        supplied = {key: row[key] for key in ("path", "sha256", "size_bytes")}
        if supplied != expected:
            raise Epoch28QualityError(f"epoch-28 frozen input drifted: {role}")
        records[role] = expected
    expected_hashes = {
        "epoch27_runtime_lock": EPOCH27_EXPECTED["runtime_lock"],
        "epoch27_receipt": EPOCH27_EXPECTED["receipt"],
        "epoch27_terminal": EPOCH27_EXPECTED["receipt"],
        "epoch27_authorization": EPOCH27_EXPECTED["authorization"],
        "epoch27_semantic_attempt": EPOCH27_EXPECTED["semantic_attempt"],
        "epoch27_labels": EPOCH27_EXPECTED["labels"],
        "epoch27_provenance": EPOCH27_EXPECTED["provenance"],
        "epoch27_fidelity": EPOCH27_EXPECTED["fidelity"],
        "epoch27_output": EPOCH27_EXPECTED["output"],
        "epoch27_sidecar": EPOCH27_EXPECTED["sidecar"],
        "epoch27_request": EPOCH27_EXPECTED["request"],
        "baseline_seed": EPOCH27_EXPECTED["baseline_seed"],
    }
    if any(records[role]["sha256"] != digest for role, digest in expected_hashes.items()):
        raise Epoch28QualityError("epoch-28 direct source hash drifted")
    receipt = _load_json(EPOCH27_ROOT / "plan-step-receipt.json", "epoch-27 receipt")
    terminal = _load_json(EPOCH27_ROOT / "terminal.json", "epoch-27 terminal")
    sidecar = _load_json(EPOCH27_ROOT / "turn/sidecar.json", "epoch-27 sidecar")
    fidelity = _load_json(EPOCH27_ROOT / "turn/semantic-fidelity.json", "epoch-27 fidelity")
    if (
        receipt != terminal
        or receipt.get("state") != "passed"
        or receipt.get("terminal_reason") != "epoch27_one_turn_canary_passed"
        or receipt.get("new_semantic_model_call_count") != 1
        or receipt.get("semantic_retry_count") != 0
        or receipt.get("new_measured_usage", {}).get("total_tokens") != 24_942
        ...
        or sidecar.get("usage", {}).get("total_tokens") != 24_942
        ...
        raise Epoch28QualityError("epoch-27 terminal evidence drifted")
    return {
        "plan": plan,
        "directive": directive,
...
def build_complete_container(
    ...
    request = _load_json(Path(records["epoch27_request"]["path"]), "epoch-27 request")
    labels = _load_json(Path(records["epoch27_labels"]["path"]), "epoch-27 labels")
    provenance = _load_json(
        Path(records["epoch27_provenance"]["path"]), "epoch-27 provenance"
    )
    ...
    if (
        ...
        raise Epoch28QualityError("epoch-27 source segment order drifted")
    if not isinstance(labels, list) or [row.get("segment_id") for row in labels] != list(
        SOURCE_SEGMENT_ORDER
    ):
        raise Epoch28QualityError("epoch-27 candidate segment order drifted")
    ...
    if not isinstance(provenance_segments, list) or [
        ...
        raise Epoch28QualityError("epoch-27 provenance segment order drifted")
    references = baseline.get("references") if isinstance(baseline, Mapping) else None
    if not isinstance(references, list):
        raise Epoch28QualityError("baseline references are missing")
    ...
    if len(reference_by_id) != len(references):
        raise Epoch28QualityError("baseline segment identities are duplicated")
    ...
    for source_index, (segment, label, provenance_segment) in enumerate(
        ...
        if (
            ...
            raise Epoch28QualityError("epoch-28 selected event counts drifted")
        baseline_wrapped = []
        for event_index, raw_event in enumerate(baseline_events):
            if not isinstance(raw_event, Mapping):
                raise Epoch28QualityError("baseline event is malformed")
            ...
            if (
                ...
                raise Epoch28QualityError("baseline exact-evidence lineage drifted")
        ...
        for event_index, (raw_event, provenance_row) in enumerate(
            ...
            if not isinstance(raw_event, Mapping) or not isinstance(provenance_row, Mapping):
                raise Epoch28QualityError("candidate event provenance is malformed")
            event = copy.deepcopy(dict(raw_event))
            evidence = event.get("evidence")
            ...
            if (
                ...
                raise Epoch28QualityError("candidate exact-evidence lineage drifted")
            event_hash = _event_sha256(event)
            lineage = {
                "segment_id": segment_id,
    ...
    try:
        ...
        raise Epoch28QualityError("epoch-28 shared witness pool construction failed") from exc
    if judge.validate_shared_witness_pool(pool):
        raise Epoch28QualityError("epoch-28 shared witness pool failed validation")
    ...
    for mapped_case in mapping.get("cases") or []:
        for witness in mapped_case.get("witnesses") or []:
            ...
            if system_id not in observed_events or not isinstance(original, Mapping):
                raise Epoch28QualityError("private witness lineage drifted")
            ...
            if system_id == SYSTEM_CANDIDATE:
                ...
                if (
                    ...
                    raise Epoch28QualityError("candidate witness lineage hash drifted")
                observed_candidate_lineage[(segment_id, raw_index, event_hash)] += 1
    expected_counts = Counter(EXPECTED_WITNESS_COUNTS)
    if observed_counts != expected_counts:
        raise Epoch28QualityError("43-baseline plus 25-candidate witness counts drifted")
    if observed_events[SYSTEM_BASELINE] != expected_baseline_events:
        raise Epoch28QualityError("baseline raw-to-witness event multiset drifted")
    if observed_events[SYSTEM_CANDIDATE] != expected_candidate_events:
        raise Epoch28QualityError("candidate raw-to-witness event multiset drifted")
    ...
    if observed_candidate_lineage != expected_lineage:
        raise Epoch28QualityError("candidate provenance is not one-to-one with witnesses")
    total = sum(
        len(case["event_set_a"]) + len(case["event_set_b"]) for case in pool["cases"]
    )
    if total != EXPECTED_TOTAL_WITNESSES:
        raise Epoch28QualityError("epoch-28 witness total drifted")
    ...
    if [row["case_id"] for row in ab_cases] != [row["case_id"] for row in ba_cases]:
        raise Epoch28QualityError("AB/BA case order drifted")
    for ab_case, ba_case in zip(ab_cases, ba_cases):
        if (
            ...
            raise Epoch28QualityError("AB/BA opaque witness order drifted")
    if SYSTEM_BASELINE in _canonical_json(pool) or SYSTEM_CANDIDATE in _canonical_json(pool):
        raise Epoch28QualityError("system origin leaked into the model-facing pool")
...
def _freeze_unlocked(root: Path) -> dict[str, Any]:
    ...
    if root.exists() and _meaningful_entries(root):
        raise Epoch28QualityError("unfrozen epoch-28 root is not empty")
    contract = load_contract()
    if contract["receipt_path"] != root / "plan-step-receipt.json":
        raise Epoch28QualityError("epoch-28 output root differs from the contract")
...
def verify_runtime_lock(path: Path, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        lock = _load_json(lock_path, "epoch-28 runtime lock")
        records = [
        ...
        if (
            ...
            raise Epoch28QualityError("epoch-28 runtime lock drifted")
        contract = load_contract()
        source_records = list(contract["source_records"].values()) + [
            _record(PLAN_PATH),
            _record(DIRECTIVE_PATH),
        ...
        if lock.get("source_records") != source_records:
            raise Epoch28QualityError("epoch-28 source record set drifted")
        rebuilt_pool, rebuilt_mapping, rebuilt_lineage = build_complete_container(contract)
        if (
            _load_json(Path(lock["pool"]["path"]), "epoch-28 pool") != rebuilt_pool
            or _load_json(Path(lock["mapping"]["path"]), "epoch-28 mapping")
            != rebuilt_mapping
            or _load_json(Path(lock["candidate_lineage"]["path"]), "epoch-28 lineage")
            != {"event_count": len(rebuilt_lineage), "events": rebuilt_lineage}
        ):
            raise Epoch28QualityError("epoch-28 reconstructed witness lineage drifted")
        for name in ("ab", "ba"):
            _turn_material(lock_path.parent, name, rebuilt_pool)
        attempt_spec = _load_json(
            Path(lock["attempt_spec"]["path"]), "epoch-28 attempt spec"
        )
        if (
            ...
            raise Epoch28QualityError("epoch-28 attempt specification drifted")
        ...
        if (
            ...
            raise Epoch28QualityError("epoch-28 capacity policy drifted")
        return lock
...
class Epoch28ReserveCapacityClient(reserve.ReserveCapacityGatedCodexAppServerClient):
    """Map the mature judge layout onto the frozen epoch-28 reserve policy."""
    ...
    def _turn_index(self, checkpoint_path: Any) -> tuple[int, Path]:
        if checkpoint_path is None:
            raise reserve.ReserveCapacityError("epoch-28 capacity checkpoint is required")
        checkpoint = Path(checkpoint_path).expanduser().resolve()
        for index in range(len(CAPACITY_TURN_NAMES)):
            if checkpoint == self._paths_for_index(index)[0]:
                return index, checkpoint
        raise reserve.ReserveCapacityError("capacity checkpoint is outside epoch 28")
    ...
    def _verify_prior_turns(self, turn_index: int) -> None:
        for index in range(turn_index):
            ...
            sidecar = _load_json(sidecar_path, "prior epoch-28 judge sidecar")
            ...
            if (
                ...
                raise reserve.ReserveCapacityError("prior epoch-28 judge turn drifted")
...
def _validate_capacity_checkpoint(path: Path) -> dict[str, Any]:
    value = _load_json(path, "epoch-28 capacity checkpoint")
    ...
    if not isinstance(value, Mapping) or any(value.get(key) != expected for key, expected in required.items()):
        raise Epoch28QualityError("epoch-28 capacity checkpoint drifted")
    ...
    for key in (
        ...
        if value.get(key) != evaluation[key]:
            raise Epoch28QualityError("epoch-28 capacity arithmetic drifted")
    if remaining != 100 - used:
        raise Epoch28QualityError("epoch-28 capacity remaining percentage drifted")
    return dict(value)
...
def _validate_launch_receipt(root: Path) -> dict[str, Any]:
    value = _load_json(root / "launch-receipt.json", "epoch-28 launch receipt")
    if (
        ...
        raise Epoch28QualityError("epoch-28 launch receipt drifted")
    return dict(value)


def _valid_usage(value: Any) -> dict[str, int]:
    if not isinstance(value, Mapping):
        raise Epoch28QualityError("judge usage is missing")
    result: dict[str, int] = {}
    for field in USAGE_FIELDS:
        item = value.get(field)
        if isinstance(item, bool) or not isinstance(item, int) or item < 0:
            raise Epoch28QualityError("judge usage is malformed")
        result[field] = item
    if (
        ...
        raise Epoch28QualityError("judge usage arithmetic drifted")
    return result
...
def _turn_material(root: Path, name: str, pool: Mapping[str, Any]) -> tuple[str, dict[str, Any], str]:
    ...
    if (
        ...
        raise Epoch28QualityError("epoch-28 frozen judge request drifted")
...
def _validate_completed_turn(
    ...
    try:
        ...
        raise Epoch28QualityError(f"epoch-28 {name} checkpoint drifted") from exc
    required = {
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch28QualityError(f"epoch-28 {name} managed sidecar contract drifted")
    usage = _valid_usage(sidecar.get("usage"))
    if sidecar.get("thread_total_usage") != usage:
        raise Epoch28QualityError(f"epoch-28 {name} thread usage drifted")
    if usage["total_tokens"] > MAXIMUM_TOTAL_TOKENS_PER_TURN:
        raise Epoch28QualityError(f"epoch-28 {name} exceeded the token ceiling")
    if judge.validate_judge_output(output, judge.build_judge_variants(pool)[name]):
        raise Epoch28QualityError(f"epoch-28 {name} judge output is invalid")
...
def _validate_partial_sidecar(
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch28QualityError(f"partial {name} sidecar lineage drifted")
    output_path = root / f"judge/output-{name}.private.json"
    if Path(str(sidecar.get("output_path") or "")).expanduser().resolve() != output_path:
        raise Epoch28QualityError(f"partial {name} output path drifted")
    thread_id = sidecar.get("thread_id")
    if not isinstance(thread_id, str) or not thread_id:
        raise Epoch28QualityError(f"partial {name} sidecar lacks thread identity")
    turn_id = sidecar.get("turn_id")
    if turn_id is not None and (not isinstance(turn_id, str) or not turn_id):
        raise Epoch28QualityError(f"partial {name} turn identity is malformed")
...
def _partial_accounting(root: Path) -> dict[str, Any]:
    ...
    turns = []
    ab_evidence = False
    pool = _load_json(root / "shared-witness-pool.private.json", "epoch-28 pool")
    for name in ("ab", "ba"):
        sidecar_path = root / f"judge/sidecars/{name}.json"
        ...
        if name == "ba" and any(present) and not ab_evidence:
            raise Epoch28QualityError("BA evidence exists without AB lineage")
        ...
        if output_path.is_file():
            ...
            if judge.validate_judge_output(output, judge.build_judge_variants(pool)[name]):
                raise Epoch28QualityError(f"partial {name} output is invalid")
        if sidecar_path.is_file():
            sidecar = _load_json(sidecar_path, f"partial {name} sidecar")
            if not isinstance(sidecar, Mapping):
                raise Epoch28QualityError(f"partial {name} sidecar is malformed")
            ...
            if state == "completed" and output_path.is_file() and capacity_path.is_file():
                ...
                raise Epoch28QualityError(f"partial {name} sidecar state drifted")
    ...
    if measured_calls + unknown_calls > MODEL_CALL_CAP:
        raise Epoch28QualityError("epoch-28 partial call count exceeded the cap")
    return {
    ...
    for case in mapping.get("cases") or []:
        for witness in case.get("witnesses") or []:
            ...
            if system_id not in {SYSTEM_BASELINE, SYSTEM_CANDIDATE} or witness_id in systems:
                raise Epoch28QualityError("private witness system lineage drifted")
    ...
    *, pool: Mapping[str, Any], mapping: Mapping[str, Any], consensus: Mapping[str, Any]
    ...
    for row in consensus.get("cases") or []:
        ...
        if case_id not in pool_cases or row.get("partition_abstained_witness_ids"):
            raise Epoch28QualityWaiting("AB/BA equivalence partition requires adjudication")
        events = {
        ...
        if grouped != set(events) or set(support) != set(events):
            raise Epoch28QualityError("shared-reference consensus omits a witness")
        group_by_witness = {
    ...
    if len(case_scores) != len(pool_cases) or not case_scores:
        raise Epoch28QualityError("macro score does not cover both cases")
    return {
        "systems": {
            system_id: {
...
def _aggregate_completed_usage(
    ...
    for name in ("ab", "ba"):
        ...
        if identity in identities:
            raise Epoch28QualityError("epoch-28 judge thread/turn identity was replayed")
    ...
    if usage["total_tokens"] > TOTAL_TOKEN_CAP:
        raise Epoch28QualityError("epoch-28 judge token cap exceeded")
    return usage, turns


def _score(root: Path) -> dict[str, Any]:
    pool = _load_json(root / "shared-witness-pool.private.json", "epoch-28 pool")
    mapping = _load_json(root / "private-witness-mapping.private.json", "epoch-28 mapping")
    consensus = _load_json(root / "judge/consensus.private.json", "epoch-28 consensus")
    report = _load_json(root / "judge/report.json", "epoch-28 judge report")
    if (
        ...
        raise Epoch28QualityError("epoch-28 judge report drifted")
    usage, turns = _aggregate_completed_usage(root, pool)
    if report.get("usage") != usage:
        raise Epoch28QualityError("epoch-28 report accounting drifted")
    try:
        ...
        if "abstained semantic equivalence partition" in str(exc):
            raise Epoch28QualityWaiting(
                "AB/BA disagreement requires separately frozen adjudication"
            ) from exc
        raise Epoch28QualityError("epoch-28 shared-reference score failed") from exc
    if set(shared.get("reference_system_ids") or []) != {SYSTEM_BASELINE, SYSTEM_CANDIDATE}:
        raise Epoch28QualityError("epoch-28 shared reference is not symmetric")
...
def _recover_terminal_mirror(root: Path) -> dict[str, Any] | None:
    ...
    value = _load_json(existing, "single epoch-28 terminal mirror")
...
def _validate_receipt_payload(root: Path, receipt: Mapping[str, Any]) -> None:
    if (
        ...
        raise Epoch28QualityError("epoch-28 terminal receipt drifted")
    ...
    if any(isinstance(item, bool) or not isinstance(item, int) or item < 0 for item in (
        ...
        raise Epoch28QualityError("epoch-28 receipt call accounting drifted")
    ...
    if (
        ...
        raise Epoch28QualityError("epoch-28 aggregate accounting drifted")
    ...
    if receipt.get("state") == "passed" or quality_terminal:
        ...
        if (
            ...
            raise Epoch28QualityError("epoch-28 quality receipt drifted")
    else:
        ...
        if (
            ...
            raise Epoch28QualityError("epoch-28 waiting receipt drifted")
...
def verify_receipt(root: Path = DEFAULT_ROOT, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        ...
        receipt = _load_json(output_root / "plan-step-receipt.json", "epoch-28 receipt")
        terminal = _load_json(output_root / "terminal.json", "epoch-28 terminal")
        if receipt != terminal:
            raise Epoch28QualityError("epoch-28 terminal mirrors differ")
...
async def _run_unlocked(
    ...
    *,
    ...
    if artifacts_exist and not launch_exists:
        raise Epoch28QualityError("semantic artifacts exist without an epoch-28 launch")
    if launch_exists:
        ...
        if report_exists:
            try:
                score = _score(root)
            except Epoch28QualityWaiting as exc:
                ...
                return _write_terminal(
                    ...
                    _receipt(
                        root,
                        state="waiting",
                        terminal_reason="epoch28_ab_ba_disagreement_requires_versioned_adjudication",
            ...
            return _write_terminal(
                ...
                _receipt(
                    ...
                    terminal_reason=(
                        "epoch28_two_case_quality_gate_passed"
                        if score["passed"]
                        else "epoch28_two_case_quality_gate_rejected"
                    ),
                    accounting=accounting,
                    score=score,
                ),
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch28_partial_or_interrupted_ab_ba_preserved_without_replay",
                accounting=accounting,
    ...
    try:
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch28_judge_structured_output_rejected",
                accounting=accounting,
        ...
        return _write_terminal(
            ...
            _receipt(
                ...
                terminal_reason=(
                    "epoch28_judge_structured_output_rejected"
                    if rejected
                    else "epoch28_judge_operational_attempt_waiting_no_replay"
                ),
                accounting=accounting,
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch28_judge_operational_attempt_waiting_no_replay",
                accounting=accounting,
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch28_judge_exact_validation_rejected",
                accounting=accounting,
                diagnostic={"class": type(exc).__name__},
            ),
        )
...
async def run(
    ...
    *,
    ...
    if output_root == DEFAULT_ROOT and (
        ...
        raise Epoch28QualityError(
            "the live epoch-28 root forbids injected judge runners or clients"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 75
    except Epoch28QualityError as exc:
