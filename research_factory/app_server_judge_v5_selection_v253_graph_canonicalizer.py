...
"""Run a bounded source-graph to global full-schema canonicalizer canary."""
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v253_graph_canonicalizer_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v253_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v253_terminal_v1"
PHASE_ID = "development_selection_v5_4_v253_graph_canonicalizer"
MODEL = "gpt-5.6-sol"
EFFORT = "low"
...
class V253GraphCanonicalizerError(RuntimeError):
    """The v253 architecture cannot proceed or be adopted safely."""


class V253OutputContractError(V253GraphCanonicalizerError):
    """A completed canonicalizer output violated the frozen projection contract."""


class V253ArchitectureStop(V253GraphCanonicalizerError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V253GraphCanonicalizerError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V253GraphCanonicalizerError(f"frozen {path.name} drifted")
        return
...
def _validate_lineage() -> dict[str, Any]:
    ...
    if (
        ...
        raise V253GraphCanonicalizerError("v253 architecture lineage drifted")
...
def _graph_ids(source_graph: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for segment in source_graph.get("segments") or []:
        ...
        if len(ids) != len(set(ids)):
            raise V253GraphCanonicalizerError("source graph event IDs are not unique")
...
def prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise V253GraphCanonicalizerError("v253 request or source graph drifted")
    return {
        "turn_name": "v253_graph_canonicalizer_" + sha256_text(source["episode_id"])[:20],
...
def project_output(
    ...
    try:
        ...
        raise V253OutputContractError("canonicalizer output schema failed") from exc
    if output.get("episode_id") != turn["episode_id"]:
        raise V253OutputContractError("canonicalizer episode id drifted")
    segments = list(output.get("segments") or [])
    if [row.get("segment_id") for row in segments] != list(turn["segment_ids"]):
        raise V253OutputContractError("canonicalizer segment coverage or order drifted")
    ...
    for position, segment in enumerate(projected["segments"]):
        ...
        for event_index, event in enumerate(segment.get("events") or []):
            if event.pop("event_id") != f"S{position}C{event_index:03d}":
                raise V253OutputContractError("canonicalizer final event ID order drifted")
            ...
            if not source_ids or any(item not in expected_set for item in source_ids):
                raise V253OutputContractError("canonicalizer event cites cross-segment graph node")
        ...
        if any(item not in expected_set for item in dropped_ids):
            raise V253OutputContractError("canonicalizer drop cites cross-segment graph node")
        observed.extend(dropped_ids)
        counts = Counter(observed)
        if set(counts) != expected_set or any(value != 1 for value in counts.values()):
            raise V253OutputContractError("canonicalizer graph coverage is not exact once")
    ...
    try:
        ...
        raise V253OutputContractError(str(exc)) from exc
    coverage = {
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v253 runtime lock")
    root = path.parent.resolve()
    spec = _load_json(root / "attempt-spec.json", "v253 spec")
    ...
    if (
        ...
        raise V253GraphCanonicalizerError("v253 runtime lock contract drifted")
    records = [
        lock.get("pinned_codex_cli"),
        *(lock.get("runtime_files") or []),
        *(lock.get("direct_lineage") or []),
        lock.get("v252_failure_audit"),
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V253GraphCanonicalizerError("v253 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V253GraphCanonicalizerError("v253 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    spec = _load_json(spec_path, "v253 spec")
    paths = _turn_paths(root, str(spec["turn_name"]))
    lineage = _validate_lineage()
    ...
    return {
        ...
        "turn": {
            **turn,
            "private_input": _load_json(paths["input"], "v253 input"),
            "prompt": paths["prompt"].read_text(encoding="utf-8"),
            "base": paths["base"].read_text(encoding="utf-8"),
            "schema": _load_json(paths["schema"], "v253 schema"),
            "projection_schema": _load_json(paths["projection_schema"], "v253 projection schema"),
            "direct_schema": _load_json(paths["direct_schema"], "v253 direct schema"),
            "paths": paths,
        },
    }


def freeze_v253(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v253 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V253GraphCanonicalizerError("unfinished v253 root is not replayable")
    ...
    v252_audit_path = root / "v252-measured-failure-audit.json"
    _write_stable_time(
        v252_audit_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "predecessor_terminal": lineage["records"]["v252_terminal"],
            "predecessor_sidecar": lineage["records"]["v252_sidecar"],
            "predecessor_output_not_used": lineage["records"]["v252_output"],
            "terminal_reported_usage_status": "unknown",
            "authoritative_sidecar_usage_status": "measured",
            "authoritative_total_tokens": 98_728,
            "declared_total_token_bound": 70_000,
            "corrected_failure_class": "measured_judge_token_bound_exceeded",
    ...
    _write_stable_time(
        authorization_path,
        {
            ...
            "scope": "one bounded source_graph_then_global_full_schema_canonicalizer canary",
    ...
    _write_stable_time(
        ranking_path,
        {
            ...
            "measured_basis": {
                ...
                "v252_measured_judge_tokens": 98_728,
                "v252_declared_judge_bound": 70_000,
            },
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "hypothesis": (
                "the measured v244 source graph can retain blind recall while one global Sol-low canonicalizer "
                "re-reads the source and corrects event boundaries and full-field semantics using v249 explicit "
                "applicability, with every graph node assigned exactly once to keep, merge, or drop"
            ),
    ...
    _write_stable_time(
        spec_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "state": "frozen_before_one_turn_graph_canonicalizer_canary",
    ...
    lock = {
        ...
        "v252_failure_audit": _record(v252_audit_path),
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        sidecar = _load_json(paths["sidecar"], "v253 sidecar")
        measured = _measured_usage(sidecar)
        if measured is not None:
            usage = measured
            unknown = 0
            measured_over_cap = usage["total_tokens"] > MAX_TOTAL_TOKENS
    semantic = isinstance(exc, (V253OutputContractError, V253ArchitectureStop)) or measured_over_cap
    ...
    terminal = {
        ...
        "terminal_reason": (
            "v253_graph_canonicalizer_structural_quality_or_cost_gate_not_passed"
            if semantic
        ...
        "exact_next_action": (
            ...
            if semantic
            else "audit immutable v253 infrastructure attempt; no retry"
        ),
    }
...
async def run_v253(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v253 terminal")
    frozen = freeze_v253(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V253GraphCanonicalizerError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V253GraphCanonicalizerError("v253 turn did not complete")
        usage = _measured_usage(_load_json(paths["sidecar"], "v253 sidecar"))
        if usage is None:
            raise V253GraphCanonicalizerError("v253 sidecar accounting or auth failed")
        if usage["total_tokens"] > MAX_TOTAL_TOKENS:
            raise V253ArchitectureStop("v253 measured turn exceeded frozen token bound")
        ...
        if not gate["passed"]:
            raise V253ArchitectureStop("v253 structural quality or cost gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "v253_architecture_structural_gate_passed",
            "terminal_reason": "v253_graph_canonicalizer_structural_cost_gate_passed",
            "semantic_attempt_count": 1,
            "semantic_retry_count": 0,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v253 source graph canonicalizer")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v253(output_dir=Path(args.output_dir))
        result = {
            "state": "frozen",
        ...
        terminal = asyncio.run(
            run_v253(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
