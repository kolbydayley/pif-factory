...
"""Low-effort recovery adapter for closed-keyed-enum compact inventory."""
...
from . import (
    app_server_canonical_v31_keyed_enum_compact_inventory_evidence_ordinal_episode_batch as parent,
)
...
ADAPTER_SCHEMA_VERSION = "pif_canonical_v31_low_effort_keyed_enum_compact_inventory_episode_batch_v1"
PROJECTION_SCHEMA_VERSION = "pif_canonical_v31_low_effort_keyed_enum_compact_inventory_projection_v1"
PROVENANCE_SCHEMA_VERSION = "pif_canonical_v31_low_effort_keyed_enum_compact_inventory_provenance_v1"
FIDELITY_SCHEMA_VERSION = "pif_canonical_v31_low_effort_keyed_enum_compact_inventory_fidelity_v1"
CANDIDATE_SYSTEM_ID = "pif_low_effort_keyed_enum_compact_inventory_ai_discourse_v3_1_v1"
CANONICAL_SCHEMA_STRATEGY = "canonical_v31_low_effort_compact_inventory_closed_keyed_enum_v1"
KEYED_ENUM_PROTOCOL_VERSION = parent.KEYED_ENUM_PROTOCOL_VERSION
...
def semantic_integrity_contract() -> dict[str, Any]:
    ...
    value["low_effort_timeout_recovery_only"] = True
    return value
...
def _parent_request_json(
    ...
    if len(matches) != 1:
        raise CanonicalV31EpisodeBatchError(
            "low-effort keyed-enum parent request cannot be reconstructed"
        )
...
def _low_effort_request(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    value["effort"] = EFFORT
    value["batch_id"] = "cv31leke_" + _sha256_text(
        _canonical_json(
            {
...
def validate_prepared_request(request: Mapping[str, Any]) -> dict[str, Any]:
    expected = _low_effort_request(_parent_request(request))
    if dict(request) != expected:
        raise CanonicalV31EpisodeBatchError("low-effort keyed-enum request drifted")
...
def validate_and_project_output(
    ...
    projected = parent.validate_and_project_output(_parent_request(validated), output)
...
def encode_parent_output_for_test(
    ...
    return parent.encode_parent_output_for_test(_parent_request(request), output)
...
def validate_turn_sidecar(
    ...
    *,
    ...
    validated = validate_prepared_request(request)
    sidecar = base._load_json(sidecar_path, "low-effort keyed-enum sidecar")  # noqa: SLF001
    ...
    if (
        ...
        raise CanonicalV31TelemetryError(
            "managed-auth low-effort keyed-enum sidecar contract failed"
        )
    ...
    if (
        ...
        raise CanonicalV31TelemetryError("low-effort keyed-enum output is absent or changed")
    return {
...
def build_six_arm_matrix_binding() -> dict[str, Any]:
    return {
        "schema_version": "pif_canonical_v31_low_effort_keyed_enum_binding_v1",
