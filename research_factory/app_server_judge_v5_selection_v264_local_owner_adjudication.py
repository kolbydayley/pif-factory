...
"""Adjudicate only the mechanically observable v263 ownership-difference closure."""
...
from . import app_server_judge_v5_selection_v263_frozen_alignment as v263
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v264_local_owner_adjudication_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v264_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v264_terminal_v1"
WINNER_VERSION = "pif_app_server_judge_v5_selection_v264_winner_v1"
PHASE_ID = "judge_v5_4_selection_v264_local_owner_adjudication"
TURN_NAME = "v264_local_side_free_owner_adjudication"
MODEL = "gpt-5.5"
EFFORT = "high"
...
EXPECTED_LOCAL_WITNESS_COUNT = 6
USAGE_FIELDS = v263.USAGE_FIELDS
PROJECT_ROOT = v263.PROJECT_ROOT
PIPELINE_ROOT = v263.PIPELINE_ROOT
V263_ROOT = v263.DEFAULT_OUTPUT_ROOT
V252_ROOT = (
...
PINNED_CODEX_0_144_1 = v263.PINNED_CODEX_0_144_1
FROZEN_V153_INSTRUCTION_HASH = (
    "f295e1bad136c68aa6ecf1ee061a8f96d1225c641813970f792da482d686e79b"
)


class V264LocalOwnerError(RuntimeError):
    """The v264 local owner adjudication cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V264LocalOwnerError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V264LocalOwnerError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    base = V263_ROOT / "turns" / "v263-frozen-alignment-base"
    canary = V263_ROOT / "turns" / "v263-frozen-alignment-balanced-canary"
    ...
    return {
        "v263_terminal": V263_ROOT / "terminal.json",
        "v263_score": V263_ROOT / "alignment-score.json",
        "v263_runtime_lock": V263_ROOT / "runtime-lock.json",
        "v263_mapping": V263_ROOT / "origin-map.private.json",
        "v263_receipts": V263_ROOT / "support-receipts.private.json",
        "v263_base_input": base / "input.private.json",
        "v263_base_output": base / "output.private.json",
        "v263_canary_input": canary / "input.private.json",
        "v263_canary_output": canary / "output.private.json",
...
def adjudication_instructions() -> str:
    value = v263.v251.v246.v130.alignment_instructions_v130() + (
    ...
    if sha256_text(value) != FROZEN_V153_INSTRUCTION_HASH:
        raise V264LocalOwnerError("frozen v153 adjudication instructions drifted")
    return value
...
def _validate_lineage() -> dict[str, Any]:
    ...
    v263.verify_runtime_lock(paths["v263_runtime_lock"])
    t251 = _load_json(paths["v263_terminal"], "v263 terminal")
    q251 = _load_json(paths["v263_score"], "v263 score")
    ...
    if (
        t251.get("terminal_reason") != "v263_alignment_quality_or_permutation_gate_not_passed"
        ...
        raise V264LocalOwnerError("v264 frozen lineage or adjudication protocol drifted")
...
def build_local_bundle(lineage: Mapping[str, Any]) -> dict[str, Any]:
    paths = lineage["paths"]
    base_input = _load_json(paths["v263_base_input"], "v263 base input")
    base_output = _load_json(paths["v263_base_output"], "v263 base output")
    canary_input = _load_json(paths["v263_canary_input"], "v263 canary input")
    canary_output = _load_json(paths["v263_canary_output"], "v263 canary output")
    receipts = _load_json(paths["v263_receipts"], "v263 support receipts")
    ...
    if len(base["cases"]) != 1 or len(canary["cases"]) != 1:
        raise V264LocalOwnerError("v264 full alignment case coverage drifted")
    ...
    swap = int(sha256_text(f"v264|candidate-order|{packet_case['case_id']}")[:2], 16) % 2 == 1
    if swap:
    ...
    if (
        ...
        raise V264LocalOwnerError("v264 local owner bundle drifted")
    return {
        "closure": closure,
        "packet": packet,
        ...
        "mapping": _load_json(paths["v263_mapping"], "v263 mapping"),
        "prompt": prompt,
        "schema": schema,
...
def _merge_local_owner(
    ...
    if any(set(group) & closure and not set(group) <= closure for group in full_case["equivalence_groups"]):
        raise V264LocalOwnerError("mechanical closure omitted a base group member")
    unaffected_pairs = [
        copy.deepcopy(row)
    ...
    if any(
        ...
        raise V264LocalOwnerError("mechanical closure omitted a base pair member")
    return {
...
def reconcile_and_score(
    *, bundle: Mapping[str, Any], output: Mapping[str, Any]
    ...
    if errors:
        raise V264LocalOwnerError(
            "v264 local adjudication output did not validate: "
            + "; ".join(sorted(set(errors)))
        )
    normalized = judge.normalize_neutral_alignment_output(output, bundle["local_input"])
    if len(normalized["cases"]) != 1:
        raise V264LocalOwnerError("v264 local output case coverage drifted")
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v264 runtime lock")
    ...
    if (
        ...
        raise V264LocalOwnerError("v264 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V264LocalOwnerError("v264 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V264LocalOwnerError("v264 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    if (
        _load_json(paths["input"], "v264 input") != bundle["local_input"]
        or _load_json(paths["packet"], "v264 packet") != bundle["packet"]
        or paths["prompt"].read_text(encoding="utf-8") != bundle["prompt"]
        or _load_json(paths["schema"], "v264 schema") != bundle["schema"]
    ):
        raise V264LocalOwnerError("v264 frozen request drifted")
    return {
        "root": root,
...
def freeze_v264(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v264 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V264LocalOwnerError("unfinished v264 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "retry_count": 0,
            "majority_voting_used": False,
            "production_amortized_extractor_ratio": v263.OBSERVED_PRODUCTION_RATIO,
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        measured = _measured_usage(_load_json(paths["sidecar"], "v264 sidecar"))
    ...
    terminal = {
        ...
        "terminal_reason": (
            "v264_local_adjudication_quality_or_cost_gate_not_passed"
        ...
        "exact_next_action": "audit immutable v264 attempt; no retry",
    }
...
def _winner(score_path: Path, audit_path: Path) -> dict[str, Any]:
    winner = v263._winner(score_path)
    winner.update(
        {
            "schema_version": WINNER_VERSION,
        "system_id": "v261_dual_decomposition_explicit_applicability_v264_local_owner_adjudicated",
            "adjudication": {
                "model": MODEL,
                "effort": EFFORT,
                "call_count": 1,
                ...
                "pre_adjudication_score": _record(V263_ROOT / "alignment-score.json"),
...
async def run_v264(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v264 terminal")
    frozen = freeze_v264(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V264LocalOwnerError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V264LocalOwnerError("local owner adjudication did not complete")
        usage = _measured_usage(_load_json(paths["sidecar"], "v264 sidecar"))
        if usage is None:
            raise V264LocalOwnerError("v264 sidecar accounting or auth failed")
        if usage["total_tokens"] > MAX_TOTAL_TOKENS:
            raise V264LocalOwnerError("v264 measured turn exceeded frozen token bound")
        ...
        terminal = {
            ...
            "terminal_reason": (
                "v264_local_owner_adjudication_passed_development_winner_frozen_holdout_authorized"
                if passed
                else "v264_local_owner_adjudication_quality_gate_not_passed"
            ),
            ...
            "overall_goal_complete": False,
            "goal_status_required": "active",
            "production_amortized_total_token_ratio": v263.OBSERVED_PRODUCTION_RATIO,
            ...
            "exact_next_action": (
                ...
                if passed
                else "reject v261 and resume ranked distinct architecture search"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v264 local owner adjudication")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v264(output_dir=Path(args.output_dir))
        result = {
            "state": "frozen",
        ...
        terminal = asyncio.run(
            run_v264(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
