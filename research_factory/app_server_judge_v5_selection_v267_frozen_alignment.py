...
"""Run the unchanged two-permutation neutral alignment judge for v265."""
...
from . import app_server_judge_v5_selection_v266_frozen_support as v266
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v267_frozen_alignment_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v267_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v267_terminal_v1"
WINNER_VERSION = "pif_app_server_judge_v5_selection_v267_winner_v1"
PHASE_ID = "judge_v5_4_selection_v267_frozen_alignment"
TURN_NAMES = ("v267_frozen_alignment_base", "v267_frozen_alignment_balanced_canary")
...
V266_ROOT = v266.DEFAULT_OUTPUT_ROOT
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v267-frozen-alignment-v265"
).resolve()
PINNED_CODEX_0_144_1 = v265.PINNED_CODEX_0_144_1


class V267FrozenAlignmentError(RuntimeError):
    """The v267 frozen alignment gate cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V267FrozenAlignmentError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V267FrozenAlignmentError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    v265_turns = sorted(V265_ROOT.glob("turns/v265-segment-*"))
    if len(v265_turns) != 2:
        raise V267FrozenAlignmentError("v265 turn membership drifted")
    return {
        ...
        "v265_sidecar_0": v265_turns[0] / "sidecar.json",
        "v265_sidecar_1": v265_turns[1] / "sidecar.json",
        "v265_normalized": V265_ROOT / "normalized-output.private.json",
        "v266_terminal": V266_ROOT / "terminal.json",
        "v266_runtime_lock": V266_ROOT / "runtime-lock.json",
        "v266_pool": V266_ROOT / "support-pool.private.json",
        "v266_origin": V266_ROOT / "origin-map.private.json",
        "v266_receipts": V266_ROOT / "support-receipts.private.json",
        "v266_score": V266_ROOT / "support-score.private.json",
        "v266_audit": V266_ROOT / "support-audit.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    v266.verify_runtime_lock(paths["v266_runtime_lock"])
    v251.v246.v224.verify_runtime_lock(paths["v224_runtime_lock"])
    t255 = _load_json(paths["v265_terminal"], "v265 terminal")
    g255 = _load_json(paths["v265_gate"], "v265 gate")
    t256 = _load_json(paths["v266_terminal"], "v266 terminal")
    a256 = _load_json(paths["v266_audit"], "v266 audit")
    ...
    if (
        t255.get("terminal_reason") != "v265_segment_isolation_structural_cost_gate_passed"
        ...
        or a256.get("support_status_counts") != {"supported": 58, "unsupported": 1}
        ...
        raise V267FrozenAlignmentError("v267 frozen alignment lineage or protocol drifted")
    return {"paths": paths, "records": records}


def _opaque_id(kind: str, value: str) -> str:
    prefix = "jcase_" if kind == "case" else "wit_"
    return prefix + sha256_text(f"v267|{kind}|{value}")[:24]


def build_alignment_bundle(lineage: Mapping[str, Any]) -> dict[str, Any]:
    raw_pool = _load_json(lineage["paths"]["v266_pool"], "v266 support pool")
    origin_rows = _load_json(lineage["paths"]["v266_origin"], "v266 origin map")["rows"]
    score_rows = _load_json(lineage["paths"]["v266_score"], "v266 support score")["rows"]
    receipts = _load_json(lineage["paths"]["v266_receipts"], "v266 support receipts")
    ...
    if set(origin_by_id) != set(status_by_id) or set(origin_by_id) != set(receipt_by_id):
        raise V267FrozenAlignmentError("support receipt and provenance coverage drifted")
    ...
    if (
        ...
        or counts[("candidate", "unsupported")]
        != EXPECTED_UNSUPPORTED_CANDIDATE_WITNESSES
        or counts[("reference", "unsupported")] != 0
        or counts[("reference", "abstain")] != 0
        or counts[("candidate", "abstain")] != 0
        or len(no_signal_rows) != 1
        or no_signal_rows[0]["origin"] != "candidate"
        or no_signal_rows[0]["support_status"] != "supported"
    ):
        raise V267FrozenAlignmentError("supported alignment partition drifted")
    pool = {
        "schema_version": app_server_llm_judge.SHARED_WITNESS_POOL_VERSION,
        "seed_sha256": sha256_text("v267-v265-supported-dense-pool"),
        "cases": [
            {
    ...
    if errors:
        raise V267FrozenAlignmentError(
    ...
    if canary_ids != list(reversed(base_ids)):
        raise V267FrozenAlignmentError("balanced witness permutation drifted")
    turns = []
    for turn_name, permutation, value in zip(TURN_NAMES, PERMUTATIONS, (base, canary)):
        ...
        if prompt_bytes > MAX_PROMPT_BYTES or schema_bytes > MAX_SCHEMA_BYTES:
            raise V267FrozenAlignmentError("alignment request size cap exceeded")
        turns.append(
            {
                "turn_name": turn_name,
...
def score_alignment(
    *, base: Mapping[str, Any], canary: Mapping[str, Any], mapping: Mapping[str, Any]
    ...
    if set(base_rows) != set(canary_rows) or len(base_rows) != EXPECTED_ALIGNMENT_CASE_COUNT:
        raise V267FrozenAlignmentError("alignment normalized case coverage drifted")
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v267 runtime lock")
    ...
    if (
        ...
        raise V267FrozenAlignmentError("v267 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V267FrozenAlignmentError("v267 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V267FrozenAlignmentError("v267 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    return {
        "root": root,
        "spec_path": spec_path,
        "spec": _load_json(spec_path, "v267 spec"),
        "runtime_lock": root / "runtime-lock.json",
        "capacity_policy": root / "capacity-policy.json",
        "bundle": {
            "mapping": _load_json(root / "origin-map.private.json", "v267 origin map"),
            "instructions": (
...
def freeze_v267(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v267 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V267FrozenAlignmentError("unfinished v267 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "strategy": "unchanged_frozen_two_permutation_neutral_alignment_over_v265",
            "model": MODEL,
            "effort": EFFORT,
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V267FrozenAlignmentError("v267 sidecar usage incomplete") from exc
    if (
        ...
        raise V267FrozenAlignmentError("v267 sidecar accounting or auth failed")
    return usage
...
def _failure_terminal(
    ...
    terminal = {
        ...
        "exact_next_action": "audit immutable v267 judge attempt; no retry",
    }
...
def _winner(score_path: Path) -> dict[str, Any]:
    turn_roots = sorted(V265_ROOT.glob("turns/v265-segment-*"))
    if len(turn_roots) != 2:
        raise V267FrozenAlignmentError("v265 winner turn membership drifted")
    return {
        "schema_version": WINNER_VERSION,
        "frozen_at": now_iso(),
        "system_id": "v265_segment_isolation_explicit_applicability_v267_frozen",
        "configuration": {
            "model": v265.MODEL,
            "reasoning_effort": v265.EFFORT,
            "max_events_per_segment": 32,
            "architecture_design": _record(V265_ROOT / "architecture-design.json"),
            "attempt_spec": _record(V265_ROOT / "attempt-spec.json"),
            "instructions": _record(turn_roots[0] / "base-instructions.private.md"),
            "schema": _record(turn_roots[0] / "schema.json"),
            "direct_projection_schema": _record(
                turn_roots[0] / "direct-projection-schema.json"
            ),
            "isolated_segment_request_templates": [
                {
                ...
                for position, turn_root in enumerate(turn_roots)
            ],
            "runtime_lock": _record(V265_ROOT / "runtime-lock.json"),
        },
        "development_evidence": {
            "structural_gate": _record(V265_ROOT / "architecture-structural-gate.json"),
            "support_terminal": _record(V266_ROOT / "terminal.json"),
...
async def run_v267(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v267 terminal")
    frozen = freeze_v267(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V267FrozenAlignmentError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        terminal = {
            ...
            "terminal_reason": (
                "v267_alignment_passed_development_winner_frozen_holdout_authorized"
                if passed
                else "v267_alignment_quality_or_permutation_gate_not_passed"
            ),
            ...
            "exact_next_action": (
                ...
                if passed
                else "reject v265 and advance to the next distinct architecture"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run frozen alignment over v265")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v267(output_dir=Path(args.output_dir))
        result = {
