...
"""Run the frozen graph canonicalizer with an app-server-compatible schema."""
...
from . import app_server_judge_v5_selection_v270_graph_canonicalizer_recovery as v270
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v273_graph_canonicalizer_schema_compat_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v273_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v273_terminal_v1"
PHASE_ID = "development_selection_v5_4_v273_graph_canonicalizer_schema_compat"
MODEL = "gpt-5.6-sol"
EFFORT = "low"
...
V270_ROOT = v270.DEFAULT_OUTPUT_ROOT
DEFAULT_OUTPUT_ROOT = (
...
class V273GraphCanonicalizerError(RuntimeError):
    """The v273 architecture cannot proceed or be adopted safely."""


class V273OutputContractError(V273GraphCanonicalizerError):
    """A completed canonicalizer output violated the frozen projection contract."""


class V273ArchitectureStop(V273GraphCanonicalizerError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V273GraphCanonicalizerError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V273GraphCanonicalizerError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "v270_launch_receipt": V270_ROOT / "launch-receipt.json",
        "v270_capacity": v270_turn / "capacity.json",
        "v270_sidecar": v270_turn / "sidecar.json",
        "v270_input": v270_turn / "input.private.json",
        "v270_prompt": v270_turn / "prompt.private.md",
        "v270_base": v270_turn / "base-instructions.private.md",
        "v270_schema": v270_turn / "schema.json",
        "v270_projection_schema": v270_turn / "projection-schema.json",
        "v270_direct_schema": v270_turn / "direct-schema.json",
    }
...
def _validate_lineage() -> dict[str, Any]:
    ...
    v270.verify_runtime_lock(paths["v270_runtime_lock"])
    ...
    schema270 = _load_json(paths["v270_schema"], "v270 schema")
    unsupported270 = llm_judge.validate_app_server_output_schema_subset(schema270)
    if (
        ...
        or t270.get("error_class") != "ReserveCapacityError"
        or t270.get("usage_status") != "unknown"
        or t270.get("accounting_complete") is not False
        or t270.get("unknown_usage_attempt_count") != 1
        or s270.get("state") != "failed"
        or s270.get("status") != "failed"
        or s270.get("usage_status") != "unknown"
        or s270.get("usage_complete") is not False
        or s270.get("error_class") != "turn_failed"
        or s270.get("auth_type") != "chatgpt"
        or t270.get("production_mutated") is not False
        or (V270_ROOT / "turns" / _source_turn_root(V270_ROOT, "v270-graph-canonicalizer-").name / "output.private.json").exists()
        or sorted(unsupported270)
        != [
            "$.$schema",
            "$.properties.segments.items.properties.events.items.properties.source_graph_event_ids.uniqueItems",
        ]
    ):
        raise V273GraphCanonicalizerError("v273 architecture lineage drifted")
    ...
    return {
        ...
        "v270_terminal": t270,
        "v270_sidecar": s270,
        "v270_unsupported_schema_paths": unsupported270,
    }
...
def _graph_ids(source_graph: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for segment in source_graph.get("segments") or []:
        ...
        if len(ids) != len(set(ids)):
            raise V273GraphCanonicalizerError("source graph event IDs are not unique")
...
def _canonicalizer_schema(
    ...
    schema = v270._canonicalizer_schema(projection_schema, graph_ids)
    unsupported = llm_judge.validate_app_server_output_schema_subset(schema)
    expected = [
        "$.$schema",
        "$.properties.segments.items.properties.events.items.properties.source_graph_event_ids.uniqueItems",
    ]
    if sorted(unsupported) != expected:
        raise V273GraphCanonicalizerError("v270 unsupported schema surface drifted")
    ...
    compatible = strip(schema)
    if llm_judge.validate_app_server_output_schema_subset(compatible):
        raise V273GraphCanonicalizerError("v273 schema remains app-server incompatible")
    return compatible
...
def prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise V273GraphCanonicalizerError("v273 request or source graph drifted")
    return {
        "turn_name": "v273_graph_canonicalizer_" + sha256_text(source["episode_id"])[:20],
...
def project_output(
    ...
    try:
        ...
        raise V273OutputContractError("canonicalizer output schema failed") from exc
    if output.get("episode_id") != turn["episode_id"]:
        raise V273OutputContractError("canonicalizer episode id drifted")
    segments = list(output.get("segments") or [])
    if [row.get("segment_id") for row in segments] != list(turn["segment_ids"]):
        raise V273OutputContractError("canonicalizer segment coverage or order drifted")
    ...
    for position, segment in enumerate(projected["segments"]):
        ...
        for event_index, event in enumerate(segment.get("events") or []):
            if event.pop("event_id") != f"S{position}C{event_index:03d}":
                raise V273OutputContractError("canonicalizer final event ID order drifted")
            ...
            if not source_ids or any(item not in expected_set for item in source_ids):
                raise V273OutputContractError("canonicalizer event cites cross-segment graph node")
        ...
        if any(item not in expected_set for item in dropped_ids):
            raise V273OutputContractError("canonicalizer drop cites cross-segment graph node")
        observed.extend(dropped_ids)
        counts = Counter(observed)
        if set(counts) != expected_set or any(value != 1 for value in counts.values()):
            raise V273OutputContractError("canonicalizer graph coverage is not exact once")
    ...
    try:
        ...
        raise V273OutputContractError(str(exc)) from exc
    coverage = {
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v273 runtime lock")
    root = path.parent.resolve()
    spec = _load_json(root / "attempt-spec.json", "v273 spec")
    ...
    if (
        ...
        raise V273GraphCanonicalizerError("v273 runtime lock contract drifted")
    records = [
        lock.get("pinned_codex_cli"),
        *(lock.get("runtime_files") or []),
        *(lock.get("direct_lineage") or []),
        lock.get("schema_compat_audit"),
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V273GraphCanonicalizerError("v273 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V273GraphCanonicalizerError("v273 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    spec = _load_json(spec_path, "v273 spec")
    paths = _turn_paths(root, str(spec["turn_name"]))
    lineage = _validate_lineage()
    ...
    return {
        ...
        "turn": {
            **turn,
            "private_input": _load_json(paths["input"], "v273 input"),
            "prompt": paths["prompt"].read_text(encoding="utf-8"),
            "base": paths["base"].read_text(encoding="utf-8"),
            "schema": _load_json(paths["schema"], "v273 schema"),
            "projection_schema": _load_json(paths["projection_schema"], "v273 projection schema"),
            "direct_schema": _load_json(paths["direct_schema"], "v273 direct schema"),
            "paths": paths,
        },
    }


def freeze_v273(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v273 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V273GraphCanonicalizerError("unfinished v273 root is not replayable")
    ...
    reused_request_names = {
        ...
        "prompt": "v270_prompt",
        "base": "v270_base",
        "projection_schema": "v270_projection_schema",
        "direct_schema": "v270_direct_schema",
    }
    if any(
        ...
        raise V273GraphCanonicalizerError("frozen v270 semantic request drifted")
    predecessor_schema = _load_json(lineage["paths"]["v270_schema"], "v270 schema")
    expected_schema = copy.deepcopy(predecessor_schema)

    def strip_unsupported(value: Any) -> Any:
        if isinstance(value, dict):
            return {
                key: strip_unsupported(child)
                for key, child in value.items()
                if key not in {"$schema", "uniqueItems"}
            }
        if isinstance(value, list):
            return [strip_unsupported(child) for child in value]
        return value

    if turn["schema"] != strip_unsupported(expected_schema):
        raise V273GraphCanonicalizerError("v273 schema delta exceeded compatibility normalization")
    capacity_paths = _capacity_policy(root, turn["turn_name"])
    schema_compat_audit_path = root / "v270-schema-compat-audit.json"
    _write_stable_time(
        schema_compat_audit_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            ...
            "failure_class": "local_output_schema_subset_incompatibility_unknown_usage",
            "unsupported_schema_paths": lineage["v270_unsupported_schema_paths"],
            "removed_schema_keywords": ["$schema", "uniqueItems"],
            "semantic_prompt_delta": False,
            "projection_delta": False,
            "deterministic_uniqueness_validation_retained": True,
            "predecessor_output_adopted": False,
            "predecessor_retried": False,
            "frozen_semantic_request_reused_byte_for_byte": True,
            "production_mutated": False,
        },
        "created_at",
    )
    ...
    _write_stable_time(
        authorization_path,
        {
            ...
            "scope": "one bounded schema-compatible execution of the frozen source graph canonicalizer",
            "semantic_prompt_delta": False,
            "structured_output_compatibility_delta_only": True,
    ...
    _write_stable_time(
        ranking_path,
        {
            ...
            "measured_basis": {
                ...
                "v270_usage_status": "unknown",
                "v270_failure_class": "local_output_schema_subset_incompatibility",
            },
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "hypothesis": (
                "the semantically unchanged v270 request with only unsupported schema keywords removed can test "
                "whether the measured v244 source graph retains blind recall "
                "while one global Sol-low canonicalizer re-reads the source and corrects event boundaries and "
                "full-field semantics using v249 explicit applicability; neither v253 nor v270 produced a "
                "semantic result, so this one fresh version is the smallest decision-changing canary"
            ),
            "representative_canary": {
                ...
                "semantic_prompt_delta_from_v270": False,
                "schema_delta_is_nonsemantic_compatibility_only": True,
    ...
    _write_stable_time(
        spec_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "state": "frozen_before_one_turn_graph_canonicalizer_schema_compat",
    ...
    lock = {
        ...
        "schema_compat_audit": _record(schema_compat_audit_path),
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        sidecar = _load_json(paths["sidecar"], "v273 sidecar")
        measured = _measured_usage(sidecar)
        if measured is not None:
            usage = measured
            unknown = 0
            measured_over_cap = usage["total_tokens"] > MAX_TOTAL_TOKENS
    semantic = isinstance(exc, (V273OutputContractError, V273ArchitectureStop)) or measured_over_cap
    ...
    terminal = {
        ...
        "exact_next_action": (
            ...
            if semantic
            else "audit immutable v273 infrastructure attempt; no retry"
        ),
    }
...
async def run_v273(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v273 terminal")
    frozen = freeze_v273(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V273GraphCanonicalizerError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V273GraphCanonicalizerError("v273 turn did not complete")
        usage = _measured_usage(_load_json(paths["sidecar"], "v273 sidecar"))
        if usage is None:
            raise V273GraphCanonicalizerError("v273 sidecar accounting or auth failed")
        if usage["total_tokens"] > MAX_TOTAL_TOKENS:
            raise V273ArchitectureStop("v273 measured turn exceeded frozen token bound")
        ...
        if not gate["passed"]:
            raise V273ArchitectureStop("v273 structural quality or cost gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "v273_architecture_structural_gate_passed",
            "terminal_reason": "v273_graph_canonicalizer_structural_cost_gate_passed",
            "semantic_attempt_count": 1,
            "semantic_retry_count": 0,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v273 source graph canonicalizer")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v273(output_dir=Path(args.output_dir))
        result = {
            "state": "frozen",
        ...
        terminal = asyncio.run(
            run_v273(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
