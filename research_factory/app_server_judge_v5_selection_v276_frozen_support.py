...
"""Run the unchanged frozen side-free support judge over v275."""
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v276_frozen_support_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v276_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v276_terminal_v1"
PHASE_ID = "judge_v5_4_selection_v276_frozen_support"
TURN_NAME = "v276_frozen_support_v275_candidate"
MODEL = v245.MODEL
EFFORT = v245.EFFORT
...
EXPECTED_CANDIDATE_WITNESSES = 33
...
class V276FrozenSupportError(RuntimeError):
    """The v276 frozen support gate cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V276FrozenSupportError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V276FrozenSupportError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "v275_sidecar": turn_root / "sidecar.json",
        "v275_normalized": V275_ROOT / "normalized-output.private.json",
        "v275_provenance": V275_ROOT / "evidence-provenance.private.json",
        "v275_input": turn_root / "input.private.json",
        "shared_reference": v245.v220.DEFAULT_OUTPUT_ROOT / "shared-reference-seed-v1.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    v245.v223.verify_runtime_lock(paths["v223_runtime_lock"])
    terminal = _load_json(paths["v275_terminal"], "v275 terminal")
    gate = _load_json(paths["v275_gate"], "v275 gate")
    sidecar = _load_json(paths["v275_sidecar"], "v275 sidecar")
    design = _load_json(paths["v223_design"], "v223 design")
    instructions = paths["v223_instructions"].read_text(encoding="utf-8")
    frozen_instructions = v245.v143.support_base_instructions_v143()
    if (
        ...
        or gate.get("dense_event_count") != 32
        or gate.get("candidate_only_nominal_no_signal_event_count") != 1
        or terminal.get("production_amortized_total_token_ratio") != 0.27203
        or (terminal.get("combined_usage") or {}).get("total_tokens") != 71_252
        or (sidecar.get("usage") or {}).get("total_tokens") != 41_890
        ...
        raise V276FrozenSupportError("v276 frozen support lineage or protocol drifted")
...
def _event_claim(event: Mapping[str, Any]) -> str:
    ...
    if not isinstance(claim, str) or not claim.strip():
        raise V276FrozenSupportError("support witness has no claim text")
    return claim


def build_support_pool(lineage: Mapping[str, Any]) -> dict[str, Any]:
    source = _load_json(lineage["paths"]["v275_input"], "v275 source input")
    candidate = _load_json(lineage["paths"]["v275_normalized"], "v275 normalized output")
    ...
    segment_ids = [v275.v234.DENSE_SEGMENT_ID, v275.v234.NO_SIGNAL_SEGMENT_ID]
    if any(
        ...
        raise V276FrozenSupportError("support pool segment coverage drifted")
    cases = []
    units = []
    origin_rows = []
    for segment_id in segment_ids:
        ...
        source_excerpt = "\n".join(
            str(unit["text"]) for unit in source_row["source_units"]
        )
        reference_row = reference_by_id[segment_id]
        if sha256_text(source_excerpt) != reference_row["text_sha256"]:
            raise V276FrozenSupportError("support source text hash drifted")
        density = "dense" if segment_id == v275.v234.DENSE_SEGMENT_ID else "no_signal"
        case_id = "case_" + sha256_text(f"v276|{segment_id}")[:24]
        ...
        for origin, events in (("reference", reference_events), ("candidate", candidate_events)):
            for event_index, event in enumerate(events):
                event_hash = sha256_text(v275._canonical_json(event))
                witness_id = "w_" + sha256_text(
                    f"v276|{case_id}|{origin}|{event_index}|{event_hash}"
                )[:24]
    ...
    if (
        ...
        raise V276FrozenSupportError("support pool witness coverage drifted")
    support_value = v245.v143._support_input(units)
    prompt = v245.v175.compact_support_prompt(units)
    schema = v245.v143.support_output_schema(support_value)
    schema_text = v275._canonical_json(schema)
    if (
        len(prompt.encode("utf-8")) > MAX_PROMPT_BYTES
        or len(schema_text.encode("utf-8")) > MAX_SCHEMA_BYTES
    ):
        raise V276FrozenSupportError("support request size cap exceeded")
    return {
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v276 runtime lock")
    if (
        ...
        raise V276FrozenSupportError("v276 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V276FrozenSupportError("v276 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V276FrozenSupportError("v276 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    return {
        "root": root,
        "spec_path": spec_path,
        "spec": _load_json(spec_path, "v276 spec"),
...
def freeze_v276(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v276 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V276FrozenSupportError("unfinished v276 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "strategy": "unchanged_frozen_side_free_support_over_v275_and_shared_reference",
            "support_instruction_hash": sha256_text(v245.v143.support_base_instructions_v143()),
            "support_model": MODEL,
    ...
    _write_private_text(instructions_path, v245.v143.support_base_instructions_v143())
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V276FrozenSupportError("v276 sidecar usage incomplete") from exc
    if (
        ...
        raise V276FrozenSupportError("v276 sidecar accounting or auth contract failed")
    return usage
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        try:
            sidecar = _load_json(paths["sidecar"], "v276 sidecar")
    ...
    terminal = {
        ...
        "exact_next_action": "audit immutable v276 judge attempt; no retry",
    }
...
async def run_v276(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v276 terminal")
    frozen = freeze_v276(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(root, frozen, V276FrozenSupportError("launch exists; replay prohibited"))
    _write_immutable(
    ...
    try:
        async with client_factory(frozen["capacity_policy"]) as client:
            result = await client.run_ephemeral_structured_turn(
                model=MODEL,
                effort=EFFORT,
                base_instructions=v245.v143.support_base_instructions_v143(),
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V276FrozenSupportError("v276 support turn did not complete")
        usage = _usage(_load_json(paths["sidecar"], "v276 sidecar"))
        audit, private_score = v245.v223.score_support_output(
            output=result.output,
            support_value=frozen["support_value"],
            origin_rows=frozen["origin_rows"],
        )
        receipts = v245.v223._support_receipts(result.output)
        ...
        terminal = {
            ...
            "terminal_reason": (
                ...
                if authorized
                else "v276_frozen_support_quality_gate_not_passed"
            ),
            ...
            "exact_next_action": (
                ...
                if authorized
                else "reject v275 on source support and advance to the next distinct architecture"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run frozen support over v275")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v276(output_dir=Path(args.output_dir))
        result = {
        ...
        terminal = asyncio.run(
            run_v276(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
