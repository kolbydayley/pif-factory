...
"""Adopt the frozen v234 inventory and realize it one proposition per event."""
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v268_inventory_atomic_realization_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v268_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v268_terminal_v1"
PHASE_ID = "development_selection_v5_4_v268_inventory_atomic_realization"
MODEL = "gpt-5.6-sol"
EFFORT = "low"
...
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v268-inventory-atomic-realization"
).resolve()
...
GLOBAL_JOIN_INSTRUCTIONS = v234.REALIZATION_INSTRUCTIONS


class V268InventoryAtomicRealizationError(RuntimeError):
    """The v268 architecture cannot proceed or be adopted safely."""


class V268OutputContractError(V268InventoryAtomicRealizationError):
    """A completed output violated the frozen ownership/projection contract."""


class V268ArchitectureStop(V268InventoryAtomicRealizationError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V268InventoryAtomicRealizationError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V268InventoryAtomicRealizationError(f"frozen {path.name} drifted")
        return
...
def _single_turn(root: Path, prefix: str) -> Path:
    ...
    if len(matches) != 1:
        raise V268InventoryAtomicRealizationError(f"{prefix} turn membership drifted")
    return matches[0]
...
def _validate_lineage() -> dict[str, Any]:
    ...
    for name, expected in EXPECTED_LINEAGE_HASHES.items():
        if records[name]["sha256"] != expected:
            raise V268InventoryAtomicRealizationError(f"frozen lineage {name} drifted")
    ...
    if (
        ...
        or any(
            ...
            for terminal in (t234, t265, t267)
        )
    ):
        raise V268InventoryAtomicRealizationError("v268 predecessor evidence drifted")
    packet = v234._source_packet(v234._validate_lineage())
    if [row["segment_id"] for row in inventory["segments"]] != packet["segment_ids"]:
        raise V268InventoryAtomicRealizationError("v234 inventory segment order drifted")
    return {
        "paths": paths,
        "records": records,
        "inventory": inventory,
...
def prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise V268InventoryAtomicRealizationError("v268 request size cap failed")
    return {
        "turn_name": "v268_inventory_atomic_realization_" + sha256_text(packet["episode_id"])[:20],
        "episode_id": packet["episode_id"],
...
def project_output(
    ...
    try:
        ...
        raise V268OutputContractError("global join output schema failed") from exc
    if output.get("episode_id") != turn["episode_id"]:
        raise V268OutputContractError("global join episode id drifted")
    rows = list(output.get("segments") or [])
    if [row.get("segment_id") for row in rows] != list(turn["segment_ids"]):
        raise V268OutputContractError("global join segment coverage or order drifted")
    source_by_id = {
    ...
    for row in rows:
        ...
        for event_index, raw_event in enumerate(events):
            ...
            if not source_ids or any(item not in position for item in source_ids):
                raise V268OutputContractError("global join cross-segment proposition ownership")
            earliest = min(position[item] for item in source_ids)
            if earliest < prior_proposition_position:
                raise V268OutputContractError("global join event proposition order drifted")
            ...
            try:
                ...
                raise V268OutputContractError(str(exc)) from exc
            if int(projection["start_index"]) < prior_evidence_position:
                raise V268OutputContractError("global join evidence order drifted")
            ...
            if any(value and value not in projection["evidence"] for value in metric_values):
                raise V268OutputContractError("global join metric literal is not in evidence")
            if any(metric_values) == (event.get("metric_direction") == "not_applicable"):
                raise V268OutputContractError("global join metric applicability drifted")
            identity = v234._event_identity(event)
            if identity in seen_global:
                raise V268OutputContractError("global join exact event identity duplicate")
        ...
        if any(item not in position for item in dropped_ids):
            raise V268OutputContractError("global join cross-segment dropped proposition")
        observed.extend(dropped_ids)
        counts = Counter(observed)
        if set(counts) != set(expected_ids) or any(value != 1 for value in counts.values()):
            raise V268OutputContractError("inventory propositions are not owned exactly once")
        if (row.get("status") == "coded") != bool(projected_events):
            raise V268OutputContractError("global join coded status drifted")
        if not projected_events and row.get("status") != "no_signal":
            raise V268OutputContractError("empty global join must be no_signal")
...
def prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    """Build the never-started v234 one-to-one realization in a new lineage."""
    turn = v234._prepare_realization_turn(lineage["packet"], lineage["inventory"])
    return {
        **turn,
        "turn_name": "v268_inventory_atomic_realization_"
        + sha256_text(turn["episode_id"])[:20],
        "packet": lineage["packet"],
        "inventory": lineage["inventory"],
    }
...
def project_output(
    ...
    except (v234.V234OutputContractError, ValidationError, ValueError, TypeError) as exc:
        raise V268OutputContractError(str(exc)) from exc
    ownership = {
        "schema_version": SCHEMA_VERSION,
        "architecture_id": "adopted_blind_inventory_atomic_one_to_one_realization",
        "segments": [
            {
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v268 runtime lock")
    root = path.parent.resolve()
    spec = _load_json(root / "attempt-spec.json", "v268 spec")
    ...
    if (
        ...
        raise V268InventoryAtomicRealizationError("v268 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V268InventoryAtomicRealizationError("v268 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V268InventoryAtomicRealizationError("v268 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    spec = _load_json(spec_path, "v268 spec")
    paths = _turn_paths(root, spec["turn_name"])
    lineage = _validate_lineage()
    ...
    return {
        ...
        "turn": {
            **turn,
            "private_input": _load_json(paths["input"], "v268 input"),
            "prompt": paths["prompt"].read_text(encoding="utf-8"),
            "base": paths["base"].read_text(encoding="utf-8"),
            "schema": _load_json(paths["schema"], "v268 schema"),
            "paths": paths,
        },
    }


def freeze_v268(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v268 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V268InventoryAtomicRealizationError("unfinished v268 root is not replayable")
    ...
    _write_stable_time(
        authorization_path,
        {
            ...
            "scope": "adopt one immutable blind inventory and run its one-to-one atomic realization",
    ...
    _write_stable_time(
        ranking_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "selected_architecture_id": "adopted_blind_inventory_atomic_one_to_one_realization",
            "architectures": [
                {"rank": 1, "id": "adopted_blind_inventory_atomic_one_to_one_realization"},
                {"rank": 2, "id": "inventory_per_event_microbatch_realization"},
            ...
            "selection_basis": {
                ...
                "v265_alignment_macro_f1": 0.786707,
                "v265_strictly_equivalent_reference_units": 11,
                "v234_original_realization_turn_started": False,
                "decision": "preserve measured inventory recall and test one-to-one realization without reducer boundary loss",
            },
            "on_failure": "reject v268 and advance to the next distinct architecture without field repair",
        },
        "created_at",
    )
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "architecture_id": "adopted_blind_inventory_atomic_one_to_one_realization",
            "hypothesis": (
                "the measured 32-proposition blind inventory preserves dense recall while one-to-one full-schema "
                "realization prevents the merge and boundary losses measured in global reducers"
            ),
    ...
    _write_stable_time(
        spec_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "state": "frozen_before_one_turn_inventory_atomic_realization_canary",
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V268InventoryAtomicRealizationError("v268 sidecar usage incomplete") from exc
    if (
        ...
        raise V268InventoryAtomicRealizationError("v268 sidecar accounting or auth failed")
    return usage
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        try:
            sidecar = _load_json(paths["sidecar"], "v268 sidecar")
    ...
    semantic = isinstance(exc, (V268OutputContractError, V268ArchitectureStop)) or measured_over_cap
    ...
    terminal = {
        ...
        "exact_next_action": (
            "reject v268 and advance to the next distinct architecture without field repair"
            if semantic
            else "audit immutable v268 infrastructure attempt; no retry"
        ),
    }
...
async def run_v268(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v268 terminal")
    frozen = freeze_v268(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V268InventoryAtomicRealizationError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V268InventoryAtomicRealizationError("v268 turn did not complete")
        new_usage = _usage(_load_json(paths["sidecar"], "v268 sidecar"))
        if new_usage["total_tokens"] > MAX_NEW_TOKENS:
            raise V268ArchitectureStop("v268 measured turn exceeded frozen token bound")
        ...
        if not gate["passed"]:
            raise V268ArchitectureStop("v268 structural quality or cost gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "v268_architecture_structural_gate_passed",
            "terminal_reason": "v268_inventory_atomic_realization_structural_cost_gate_passed",
            "new_semantic_attempt_count": 1,
            "semantic_retry_count": 0,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v268 adopted inventory atomic realization")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v268(output_dir=Path(args.output_dir))
        result = {
        ...
        terminal = asyncio.run(
            run_v268(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
