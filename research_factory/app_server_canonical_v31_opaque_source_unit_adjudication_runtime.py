...
"""Epoch-31 one-call source-unit adjudication of the epoch-28 disagreement."""
...
from . import app_server_canonical_v31_adjudication_exact_validation_recovery as epoch30
...
EPOCH30_ROOT = epoch30.DEFAULT_ROOT
DEFAULT_ROOT = (
    PIPELINE_ROOT
...
DIRECTIVE_PATH = (
    PROJECT_ROOT
    / "automation/pif-evaluation-epoch31-opaque-source-unit-adjudication-v31.json"
).resolve()
PLAN_PATH = (
    PROJECT_ROOT / "automation/pif-evaluation-semantic-plan-v31.json"
).resolve()
...
SCHEMA_VERSION = "pif_canonical_v31_opaque_source_unit_adjudication_v31"
LOCK_VERSION = "pif_canonical_v31_opaque_source_unit_adjudication_lock_v1"
LAUNCH_VERSION = "pif_canonical_v31_opaque_source_unit_adjudication_launch_v1"
RECEIPT_VERSION = "pif_semantic_plan_step_receipt_v1"
THREAD_ID = epoch28.THREAD_ID
PLAN_EPOCH = 31
STEP_ID = "canonical_v31_epoch31_opaque_source_unit_adjudication_v31"
MODEL = epoch28.MODEL
...
CAPACITY_TURN_NAMES = ("epoch31_quality_adjudication",)
TURN_DIRECTORY_NAME = "epoch31-quality-adjudication"
...
PREDECESSOR_QUALITY_USAGE = {
    "input_tokens": 143_859,
    "cached_input_tokens": 4_224,
    "output_tokens": 47_607,
    "reasoning_output_tokens": 21_372,
    "total_tokens": 191_466,
}
EPOCH28_USAGE = {
...
EPOCH29_USAGE = dict(epoch30.EXPECTED_USAGE)
PREDECESSOR_AGGREGATE_MODEL_CALLS = 8
PREDECESSOR_AGGREGATE_UNKNOWN_USAGE = 2
PREDECESSOR_AGGREGATE_MEASURED_TOKENS = 265_792
...
PROCESS_LOCK_NAME = ".canonical-v31-epoch31-source-unit-adjudication.lock"
USAGE_FIELDS = epoch28.USAGE_FIELDS
_FORBIDDEN_AUTH_ENVIRONMENT = epoch28._FORBIDDEN_AUTH_ENVIRONMENT  # noqa: SLF001


class Epoch31SourceUnitAdjudicationError(RuntimeError):
    """Epoch-31 contract, lineage, or immutable evidence drifted."""


class Epoch31SourceUnitAdjudicationWaiting(Epoch31SourceUnitAdjudicationError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise Epoch31SourceUnitAdjudicationError(f"cannot read {label}") from exc
...
def _write_immutable_json(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise Epoch31SourceUnitAdjudicationError(f"frozen {path.name} drifted")
        return
...
def _process_lock(root: Path) -> Iterator[None]:
    ...
    with path.open("a+", encoding="ascii") as handle:
        try:
            ...
            raise Epoch31SourceUnitAdjudicationWaiting(
                "another epoch-31 adjudication process owns this root"
            ) from exc
        try:
            yield
        finally:
...
def _runtime_paths() -> list[Path]:
    return [
        ...
        Path(epoch30.__file__).resolve(),
        Path(epoch29.__file__).resolve(),
...
def _expected_predecessor_records() -> list[dict[str, Any]]:
    receipt = _validate_epoch30_terminal()
    records = [
        _record(EPOCH30_ROOT / "runtime-lock.json"),
        _record(EPOCH30_ROOT / "plan-step-receipt.json"),
        _record(EPOCH30_ROOT / "terminal.json"),
        _record(EPOCH28_ROOT / "shared-witness-pool.private.json"),
        _record(EPOCH28_ROOT / "private-witness-mapping.private.json"),
        _record(EPOCH28_ROOT / "judge/output-ab.private.json"),
        _record(EPOCH28_ROOT / "judge/output-ba.private.json"),
        _record(EPOCH28_ROOT / "judge/consensus.private.json"),
    ]
    artifact_records = receipt.get("recovered_artifact_records")
    if not isinstance(artifact_records, Mapping):
        raise Epoch31SourceUnitAdjudicationError("epoch-30 artifact closure is missing")
    for role in sorted(artifact_records):
        row = artifact_records[role]
        if not isinstance(row, Mapping) or not _verify_record(row):
            raise Epoch31SourceUnitAdjudicationError(
                f"epoch-30 artifact closure drifted: {role}"
            )
...
def _validate_epoch28_terminal() -> dict[str, Any]:
    ...
    if (
        ...
        or receipt.get("usage") != EPOCH28_USAGE
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-28 terminal contract drifted")
    return receipt


def _validate_epoch30_terminal() -> dict[str, Any]:
    receipt = epoch30.verify_receipt(EPOCH30_ROOT)
    if (
        receipt.get("state") != "rejected"
        or receipt.get("terminal_reason")
        != "epoch29_adjudication_exact_evidence_validation_rejected"
        or receipt.get("semantic_model_call_count") != 0
        or receipt.get("semantic_retry_count") != 0
        or receipt.get("recovered_predecessor_semantic_model_call_count") != 1
        or receipt.get("recovered_predecessor_usage") != EPOCH29_USAGE
        or receipt.get("exact_validation_error_count") != 2
        or receipt.get("exact_validation_errors_sha256")
        != epoch30.EXPECTED_ERRORS_SHA256
        or receipt.get("aggregate_architecture_semantic_model_call_count") != 8
        or receipt.get("aggregate_architecture_unknown_usage_turn_count") != 2
        or receipt.get("aggregate_architecture_measured_total_tokens") != 265_792
        or receipt.get("semantic_output_repair_applied") is not False
        or receipt.get("winner_frozen") is not False
        or receipt.get("holdout_authorized") is not False
        or receipt.get("production_mutated") is not False
    ):
        raise Epoch31SourceUnitAdjudicationError("epoch-30 terminal contract drifted")
    return receipt


def load_contract() -> dict[str, Any]:
    plan = _load_json(PLAN_PATH, "epoch-31 semantic plan")
    directive = _load_json(DIRECTIVE_PATH, "epoch-31 directive")
    ...
    if (
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-31 semantic plan drifted")
    ...
    if (
        ...
        != "pif_evaluation_epoch31_opaque_source_unit_adjudication_directive_v1"
        ...
        != "epoch29_adjudication_exact_evidence_validation_rejected"
        or predecessor.get("recovered_measured_model_call_count") != 1
        or predecessor.get("recovered_measured_total_tokens")
        != EPOCH29_USAGE["total_tokens"]
        or predecessor.get("quality_measured") is not False
        or predecessor.get("runtime_lock") != _record(EPOCH30_ROOT / "runtime-lock.json")
        or predecessor.get("receipt")
        != _record(EPOCH30_ROOT / "plan-step-receipt.json")
        or predecessor.get("terminal") != _record(EPOCH30_ROOT / "terminal.json")
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-31 directive values drifted")
    receipt = _validate_epoch30_terminal()
    return {
        "plan": plan,
        "directive": directive,
...
def _base_bundle() -> dict[str, Any]:
    ...
    for name in ("ab", "ba"):
        if judge.validate_judge_output(outputs[name], variants[name]):
            raise Epoch31SourceUnitAdjudicationError(f"epoch-28 {name} output drifted")
    ...
    if consensus != _load_json(
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-28 consensus reconstruction drifted")
    return {
        "pool": pool,
        "mapping": mapping,
        "outputs": outputs,
...
def observable_disagreement_case_ids(
    ...
    for name in ("ab", "ba"):
        if judge.validate_judge_output(outputs[name], variants[name]):
            raise Epoch31SourceUnitAdjudicationError("base judge output failed exact validation")
    ...
    if sorted(disagreement) != sorted(EXPECTED_DISAGREEMENT_CASE_IDS):
        raise Epoch31SourceUnitAdjudicationError("observable disagreement case set drifted")
...
def build_adjudication_variant(
    ...
    for case_position, case in enumerate(pool["cases"]):
        ...
        if not isinstance(source, str) or not source:
            raise Epoch31SourceUnitAdjudicationError("adjudication source is empty")
        boundaries = [
            {
                "window_id": 0,
                "owner_start": 0,
    ...
    if not cases or {row["case_id"] for row in cases} != selected:
        raise Epoch31SourceUnitAdjudicationError("adjudication case coverage drifted")
    variant = {
    ...
    if epoch28.SYSTEM_BASELINE in serialized or epoch28.SYSTEM_CANDIDATE in serialized:
        raise Epoch31SourceUnitAdjudicationError("system origin leaked into adjudication")
    return variant
...
def _freeze_unlocked(root: Path) -> dict[str, Any]:
    ...
    if root.exists() and _meaningful_entries(root):
        raise Epoch31SourceUnitAdjudicationError("unfrozen epoch-31 root is not empty")
    contract = load_contract()
    if contract["receipt_path"] != root / "plan-step-receipt.json":
        raise Epoch31SourceUnitAdjudicationError("epoch-31 output root differs from contract")
    ...
    _write_immutable_json(
        attempt_path,
        {
            ...
            "step_id": STEP_ID,
            "predecessor_receipt": _record(EPOCH30_ROOT / "plan-step-receipt.json"),
...
def verify_runtime_lock(path: Path, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        lock = _load_json(lock_path, "epoch-31 runtime lock")
        records = [
        ...
        if (
            ...
            raise Epoch31SourceUnitAdjudicationError("epoch-31 runtime lock drifted")
        load_contract()
        variant, base, prompt, schema = _request_material()
        expected_requests = [
        ...
        if (
            ...
            raise Epoch31SourceUnitAdjudicationError("epoch-31 request artifacts drifted")
        attempt = _load_json(
            Path(str(lock["attempt_spec"]["path"])), "epoch-31 attempt spec"
        )
        if (
            ...
            raise Epoch31SourceUnitAdjudicationError("epoch-31 attempt specification drifted")
        ...
        if (
            ...
            raise Epoch31SourceUnitAdjudicationError("epoch-31 capacity policy drifted")
        return lock
...
def _validate_capacity_checkpoint(root: Path) -> dict[str, Any]:
    ...
    value = _load_json(path, "epoch-31 capacity checkpoint")
    ...
    if not isinstance(value, Mapping) or any(
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-31 capacity checkpoint drifted")
    ...
    for key in (
        ...
        if value.get(key) != evaluation[key]:
            raise Epoch31SourceUnitAdjudicationError("epoch-31 capacity arithmetic drifted")
    return dict(value)
...
def _validate_launch_receipt(root: Path) -> dict[str, Any]:
    value = _load_json(root / "launch-receipt.json", "epoch-31 launch receipt")
    expected = {
        "runtime_lock": _record(root / "runtime-lock.json"),
        "predecessor_receipt": _record(EPOCH30_ROOT / "plan-step-receipt.json"),
        "attempt_spec": _record(root / "attempt-spec.json"),
    ...
    if (
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-31 launch receipt drifted")
    return dict(value)
...
def _valid_usage(value: Any) -> dict[str, int]:
    try:
        ...
        raise Epoch31SourceUnitAdjudicationError(str(exc)) from exc
...
def _validate_partial_sidecar(root: Path, sidecar: Mapping[str, Any]) -> None:
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch31SourceUnitAdjudicationError("partial adjudication sidecar lineage drifted")
    thread_id = sidecar.get("thread_id")
    turn_id = sidecar.get("turn_id")
    if not isinstance(thread_id, str) or not thread_id:
        raise Epoch31SourceUnitAdjudicationError("partial adjudication lacks thread identity")
    if turn_id is not None and (not isinstance(turn_id, str) or not turn_id):
        raise Epoch31SourceUnitAdjudicationError("partial adjudication turn identity drifted")
    output_path = _turn_root(root) / "output.private.json"
    if Path(str(sidecar.get("output_path") or "")).expanduser().resolve() != output_path:
        raise Epoch31SourceUnitAdjudicationError("partial adjudication output path drifted")
...
def _validate_completed_turn(
    ...
    try:
        ...
        raise Epoch31SourceUnitAdjudicationError("completed adjudication checkpoint drifted") from exc
    required = {
        "schema_version": codex_app_server.TURN_SIDECAR_SCHEMA_VERSION,
        "state": "completed",
        "status": "completed",
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch31SourceUnitAdjudicationError("managed adjudication sidecar contract drifted")
    usage = _valid_usage(sidecar.get("usage"))
    if sidecar.get("thread_total_usage") != usage:
        raise Epoch31SourceUnitAdjudicationError("adjudication thread usage drifted")
    if usage["total_tokens"] > NEW_TOTAL_TOKEN_CAP:
        raise Epoch31SourceUnitAdjudicationError("adjudication exceeded its token ceiling")
    validation_errors = validate_source_unit_output(output, variant)
    _validate_capacity_checkpoint(root)
    epoch28_receipt = _validate_epoch28_terminal()
    epoch30_receipt = _validate_epoch30_terminal()
    prior_ids = {
        (str(row["thread_id"]), str(row["turn_id"]))
        for row in epoch28_receipt.get("turns") or []
    }
    prior_ids.add(
        (
            str(epoch30_receipt["recovered_predecessor_thread_id"]),
            str(epoch30_receipt["recovered_predecessor_turn_id"]),
        )
    )
    ...
    if identity in prior_ids or len(identity[0]) == 0 or len(identity[1]) == 0:
        raise Epoch31SourceUnitAdjudicationError("adjudication reused a prior thread or turn")
...
def merge_adjudication(
    ...
    if set(adjudicated) != selected:
        raise Epoch31SourceUnitAdjudicationError("adjudication output case coverage drifted")
    cases = []
...
def _score(
    ...
    standard_variant = epoch29.build_adjudication_variant(bundle["pool"], case_ids)
    ...
    try:
        ...
        raise Epoch31SourceUnitAdjudicationError("adjudicated shared-reference score failed") from exc
...
def _receipt(
    ...
    *,
    ...
    if state not in {"passed", "rejected", "waiting"}:
        raise Epoch31SourceUnitAdjudicationError("epoch-31 receipt state is invalid")
...
def _recover_terminal_mirror(root: Path) -> dict[str, Any] | None:
    ...
    value = _load_json(existing, "single epoch-31 terminal mirror")
...
def _validate_receipt_payload(root: Path, receipt: Mapping[str, Any]) -> None:
    if (
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-31 terminal receipt drifted")
    ...
    if (
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-31 call accounting drifted")
    ...
    if (
        ...
        raise Epoch31SourceUnitAdjudicationError("epoch-31 aggregate accounting drifted")
    if receipt.get("score") is not None or receipt.get("state") == "passed":
        ...
        if (
            ...
            or _load_json(
                root / "projected-adjudication.private.json",
                "epoch-31 projected adjudication",
            )
            != score["projected_output"]
            or _load_json(root / "consensus.private.json", "epoch-31 consensus")
            != score["consensus"]
        ):
            raise Epoch31SourceUnitAdjudicationError("epoch-31 quality receipt drifted")
    else:
        if receipt.get("terminal_reason") == "epoch31_source_unit_output_rejected":
            ...
            if (
                ...
                raise Epoch31SourceUnitAdjudicationError(
                    "epoch-31 rejected output receipt drifted"
                )
            return
        ...
        if (
            ...
            or (
                ...
                != "epoch31_adjudication_structured_output_rejected"
            )
            ...
            raise Epoch31SourceUnitAdjudicationError("epoch-31 waiting receipt drifted")
...
def verify_receipt(root: Path = DEFAULT_ROOT, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        ...
        receipt = _load_json(output_root / "plan-step-receipt.json", "epoch-31 receipt")
        terminal = _load_json(output_root / "terminal.json", "epoch-31 terminal")
        if receipt != terminal:
            raise Epoch31SourceUnitAdjudicationError("epoch-31 terminal mirrors differ")
...
def _finalize_completed(root: Path) -> dict[str, Any]:
    ...
    if validation_errors:
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch31_source_unit_output_rejected",
    ...
    return _write_terminal(
        ...
        _receipt(
            ...
            terminal_reason=(
                "epoch31_source_unit_adjudicated_quality_gate_passed"
                if score["passed"]
                else "epoch31_source_unit_adjudicated_quality_gate_rejected"
            ),
            accounting=accounting,
            score=score,
        ),
...
async def _run_unlocked(
    ...
    if artifacts_exist and not launch_exists:
        raise Epoch31SourceUnitAdjudicationError("semantic artifacts exist without epoch-31 launch")
    if launch_exists:
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch31_partial_or_interrupted_adjudication_preserved_without_replay",
                accounting=accounting,
    ...
    try:
        ...
        if not result.status_ok or not isinstance(result.output, Mapping):
            raise Epoch31SourceUnitAdjudicationWaiting("adjudication turn did not complete")
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch31_adjudication_structured_output_rejected",
                accounting=accounting,
        ...
        Epoch31SourceUnitAdjudicationWaiting,
        OSError,
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch31_adjudication_operational_waiting_no_replay",
                accounting=accounting,
...
async def run(
    ...
    *,
    ...
    if output_root == DEFAULT_ROOT and client_factory is not None:
        raise Epoch31SourceUnitAdjudicationError("the live epoch-31 root forbids injected clients")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except Epoch31SourceUnitAdjudicationWaiting as exc:
        print(json.dumps({"state": "waiting", "error": str(exc)}, sort_keys=True))
        return 75
    except Epoch31SourceUnitAdjudicationError as exc:
