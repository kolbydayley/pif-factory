...
"""Run one immutable extraction request through a different frontier model lane."""
...
CONFIG_VERSION = "pif_frozen_request_model_lane_config_v1"
LOCK_VERSION = "pif_frozen_request_model_lane_runtime_lock_v1"
TERMINAL_VERSION = "pif_frozen_request_model_lane_terminal_v1"
ARCHITECTURE_ID = "one_pass_explicit_applicability_alternate_frontier_model"
TURN_NAME = "frozen_request_frontier_model_lane"
MODEL = "gpt-5.5"
EFFORT = "high"
...
class FrozenRequestModelLaneError(RuntimeError):
    pass


class FrozenRequestOutputError(FrozenRequestModelLaneError):
    pass


class FrozenRequestArchitectureStop(FrozenRequestModelLaneError):
    pass
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise FrozenRequestModelLaneError(f"cannot read {label}") from exc
...
def _verify_record(record: Mapping[str, Any], *, cache: ContentHashCache) -> None:
    if not cache.verify_record(record):
        raise FrozenRequestModelLaneError("frozen direct-lineage artifact drifted")
...
def _validate_config(config_path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise FrozenRequestModelLaneError("experiment config contract drifted")
    if Path(str(value.get("output_root") or "")).expanduser().resolve() != config_path.parent.resolve():
        raise FrozenRequestModelLaneError("experiment output root drifted")
    ...
    if projected > 0.28:
        raise FrozenRequestModelLaneError("frozen token ceiling exceeds production target")
    return value
...
def _capacity_policy(root: Path, config: Mapping[str, Any]) -> dict[str, Path]:
    ...
    projected_points = math.ceil(
        MAX_TOTAL_TOKENS * int(config["quota_points_per_million_tokens"]) / 1_000_000
    )
...
def freeze_experiment(config_path: Path) -> dict[str, Any]:
    ...
    for destination, source_key in copy_names.items():
        ...
        if _record(turn[destination], cache=cache)["sha256"] != config[source_key]["sha256"]:
            raise FrozenRequestModelLaneError("frozen request bytes changed during copy")
...
def verify_frozen(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise FrozenRequestModelLaneError("runtime lock verification failed") from exc
    if result.manifest.get("pinned_codex_cli") != _record(PINNED_CODEX):
        raise FrozenRequestModelLaneError("pinned Codex binary drifted")
    ...
    if launch.exists():
        ...
        if value.get("semantic_attempt_count") != 1 or value.get("retry_count") != 0:
            raise FrozenRequestModelLaneError("launch receipt contract drifted")
    return {
        "root": root,
        "config": config,
...
def _unwrap_text(value: Mapping[str, Any], field: str) -> str:
    state, text = str(value["applicability"]), str(value["value"])
    if (state == "present" and not text.strip()) or (
        state == "not_applicable" and text != ""
    ):
        raise FrozenRequestOutputError(f"{field} applicability conflicts with value")
    return text


def _unwrap_enum(value: Mapping[str, Any], field: str, absent: str) -> str:
    state, item = str(value["applicability"]), str(value["value"])
...
def _project_event(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    projected["speaker_name"], projected["speaker_role"] = _unwrap_party(event["speaker"], "speaker")
    projected["actor_name"], projected["actor_type"] = _unwrap_party(event["actor"], "actor")
    projected["reported_actor_name"], projected["reported_actor_type"] = _unwrap_party(
        event["reported_actor"], "reported_actor"
    )
    metric = event["metric"]
    values = [str(metric[name]) for name in ("value", "unit", "comparator", "raw_text")]
    if (
        metric["applicability"] == "present"
        and (not any(values) or metric["direction"] == "not_applicable")
    ) or (
        metric["applicability"] == "not_applicable"
        and (any(values) or metric["direction"] != "not_applicable")
    ):
        raise FrozenRequestOutputError("metric applicability conflicts with value")
    projected.update(
        {
            "metric_value": values[0],
            "metric_unit": values[1],
            "metric_comparator": values[2],
            "metric_raw_text": values[3],
...
def validate_and_project_output(
    ...
    try:
        ...
        raise FrozenRequestOutputError("structured output schema failed") from exc
    ...
    if output.get("episode_id") != source.get("episode_id") or [
        ...
        raise FrozenRequestOutputError("episode or segment order drifted")
    ...
    for row in rows:
        ...
        if [receipt.get("unit_id") for receipt in receipts] != expected_ids:
            raise FrozenRequestOutputError("source unit receipt order drifted")
        ...
        if coverage.get("all_source_units_reviewed") is not True or coverage.get("unresolved_count") != 0:
            raise FrozenRequestOutputError("source unit coverage audit failed")
        receipt_counts = {
            str(receipt["unit_id"]): int(receipt["eligible_event_count"])
            for receipt in receipts
        }
        if any(receipt.get("unresolved_count") != 0 for receipt in receipts):
            raise FrozenRequestOutputError("source unit receipt is unresolved")
        raw_events = list(row.get("events") or [])
        if sum(receipt_counts.values()) != len(raw_events):
            raise FrozenRequestOutputError("source unit event total drifted")
        if (row.get("status") == "coded") != bool(raw_events) or len(raw_events) > 32:
            raise FrozenRequestOutputError("segment status or event cap drifted")
        ...
        for event_index, raw_event in enumerate(raw_events):
            for field in (
                ...
                *ENTITY_FIELDS,
                ...
                state = str(raw_event[field]["applicability"])
                bucket = state_counts.setdefault(field, {})
                bucket[state] = bucket.get(state, 0) + 1
            event = _project_event(raw_event)
            ...
            if unit_index[start_id] < prior_start:
                raise FrozenRequestOutputError("event source order drifted")
            ...
            if any(value and value not in evidence for value in metric_values):
                raise FrozenRequestOutputError("metric literal is not in exact evidence")
            if any(metric_values) == (event.get("metric_direction") == "not_applicable"):
                raise FrozenRequestOutputError("metric direction applicability drifted")
            identity = json.dumps(
            ...
            if identity in seen:
                raise FrozenRequestOutputError("exact event identity duplicate")
            seen.add(identity)
        ...
        if start_counts != receipt_counts:
            raise FrozenRequestOutputError("source unit event ownership drifted")
    ...
    normalized = {"episode_id": source["episode_id"], "segments": normalized_rows}
    projection_schema = _load_json(
        turn["projection_schema"], "direct projection schema"
    )
    try:
        _validate_schema(projection_schema, normalized, path="$")
    except (ValidationError, TypeError, ValueError) as exc:
        raise FrozenRequestOutputError("direct projection schema failed") from exc
    provenance = {
    ...
    return normalized, provenance, {
        ...
        "applicability_state_counts": state_counts,
...
def _usage_from_sidecar(path: Path) -> dict[str, int]:
    ...
    if not isinstance(values, Mapping):
        raise FrozenRequestModelLaneError("turn usage is absent")
    usage: dict[str, int] = {}
    for field in USAGE_FIELDS:
        value = values.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise FrozenRequestModelLaneError("turn usage is incomplete")
        usage[field] = value
    if (
        ...
        raise FrozenRequestModelLaneError("measured sidecar contract failed")
    return usage
...
def _gate(usage: Mapping[str, int], diagnostics: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "pif_frozen_request_model_lane_structural_gate_v1",
        "passed": not failed,
        "checks": checks,
...
def _failure_terminal(
    ...
    if turn["sidecar"].exists():
        ...
        unknown = 1
    semantic = isinstance(exc, (FrozenRequestOutputError, FrozenRequestArchitectureStop))
    ...
    terminal = {
        ...
        "terminal_reason": (
            "frozen_request_model_lane_structural_or_cost_gate_not_passed"
            if semantic
        ...
        "exact_next_action": (
            "freeze this model lane as rejected; do not field-patch it"
            if semantic
    ...
    try:
        ...
        if not launch.exists():
            configured._write_stable_time(  # noqa: SLF001
                launch,
                {
                    "schema_version": "pif_frozen_request_model_lane_launch_v1",
                    "launched_at": now_iso(),
        ...
        if not turn["sidecar"].exists() and (
            ...
            raise FrozenRequestModelLaneError("attempt artifact exists without sidecar")
        ...
        if usage["total_tokens"] > MAX_TOTAL_TOKENS:
            raise FrozenRequestArchitectureStop("turn exceeded frozen token ceiling")
        ...
        if not gate["passed"]:
            raise FrozenRequestArchitectureStop("structural or production-cost gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "frozen_request_model_lane_structural_cost_passed",
            "terminal_reason": "frozen_request_model_lane_structural_cost_passed_support_alignment_required",
            "semantic_attempt_count": 1,
            "semantic_retry_count": 0,
            ...
            "exact_next_action": "run the unchanged frozen support-first and neutral two-permutation alignment evaluator",
        }
        configured._write_stable_time(root / "terminal.json", terminal, "terminal_at")  # noqa: SLF001
        return terminal
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "freeze":
        ...
        print(json.dumps({"state": "frozen", "root": str(frozen["root"])}, sort_keys=True))
        return 0
    if args.command == "verify":
        frozen = verify_frozen(args.config.expanduser().resolve().parent)
        print(json.dumps({"state": "verified", "root": str(frozen["root"])}, sort_keys=True))
        return 0
