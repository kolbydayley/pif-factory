...
def release_unstarted_scorer_reservation(
    conn: sqlite3.Connection, *, reservation_id: str
) -> None:
    """Forget an A2 reservation when foreground priority blocked any call."""

    changed = conn.execute(
        "DELETE FROM signal_desk_scorer_reservations WHERE id=? AND status='active'",
        (reservation_id,),
    ).rowcount
    conn.commit()
    if changed != 1:
        raise RuntimeError("unstarted A2 reservation not found")
        ...
        async def worker(index: int) -> None:
            async with CodexAppServerClient(
                ...
                while not stop.is_set():
                    async with lock:
                        lease = acquire_lease(
                            dispatch, lease_owner=f"a2-{index}", lease_seconds=1800
                        )
                    if lease is None:
                        return
                    ...
                    task_key = (
                        f"a2:{case_id}:attempt:{lease['current_attempt_id']}:"
                    ...
                    if not foreground.allowed:
                        release_attempt_for_retry(
                            ...
                            failure_code="a2_foreground_reserved",
                            failure_detail=(
                                "foreground Codex has priority over background A2; "
                    ...
                    if not admission["allowed"]:
                        release_attempt_for_retry(
                            ...
                            failure_detail="shared GPT-5.6-Sol capacity circuit deferred A2",
                        )
                    ...
                    try:
                        async def invoke_provider_turn() -> Any:
                            ...
                            if not reservation["allowed"]:
                                raise RuntimeError(f"a2_budget_stall:{reservation['reason']}")
                        ...
                        if reservation and reservation.get("allowed") and not settled:
                            if provider_started:
                                ...
                                release_unstarted_scorer_reservation(
                                    budget, reservation_id=str(reservation["reservation_id"])
                                )
                        ...
                        if foreground_yield:
                            ...
                            release_attempt_for_retry(
                                ...
                                failure_code="a2_foreground_reserved",
                                failure_detail=(
                                    "foreground Codex resumed; cancelled background A2 "
                        ...
                        if provider_capacity:
                            ...
                            release_gold_admission(budget, admission_id=admission_id)
                            failure_code = "a2_infrastructure_failure"
                        release_attempt_for_retry(
                            dispatch, attempt_id=int(lease["current_attempt_id"]),
                            lease_owner=str(lease["lease_owner"]),
                            lease_generation=int(lease["lease_generation"]),
