...
"""Low-effort system variant of the exact-token-ID canonical adapter."""
...
ADAPTER_SCHEMA_VERSION = "pif_canonical_v31_unit_owned_token_id_low_episode_batch_v1"
PROJECTION_SCHEMA_VERSION = "pif_canonical_v31_unit_owned_token_id_low_projection_v1"
PROVENANCE_SCHEMA_VERSION = "pif_canonical_v31_unit_owned_token_id_low_provenance_v1"
FIDELITY_SCHEMA_VERSION = "pif_canonical_v31_unit_owned_token_id_low_fidelity_v1"
CANDIDATE_SYSTEM_ID = "pif_unit_owned_token_id_low_ai_discourse_v3_1_v1"
CANONICAL_SCHEMA_STRATEGY = "canonical_v31_unit_owned_exact_token_id_low_effort_v1"
    ...
    value.update(
        {
            ...
            "semantic_system_variant": "exact_token_id_low_effort_v1",
            "parent_prompt_unchanged": True,
            "parent_output_schema_unchanged": True,
...
def _parent_request(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise CanonicalV31EpisodeBatchError("low-effort parent request cannot be reconstructed")
    return matches[0]


def _low_request(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    value["effort"] = EFFORT
    value["batch_id"] = "cv31uotil_" + _sha256_text(
        _canonical_json(
            {
...
def validate_prepared_request(request: Mapping[str, Any]) -> dict[str, Any]:
    expected = _low_request(_parent_request(request))
    if dict(request) != expected:
        raise CanonicalV31EpisodeBatchError("unit-owned token-ID low-effort request drifted")
...
def validate_turn_sidecar(
    ...
    *,
    ...
    validated = validate_prepared_request(request)
    sidecar = base._load_json(sidecar_path, "unit-owned token-ID low-effort sidecar")  # noqa: SLF001
    ...
    if (
        ...
        raise CanonicalV31TelemetryError("managed-auth token-ID low-effort sidecar contract failed")
    ...
    if (
        ...
        raise CanonicalV31TelemetryError("sidecar-bound low-effort output is absent or changed")
    return {
...
def build_six_arm_matrix_binding() -> dict[str, Any]:
    return {
        "schema_version": "pif_canonical_v31_unit_owned_token_id_low_binding_v1",
        ...
        "model": MODEL,
        "effort": EFFORT,
        "prompt_unchanged_from_parent": True,
        "output_schema_unchanged_from_parent": True,
        "deterministic_semantic_pruning": False,
        "deterministic_deduplication": False,
        "deterministic_relabeling": False,
    }
