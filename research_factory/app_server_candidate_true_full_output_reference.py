...
"""Run the checksum-bound true-full-output shared reference evaluation.

This adapter intentionally performs no extraction. It reprojects the frozen
60-event development pool into the mature full-event judge, runs blinded AB/BA
...
SCHEMA_VERSION = "pif_candidate_true_full_output_reference_v2"
LOCK_VERSION = "pif_candidate_true_full_output_reference_lock_v2"
...
class TrueFullOutputReferenceError(RuntimeError):
    """The frozen full-output contract or one of its artifacts is invalid."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise TrueFullOutputReferenceError(f"cannot read {label}") from exc
...
def _write_immutable_json(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise TrueFullOutputReferenceError(f"frozen {path.name} drifted")
        return
...
def load_contract() -> dict[str, Any]:
    ...
    if (
        ...
        != "pif_evaluation_true_full_output_reference_directive_v2"
        ...
        raise TrueFullOutputReferenceError("semantic plan or directive contract drifted")
    ...
    if (
        ...
        raise TrueFullOutputReferenceError("expected receipt path drifted")
    required_roles = {
        "complete_60_submission_bundle",
        "complete_bundle_runtime_lock",
        "candidate_v249_complete_output",
        "baseline_complete_output",
        "blind_source_packet",
        "epoch_1_diagnostic_score",
    }
    frozen_records = []
    frozen_by_role = {}
    for row in directive.get("frozen_inputs") or []:
        if not isinstance(row, Mapping):
            raise TrueFullOutputReferenceError("directive frozen input is malformed")
        role = str(row.get("role") or "")
        if role not in required_roles or role in frozen_by_role:
            raise TrueFullOutputReferenceError("directive frozen input role drifted")
        path = Path(str(row.get("path"))).resolve()
        if not path.is_file() or row.get("sha256") != _sha256_file(path):
            raise TrueFullOutputReferenceError("directive frozen input drifted")
        record = _record(path)
        frozen_records.append(record)
        frozen_by_role[role] = record
    if set(frozen_by_role) != required_roles or len(frozen_records) != 6:
        raise TrueFullOutputReferenceError("directive frozen input coverage drifted")
    predecessor = directive.get("predecessor_receipt")
    if not isinstance(predecessor, Mapping):
        raise TrueFullOutputReferenceError("epoch-1 predecessor receipt is missing")
    predecessor_path = Path(str(predecessor.get("path"))).resolve()
    predecessor_receipt = _load_json(predecessor_path, "epoch-1 predecessor receipt")
    if (
        predecessor.get("sha256") != _sha256_file(predecessor_path)
        or predecessor.get("state") != "rejected"
        or predecessor.get("final_acceptance_evidence") is not False
        or predecessor_receipt.get("plan_epoch") != 1
        or predecessor_receipt.get("state") != "rejected"
    ):
        raise TrueFullOutputReferenceError("epoch-1 diagnostic lineage drifted")
    scope = directive.get("scope_correction")
    if not isinstance(scope, Mapping) or any(
        scope.get(key) != value
        for key, value in {
            "reference_submission_count": 27,
            "candidate_submission_count": 33,
            "total_submission_count": 60,
            "dense_reference_submission_count": 27,
            "dense_candidate_submission_count": 32,
            "no_signal_reference_submission_count": 0,
            "no_signal_candidate_submission_count": 1,
            "epoch_1_candidate_witness_count": 25,
            "omitted_candidate_submission_count": 8,
            "all_submitted_events_must_reach_llm_support_and_equivalence_judgment": True,
            "prior_support_or_alignment_filtering_allowed": False,
        }.items()
    ):
        raise TrueFullOutputReferenceError("full-output scope correction drifted")
    return {
        "plan": plan,
        "directive": directive,
        "receipt_path": receipt_path,
        "frozen_records": frozen_records,
        "frozen_by_role": frozen_by_role,
        "predecessor_receipt": _record(predecessor_path),
    }


def build_shared_pool(contract: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    by_role = contract.get("frozen_by_role")
    if not isinstance(by_role, Mapping):
        raise TrueFullOutputReferenceError("frozen role mapping is missing")
    support_path = Path(str(by_role["complete_60_submission_bundle"]["path"]))
    candidate_path = Path(str(by_role["candidate_v249_complete_output"]["path"]))
    baseline_path = Path(str(by_role["baseline_complete_output"]["path"]))
    support = _load_json(support_path, "complete 60-submission bundle")
    candidate = _load_json(candidate_path, "complete candidate output")
    baseline = _load_json(baseline_path, "complete baseline output")
    if (
        support.get("schema_version") != "pif_candidate_semantic_evaluation_bundle_v1"
        or not isinstance(support.get("cases"), list)
        or not isinstance(support.get("origins"), list)
    ):
        raise TrueFullOutputReferenceError("frozen 60-witness source shape drifted")
    counts = support.get("counts")
    grounding = support.get("grounding")
    if (
        not isinstance(counts, Mapping)
        or counts.get("reference_witness_count") != 27
        or counts.get("candidate_witness_count") != 33
        or counts.get("total_witness_count") != 60
        or not isinstance(grounding, Mapping)
        or grounding.get("candidate_event_count") != 33
        or grounding.get("exact_evidence_event_count") != 33
        or grounding.get("exact_evidence_rate") != 1.0
        or grounding.get("event_cap_violation_count") != 0
        or grounding.get("metric_grounding_error_event_count") != 0
    ):
        raise TrueFullOutputReferenceError("complete bundle grounding invariant drifted")
    origin_by_id = {str(row.get("witness_id")): row for row in support["origins"]}
    if len(origin_by_id) != 60:
        raise TrueFullOutputReferenceError("frozen witness mapping is not 60 unique rows")
    ...
    def event_hash(value: Mapping[str, Any]) -> str:
        return hashlib.sha256(_canonical_json(compact(value)).encode("utf-8")).hexdigest()
    ...
    target_segment_ids = {str(row.get("segment_id")) for row in support["origins"]}
    baseline_hashes = {
        event_hash(event)
        for row in baseline.get("references") or []
        if str(row.get("segment_id")) in target_segment_ids
        for event in ((row.get("golden_output") or {}).get("discourse_events") or [])
        if isinstance(event, Mapping)
    }
    bundle_candidate_hashes = {
        str(row.get("event_sha256"))
        for row in support["origins"]
        if row.get("origin") == "candidate"
    }
    bundle_baseline_hashes = {
        str(row.get("event_sha256"))
        for row in support["origins"]
        if row.get("origin") == "reference"
    }
    if (
        len(candidate_hashes) != 33
        or candidate_hashes != bundle_candidate_hashes
        or len(baseline_hashes) != 27
        or baseline_hashes != bundle_baseline_hashes
    ):
        raise TrueFullOutputReferenceError("direct 27+33 submission lineage drifted")
    ...
    for case in support["cases"]:
        ...
        for witness in case.get("witnesses") or []:
            ...
            if legacy_id in seen_ids or legacy_id not in origin_by_id:
                raise TrueFullOutputReferenceError("support pool witness mapping drifted")
            seen_ids.add(legacy_id)
            row = origin_by_id[legacy_id]
            if row.get("case_id") != case.get("case_id") or row.get("density_stratum") != density:
                raise TrueFullOutputReferenceError("support pool case provenance drifted")
            ...
            if origin_name == "reference":
                ...
                raise TrueFullOutputReferenceError("unknown source system membership")
    ...
    if seen_ids != set(origin_by_id) or observed_counts != EXPECTED_WITNESS_COUNTS:
        raise TrueFullOutputReferenceError("frozen 27+33 witness membership drifted")
    pool, mapping = judge.make_shared_witness_pool(
        raw_cases,
        seed="true_full_output_shared_reference_v2",
    )
    errors = judge.validate_shared_witness_pool(pool)
    if errors:
        raise TrueFullOutputReferenceError("generated shared witness pool is invalid")
    exact_count = sum(
    ...
    if len(pool["cases"]) != 2 or witness_count != 60 or exact_count != 60:
        raise TrueFullOutputReferenceError("shared witness exact-evidence invariant drifted")
    return pool, mapping
...
def freeze_run(output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    ...
    if root.exists() and any(root.iterdir()):
        raise TrueFullOutputReferenceError("unfrozen output root is not empty")
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise TrueFullOutputReferenceError("true-full-output runtime lock drifted")
    if not records or any(not _verify_record(row) for row in records):
        raise TrueFullOutputReferenceError("true-full-output runtime artifact drifted")
    load_contract()
    return lock
...
def _sidecar_usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    if (
        ...
        raise TrueFullOutputReferenceError("judge sidecar is not measured managed-auth output")
    normalized = {}
    for field in USAGE_FIELDS:
        value = usage.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise TrueFullOutputReferenceError("judge usage telemetry is incomplete")
    ...
    if (
        ...
        raise TrueFullOutputReferenceError("judge usage accounting is inconsistent")
    return normalized
...
def _observable_disagreement_case_ids(
    ...
    for name in ("ab", "ba"):
        ...
        if errors:
            raise TrueFullOutputReferenceError("judge output failed validation")
...
def _build_adjudication_variant(
    ...
    if not cases or {row["case_id"] for row in cases} != selected:
        raise TrueFullOutputReferenceError("adjudication disagreement coverage drifted")
    return {
...
async def _run_adjudication(
    *,
    ...
    if (phase_root / "sidecar.json").exists() or (phase_root / "output.private.json").exists():
        raise TrueFullOutputReferenceError("adjudication attempt already exists; retry is forbidden")
    ...
    if any(not _verify_record(row) for row in predecessor_records):
        raise TrueFullOutputReferenceError("adjudication predecessor drifted")
    ...
    if not result.status_ok or not isinstance(result.output, Mapping):
        raise TrueFullOutputReferenceError("side-free adjudication turn failed")
    output = _load_json(output_path, "adjudication output")
    errors = judge.validate_judge_output(output, variant)
    if errors:
        raise TrueFullOutputReferenceError("side-free adjudication output is invalid")
    ...
    if (
        ...
        raise TrueFullOutputReferenceError("adjudication capacity checkpoint is invalid")
...
def _merge_adjudication(
    *,
    ...
    if set(adjudicated) != selected:
        raise TrueFullOutputReferenceError("adjudication output case coverage drifted")
    cases = []
    ...
    for case in mapping["cases"]:
        ...
        for witness in case["witnesses"]:
            ...
            if system_id not in {SYSTEM_BASELINE, SYSTEM_CANDIDATE}:
                raise TrueFullOutputReferenceError("private system membership drifted")
    ...
    *,
    ...
    for row in consensus["cases"]:
        ...
        if row.get("partition_abstained_witness_ids"):
            raise TrueFullOutputReferenceError("cannot score an abstained shared partition")
        witness_event = {
        ...
        if set().union(*groups) != set(witness_event):
            raise TrueFullOutputReferenceError("shared partition does not cover the case")
        group_by_witness = {
...
def score_shared_reference(
    *,
    ...
    if shared.get("reference_system_ids") != [SYSTEM_BASELINE, SYSTEM_CANDIDATE]:
        raise TrueFullOutputReferenceError("shared reference did not include both systems")
    ...
    checks = {
        "full_60_witness_pool": witness_count == 60,
...
def _receipt(
    *,
    ...
    if state not in {"passed", "rejected", "waiting"}:
        raise TrueFullOutputReferenceError("invalid plan-step receipt state")
    payload = {
        ...
        "next_action": load_contract()["directive"]["branch_after_receipt"][state],
    ...
    *,
    ...
    if receipt_path.is_file():
        ...
        if (
            ...
            raise TrueFullOutputReferenceError("existing plan-step receipt drifted")
        return receipt
    ...
    try:
        ...
        epoch_1_score_path = Path(
            str(load_contract()["frozen_by_role"]["epoch_1_diagnostic_score"]["path"])
        )
        epoch_1_score = _load_json(epoch_1_score_path, "epoch-1 diagnostic score")
        ratio = float(epoch_1_score["production_amortized_total_token_ratio"])
        ...
        terminal_reason = (
            "true_full_output_shared_reference_quality_gate_passed"
            if score["passed"]
            else "true_full_output_shared_reference_quality_gate_not_passed"
        )
        receipt = _receipt(
            root=root,
            state=state,
        ...
        except TrueFullOutputReferenceError:
            accounting = {
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the true full-output shared reference gate")
