...
"""Recover v271 by removing unsupported app-server schema keywords only."""
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v272_compact_delta_schema_recovery_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v272_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v272_terminal_v1"
PHASE_ID = "development_selection_v5_4_v272_compact_delta_schema_recovery"
MODEL = "gpt-5.6-sol"
EFFORT = "low"
...
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v272-compact-delta-schema-recovery"
).resolve()
PINNED_CODEX_0_144_1 = v249.PINNED_CODEX_0_144_1


class V272DeltaSetEditorError(RuntimeError):
    """The v272 architecture cannot proceed or be adopted safely."""


class V272OutputContractError(V272DeltaSetEditorError):
    """A completed canonicalizer output violated the frozen projection contract."""


class V272ArchitectureStop(V272DeltaSetEditorError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V272DeltaSetEditorError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V272DeltaSetEditorError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    ...
    v271_turn = _source_turn_root(V271_ROOT, "v271-compact-delta-set-editor-")
    return {
        ...
        "v271_terminal": V271_ROOT / "terminal.json",
        "v271_runtime_lock": V271_ROOT / "runtime-lock.json",
        "v271_capacity": v271_turn / "capacity.json",
        "v271_sidecar": v271_turn / "sidecar.json",
        "v271_input": v271_turn / "input.private.json",
        "v271_prompt": v271_turn / "prompt.private.md",
        "v271_base": v271_turn / "base-instructions.private.md",
        "v271_schema": v271_turn / "schema.json",
        "v271_projection_schema": v271_turn / "projection-schema.json",
        "v271_direct_schema": v271_turn / "direct-schema.json",
    }
...
def _validate_lineage() -> dict[str, Any]:
    ...
    t271 = _load_json(paths["v271_terminal"], "v271 terminal")
    s271 = _load_json(paths["v271_sidecar"], "v271 sidecar")
    schema271 = _load_json(paths["v271_schema"], "v271 schema")
    if (
        ...
        or t271.get("terminal_reason") != "infrastructure_or_judge_attempt_failed"
        or t271.get("error_message_sha256")
        != "a34941799b9e25bd8b4d4935d5774f08db2b114ecb2d589da9a665e3ee018d4a"
        or t271.get("usage_status") != "unknown"
        or s271.get("state") != "failed"
        or s271.get("usage_status") != "unknown"
        or s271.get("error_class") != "turn_failed"
        or t271.get("production_mutated") is not False
        or validate_app_server_output_schema_subset(schema271)
        != [
            "$.$schema",
            "$.properties.segments.items.properties.replacement_events.items.properties.source_proposal_ids.uniqueItems",
        ]
    ):
        raise V272DeltaSetEditorError("v272 architecture lineage drifted")
...
def _base_packet(lineage: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, list[str]]]:
    ...
    if [row["segment_id"] for row in raw_segments] != [
        ...
        raise V272DeltaSetEditorError("v249 base segment order drifted")
    for position, (raw, normalized) in enumerate(zip(raw_segments, normalized_segments)):
        ...
        if len(raw["events"]) != len(normalized["events"]):
            raise V272DeltaSetEditorError("v249 raw and normalized event counts drifted")
...
def repair_schema_subset(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if validate_app_server_output_schema_subset(corrected):
        raise V272DeltaSetEditorError("v272 corrected schema remains unsupported")
    return corrected
...
def _episode_context(base: str) -> Any:
    ...
    if marker not in base:
        raise V272DeltaSetEditorError("immutable episode context marker missing")
    payload = base.split(marker, 1)[1].split("\n\n# Explicit applicability contract", 1)[0]
    try:
        return json.loads(payload.strip())
    except json.JSONDecodeError as exc:
        raise V272DeltaSetEditorError("immutable episode context is malformed") from exc


def prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    source = lineage["source_turn"]
    packet, proposal_ids = _base_packet(lineage)
    schema = repair_schema_subset(_delta_schema(source, proposal_ids))
    ...
    if (
        ...
        raise V272DeltaSetEditorError("v272 request size or base coverage drifted")
    return {
        "turn_name": "v272_compact_delta_set_editor_" + sha256_text(source["episode_id"])[:20],
...
def project_output(
    ...
    try:
        ...
        raise V272OutputContractError("delta set-editor output schema failed") from exc
    if output.get("episode_id") != turn["episode_id"]:
        raise V272OutputContractError("delta set-editor episode id drifted")
    rows = list(output.get("segments") or [])
    if [row.get("segment_id") for row in rows] != list(turn["segment_ids"]):
        raise V272OutputContractError("delta set-editor segment coverage or order drifted")
    ...
    for position, row in enumerate(rows):
        ...
        if (
            ...
            raise V272OutputContractError("delta set-editor receipt coverage failed")
        ...
        for index, raw_event in enumerate(row["replacement_events"]):
            ...
            if (
                ...
                raise V272OutputContractError("delta replacement identity or ownership drifted")
        ...
        for receipt in receipts:
            ...
            if action == "keep":
                if final_event_id != proposal_id:
                    raise V272OutputContractError("kept proposal final ID drifted")
            elif action == "drop":
                if final_event_id:
                    raise V272OutputContractError("dropped proposal names a final event")
            else:
                if final_event_id not in replacement_by_id:
                    raise V272OutputContractError("changed proposal lacks replacement event")
        ...
        for event_id, source_ids in replacement_sources.items():
            ...
            if source_ids != [str(item["proposal_id"]) for item in group]:
                raise V272OutputContractError("replacement receipt mapping drifted")
            if len(group) == 1 and group[0]["action"] != "replace":
                raise V272OutputContractError("single changed proposal must be replace")
            if len(group) > 1 and any(item["action"] != "merge" for item in group):
                raise V272OutputContractError("multi-proposal replacement must be merge")
        if set(grouped) != set(replacement_by_id):
            raise V272OutputContractError("unmapped replacement event")
        final_events = []
        emitted = set()
        for receipt in receipts:
            proposal_id = str(receipt["proposal_id"])
        ...
        try:
            ...
            raise V272OutputContractError("final event evidence belongs to another segment") from exc
        if len(final_events) > 32 or (row["final_status"] == "coded") != bool(final_events):
            raise V272OutputContractError("final status or event cap drifted")
    ...
    try:
        ...
        raise V272OutputContractError(str(exc)) from exc
    coverage = {
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v272 runtime lock")
    root = path.parent.resolve()
    spec = _load_json(root / "attempt-spec.json", "v272 spec")
    ...
    if (
        ...
        raise V272DeltaSetEditorError("v272 runtime lock contract drifted")
    records = [
        lock.get("pinned_codex_cli"),
        *(lock.get("runtime_files") or []),
        *(lock.get("direct_lineage") or []),
        lock.get("v271_schema_recovery_audit"),
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V272DeltaSetEditorError("v272 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V272DeltaSetEditorError("v272 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    spec = _load_json(spec_path, "v272 spec")
    paths = _turn_paths(root, str(spec["turn_name"]))
    lineage = _validate_lineage()
    ...
    return {
        ...
        "turn": {
            **turn,
            "private_input": _load_json(paths["input"], "v272 input"),
            "prompt": paths["prompt"].read_text(encoding="utf-8"),
            "base": paths["base"].read_text(encoding="utf-8"),
            "schema": _load_json(paths["schema"], "v272 schema"),
            "projection_schema": _load_json(paths["projection_schema"], "v272 projection schema"),
            "direct_schema": _load_json(paths["direct_schema"], "v272 direct schema"),
            "paths": paths,
        },
    }


def freeze_v272(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v272 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V272DeltaSetEditorError("unfinished v272 root is not replayable")
    ...
    unchanged_request_names = {
        "input": "v271_input",
        "prompt": "v271_prompt",
        "base": "v271_base",
        "projection_schema": "v271_projection_schema",
        "direct_schema": "v271_direct_schema",
    }
    if any(
        _record(paths[name])["sha256"]
        != lineage["records"][predecessor_name]["sha256"]
        for name, predecessor_name in unchanged_request_names.items()
    ):
        raise V272DeltaSetEditorError("v271 non-schema request content drifted")
    unsupported_before = validate_app_server_output_schema_subset(
        lineage["v271_schema"]
    )
    unsupported_after = validate_app_server_output_schema_subset(turn["schema"])
    if turn["schema"] != repair_schema_subset(lineage["v271_schema"]):
        raise V272DeltaSetEditorError("v272 schema changed beyond subset normalization")
    capacity_paths = _capacity_policy(root, turn["turn_name"])
    schema_audit_path = root / "v271-schema-subset-recovery-audit.json"
    _write_stable_time(
        schema_audit_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "predecessor_terminal": lineage["records"]["v271_terminal"],
            "predecessor_sidecar": lineage["records"]["v271_sidecar"],
            "predecessor_runtime_lock": lineage["records"]["v271_runtime_lock"],
            "predecessor_schema": lineage["records"]["v271_schema"],
            ...
            "failure_class": "unsupported_structured_output_schema_keywords",
            "unsupported_paths_before": unsupported_before,
            "unsupported_paths_after": unsupported_after,
            "removed_keywords": ["$schema", "uniqueItems"],
            "prompt_input_and_semantic_contract_unchanged": True,
            "deterministic_uniqueness_validator_retained": True,
    ...
    _write_stable_time(
        authorization_path,
        {
            ...
            "scope": "remove unsupported Structured Outputs schema keywords and run one fresh bounded attempt",
            "semantic_prompt_or_schema_delta": False,
            "schema_formatting_delta_only": True,
    ...
    _write_stable_time(
        ranking_path,
        {
            ...
            "measured_basis": {
                ...
                "v271_semantic_output_observed": False,
                "v271_usage_status": "unknown",
                "v271_unsupported_schema_paths": unsupported_before,
            },
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "hypothesis": (
                ...
                "empty input fields leaves enough token budget for a decision-changing second stage; v272 "
                "changes only unsupported schema formatting, not that semantic hypothesis"
            ),
    ...
    _write_stable_time(
        spec_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "state": "frozen_before_one_turn_compact_delta_schema_recovery",
    ...
    lock = {
        ...
        "v271_schema_recovery_audit": _record(schema_audit_path),
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        sidecar = _load_json(paths["sidecar"], "v272 sidecar")
        measured = _measured_usage(sidecar)
        if measured is not None:
            usage = measured
            unknown = 0
            measured_over_cap = usage["total_tokens"] > MAX_TOTAL_TOKENS
    semantic = isinstance(exc, (V272OutputContractError, V272ArchitectureStop)) or measured_over_cap
    ...
    terminal = {
        ...
        "terminal_reason": (
            "v272_compact_delta_set_editor_structural_quality_or_cost_gate_not_passed"
            if semantic
        ...
        "exact_next_action": (
            ...
            if semantic
            else "audit immutable v272 infrastructure attempt; no retry"
        ),
    }
...
async def run_v272(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v272 terminal")
    frozen = freeze_v272(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V272DeltaSetEditorError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V272DeltaSetEditorError("v272 turn did not complete")
        usage = _measured_usage(_load_json(paths["sidecar"], "v272 sidecar"))
        if usage is None:
            raise V272DeltaSetEditorError("v272 sidecar accounting or auth failed")
        if usage["total_tokens"] > MAX_TOTAL_TOKENS:
            raise V272ArchitectureStop("v272 measured turn exceeded frozen token bound")
        ...
        if not gate["passed"]:
            raise V272ArchitectureStop("v272 structural quality or cost gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "v272_architecture_structural_gate_passed",
            "terminal_reason": "v272_compact_delta_set_editor_structural_cost_gate_passed",
            "semantic_attempt_count": 1,
            "semantic_retry_count": 0,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v272 compact global delta set editor")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v272(output_dir=Path(args.output_dir))
        result = {
            "state": "frozen",
        ...
        terminal = asyncio.run(
            run_v272(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
