...
"""Run the unchanged frozen side-free support judge over v255."""
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v256_frozen_support_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v256_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v256_terminal_v1"
PHASE_ID = "judge_v5_4_selection_v256_frozen_support"
TURN_NAME = "v256_frozen_support_v255_candidate"
MODEL = v250.MODEL
EFFORT = v250.EFFORT
...
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v256-frozen-support-v255"
).resolve()
PINNED_CODEX_0_144_1 = v255.PINNED_CODEX_0_144_1


class V256FrozenSupportError(RuntimeError):
    """The v256 frozen support gate cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V256FrozenSupportError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V256FrozenSupportError(f"frozen {path.name} drifted")
        return
...
def _v255_turn_root() -> Path:
    return next(V255_ROOT.glob("turns/v255-atomic-ledger-*"))


def _lineage_paths() -> dict[str, Path]:
    turn_root = _v255_turn_root()
    return {
        ...
        "v255_sidecar": turn_root / "sidecar.json",
        "v255_normalized": V255_ROOT / "normalized-output.private.json",
        "v255_provenance": V255_ROOT / "evidence-provenance.private.json",
        "v255_input": turn_root / "input.private.json",
...
def _validate_lineage() -> dict[str, Any]:
    ...
    terminal = _load_json(paths["v255_terminal"], "v255 terminal")
    gate = _load_json(paths["v255_gate"], "v255 gate")
    sidecar = _load_json(paths["v255_sidecar"], "v255 sidecar")
    ...
    if (
        ...
        != "v255_atomic_ledger_structural_cost_gate_passed"
        ...
        or gate.get("production_amortized_total_token_ratio") != 0.198406
        or (sidecar.get("usage") or {}).get("total_tokens") != 46_550
        ...
        raise V256FrozenSupportError("v256 frozen support lineage or protocol drifted")
...
def _event_claim(event: Mapping[str, Any]) -> str:
    ...
    if not isinstance(claim, str) or not claim.strip():
        raise V256FrozenSupportError("support witness has no claim text")
    return claim


def build_support_pool(lineage: Mapping[str, Any]) -> dict[str, Any]:
    source = _load_json(lineage["paths"]["v255_input"], "v255 source input")
    candidate = _load_json(
        lineage["paths"]["v255_normalized"], "v255 normalized output"
    )
    ...
    segment_ids = [v255.v249.v239.DENSE_SEGMENT_ID, v255.v249.v239.NO_SIGNAL_SEGMENT_ID]
    if any(
        ...
        raise V256FrozenSupportError("support pool segment coverage drifted")
    ...
    for segment_id in segment_ids:
        ...
        if sha256_text(source_excerpt) != reference_row["text_sha256"]:
            raise V256FrozenSupportError("support source text hash drifted")
        density = str(source_row["density_stratum"])
        case_id = "case_" + sha256_text(f"v256|{segment_id}")[:24]
        ...
        for origin, events in (
            ...
            for event_index, event in enumerate(events):
                event_hash = sha256_text(v255._canonical_json(event))
                witness_id = "w_" + sha256_text(
                    f"v256|{case_id}|{origin}|{event_index}|{event_hash}"
                )[:24]
    ...
    if (
        ...
        raise V256FrozenSupportError("support pool witness coverage drifted")
    ...
    schema_text = v255._canonical_json(schema)
    if (
        len(prompt.encode("utf-8")) > MAX_PROMPT_BYTES
        or len(schema_text.encode("utf-8")) > MAX_SCHEMA_BYTES
    ):
        raise V256FrozenSupportError("support request size cap exceeded")
    return {
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v256 runtime lock")
    if (
        ...
        raise V256FrozenSupportError("v256 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V256FrozenSupportError("v256 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V256FrozenSupportError("v256 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    return {
        "root": root,
        "spec_path": spec_path,
        "spec": _load_json(spec_path, "v256 spec"),
...
def freeze_v256(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v256 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V256FrozenSupportError("unfinished v256 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "strategy": "unchanged_frozen_side_free_support_over_v255_and_shared_reference",
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V256FrozenSupportError("v256 sidecar usage incomplete") from exc
    if (
        ...
        raise V256FrozenSupportError("v256 sidecar accounting or auth contract failed")
    return usage
...
def _failure_terminal(
    ...
    if paths["sidecar"].is_file():
        ...
        try:
            sidecar = _load_json(paths["sidecar"], "v256 sidecar")
            usage = {
    ...
    terminal = {
        ...
        "exact_next_action": "audit immutable v256 judge attempt; no retry",
    }
...
async def run_v256(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v256 terminal")
    frozen = freeze_v256(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V256FrozenSupportError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V256FrozenSupportError("v256 support turn did not complete")
        usage = _usage(_load_json(paths["sidecar"], "v256 sidecar"))
        ...
        terminal = {
            ...
            "terminal_reason": (
                ...
                if authorized
                else "v256_frozen_support_quality_gate_not_passed"
            ),
            ...
            "exact_next_action": (
                ...
                if authorized
                else "reject v255 on source support and advance to the next distinct architecture"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run frozen support over v255")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v256(output_dir=Path(args.output_dir))
        result = {
