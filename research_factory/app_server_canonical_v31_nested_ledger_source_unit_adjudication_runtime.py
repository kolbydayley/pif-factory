...
"""Epoch-36 one-call source-unit adjudication of the epoch-35 disagreement."""
...
from . import app_server_canonical_v31_nested_ledger_quality_runtime as epoch35
...
DEFAULT_ROOT = (
    PIPELINE_ROOT
    / "canonical-v31-epoch36-nested-ledger-source-unit-adjudication-v1"
).resolve()
DIRECTIVE_PATH = (
    PROJECT_ROOT
    / "automation/pif-evaluation-epoch36-nested-ledger-source-unit-adjudication-v36.json"
).resolve()
PLAN_PATH = (
    PROJECT_ROOT / "automation/pif-evaluation-semantic-plan-v36.json"
).resolve()
...
SCHEMA_VERSION = "pif_canonical_v31_nested_ledger_source_unit_adjudication_v36"
LOCK_VERSION = "pif_canonical_v31_nested_ledger_source_unit_adjudication_lock_v1"
LAUNCH_VERSION = "pif_canonical_v31_nested_ledger_source_unit_adjudication_launch_v1"
RECEIPT_VERSION = "pif_semantic_plan_step_receipt_v1"
THREAD_ID = epoch35.THREAD_ID
PLAN_EPOCH = 36
STEP_ID = "canonical_v31_epoch36_nested_ledger_source_unit_adjudication_v36"
MODEL = epoch35.MODEL
EFFORT = epoch35.EFFORT
TRANSPORT = epoch35.TRANSPORT
...
QUALITY_THRESHOLD = epoch35.QUALITY_THRESHOLD
EXPECTED_TOTAL_WITNESSES = epoch35.EXPECTED_TOTAL_WITNESSES
EXPECTED_BASELINE_WITNESSES = epoch35.EXPECTED_BASELINE_WITNESSES
EXPECTED_CANDIDATE_WITNESSES = epoch35.EXPECTED_CANDIDATE_WITNESSES
EXPECTED_DISAGREEMENT_CASE_IDS = (
    "jcase_7a7b8e3ffc335e72edd364d5",
    "jcase_f94ed7887f44fd47704962b7",
)
CAPACITY_TURN_NAMES = ("epoch36_quality_adjudication",)
TURN_DIRECTORY_NAME = "epoch36-quality-adjudication"
EXPECTED_INSTRUCTION_SOURCES_COUNT = epoch35.EXPECTED_INSTRUCTION_SOURCES_COUNT
EXPECTED_INSTRUCTION_SOURCES_SHA256 = epoch35.EXPECTED_INSTRUCTION_SOURCES_SHA256
PREDECESSOR_QUALITY_USAGE = {
    "input_tokens": 112_520,
    "cached_input_tokens": 2_816,
    "output_tokens": 36_129,
    "reasoning_output_tokens": 14_419,
    "total_tokens": 148_649,
}
EPOCH35_USAGE = {
    "input_tokens": 112_520,
    "cached_input_tokens": 2_816,
...
PREDECESSOR_AGGREGATE_MODEL_CALLS = 13
PREDECESSOR_AGGREGATE_UNKNOWN_USAGE = 2
PREDECESSOR_AGGREGATE_MEASURED_TOKENS = 523_210
...
PROCESS_LOCK_NAME = ".canonical-v31-epoch36-source-unit-adjudication.lock"
USAGE_FIELDS = epoch35.USAGE_FIELDS
_FORBIDDEN_AUTH_ENVIRONMENT = epoch35._FORBIDDEN_AUTH_ENVIRONMENT  # noqa: SLF001


class Epoch36SourceUnitAdjudicationError(RuntimeError):
    """Epoch-36 contract, lineage, or immutable evidence drifted."""


class Epoch36SourceUnitAdjudicationWaiting(Epoch36SourceUnitAdjudicationError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise Epoch36SourceUnitAdjudicationError(f"cannot read {label}") from exc
...
def _write_immutable_json(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise Epoch36SourceUnitAdjudicationError(f"frozen {path.name} drifted")
        return
...
def _process_lock(root: Path) -> Iterator[None]:
    ...
    with path.open("a+", encoding="ascii") as handle:
        try:
            ...
            raise Epoch36SourceUnitAdjudicationWaiting(
                "another epoch-36 adjudication process owns this root"
            ) from exc
        try:
            yield
        finally:
...
def _expected_predecessor_records() -> list[dict[str, Any]]:
    _validate_epoch35_terminal()
    records = [
        _record(EPOCH35_ROOT / "runtime-lock.json"),
        _record(EPOCH35_ROOT / "plan-step-receipt.json"),
        _record(EPOCH35_ROOT / "terminal.json"),
        _record(EPOCH35_ROOT / "launch-receipt.json"),
        _record(EPOCH35_ROOT / "shared-witness-pool.private.json"),
        _record(EPOCH35_ROOT / "private-witness-mapping.private.json"),
        _record(EPOCH35_ROOT / "candidate-lineage.json"),
        _record(EPOCH35_ROOT / "judge/output-ab.private.json"),
        _record(EPOCH35_ROOT / "judge/output-ba.private.json"),
        _record(EPOCH35_ROOT / "judge/consensus.private.json"),
        _record(EPOCH35_ROOT / "judge/report.json"),
        _record(EPOCH35_ROOT / "judge/sidecars/ab.json"),
        _record(EPOCH35_ROOT / "judge/sidecars/ba.json"),
        _record(EPOCH35_ROOT / "judge/sidecars/ab.capacity.json"),
        _record(EPOCH35_ROOT / "judge/sidecars/ba.capacity.json"),
    ]
    return records


def _validate_epoch35_terminal() -> dict[str, Any]:
    receipt = epoch35.verify_receipt(EPOCH35_ROOT)
    if (
        receipt.get("state") != "waiting"
        or receipt.get("terminal_reason")
        != "epoch35_ab_ba_disagreement_requires_versioned_adjudication"
        ...
        or receipt.get("usage") != EPOCH35_USAGE
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-35 terminal contract drifted")
    return receipt


def load_contract() -> dict[str, Any]:
    plan = _load_json(PLAN_PATH, "epoch-36 semantic plan")
    directive = _load_json(DIRECTIVE_PATH, "epoch-36 directive")
    ...
    if (
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-36 semantic plan drifted")
    ...
    if (
        ...
        != "pif_evaluation_epoch36_nested_ledger_source_unit_adjudication_directive_v1"
        ...
        or predecessor.get("measured_total_tokens") != EPOCH35_USAGE["total_tokens"]
        ...
        or predecessor.get("runtime_lock") != _record(EPOCH35_ROOT / "runtime-lock.json")
        or predecessor.get("receipt")
        != _record(EPOCH35_ROOT / "plan-step-receipt.json")
        or predecessor.get("terminal") != _record(EPOCH35_ROOT / "terminal.json")
        ...
        != epoch35.FULL_SIX_CASE_TOKEN_PROJECTION
        or promotion.get("full_six_case_token_ceiling")
        != epoch35.FULL_SIX_CASE_TOKEN_CEILING
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-36 directive values drifted")
    receipt = _validate_epoch35_terminal()
    return {
        "plan": plan,
        "directive": directive,
...
def _base_bundle() -> dict[str, Any]:
    _validate_epoch35_terminal()
    pool = _load_json(EPOCH35_ROOT / "shared-witness-pool.private.json", "epoch-35 pool")
    mapping = _load_json(
        EPOCH35_ROOT / "private-witness-mapping.private.json", "epoch-35 mapping"
    )
    outputs = {
        name: _load_json(
            EPOCH35_ROOT / f"judge/output-{name}.private.json", f"epoch-35 {name} output"
        )
        for name in ("ab", "ba")
    }
    ...
    for name in ("ab", "ba"):
        if judge.validate_judge_output(outputs[name], variants[name]):
            raise Epoch36SourceUnitAdjudicationError(f"epoch-35 {name} output drifted")
    consensus = judge.combine_judge_consensus(pool, outputs)
    if consensus != _load_json(
        EPOCH35_ROOT / "judge/consensus.private.json", "epoch-35 consensus"
    ):
        raise Epoch36SourceUnitAdjudicationError("epoch-35 consensus reconstruction drifted")
    return {
        "pool": pool,
        "mapping": mapping,
        "outputs": outputs,
...
def observable_disagreement_case_ids(
    ...
    for name in ("ab", "ba"):
        if judge.validate_judge_output(outputs[name], variants[name]):
            raise Epoch36SourceUnitAdjudicationError("base judge output failed exact validation")
    ...
    if sorted(disagreement) != sorted(EXPECTED_DISAGREEMENT_CASE_IDS):
        raise Epoch36SourceUnitAdjudicationError("observable disagreement case set drifted")
...
def build_adjudication_variant(
    ...
    for case_position, case in enumerate(pool["cases"]):
        ...
        if not isinstance(source, str) or not source:
            raise Epoch36SourceUnitAdjudicationError("adjudication source is empty")
        boundaries = [
            {
                "window_id": 0,
                "owner_start": 0,
        ...
        try:
            ...
            raise Epoch36SourceUnitAdjudicationError(
    ...
    if not cases or {row["case_id"] for row in cases} != selected:
        raise Epoch36SourceUnitAdjudicationError("adjudication case coverage drifted")
    variant = {
    ...
    if epoch35.SYSTEM_BASELINE in serialized or epoch35.SYSTEM_CANDIDATE in serialized:
        raise Epoch36SourceUnitAdjudicationError("system origin leaked into adjudication")
    return variant
...
def _freeze_unlocked(root: Path) -> dict[str, Any]:
    ...
    if root.exists() and _meaningful_entries(root):
        raise Epoch36SourceUnitAdjudicationError("unfrozen epoch-36 root is not empty")
    contract = load_contract()
    if contract["receipt_path"] != root / "plan-step-receipt.json":
        raise Epoch36SourceUnitAdjudicationError("epoch-36 output root differs from contract")
    ...
    _write_immutable_json(
        attempt_path,
        {
            ...
            "step_id": STEP_ID,
            "predecessor_receipt": _record(EPOCH35_ROOT / "plan-step-receipt.json"),
...
def verify_runtime_lock(path: Path, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        lock = _load_json(lock_path, "epoch-36 runtime lock")
        records = [
        ...
        if (
            ...
            raise Epoch36SourceUnitAdjudicationError("epoch-36 runtime lock drifted")
        load_contract()
        variant, base, prompt, schema = _request_material()
        expected_requests = [
        ...
        if (
            ...
            raise Epoch36SourceUnitAdjudicationError("epoch-36 request artifacts drifted")
        attempt = _load_json(
            Path(str(lock["attempt_spec"]["path"])), "epoch-36 attempt spec"
        )
        if (
            ...
            raise Epoch36SourceUnitAdjudicationError("epoch-36 attempt specification drifted")
        ...
        if (
            ...
            raise Epoch36SourceUnitAdjudicationError("epoch-36 capacity policy drifted")
        return lock
...
def _validate_capacity_checkpoint(root: Path) -> dict[str, Any]:
    ...
    value = _load_json(path, "epoch-36 capacity checkpoint")
    ...
    if not isinstance(value, Mapping) or any(
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-36 capacity checkpoint drifted")
    ...
    for key in (
        ...
        if value.get(key) != evaluation[key]:
            raise Epoch36SourceUnitAdjudicationError("epoch-36 capacity arithmetic drifted")
    return dict(value)
...
def _validate_launch_receipt(root: Path) -> dict[str, Any]:
    value = _load_json(root / "launch-receipt.json", "epoch-36 launch receipt")
    expected = {
        "runtime_lock": _record(root / "runtime-lock.json"),
        "predecessor_receipt": _record(EPOCH35_ROOT / "plan-step-receipt.json"),
        "attempt_spec": _record(root / "attempt-spec.json"),
    ...
    if (
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-36 launch receipt drifted")
    return dict(value)


def _valid_usage(value: Any) -> dict[str, int]:
    try:
        return epoch35._valid_usage(value)  # noqa: SLF001
    except epoch35.Epoch35QualityError as exc:
        raise Epoch36SourceUnitAdjudicationError(str(exc)) from exc
...
def _validate_partial_sidecar(root: Path, sidecar: Mapping[str, Any]) -> None:
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch36SourceUnitAdjudicationError("partial adjudication sidecar lineage drifted")
    thread_id = sidecar.get("thread_id")
    turn_id = sidecar.get("turn_id")
    if not isinstance(thread_id, str) or not thread_id:
        raise Epoch36SourceUnitAdjudicationError("partial adjudication lacks thread identity")
    if turn_id is not None and (not isinstance(turn_id, str) or not turn_id):
        raise Epoch36SourceUnitAdjudicationError("partial adjudication turn identity drifted")
    output_path = _turn_root(root) / "output.private.json"
    if Path(str(sidecar.get("output_path") or "")).expanduser().resolve() != output_path:
        raise Epoch36SourceUnitAdjudicationError("partial adjudication output path drifted")
...
def _validate_completed_turn(
    ...
    try:
        ...
        raise Epoch36SourceUnitAdjudicationError("completed adjudication checkpoint drifted") from exc
    required = {
        "schema_version": codex_app_server.TURN_SIDECAR_SCHEMA_VERSION,
        "state": "completed",
        "status": "completed",
    ...
    if any(sidecar.get(key) != expected for key, expected in required.items()):
        raise Epoch36SourceUnitAdjudicationError("managed adjudication sidecar contract drifted")
    usage = _valid_usage(sidecar.get("usage"))
    if sidecar.get("thread_total_usage") != usage:
        raise Epoch36SourceUnitAdjudicationError("adjudication thread usage drifted")
    if usage["total_tokens"] > NEW_TOTAL_TOKEN_CAP:
        raise Epoch36SourceUnitAdjudicationError("adjudication exceeded its token ceiling")
    validation_errors = validate_source_unit_output(output, variant)
    _validate_capacity_checkpoint(root)
    epoch35_receipt = _validate_epoch35_terminal()
    prior_ids = {
        (str(row["thread_id"]), str(row["turn_id"]))
        for row in epoch35_receipt.get("turns") or []
    }
    ...
    if identity in prior_ids or len(identity[0]) == 0 or len(identity[1]) == 0:
        raise Epoch36SourceUnitAdjudicationError("adjudication reused a prior thread or turn")
...
def merge_adjudication(
    ...
    if set(adjudicated) != selected:
        raise Epoch36SourceUnitAdjudicationError("adjudication output case coverage drifted")
    cases = []
...
def _score(
    ...
    try:
        ...
        macro = epoch35._macro_scores(  # noqa: SLF001
            pool=bundle["pool"], mapping=bundle["mapping"], consensus=consensus
        )
    except (ValueError, epoch35.Epoch35QualityError) as exc:
        raise Epoch36SourceUnitAdjudicationError("adjudicated shared-reference score failed") from exc
    baseline_macro = macro["systems"][epoch35.SYSTEM_BASELINE][
        "strict_full_field_macro_f1"
    ]
    candidate_macro = macro["systems"][epoch35.SYSTEM_CANDIDATE][
        "strict_full_field_macro_f1"
    ]
    witness_count = sum(
    ...
    return {
        ...
        "full_six_case_token_projection": epoch35.FULL_SIX_CASE_TOKEN_PROJECTION,
        "full_six_case_token_ceiling": epoch35.FULL_SIX_CASE_TOKEN_CEILING,
...
def _receipt(
    ...
    *,
    ...
    if state not in {"passed", "rejected", "waiting"}:
        raise Epoch36SourceUnitAdjudicationError("epoch-36 receipt state is invalid")
...
def _recover_terminal_mirror(root: Path) -> dict[str, Any] | None:
    ...
    value = _load_json(existing, "single epoch-36 terminal mirror")
...
def _validate_receipt_payload(root: Path, receipt: Mapping[str, Any]) -> None:
    if (
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-36 terminal receipt drifted")
    ...
    if (
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-36 call accounting drifted")
    ...
    if (
        ...
        raise Epoch36SourceUnitAdjudicationError("epoch-36 aggregate accounting drifted")
    if receipt.get("score") is not None or receipt.get("state") == "passed":
        ...
        if (
            ...
            or _load_json(
                root / "projected-adjudication.private.json",
                "epoch-36 projected adjudication",
            )
            != score["projected_output"]
            or _load_json(root / "consensus.private.json", "epoch-36 consensus")
            != score["consensus"]
        ):
            raise Epoch36SourceUnitAdjudicationError("epoch-36 quality receipt drifted")
    else:
        if receipt.get("terminal_reason") == "epoch36_source_unit_output_rejected":
            ...
            if (
                ...
                raise Epoch36SourceUnitAdjudicationError(
                    "epoch-36 rejected output receipt drifted"
                )
            return
        ...
        if (
            ...
            or (
                ...
                != "epoch36_adjudication_structured_output_rejected"
            )
            ...
            raise Epoch36SourceUnitAdjudicationError("epoch-36 waiting receipt drifted")
...
def verify_receipt(root: Path = DEFAULT_ROOT, *, acquire_lock: bool = True) -> dict[str, Any]:
    ...
    def verify() -> dict[str, Any]:
        ...
        receipt = _load_json(output_root / "plan-step-receipt.json", "epoch-36 receipt")
        terminal = _load_json(output_root / "terminal.json", "epoch-36 terminal")
        if receipt != terminal:
            raise Epoch36SourceUnitAdjudicationError("epoch-36 terminal mirrors differ")
...
def _finalize_completed(root: Path) -> dict[str, Any]:
    ...
    if validation_errors:
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch36_source_unit_output_rejected",
    ...
    return _write_terminal(
        ...
        _receipt(
            ...
            terminal_reason=(
                "epoch36_source_unit_adjudicated_quality_gate_passed"
                if score["passed"]
                else "epoch36_source_unit_adjudicated_quality_gate_rejected"
            ),
            accounting=accounting,
            score=score,
        ),
...
async def _run_unlocked(
    ...
    if artifacts_exist and not launch_exists:
        raise Epoch36SourceUnitAdjudicationError("semantic artifacts exist without epoch-36 launch")
    if launch_exists:
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch36_partial_or_interrupted_adjudication_preserved_without_replay",
                accounting=accounting,
    ...
    try:
        ...
        if not result.status_ok or not isinstance(result.output, Mapping):
            raise Epoch36SourceUnitAdjudicationWaiting("adjudication turn did not complete")
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="rejected",
                terminal_reason="epoch36_adjudication_structured_output_rejected",
                accounting=accounting,
        ...
        Epoch36SourceUnitAdjudicationWaiting,
        OSError,
        ...
        return _write_terminal(
            ...
            _receipt(
                root,
                state="waiting",
                terminal_reason="epoch36_adjudication_operational_waiting_no_replay",
                accounting=accounting,
...
async def run(
    ...
    *,
    ...
    if output_root == DEFAULT_ROOT and client_factory is not None:
        raise Epoch36SourceUnitAdjudicationError("the live epoch-36 root forbids injected clients")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except Epoch36SourceUnitAdjudicationWaiting as exc:
        print(json.dumps({"state": "waiting", "error": str(exc)}, sort_keys=True))
        return 75
    except Epoch36SourceUnitAdjudicationError as exc:
