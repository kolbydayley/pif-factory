...
"""One-call repair of the observable v91 speaker abstention."""
...
V92_INPUT_VERSION = "pif_app_server_judge_v5_4_v92_speaker_repair_input_v1"
V92_SPEC_VERSION = "pif_app_server_judge_v5_4_v92_speaker_repair_spec_v1"
V92_OUTPUT_VERSION = "pif_app_server_judge_v5_4_v92_speaker_repair_output_v1"
V92_SCORE_VERSION = "pif_app_server_judge_v5_4_v92_speaker_repair_score_v1"
V92_AUDIT_VERSION = "pif_app_server_judge_v5_4_v92_reconciliation_audit_v1"
V92_FAILURE_VERSION = "pif_app_server_judge_v5_4_v92_failure_v1"
V92_TERMINAL_VERSION = "pif_app_server_judge_v5_4_v92_terminal_v1"
V92_CAPACITY_AUDIT_VERSION = "pif_app_server_capacity_policy_audit_v20"
V92_CAPACITY_POLICY_VERSION = "pif_app_server_capacity_policy_v20"
V92_PHASE_ID = "judge_v5_4_v92_speaker_repair"
TURN_NAME = "speaker_abstention_repair"
MODEL = "gpt-5.5"
EFFORT = "high"
TIMEOUT_SECONDS = 600.0
DEFAULT_OUTPUT_ROOT = (
    DEFAULT_V91_ROOT.parent / "judge-calibration-v5_4-v92-speaker-repair"
).resolve()


class JudgeV5CalibrationV92Error(RuntimeError):
    """The v92 bounded repair cannot preserve its frozen contract."""


def _validate_v91(root: Path) -> dict[str, Any]:
    paths = {
        "terminal": root / "terminal.json",
        "spec": root / "fresh-luna-spec.json",
        "score": root / "fresh-luna-score.json",
        "input": root / "fresh-luna-input.private.json",
        "truth": root / "fresh-luna-truth.private.json",
        "output": root / "fresh-luna-output.private.json",
        "canary_output": root / "permutation-canary-output.private.json",
    }
    values = {name: _load_json(path, f"v91 {name}") for name, path in paths.items()}
    terminal = values["terminal"]
    spec = values["spec"]
    score = values["score"]
    trigger_ids = score.get("observable_repair_task_ids") or []
    trigger = trigger_ids[0] if len(trigger_ids) == 1 else None
    truth_row = next(
        (row for row in values["truth"].get("tasks") or [] if row["task_id"] == trigger), None
    )
    if (
        terminal.get("state") != "inactive"
        or terminal.get("development_terminal_reason") != "v91_observable_repair_required"
        ...
        or not truth_row
        or truth_row.get("field") != "speaker"
        or truth_row.get("expected_status") != "incorrect"
        ...
        raise JudgeV5CalibrationV92Error("v91 speaker-repair contract drifted")
    return {
        "paths": paths,
        "values": values,
...
def build_repair_input(v91: Mapping[str, Any]) -> dict[str, Any]:
    trigger = v91["trigger_task_id"]
    tasks = [
        deepcopy(row)
        for row in v91["values"]["input"].get("tasks") or []
        if row.get("task_id") == trigger
    ]
    if len(tasks) != 1:
        raise JudgeV5CalibrationV92Error("v92 trigger task coverage drifted")
    return {
        "schema_version": V92_INPUT_VERSION,
        "task_count": 1,
        "tasks": tasks,
        "repair_scope": "one_observable_speaker_abstention_only",
...
def repair_instructions() -> str:
    return (
        base_instructions_v88()
        + " Judge whether the existing field value is correct, not whether you can supply a replacement. If "
        "the aligned source proposition rules out the existing speaker, return incorrect even when the true "
        "speaker is unnamed. Abstain only when the source cannot determine whether the existing value is right "
        "or wrong. Never borrow the speaker of an adjacent proposition."
    )
...
def reconcile_output(
    ...
    if len(repair_rows) != 1 or repair_rows[0].get("task_id") != trigger_task_id:
        raise JudgeV5CalibrationV92Error("v92 repair output coverage drifted")
    decisions = []
    replaced = 0
    ...
    if replaced != 1 or len(decisions) != 15:
        raise JudgeV5CalibrationV92Error("v92 reconciliation coverage drifted")
    return {"schema_version": V92_OUTPUT_VERSION, "decisions": decisions}
...
def residual_mismatches(
    ...
    if set(expected) != set(observed):
        raise JudgeV5CalibrationV92Error("v92 residual coverage drifted")
    return [
        {
            "task_id": task_id,
            "field": row["field"],
...
def _build_capacity_policy(root: Path, predecessor: Mapping[str, Any]) -> dict[str, Path]:
    ...
    audit = {
        "schema_version": V92_CAPACITY_AUDIT_VERSION,
        "phase_id": V92_PHASE_ID,
        "created_at": now_iso(),
    ...
    _write_stable_created(audit_path, audit, "v92 capacity audit")
    policy = {
        "schema_version": V92_CAPACITY_POLICY_VERSION,
        "phase_id": V92_PHASE_ID,
        "created_at": now_iso(),
    ...
    _write_stable_created(policy_path, policy, "v92 capacity policy")
...
def freeze_v92(
    *,
    ...
    predecessor = _validate_v91(v91_root.resolve())
    value = build_repair_input(predecessor)
    input_path = root / "speaker-repair-input.private.json"
    _write_immutable(input_path, value)
    prompt = build_prompt_v88(value)
    ...
    spec = {
        "schema_version": V92_SPEC_VERSION,
        ...
        "strategy": "single_stronger_owner_repair_existing_speaker_correctness_without_replacement_requirement",
        "turn_plan": [TURN_NAME],
        "task_count": 1,
        ...
        "retry_count_per_turn": 0,
        "reconciliation_rule": "replace_exactly_the_single_trigger_task_and_preserve_all_other_v91_decisions",
        "promotion_rule": "full_v91_gate_must_pass_after_single_replacement",
        ...
        "runtime_files": [
            ...
            _record(runtime_dir / "app_server_judge_v5_calibration_v91_fresh_luna_diagnostic.py"),
    ...
    spec_path = root / "speaker-repair-spec.json"
    if spec_path.exists():
        prior = _load_json(spec_path, "v92 spec")
        candidate = deepcopy(spec)
        candidate["created_at"] = prior.get("created_at")
        if candidate != prior:
            raise JudgeV5CalibrationV92Error("immutable v92 spec drifted")
        spec = prior
    else:
...
def _write_failure(root: Path, turn_name: Optional[str], error_class: str) -> dict[str, Any]:
    ...
    for attempt in attempts:
        ...
        if not isinstance(record, Mapping):
            ...
            continue
        try:
            measured = _validate_usage(_load_json(Path(record["path"]), "v92 sidecar"))
        except Exception:
            unknown += 1
            continue
    ...
    complete = unknown == 0
    failure = {
        "schema_version": V92_FAILURE_VERSION,
        "terminal_at": now_iso(),
    ...
    terminal = {
        "schema_version": V92_TERMINAL_VERSION,
        "state": "failed",
...
async def run_v92(
    *,
    ...
    if terminal_path.exists():
        return _load_json(terminal_path, "v92 terminal")
    frozen = freeze_v92(output_dir=root, timeout_seconds=timeout_seconds)
    ...
    try:
        ...
        if validate_output(projected, frozen["value"]):
            raise JudgeV5CalibrationV92Error("projected v92 output is invalid")
        repair_path = root / "speaker-repair-output.private.json"
        ...
        score = score_v91(reconciled, predecessor["canary_output"], predecessor["truth"])
        score["schema_version"] = V92_SCORE_VERSION
        ...
        reference_audit = (
            ...
            and metrics["canary_abstention_count"] <= 1
        )
        score["residual_reference_audit_authorized"] = reference_audit
        score_path = root / "speaker-repair-score.json"
        _write_immutable(score_path, score)
        audit = {
            "schema_version": V92_AUDIT_VERSION,
            "created_at": now_iso(),
            "replacement_count": 1,
        ...
        terminal = {
            ...
            "terminal_at": now_iso(),
            "terminal_reason": (
                "v92_repair_passed_full_development_calibration_authorized"
                if passed
            ...
            "development_terminal_reason": (
                "v92_repair_passed_full_development_calibration_authorized"
                if passed
                else "v92_repair_completed_residual_reference_audit_required"
            ),
            "overall_evaluation_complete": False,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run v92 speaker repair")
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    terminal = asyncio.run(
        run_v92(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
    )
    print(
        json.dumps(
            {
