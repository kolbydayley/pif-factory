...
"""Realize the frozen blind inventory through one compact global semantic join."""
...
from . import app_server_judge_v5_selection_v274_compact_applicability_ledger as v274
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v275_inventory_compact_global_join_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v275_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v275_terminal_v1"
PHASE_ID = "development_selection_v5_4_v275_inventory_compact_global_join"
MODEL = "gpt-5.6-sol"
EFFORT = "high"
...
V274_ROOT = v274.DEFAULT_OUTPUT_ROOT
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v275-inventory-compact-global-join"
).resolve()
...
EXPECTED_LINEAGE_HASHES = {
    ...
    "v274_terminal": "659920f58dab89d49db898754e0c7c971529027203c3aac526d42c442fa5dced",
    "v274_runtime_lock": "270bfeb58c5c8aa05b736d1856a7e0f56c0285644f7de6215299600648d2f579",
    "v274_sidecar": "22729fa8849de2147a34a9d97af3c7fb21153f87e908794b6a00b13c74c79558",
}

class V275InventoryCompactGlobalJoinError(RuntimeError):
    """The v275 architecture cannot proceed or be adopted safely."""


class V275OutputContractError(V275InventoryCompactGlobalJoinError):
    """A completed output violated the frozen ownership/projection contract."""


class V275ArchitectureStop(V275InventoryCompactGlobalJoinError):
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V275InventoryCompactGlobalJoinError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V275InventoryCompactGlobalJoinError(f"frozen {path.name} drifted")
        return
...
def _single_turn(root: Path, prefix: str) -> Path:
    ...
    if len(matches) != 1:
        raise V275InventoryCompactGlobalJoinError(f"{prefix} turn membership drifted")
    return matches[0]
...
def _lineage_paths() -> dict[str, Path]:
    ...
    v274_turn = _single_turn(V274_ROOT, "v274-compact-applicability-")
    return {
        ...
        "v274_terminal": V274_ROOT / "terminal.json",
        "v274_runtime_lock": V274_ROOT / "runtime-lock.json",
        "v274_sidecar": v274_turn / "sidecar.json",
    }
...
def _validate_lineage() -> dict[str, Any]:
    ...
    for name, expected in EXPECTED_LINEAGE_HASHES.items():
        if records[name]["sha256"] != expected:
            raise V275InventoryCompactGlobalJoinError(f"frozen lineage {name} drifted")
    ...
    v274.verify_runtime_lock(paths["v274_runtime_lock"])
    ...
    t274 = _load_json(paths["v274_terminal"], "v274 terminal")
    s274 = _load_json(paths["v274_sidecar"], "v274 sidecar")
    ...
    if (
        ...
        or t274.get("terminal_reason")
        != "v274_compact_applicability_structural_quality_or_cost_gate_not_passed"
        or t274.get("error_class") != "V274OutputContractError"
        or t274.get("error_message_sha256")
        != "e295f83d99f674fd1be3bee9428aab65c2ebbfcea86bb710187dab4d8c9b9055"
        or (s274.get("usage") or {}).get("total_tokens") != 42_872
        or s274.get("usage_status") != "measured"
        or s274.get("usage_complete") is not True
        or any(
            terminal.get("production_mutated") is not False
            for terminal in (t234, t265, t267, t268, t274)
        )
    ):
        raise V275InventoryCompactGlobalJoinError("v275 predecessor evidence drifted")
    packet = v234._source_packet(v234._validate_lineage())
    if [row["segment_id"] for row in inventory["segments"]] != packet["segment_ids"]:
        raise V275InventoryCompactGlobalJoinError("v234 inventory segment order drifted")
    return {
        "paths": paths,
        "records": records,
        "inventory": inventory,
...
def _explicit_realization_schema(
    ...
    event_path["items"] = v274._explicit_event_schema(source_event)
    if v274.llm_judge.validate_app_server_output_schema_subset(schema):
        raise V275InventoryCompactGlobalJoinError("v275 schema is not app-server compatible")
...
def prepare_turn(lineage: Mapping[str, Any]) -> dict[str, Any]:
    ...
    base = (
        ...
        + v274.APPLICABILITY_INSTRUCTIONS
        + "\n"
    )
    ...
    if (
        ...
        raise V275InventoryCompactGlobalJoinError("v275 request size cap failed")
    return {
        **direct,
        "turn_name": "v275_inventory_compact_global_join_"
    ...
    try:
        ...
        for segment in direct.get("segments") or []:
            ...
            for event in segment.get("events") or []:
                projected = v274._project_event(event)
        ...
        v274.V274OutputContractError,
        ValidationError,
        ValueError,
        TypeError,
    ) as exc:
        raise V275OutputContractError(str(exc)) from exc
    ownership = {
        "schema_version": SCHEMA_VERSION,
        "architecture_id": "adopted_blind_inventory_compact_applicability_global_join",
        "segments": [
            {
        ...
        "all_compact_applicability_states_selected_by_llm": True,
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v275 runtime lock")
    root = path.parent.resolve()
    spec = _load_json(root / "attempt-spec.json", "v275 spec")
    ...
    if (
        ...
        raise V275InventoryCompactGlobalJoinError("v275 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V275InventoryCompactGlobalJoinError("v275 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V275InventoryCompactGlobalJoinError("v275 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    spec = _load_json(spec_path, "v275 spec")
    paths = _turn_paths(root, spec["turn_name"])
    lineage = _validate_lineage()
    ...
    return {
        ...
        "turn": {
            **turn,
            "private_input": _load_json(paths["input"], "v275 input"),
            "prompt": paths["prompt"].read_text(encoding="utf-8"),
            "base": paths["base"].read_text(encoding="utf-8"),
            "schema": _load_json(paths["schema"], "v275 schema"),
            "paths": paths,
        },
    }


def freeze_v275(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v275 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V275InventoryCompactGlobalJoinError("unfinished v275 root is not replayable")
    ...
    _write_stable_time(
        authorization_path,
        {
            ...
            "scope": "adopt one immutable blind inventory and run one compact-applicability global join",
    ...
    _write_stable_time(
        ranking_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "selected_architecture_id": "adopted_blind_inventory_compact_applicability_global_join",
            "architectures": [
                {"rank": 1, "id": "adopted_blind_inventory_compact_applicability_global_join"},
            ...
            "selection_basis": {
                ...
                "v274_tokens": 42_872,
                "v274_failure_class": "compact_stance_state_value_conflict",
                "decision": "combine frozen inventory-owned boundaries with a compact full-schema join so one model turn focuses on realization rather than rediscovery",
            },
            "on_failure": "reject v275 and advance to the next distinct architecture without field repair",
        },
        "created_at",
    )
    ...
    _write_stable_time(
        design_path,
        {
            ...
            "architecture_id": "adopted_blind_inventory_compact_applicability_global_join",
            "hypothesis": (
                "the frozen blind inventory supplies recall and atomic proposition ownership while one Sol-high "
                "compact-applicability global join re-reads the source and realizes every full field; this should "
                "avoid v274 rediscovery pressure and stay below the combined production token ceiling"
            ),
    ...
    _write_stable_time(
        spec_path,
        {
            ...
            "created_at": now_iso(),
            "phase_id": PHASE_ID,
            "state": "frozen_before_one_turn_inventory_compact_global_join_canary",
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V275InventoryCompactGlobalJoinError("v275 sidecar usage incomplete") from exc
    if (
        ...
        raise V275InventoryCompactGlobalJoinError("v275 sidecar accounting or auth failed")
    return usage
...
def _failure_terminal(root: Path, frozen: Mapping[str, Any], exc: BaseException) -> dict[str, Any]:
    ...
    if paths["sidecar"].is_file():
        ...
        try:
            sidecar = _load_json(paths["sidecar"], "v275 sidecar")
    ...
    semantic = isinstance(exc, (V275OutputContractError, V275ArchitectureStop)) or measured_over_cap
    ...
    terminal = {
        ...
        "terminal_reason": (
            "v275_inventory_compact_global_join_structural_quality_or_cost_gate_not_passed"
            if semantic
        ...
        "exact_next_action": (
            "reject v275 and advance to the next distinct architecture without field repair"
            if semantic
            else "audit immutable v275 infrastructure attempt; no retry"
        ),
    }
...
async def run_v275(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v275 terminal")
    frozen = freeze_v275(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V275InventoryCompactGlobalJoinError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        if result.status_ok is not True or not isinstance(result.output, Mapping):
            raise V275InventoryCompactGlobalJoinError("v275 turn did not complete")
        new_usage = _usage(_load_json(paths["sidecar"], "v275 sidecar"))
        if new_usage["total_tokens"] > MAX_NEW_TOKENS:
            raise V275ArchitectureStop("v275 measured turn exceeded frozen token bound")
        ...
        if not gate["passed"]:
            raise V275ArchitectureStop("v275 structural quality or cost gate failed")
        terminal = {
            "schema_version": TERMINAL_VERSION,
            "terminal_at": now_iso(),
            "state": "v275_architecture_structural_gate_passed",
            "terminal_reason": "v275_inventory_compact_global_join_structural_cost_gate_passed",
            "new_semantic_attempt_count": 1,
            "semantic_retry_count": 0,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run v275 adopted-inventory compact global join"
    )
    ...
    if args.action == "freeze":
        frozen = freeze_v275(output_dir=Path(args.output_dir))
        result = {
        ...
        terminal = asyncio.run(
            run_v275(output_dir=Path(args.output_dir), timeout_seconds=args.timeout_seconds)
        )
        result = {
