from __future__ import annotations

"""Build runtime-lock v12 for the frozen judge-v5.4 full calibration."""

import argparse
import hashlib
import json
from pathlib import Path
from typing import Any, Optional, Sequence

from .app_server_judge_v5_calibration_runner import verify_judge_freeze_receipt
from .app_server_runtime_lock import (
    JUDGE_DIAGNOSTIC_V6_RUNTIME_LOCK_VERSION,
    JUDGE_FULL_CALIBRATION_RUNTIME_LOCK_VERSION,
    verify_runtime_lock,
)
from .app_server_runtime_lock_v11 import CURRENT_RUNTIME_FILES as V11_RUNTIME_FILES
from .util import now_iso, write_text_atomic


CURRENT_RUNTIME_FILES = tuple(
    "automation/resume-app-server-evaluation-v13.sh"
    if path == "automation/resume-app-server-evaluation-v12.sh"
    else "research_factory/app_server_runtime_lock_v12.py"
    if path == "research_factory/app_server_runtime_lock_v11.py"
    else path
    for path in V11_RUNTIME_FILES
) + (
    "research_factory/app_server_judge_v5_calibration.py",
    "research_factory/app_server_judge_v5_calibration_runner.py",
    "work/app-server-development-v2/unattended-pipeline-v5/judge-v5_4-freeze-receipt-v1.json",
)


SUPERSEDED_ARTIFACTS = (
    ("runtime_lock_v11", "work/app-server-development-v2/unattended-runtime-lock-v11.json"),
    (
        "launch_receipt_v11",
        "work/app-server-development-v2/unattended-control-v11/launch-receipt-v11.json",
    ),
    (
        "terminal_receipt_v11",
        "work/app-server-development-v2/unattended-control-v11/terminal-receipt-v11.json",
    ),
    (
        "diagnostic_v6_spec",
        "work/app-server-development-v2/unattended-pipeline-v5/judge-diagnostic-v6/diagnostic-spec.json",
    ),
    (
        "diagnostic_v6_terminal",
        "work/app-server-development-v2/unattended-pipeline-v5/judge-diagnostic-v6/terminal.json",
    ),
    (
        "diagnostic_v6_score",
        "work/app-server-development-v2/unattended-pipeline-v5/judge-diagnostic-v6/diagnostic-score.json",
    ),
    (
        "judge_v5_4_freeze_receipt",
        "work/app-server-development-v2/unattended-pipeline-v5/judge-v5_4-freeze-receipt-v1.json",
    ),
    (
        "diagnostic_fixture_patch_v3",
        "research_factory/evaluation/judge_v5_diagnostic_v3.json",
    ),
)


def _sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def _record(root: Path, relative: str) -> dict[str, Any]:
    path = (root / relative).resolve()
    try:
        path.relative_to(root)
    except ValueError as exc:
        raise ValueError("runtime lock path escapes the repository") from exc
    if not path.is_file():
        raise ValueError("runtime lock input is missing: %s" % relative)
    return {
        "path": relative,
        "sha256": _sha256_file(path),
        "size_bytes": path.stat().st_size,
    }


def build_runtime_lock_v12(
    *, repo_root: Path, manifest_path: Path, judge_freeze_receipt_path: Path
) -> dict[str, Any]:
    root = repo_root.expanduser().resolve()
    manifest = manifest_path.expanduser().resolve()
    freeze_path = judge_freeze_receipt_path.expanduser().resolve()
    freeze = verify_judge_freeze_receipt(freeze_path)
    expected = (
        root
        / "work/app-server-development-v2/unattended-pipeline-v5/"
        "judge-v5_4-freeze-receipt-v1.json"
    ).resolve()
    if freeze_path != expected or freeze.get("full_calibration_authorized") is not True:
        raise ValueError("runtime lock v12 requires canonical passing judge freeze")
    if manifest.exists():
        report = verify_runtime_lock(repo_root=root, manifest_path=manifest)
        value = json.loads(manifest.read_text(encoding="utf-8"))
        if report.get("schema_version") != JUDGE_FULL_CALIBRATION_RUNTIME_LOCK_VERSION:
            raise ValueError("existing runtime lock is not v12")
        return value
    payload = {
        "schema_version": JUDGE_FULL_CALIBRATION_RUNTIME_LOCK_VERSION,
        "created_at": now_iso(),
        "purpose": (
            "Supersede the passing immutable diagnostic-v6 and run one full fresh "
            "66-case sharded calibration with the frozen judge-v5.4 protocol."
        ),
        "supersedes": {
            "schema_version": "pif_app_server_unattended_runtime_lock_supersession_v11",
            "prior_runtime_lock_version": JUDGE_DIAGNOSTIC_V6_RUNTIME_LOCK_VERSION,
            "diagnostic_v6_status": "passed",
            "diagnostic_v6_terminal_classification": (
                "judge_diagnostic_passed_full_calibration_authorized"
            ),
            "diagnostic_v6_protocol_version": "pif_app_server_judge_protocol_v5_4",
            "diagnostic_v6_completed_turn_count": 3,
            "diagnostic_v6_usage_status": "complete",
            "diagnostic_v6_total_tokens": 122896,
            "diagnostic_v6_full_calibration_authorized": True,
            "diagnostic_v6_replay_allowed": False,
            "diagnostic_v6_output_reuse_allowed": False,
            "artifacts": [
                {"label": label, **_record(root, relative)}
                for label, relative in SUPERSEDED_ARTIFACTS
            ],
        },
        "policy": {
            "managed_chatgpt_auth_only": True,
            "api_key_billing_allowed": False,
            "raw_session_token_access_allowed": False,
            "codex_exec_semantic_calls_allowed": False,
            "official_persistent_codex_app_server_only": True,
            "production_mutation_allowed": False,
            "semantic_turn_maximum_primary_used_percent": 20,
            "per_turn_capacity_checkpoint_required": True,
            "retry_count_per_semantic_turn": 0,
            "all_semantic_turns_run_fresh": True,
            "diagnostic_output_reuse_allowed": False,
            "judge_protocol_version": "pif_app_server_judge_protocol_v5_4",
            "judge_prompt_changes_allowed": False,
            "gate_threshold_changes_allowed": False,
            "pointwise_shard_count": 6,
            "base_alignment_shard_count": 6,
            "canary_case_count": 12,
            "adjudication_call_cap": 1,
            "system_selection_allowed_before_calibration_pass": False,
            "verify_before_semantic_work": True,
        },
        "files": [_record(root, relative) for relative in CURRENT_RUNTIME_FILES],
    }
    manifest.parent.mkdir(parents=True, exist_ok=True)
    write_text_atomic(
        manifest,
        json.dumps(payload, ensure_ascii=True, indent=2, sort_keys=True) + "\n",
    )
    verify_runtime_lock(repo_root=root, manifest_path=manifest)
    return payload


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Build app-server runtime lock v12")
    parser.add_argument("--repo-root", required=True)
    parser.add_argument("--manifest", required=True)
    parser.add_argument("--judge-freeze-receipt", required=True)
    args = parser.parse_args(argv)
    payload = build_runtime_lock_v12(
        repo_root=Path(args.repo_root),
        manifest_path=Path(args.manifest),
        judge_freeze_receipt_path=Path(args.judge_freeze_receipt),
    )
    print(json.dumps({
        "ok": True,
        "schema_version": payload["schema_version"],
        "file_count": len(payload["files"]),
        "superseded_artifact_count": len(payload["supersedes"]["artifacts"]),
    }, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
