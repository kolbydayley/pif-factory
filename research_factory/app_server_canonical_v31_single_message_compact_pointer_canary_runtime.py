...
"""Epoch-19 one-turn canary for single-message compact canonical output."""
...
from . import app_server_canonical_v31_compact_unit_pointer_canary_runtime as epoch18
from . import app_server_canonical_v31_metric_compiler_canary_runtime as epoch15
from . import app_server_canonical_v31_single_message_compact_pointer_episode_batch as adapter
...
PREDECESSOR_ROOT = epoch18.DEFAULT_ROOT
DIRECTIVE_PATH = (
    PROJECT_ROOT
    / "automation/pif-evaluation-epoch19-single-message-compact-pointer-v19.json"
)
PLAN_PATH = PROJECT_ROOT / "automation/pif-evaluation-semantic-plan-v19.json"
DIRECTIVE_SHA256 = "547259078c291846c589af50ce8e5c3804987ed5c3e8429d99c0b90a3c53e66d"
THREAD_ID = "019f4cf1-c46e-7db3-acd2-bf03c4459a10"
PLAN_EPOCH = 19
STEP_ID = "canonical_v31_epoch19_single_message_compact_pointer_v19"
TURN_NAME = "epoch19_single_message_compact_547259078c291846c589af50"
AUTHORIZATION_STATEMENT = (
    "You have my full permission to continue. No need to seek out my approval anymore."
)
AUTHORIZATION_ID_DEFAULT = "kolby-epoch19-single-message-compact-20260719"
...
EXPECTED_BASE_BYTES = 20_758
EXPECTED_SCHEMA_BYTES = 12_787
EXPECTED_REQUEST_BYTES = 845_719
...
PREDECESSOR_RECEIPT_SHA256 = (
    "57557a1d9de10cc4581b6775eb4cc9c8b0872326ac95b8b8e9c6d26c2f6a674f"
)
PREDECESSOR_RUNTIME_LOCK_SHA256 = (
    "5c8dc3260f7fcf7b41c2ce646638889875f37d09205c4720660c87241ca1c315"
)
PREDECESSOR_OUTPUT_SHA256 = (
    "df6b7d4af30ba009bfdd01243b5a7d10b9f79b16b0bc3f24e790a789648543da"
)
PREDECESSOR_SIDECAR_SHA256 = (
    "d643f8e6b0bbd9c4a627f2995843c351ecca85b3c55714493daad59b368bc67d"
)
PREDECESSOR_LABELS_SHA256 = (
    "b12e8d05f3ee81a87ee8e3d8a00a3f3749f893a489db703142b75382462f75a0"
)
...
SingleMessageCompactCanaryError = one_turn.OneTurnCanaryError
SingleMessageCompactCanaryWaiting = one_turn.OneTurnCanaryWaiting
...
def _build_request() -> dict[str, Any]:
    ...
    if len(requests) != 1:
        raise SingleMessageCompactCanaryError("epoch-19 request count drifted")
    request = requests[0]
    if (
        ...
        or not request["base_instructions"].startswith(
            adapter.SINGLE_MESSAGE_INSTRUCTIONS
        )
    ):
        raise SingleMessageCompactCanaryError("epoch-19 frozen request shape drifted")
...
def _validate_predecessor(full_verify: bool) -> None:
    ...
    predecessor = (
        epoch18.verify_receipt(PREDECESSOR_ROOT)
        if full_verify
        else _load(predecessor_receipt_path, "epoch-18 receipt")
    )
    predecessor_sidecar = _load(
        PREDECESSOR_ROOT / "turn/sidecar.json", "epoch-18 sidecar"
    )
    if (
        ...
        or predecessor.get("terminal_reason") != "epoch18_one_turn_canary_cost_rejected"
        ...
        or predecessor.get("new_measured_usage", {}).get("total_tokens") != 84_866
        or predecessor.get("failed_checks")
        != ["measured_total_token_acceptance_ceiling"]
        or predecessor.get("diagnostic") is not None
        or predecessor.get("production_amortized_ratio") != 0.31260654044846187
        or predecessor.get("production_mutated") is not False
        or predecessor.get("holdout_authorized") is not False
        or predecessor_sidecar.get("usage", {}).get("total_tokens") != 56_525
        or predecessor_sidecar.get("thread_total_usage", {}).get("total_tokens")
        != 84_866
    ):
        raise SingleMessageCompactCanaryError("epoch-19 predecessor evidence drifted")
    request = _build_request()
    epoch17._validate_representability(request)  # noqa: SLF001
    compact_request = adapter._compact_request(request)  # noqa: SLF001
    if (
        compact_request["prompt"] != request["prompt"]
        or compact_request["output_schema"] != request["output_schema"]
    ):
        raise SingleMessageCompactCanaryError("epoch-19 semantic schema drifted")
...
def _runtime_module_paths() -> Sequence[Path]:
    return (
        ...
        Path(adapter.compact.__file__),
        Path(adapter.compact.local.__file__),
        Path(adapter.compact.local.bounded_span.__file__),
        Path(adapter.compact.local.pointer.__file__),
        Path(adapter.base.__file__),
        Path(epoch13.__file__),
        Path(epoch15.__file__),
        Path(epoch17.__file__),
        Path(epoch18.__file__),
    )
...
def _validate_directive(value: Mapping[str, Any]) -> None:
    ...
    if dict(value) != expected:
        raise SingleMessageCompactCanaryError("epoch-19 directive contract drifted")
    if (
        expected.get("schema_version")
        != "pif_evaluation_epoch19_single_message_compact_pointer_directive_v1"
        ...
        or expected["architecture_ranking"][0]["selected"] is not True
        ...
        raise SingleMessageCompactCanaryError("epoch-19 directive invariants drifted")
...
def _receipt_metadata(outcome: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if outcome["state"] == "passed" and not all(gate.values()):
        raise SingleMessageCompactCanaryError(
            "epoch-19 pass violates production cost gate"
        )
    return {
        ...
        "single_message_compact_pointer_contract": True,
    }
...
SPEC = one_turn.OneTurnCanarySpec(
    ...
    step_id=STEP_ID,
    turn_name=TURN_NAME,
    stage="single_message_compact_pointer_full_episode",
    architecture_class=(
        "one_turn_full_canonical_single_final_message_compact_pointer_extraction_v1"
    ),
    authorization_statement=AUTHORIZATION_STATEMENT,
    authorization_state="authorized_for_exactly_one_single_message_compact_turn",
    model=adapter.MODEL,
    ...
    reject_next_action="reject_single_message_compact_pointer_architecture",
)
...
def _runtime_status(status: Mapping[str, Any], root: Path) -> dict[str, Any]:
    request = _load(
        ...
        "epoch-19 prepared request",
    )
    shape = _request_shape(request)
    if shape != _expected_shape():
        raise SingleMessageCompactCanaryError("epoch-19 runtime status shape drifted")
...
__all__: Sequence[str] = (
    ...
    "DEFAULT_ROOT",
    "SingleMessageCompactCanaryError",
    "authorize_canary",
    "execute_canary",
    "prepare_canary",
    "status_canary",
