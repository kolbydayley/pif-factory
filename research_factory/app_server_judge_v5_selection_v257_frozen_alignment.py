...
"""Run the unchanged two-permutation neutral alignment judge for v255."""
...
from . import app_server_judge_v5_selection_v255_atomic_ledger as v255
from . import app_server_judge_v5_selection_v256_frozen_support as v256
...
SCHEMA_VERSION = "pif_app_server_judge_v5_selection_v257_frozen_alignment_v1"
RUNTIME_LOCK_VERSION = "pif_app_server_judge_v5_selection_v257_runtime_lock_v1"
TERMINAL_VERSION = "pif_app_server_judge_v5_selection_v257_terminal_v1"
WINNER_VERSION = "pif_app_server_judge_v5_selection_v257_winner_v1"
PHASE_ID = "judge_v5_4_selection_v257_frozen_alignment"
TURN_NAMES = ("v257_frozen_alignment_base", "v257_frozen_alignment_balanced_canary")
PERMUTATIONS = ("base", "balanced_canary")
MODEL = v251.MODEL
EFFORT = v251.EFFORT
MAX_TOTAL_TOKENS_PER_TURN = v251.MAX_TOTAL_TOKENS_PER_TURN
MAX_PROMPT_BYTES = v251.MAX_PROMPT_BYTES
MAX_SCHEMA_BYTES = v251.MAX_SCHEMA_BYTES
TIMEOUT_SECONDS = v251.TIMEOUT_SECONDS
MIN_REMAINING_RESERVE_PERCENT = 20
QUOTA_POINTS_PER_MILLION_TOKENS = 17
EXPECTED_ALIGNMENT_CASE_COUNT = 1
EXPECTED_SUPPORTED_REFERENCE_WITNESSES = 27
EXPECTED_SUPPORTED_CANDIDATE_WITNESSES = 31
EXPECTED_ALIGNMENT_WITNESSES = 58
EXPECTED_TOTAL_CANDIDATE_WITNESSES = 32
NONINFERIORITY_MARGIN = 0.03
TOKEN_RATIO_TARGET = 0.28
OBSERVED_PRODUCTION_RATIO = 0.198406
USAGE_FIELDS = v255.USAGE_FIELDS
PROJECT_ROOT = v255.PROJECT_ROOT
PIPELINE_ROOT = v255.PIPELINE_ROOT
V255_ROOT = v255.DEFAULT_OUTPUT_ROOT
V256_ROOT = v256.DEFAULT_OUTPUT_ROOT
DEFAULT_OUTPUT_ROOT = (
    PIPELINE_ROOT / "development-selection-v5_4-v257-frozen-alignment-v255"
).resolve()
PINNED_CODEX_0_144_1 = v255.PINNED_CODEX_0_144_1


class V257FrozenAlignmentError(RuntimeError):
    """The v257 frozen alignment gate cannot proceed or be adopted safely."""
...
def _load_json(path: Path, label: str) -> Any:
    try:
        ...
        raise V257FrozenAlignmentError(f"cannot read {label}") from exc
...
def _write_immutable(path: Path, value: Any) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise V257FrozenAlignmentError(f"frozen {path.name} drifted")
        return
...
def _lineage_paths() -> dict[str, Path]:
    v255_turn = next(V255_ROOT.glob("turns/v255-atomic-ledger-*"))
    return {
        "v255_terminal": V255_ROOT / "terminal.json",
        "v255_gate": V255_ROOT / "architecture-structural-gate.json",
        "v255_runtime_lock": V255_ROOT / "runtime-lock.json",
        "v255_sidecar": v255_turn / "sidecar.json",
        "v255_normalized": V255_ROOT / "normalized-output.private.json",
        "v256_terminal": V256_ROOT / "terminal.json",
        "v256_runtime_lock": V256_ROOT / "runtime-lock.json",
        "v256_pool": V256_ROOT / "support-pool.private.json",
        "v256_origin": V256_ROOT / "origin-map.private.json",
        "v256_receipts": V256_ROOT / "support-receipts.private.json",
        "v256_score": V256_ROOT / "support-score.private.json",
        "v256_audit": V256_ROOT / "support-audit.json",
        "v224_runtime_lock": v251.v246.v224.DEFAULT_OUTPUT_ROOT / "runtime-lock.json",
        "v224_instructions": v251.v246.v224.DEFAULT_OUTPUT_ROOT
        / "alignment-instructions.private.md",
        "v224_design": v251.v246.v224.DEFAULT_OUTPUT_ROOT / "alignment-design.json",
    }
...
def _validate_lineage() -> dict[str, Any]:
    ...
    v255.verify_runtime_lock(paths["v255_runtime_lock"])
    v256.verify_runtime_lock(paths["v256_runtime_lock"])
    v251.v246.v224.verify_runtime_lock(paths["v224_runtime_lock"])
    t255 = _load_json(paths["v255_terminal"], "v255 terminal")
    g255 = _load_json(paths["v255_gate"], "v255 gate")
    t256 = _load_json(paths["v256_terminal"], "v256 terminal")
    a256 = _load_json(paths["v256_audit"], "v256 audit")
    design = _load_json(paths["v224_design"], "v224 design")
    instructions = paths["v224_instructions"].read_text(encoding="utf-8")
    frozen_instructions = v251.v246.v130.alignment_instructions_v130()
    if (
        t255.get("terminal_reason") != "v255_atomic_ledger_structural_cost_gate_passed"
        or t255.get("support_alignment_authorized") is not True
        or t255.get("production_amortized_total_token_ratio")
        != OBSERVED_PRODUCTION_RATIO
        or g255.get("passed") is not True
        or t256.get("terminal_reason")
        != "v256_frozen_support_completed_alignment_authorized"
        or t256.get("alignment_audit_authorized") is not True
        or a256.get("support_status_counts") != {"supported": 59}
        or a256.get("no_signal_candidate_event_support_status") != "supported"
        ...
        or t255.get("production_mutated") is not False
        or t256.get("production_mutated") is not False
    ):
        raise V257FrozenAlignmentError("v257 frozen alignment lineage or protocol drifted")
    return {"paths": paths, "records": records}


def _opaque_id(kind: str, value: str) -> str:
    prefix = "jcase_" if kind == "case" else "wit_"
    return prefix + sha256_text(f"v257|{kind}|{value}")[:24]


def build_alignment_bundle(lineage: Mapping[str, Any]) -> dict[str, Any]:
    raw_pool = _load_json(lineage["paths"]["v256_pool"], "v256 support pool")
    origin_rows = _load_json(lineage["paths"]["v256_origin"], "v256 origin map")["rows"]
    score_rows = _load_json(lineage["paths"]["v256_score"], "v256 support score")["rows"]
    receipts = _load_json(lineage["paths"]["v256_receipts"], "v256 support receipts")
    origin_by_id = {str(row["witness_id"]): row for row in origin_rows}
    status_by_id = {
        str(row["witness_id"]): str(row["support_status"]) for row in score_rows
    }
    ...
    if set(origin_by_id) != set(status_by_id) or set(origin_by_id) != set(receipt_by_id):
        raise V257FrozenAlignmentError("support receipt and provenance coverage drifted")
    ...
    if (
        counts[("reference", "supported")]
        != EXPECTED_SUPPORTED_REFERENCE_WITNESSES
        or counts[("candidate", "supported")]
        != EXPECTED_SUPPORTED_CANDIDATE_WITNESSES
        or sum(
            count for (_origin, status), count in counts.items() if status != "supported"
        )
        != 0
        ...
        raise V257FrozenAlignmentError("supported alignment partition drifted")
    pool = {
        "schema_version": app_server_llm_judge.SHARED_WITNESS_POOL_VERSION,
        "seed_sha256": sha256_text("v257-v255-supported-dense-pool"),
        "cases": [
            {
    ...
    if errors:
        raise V257FrozenAlignmentError(
    ...
    if canary_ids != list(reversed(base_ids)):
        raise V257FrozenAlignmentError("balanced witness permutation drifted")
    turns = []
    for turn_name, permutation, value in zip(TURN_NAMES, PERMUTATIONS, (base, canary)):
        prompt = v251.v246.v130.alignment_prompt_v130(value)
        ...
        if prompt_bytes > MAX_PROMPT_BYTES or schema_bytes > MAX_SCHEMA_BYTES:
            raise V257FrozenAlignmentError("alignment request size cap exceeded")
        turns.append(
            {
                "turn_name": turn_name,
    ...
    return {
        ...
        "mapping": {
            "schema_version": SCHEMA_VERSION,
            "rows": sorted(
                mapping_rows, key=lambda row: str(row["legacy_witness_id"])
            ),
...
def score_alignment(
    *, base: Mapping[str, Any], canary: Mapping[str, Any], mapping: Mapping[str, Any]
    ...
    if set(base_rows) != set(canary_rows) or len(base_rows) != EXPECTED_ALIGNMENT_CASE_COUNT:
        raise V257FrozenAlignmentError("alignment normalized case coverage drifted")
    disagreement_case_ids = sorted(
        case_id
        for case_id in base_rows
        if v251.v246.v130._owner_projection(base_rows[case_id])
        != v251.v246.v130._owner_projection(canary_rows[case_id])
    )
    consistency_issues = v251.v246.v224._alignment_consistency_issues(base)
    abstention_case_ids = sorted(
        case_id
        for case_id, row in base_rows.items()
        if v251.v246.v224._case_has_abstention(row)
    )
    mapping_rows = mapping["rows"]
    supported_mapping = [row for row in mapping_rows if row["witness_id"]]
    origin_by_id = {
        str(row["witness_id"]): str(row["origin"]) for row in supported_mapping
    }
    ...
    strict_represented = {
        index
        for index, group in enumerate(groups)
        if {origin_by_id[witness_id] for witness_id in group}
        == {"reference", "candidate"}
    }
    ...
    no_signal_unsupported = sum(
        row["support_status"] != "supported" for row in no_signal_rows
    )
        ...
        sorted(
            {
                ...
                *v256._runtime_files(),
                ...
                Path(v255.__file__).resolve(),
                Path(v256.__file__).resolve(),
...
def verify_runtime_lock(path: Path) -> dict[str, Any]:
    lock = _load_json(path, "v257 runtime lock")
    ...
    if (
        ...
        raise V257FrozenAlignmentError("v257 runtime lock contract drifted")
    records = [
    ...
    if any(not _verify_record(record or {}) for record in records):
        raise V257FrozenAlignmentError("v257 runtime lock record drifted")
    lineage = _validate_lineage()
    if {row["path"] for row in lock["direct_lineage"]} != {
        row["path"] for row in lineage["records"].values()
    }:
        raise V257FrozenAlignmentError("v257 direct lineage set drifted")
...
def _load_frozen(root: Path) -> dict[str, Any]:
    ...
    return {
        "root": root,
        "spec_path": spec_path,
        "spec": _load_json(spec_path, "v257 spec"),
        "runtime_lock": root / "runtime-lock.json",
        "capacity_policy": root / "capacity-policy.json",
        "bundle": {
            "mapping": _load_json(root / "origin-map.private.json", "v257 origin map"),
            "instructions": (
                root / "alignment-instructions.private.md"
...
def freeze_v257(*, output_dir: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    root = output_dir.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    if (root / "terminal.json").exists():
        return {"root": root, "terminal": _load_json(root / "terminal.json", "v257 terminal")}
    if any(root.iterdir()):
        if not (root / "runtime-lock.json").is_file():
            raise V257FrozenAlignmentError("unfinished v257 root is not replayable")
    ...
    _write_stable_time(
        design_path,
        {
            "schema_version": SCHEMA_VERSION,
            "created_at": now_iso(),
            "strategy": "unchanged_frozen_two_permutation_neutral_alignment_over_v255",
            "model": MODEL,
            "effort": EFFORT,
            "alignment_instruction_hash": sha256_text(
                v251.v246.v130.alignment_instructions_v130()
            ),
    ...
    _write_private_text(
        instructions_path, v251.v246.v130.alignment_instructions_v130()
    )
...
def _usage(sidecar: Mapping[str, Any]) -> dict[str, int]:
    ...
    try:
        ...
        raise V257FrozenAlignmentError("v257 sidecar usage incomplete") from exc
    if (
        ...
        raise V257FrozenAlignmentError("v257 sidecar accounting or auth failed")
    return usage
...
def _failure_terminal(
    ...
    for turn_name in TURN_NAMES:
        ...
        if paths["sidecar"].is_file():
            ...
            try:
                ...
                row_usage = {
                    field: int((sidecar.get("usage") or {})[field])
                ...
                if (
                    ...
                    raise ValueError("usage not measured")
                measured += 1
                for field in USAGE_FIELDS:
    ...
    terminal = {
        ...
        "exact_next_action": "audit immutable v257 judge attempt; no retry",
    }
...
def _winner(score_path: Path) -> dict[str, Any]:
    turn_root = next(V255_ROOT.glob("turns/v255-atomic-ledger-*"))
    return {
        "schema_version": WINNER_VERSION,
        "frozen_at": now_iso(),
        "system_id": "v255_atomic_ledger_explicit_applicability_v257_frozen",
        "configuration": {
            "model": v255.MODEL,
            "reasoning_effort": v255.EFFORT,
            "max_events_per_segment": v255.MAX_PROPOSITIONS_PER_SEGMENT,
            "architecture_design": _record(V255_ROOT / "architecture-design.json"),
            "attempt_spec": _record(V255_ROOT / "attempt-spec.json"),
            "instructions": _record(turn_root / "base-instructions.private.md"),
            "schema": _record(turn_root / "schema.json"),
            "v249_projection_schema": _record(
                turn_root / "v249-projection-schema.json"
            ),
            "direct_projection_schema": _record(
                turn_root / "direct-projection-schema.json"
            ),
            "runtime_lock": _record(V255_ROOT / "runtime-lock.json"),
        },
        "development_evidence": {
            "structural_gate": _record(V255_ROOT / "architecture-structural-gate.json"),
            "support_terminal": _record(V256_ROOT / "terminal.json"),
...
async def run_v257(
    *,
    ...
    root = output_dir.expanduser().resolve()
    if (root / "terminal.json").exists():
        return _load_json(root / "terminal.json", "v257 terminal")
    frozen = freeze_v257(output_dir=root)
    verify_runtime_lock(frozen["runtime_lock"])
    if (root / "launch-receipt.json").exists():
        return _failure_terminal(
            root, frozen, V257FrozenAlignmentError("launch exists; replay prohibited")
        )
    _write_immutable(
    ...
    try:
        ...
        async with client_factory(frozen["capacity_policy"]) as client:
            for turn in frozen["bundle"]["turns"]:
                ...
                usages.append(
                    _usage(_load_json(paths["sidecar"], f"{turn['turn_name']} sidecar"))
                )
        ...
        terminal = {
            ...
            "terminal_reason": (
                "v257_alignment_passed_development_winner_frozen_holdout_authorized"
                if passed
                else "v257_alignment_quality_or_permutation_gate_not_passed"
            ),
            ...
            "exact_next_action": (
                ...
                if passed
                else "reject v255 and advance to the next distinct architecture"
            ),
        }
        _write_stable_time(root / "terminal.json", terminal, "terminal_at")
        return terminal
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run frozen alignment over v255")
    parser.add_argument("action", choices=["freeze", "run"])
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--timeout-seconds", type=float, default=TIMEOUT_SECONDS)
    args = parser.parse_args(argv)
    if args.action == "freeze":
        frozen = freeze_v257(output_dir=Path(args.output_dir))
        result = {
        ...
        result = {
            ...
            "development_quality_passed": terminal.get(
                "development_quality_passed", False
            ),
            "development_winner_frozen": terminal.get(
                "development_winner_frozen", False
            ),
